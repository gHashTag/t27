...
module tnf17_jtag #(parameter integer JTAG_CHAIN_N = 3);
    ...
    reg [31:0] sr = 32'hA5A529F4;
    always @(posedge drck)
        if (sel) begin
            // W820 (T548), migrated W839: LAYOUT v2, DESIGN 9 -- the clause
            // bits ride in the word, and the design nibble names whose they are.
            // inv_acc = negation is an involution on every probe; moved = at least one probe
    // actually changed, which is what stops an identity module passing; swept =
    // the sweep finished. One PADDING bit.
            ...
            if (capture)    sr <= {16'hA5A5, 4'd2, 4'd9, inv_acc, moved, swept, 1'b1,
                                   1'b0, 1'b1, beat, ok};
