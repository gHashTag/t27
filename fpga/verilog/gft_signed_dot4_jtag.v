...
// W821: a signed four-term dot product on our die, checked by algebra. LAYOUT v1.
//
...
//
// WORD LAYOUT v1 (T548):
//     {20'hA5A5A, 4'd1, c_can, c_ann, c_com, c_non, 1'b0, 1'b1, beat, ok}
...
module gft_signed_dot4_jtag #(parameter integer JTAG_CHAIN_N = 3);
    ...
    always @(posedge drck)
        if (sel) begin
            if (capture) sr <= {20'hA5A5A, 4'd1, c_can, c_ann, c_com, c_non,
                                1'b0, 1'b1, beat, ok};
