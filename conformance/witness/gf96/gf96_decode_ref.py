...
# -*- coding: utf-8 -*-
"""
gf96_decode_ref.py -- GOLDEN software decode oracle for gf96
(GoldenFloat96: S1 E36 M59, BIAS=34359738367 = 2^35-1); exact-rational target.
...
Why NOT an FP-target (contrast with gf48):
  ...
  edge. gf96 has M=59 > 52 (binary64 mantissa), so binary64 CANNOT hold the
  mantissa exactly and a binary64 lowering WOULD round. The pack therefore keeps
  every gf96 value as an EXACT dyadic literal A*2^B (value_encoding=dyadic), and
  the conformance target is the exact rational value itself, not an FP lowering.
  Consequently there is NO rounding in the decode path: every representable gf96
...
def decode_fraction(raw):
    ...
    Independent path. CRITICAL: the exponent range of gf96 is +-2^35, so 2^(exp-BIAS)
    must NEVER be materialized as an integer (2^34e9 ~ 4 GB -> OOM). Instead this
    ...
    (mantissa arithmetic only, exponents <= M+1 = 60 -> tiny), and shift is a plain
    Python int (may be ~ -3.4e10). This is a DIFFERENT internal decomposition from
...
if __name__ == "__main__":
    ...
    if ok == tot and not fails:
        print("VERDICT: gf96 Fraction-oracle path agrees with pack, abs_error=0")
