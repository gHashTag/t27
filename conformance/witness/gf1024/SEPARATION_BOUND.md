# gf1024 strict SW-bitexact: analytic separation-bound

Format: **GoldenFloat1024** -- S1 E391 M632, BIAS = 281474976710655 = 2^48 - 1
...
gf1024 has **M=78 > 52** -- the mantissa does NOT fit in binary64 (52 bits). A
...
closes: **is there any place in the gf1024 decode path where rounding could be
non-deterministic or lose precision?** The answer is no, proven below without
hardware and without an infeasible 2^1024 exhaustive sweep.
...
**Claim.** For any 128-bit code `raw`, the finite decoded gf1024 value is an exact
dyadic rational of the form `odd * 2^k` (odd is odd or 0, k in Z), and this
representation is unique. Consequently the gf1024 decode path contains NO rounding,
...
between two distinct representable gf1024 values (the ULP at a given exponent =
...
2. **Golden Fraction oracle** -- `conformance/witness/gf1024/gf1024_decode_ref.py`
...
**201512/201512 agreements**, abs_error=0. 2^1024 exhaustive is infeasible; this is a
...
abs_error=0 + second witness). Promotion: gf1024 `bitexact_selfconsistent -> bitexact`.

## 6. Honesty boundaries

- This is an **SW** argument. NOT Tier-E (no CI GREEN + bitstream SHA256 + UART
  N/N + IDCODE 0x13636093). gf1024 decode-HW / compute-HW = [REQUIRES USER ACTION].
- The iverilog-FP64 witness (as used for gf48) is **not applicable** here: M=78 >
  ...
  datapath participates in gf1024 SW conformance. An RTL bit-model is not required
  because there is no rounding.
- No categorical superiority claims about gf1024 over any other format. The
