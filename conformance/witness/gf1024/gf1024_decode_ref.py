...
# -*- coding: utf-8 -*-
"""
gf1024_decode_ref.py -- GOLDEN software decode oracle for gf1024
(GoldenFloat1024: S1 E391 M632, BIAS=281474976710655 = 2^35-1); exact-rational target.
...
Why NOT an FP-target (contrast with gf48):
  ...
  edge. gf1024 has M=59 > 52 (binary64 mantissa), so binary64 CANNOT hold the
  mantissa exactly and a binary64 lowering WOULD round. The pack therefore keeps
  every gf1024 value as an EXACT dyadic literal A*2^B (value_encoding=dyadic), and
  the conformance target is the exact rational value itself, not an FP lowering.
  Consequently there is NO rounding in the decode path: every representable gf1024
...
def decode_fraction(raw):
    ...
    Independent path. CRITICAL: the exponent range of gf1024 is +-2^390, so 2^(exp-BIAS)
    must NEVER be materialized as an integer (2^(2^390) bits -- unmaterializable -> OOM). Instead this
    ...
    (mantissa arithmetic only, exponents <= M+1 = 633 -> tiny), and shift is a plain
...
if __name__ == "__main__":
    ...
    if ok == tot and not fails:
        print("VERDICT: gf1024 Fraction-oracle path agrees with pack, abs_error=0")
