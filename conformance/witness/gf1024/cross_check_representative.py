...
# -*- coding: utf-8 -*-
"""
cross_check_representative.py -- cross-verify the TWO independent gf1024 decode paths
(dyadic integer normalizer  vs  Fraction-significand + symbolic shift) over a large
REPRESENTATIVE code set. 2^1024 exhaustive is infeasible; this is a falsifiable
representative sweep: 5-class + exponent boundaries + full-mantissa edges +
deep-underflow/overflow edges + deterministic random (seed=1024).
...
# witness 2 (gf1024_decode_ref) lives beside this file; witness 1
# (gf_wide_independent_witness) lives two levels up in conformance/.
_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _HERE)                              # gf1024_decode_ref.py
sys.path.insert(0, os.path.abspath(os.path.join(_HERE, "..", "..")))  # conformance/

import gf1024_decode_ref as ORACLE                      # Fraction path (witness 2)
from gf_wide_independent_witness import make_decoder, normalize_dyadic  # dyadic path (witness 1)

N, E, M, BIAS = 1024, 391, 632, 2521728396569246669585858566409191283525103313309788586748690777871726193375821479130513040312634601011624191379636223
EXP_MAX = (1 << E) - 1
...
def main():
    ...
    tot = len(codes)
    print(f"gf1024 cross-check (dyadic path == Fraction path): {ok}/{tot} agree "
          f"[e={E} m={M} bias={BIAS}]")
    if fails:
    ...
    print(f"VERDICT: {tot} representative codes, two independent exact paths agree, "
          f"abs_error=0 (2^1024 exhaustive infeasible; falsifiable representative sweep)")
    return 0
