...
# -*- coding: utf-8 -*-
"""
gf512_decode_ref.py -- GOLDEN software decode oracle for gf512
(GoldenFloat512: S1 E195 M316, BIAS=281474976710655 = 2^35-1); exact-rational target.
...
Why NOT an FP-target (contrast with gf48):
  ...
  edge. gf512 has M=59 > 52 (binary64 mantissa), so binary64 CANNOT hold the
  mantissa exactly and a binary64 lowering WOULD round. The pack therefore keeps
  every gf512 value as an EXACT dyadic literal A*2^B (value_encoding=dyadic), and
  the conformance target is the exact rational value itself, not an FP lowering.
  Consequently there is NO rounding in the decode path: every representable gf512
...
N, E, M, BIAS = 512, 195, 316, 25108406941546723055343157692830665664409421777856138051583
EXP_MAX = (1 << E) - 1
...
def decode_fraction(raw):
    ...
    Independent path. CRITICAL: the exponent range of gf512 is +-2^194, so 2^(exp-BIAS)
    must NEVER be materialized as an integer (2^(2^194) bits -- unmaterializable -> OOM). Instead this
    ...
    (mantissa arithmetic only, exponents <= M+1 = 317 -> tiny), and shift is a plain
...
if __name__ == "__main__":
    ...
    if ok == tot and not fails:
        print("VERDICT: gf512 Fraction-oracle path agrees with pack, abs_error=0")
