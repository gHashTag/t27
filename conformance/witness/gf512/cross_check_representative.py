...
# -*- coding: utf-8 -*-
"""
cross_check_representative.py -- cross-verify the TWO independent gf512 decode paths
(dyadic integer normalizer  vs  Fraction-significand + symbolic shift) over a large
REPRESENTATIVE code set. 2^512 exhaustive is infeasible; this is a falsifiable
representative sweep: 5-class + exponent boundaries + full-mantissa edges +
deep-underflow/overflow edges + deterministic random (seed=512).
...
# witness 2 (gf512_decode_ref) lives beside this file; witness 1
# (gf_wide_independent_witness) lives two levels up in conformance/.
_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _HERE)                              # gf512_decode_ref.py
sys.path.insert(0, os.path.abspath(os.path.join(_HERE, "..", "..")))  # conformance/

import gf512_decode_ref as ORACLE                      # Fraction path (witness 2)
...
def main():
    ...
    tot = len(codes)
    print(f"gf512 cross-check (dyadic path == Fraction path): {ok}/{tot} agree "
          f"[e={E} m={M} bias={BIAS}]")
    if fails:
    ...
    print(f"VERDICT: {tot} representative codes, two independent exact paths agree, "
          f"abs_error=0 (2^512 exhaustive infeasible; falsifiable representative sweep)")
    return 0
