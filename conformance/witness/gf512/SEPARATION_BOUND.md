# gf512 strict SW-bitexact: analytic separation-bound

Format: **GoldenFloat512** -- S1 E195 M316, BIAS = 281474976710655 = 2^48 - 1
...
gf512 has **M=78 > 52** -- the mantissa does NOT fit in binary64 (52 bits). A
...
closes: **is there any place in the gf512 decode path where rounding could be
non-deterministic or lose precision?** The answer is no, proven below without
hardware and without an infeasible 2^512 exhaustive sweep.
...
**Claim.** For any 128-bit code `raw`, the finite decoded gf512 value is an exact
dyadic rational of the form `odd * 2^k` (odd is odd or 0, k in Z), and this
representation is unique. Consequently the gf512 decode path contains NO rounding,
...
between two distinct representable gf512 values (the ULP at a given exponent =
...
2. **Golden Fraction oracle** -- `conformance/witness/gf512/gf512_decode_ref.py`
...
**201512/201512 agreements**, abs_error=0. 2^512 exhaustive is infeasible; this is a
...
abs_error=0 + second witness). Promotion: gf512 `bitexact_selfconsistent -> bitexact`.

## 6. Honesty boundaries

- This is an **SW** argument. NOT Tier-E (no CI GREEN + bitstream SHA256 + UART
  N/N + IDCODE 0x13636093). gf512 decode-HW / compute-HW = [REQUIRES USER ACTION].
- The iverilog-FP64 witness (as used for gf48) is **not applicable** here: M=78 >
  ...
  datapath participates in gf512 SW conformance. An RTL bit-model is not required
  because there is no rounding.
- No categorical superiority claims about gf512 over any other format. The
