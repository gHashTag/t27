# gf512 strict SW-bitexact witness chain (exact-dyadic target)

Promotion of `gf512` (GoldenFloat512: S1 E195 M316, BIAS=281474976710655=2^194-1) from
...
gf512 has **M=78 > 52**, so binary64 CANNOT hold the mantissa exactly and a binary64
lowering WOULD round. The pack therefore keeps every gf512 value as an EXACT dyadic
...
representable gf512 code maps to an exact dyadic rational (see `SEPARATION_BOUND.md`,
...
2. **Golden Fraction oracle** -- `gf512_decode_ref.py`
   ...
   ```
   python3 conformance/witness/gf512/gf512_decode_ref.py \
     conformance/vectors/gf512_conformance_v0.json
   ```
...
```
cd conformance/witness/gf512 && python3 cross_check_representative.py
```
Expected: `201512/201512 agree` (count is deterministic for seed=96).

## Memory note (why no giant integers)

The gf512 exponent range is +-2^194, so `2^(exp-BIAS)` must NEVER be materialized as
...
| File | Role |
|---|---|
| `gf512_decode_ref.py` | Golden Fraction decode oracle (witness 2). |
