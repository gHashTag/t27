# Wave Loop 835 Cooperation Plan
...
| Parent branch | `wave-loop-834` HEAD because earlier waves' PRs remain open |
| Status | planned |
...
### A — Recommended: `[489][2]^6 Pt` module-scope AoS variable from call with indexed signed writes

- Outer dimension += 2 (487 → 489), keeping the inner `2^6` and `Pt { x : i16, y : i16 }` pattern.
- Expected 31,296 elements, 1,001,472-bit packed vector (~0.955 MiBit).
- Still well under the 4-MiBit packed-vector cliff.
- Expected zero compiler / reference-model / `FROZEN_HASH` changes.
- Mechanical generator copy-hazard fix required (`w835` / `489` / `MID_IDX = 244`).

### B — Implementation-heavy: `[487][3]^6 Pt` stride scaling

- Keep outer dimension at 487 but grow the second inner dimension from `2` to `3`.
- Changes the flattened stride and element count (487 × 3^6 = 355,023 elements, ~11.36 MiBit), which may cross simulation wall-clock or width limits.
...
### C — Process/tooling: `[487][2]^6 Pt` with negative-index writes

- Same shape as W834 but add signed negative indices to exercise wrap-around addressing.
...
- [ ] Generator `scripts/gen_w835.py` with correct `OUTER` and `MID_IDX`.
- [ ] Witness generated and passes `t27c parse|icarus-lowerable|icarus-simulate|icarus-cocotb|seal --save`.
- [ ] Integration test added; full `icarus_lowerable` suite expected to be 295/0.
