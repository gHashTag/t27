# Wave Loop 615 Plan — module-scope `[49][2]^6 Pt` packed AoS variable from call with indexed signed writes

## Goal
Close Issue #1586 by adding a module-scope non-power-of-two outer-dimension
witness `[49][2]^6 Pt` (3,136 elements, 100,352 bits, ≈0.096 MiBit), initialized
from a function call and exercised with indexed signed field writes, without
compiler or reference-model changes.

## Decomposition
1. **Spec / TDD** — create `specs/scratch/w615_bench_module_49x2p6_aos_var_call_write.t27`:
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [49][2]^6 Pt` returning a multi-line
   ...
   - `test module_var_49x2p6_call_write`: initial equality, corner reads,
     explicit `make_grid(32768)` modulo-wrap check.
   - `bench module_bench_49x2p6_call_write`: whole-array equality, indexed reads,
     signed writes, read-back, frame-condition, whole-array inequality.
2. **Generate** — run generator script; verify leaf count = 49 × 2⁶ = 3136 and
   bit width = 3136 × 32 = 100,352.
...
- First module-scope outer dimension 49 in the packed AoS ladder.
- Stride by 49 must be correct in generated Verilog and reference model.
...
- Icarus Verilog packed-array handling (issue #1171) — W615 stays far below the
...
## Sizing
- Elements: 49 × 64 = 3,136.
- Bits: 3,136 × 32 = 100,352 (≈0.096 MiBit).
- Expected wall-clock: seconds for parse, seconds for Icarus simulation,
  seconds for cocotb reference model.

## Success criteria
- `cargo build --release -p t27c` green.
- `cargo test -p t27c --test icarus_lowerable` includes W615 and passes 75/0.
...
## Cooperation variants for Wave Loop 616
...
3. **Variant C — `[49][2]^6 Pt` conditional whole-array reassignment inside `if`, then indexed writes.**
   100,352-bit; tests control-flow guarded packed-reg reassignment.
