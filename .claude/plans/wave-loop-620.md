# Wave Loop 620 Plan — module-scope `[59][2]^6 Pt` packed AoS variable from call with indexed signed writes

## Goal
Close Issue #1591 by adding a module-scope non-power-of-two outer-dimension
witness `[59][2]^6 Pt` (3,776 elements, 120,832 bits, ≈0.115 MiBit), initialized
from a function call and exercised with indexed signed field writes, without
compiler or reference-model changes.

## Decomposition
1. **Spec / TDD** — create `specs/scratch/w620_bench_module_59x2p6_aos_var_call_write.t27`:
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [59][2]^6 Pt` returning a multi-line
   ...
   - `test module_var_59x2p6_call_write`: initial equality, corner reads,
     explicit `make_grid(32768)` modulo-wrap check.
   - `bench module_bench_59x2p6_call_write`: whole-array equality, indexed reads,
     signed writes, read-back, frame-condition, whole-array inequality.
2. **Generate** — run generator script; verify leaf count = 59 × 2⁶ = 3776 and
   bit width = 3776 × 32 = 120,832.
...
- First module-scope outer dimension 59 in the packed AoS ladder.
- Stride by 59 must be correct in generated Verilog and reference model.
...
- Icarus Verilog packed-array handling (issue #1171) — W620 stays far below the
...
## Sizing
- Elements: 59 × 64 = 3,776.
- Bits: 3,776 × 32 = 120,832 (≈0.115 MiBit).
- Expected wall-clock: seconds for parse, seconds for Icarus simulation,
  seconds for cocotb reference model.

## Success criteria
- `cargo build --release -p t27c` green.
- `cargo test -p t27c --test icarus_lowerable` includes W620 and passes 80/0.
...
## Cooperation variants for Wave Loop 621
...
3. **Variant C — `[59][2]^6 Pt` conditional whole-array reassignment inside `if`, then indexed writes.**
   120,832-bit; tests control-flow guarded packed-reg reassignment.
