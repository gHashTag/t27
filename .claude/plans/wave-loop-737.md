# Wave Loop 737 — Decomposed Plan

**Issue:** #1708 (expected)  
**Branch:** `wave-loop-737`  
**Date:** 2026-07-07  
**Previous:** Wave Loop 736 (#1707, branch `wave-loop-736`)

## Goal

Validate a module-scope `[293][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 600,064-bit packed vector (18,752 elements,
≈0.573 MiBit), continuing the odd outer-dimension ladder while staying far
...
- Icarus issue #1171 — elaboration freeze on very large packed vectors; W737
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W737 must continue using
...
3. **Natural modulo wrap already present.** With 18,752 elements, the last raw
   `x` value is `2·18751 = 37502`, which exceeds 32768 and wraps to
   `37502 mod 32768 = 4734`. The explicit `make_grid(32768)` call remains a
   period-identity regression check (adding one full period is a no-op modulo
   32768), not a first-time wrap test.
4. **No systematic wall-clock limit test yet.** At 0.573 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 293.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 293 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 146` for `OUTER = 293`.
   The mid-row check uses `[146][1][0][0][0][0][0]`, element index
   `146·64 + 32 = 9376`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w736.py` to
      `scripts/gen_w737.py`; set `OUTER = 293` and `MID_IDX = 146`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w737.py` to produce
      `specs/scratch/w737_bench_module_293x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w737_bench_module_293x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W737 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W737_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W737 with `Closes #1708`, record session
      log and commit count, create `wave-loop-738` with W738 cooperation
...
## Next Wave Loop 738 cooperation variants

1. **Variant A (recommended): `[295][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - 604,160-bit packed vector, 18,880 elements.
   - Continues the odd outer-dimension ladder and confirms non-p2 stride 295.
   - **Recommended.**

2. **Variant B: `[293][2]^6 Pt` bench-local (function-local) packed array var
   from call with indexed signed writes.**
   - Same width as W737 but tests the mutable `reg` declared inside a bench or
     function rather than at module scope.

3. **Variant C: `[293][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   - Stays at 0.573 MiBit and tests control-flow-guarded writes on a packed reg.

## Definition of done

- [ ] `.claude/plans/wave-loop-737.md` exists and covers the above.
- [ ] `scripts/gen_w737.py` created with `OUTER = 293` and `MID_IDX = 146`.
- [ ] `specs/scratch/w737_bench_module_293x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w737_bench_module_293x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-738` created and W737 committed with `Closes #1708`.
