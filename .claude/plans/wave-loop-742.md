...
Validate a module-scope `[303][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed
signed field writes. The witness is a 620,544-bit packed vector (19,392
elements, ≈0.591 MiBit), continuing the odd outer-dimension ladder while staying
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W742 must continue using
...
3. **Natural modulo wrap already present.** With 19,392 elements, the last raw
   `x` value is `2·19391 = 38782`, which exceeds 32768 and wraps to
   `38782 mod 32768 = 6014`. The explicit `make_grid(32768)` call remains a
...
4. **No systematic wall-clock limit test yet.** At 0.591 MiBit we remain
   ...
   backlog.
5. **Outer stride 303.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 303 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 151` for `OUTER = 303`.
   The mid-row check uses `[151][1][0][0][0][0][0]`, element index
   `151·64 + 32 = 9696`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w741.py` to
      `scripts/gen_w742.py`; set `OUTER = 303` and `MID_IDX = 151`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w742.py` to produce
      `specs/scratch/w742_bench_module_303x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w742_bench_module_303x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W742 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W742_2026-07-21.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W742 with `Closes #1713`, record session
      log and commit count, create `wave-loop-743` with W743 cooperation
...
- [ ] `.claude/plans/wave-loop-742.md` exists and covers the above.
- [ ] `scripts/gen_w742.py` created with `OUTER = 303` and `MID_IDX = 151`.
- [ ] `specs/scratch/w742_bench_module_303x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w742_bench_module_303x2p6_aos_var_call_write` added and passing.
