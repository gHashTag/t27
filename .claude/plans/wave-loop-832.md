# Wave Loop 832 Cooperation Plan
...
| Parent branch | `wave-loop-831` HEAD because earlier waves' PRs remain open |
| Status | planned |
...
### A — Recommended: `[483][2]^6 Pt` module-scope AoS variable from call with indexed signed writes

- Outer dimension += 2 (481 → 483), keeping the inner `2^6` and `Pt { x : i16, y : i16 }` pattern.
...
- Mechanical generator copy-hazard fix required (`w832` / `483` / `MID_IDX = 241`).

### B — Implementation-heavy: `[481][3]^6 Pt` stride scaling

- Keep outer dimension at 481 but grow the second inner dimension from `2` to `3`.
- Changes the flattened stride and element count (481 × 3^6 = 349,551 elements, ~11.18 MiBit), which may cross simulation wall-clock or width limits.
...
### C — Process/tooling: `[481][2]^6 Pt` with negative-index writes

- Same shape as W831 but add signed negative indices to exercise wrap-around addressing.
...
- [ ] Generator `scripts/gen_w832.py` with correct `OUTER` and `MID_IDX`.
- [ ] Witness generated and passes `t27c parse|icarus-lowerable|icarus-simulate|icarus-cocotb|seal --save`.
- [ ] Integration test added; full `icarus_lowerable` suite expected to be 292/0.
