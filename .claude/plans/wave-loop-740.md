...
Validate a module-scope `[299][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 612,352-bit packed vector (19,136 elements,
≈0.584 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W740 must continue using
...
3. **Natural modulo wrap already present.** With 19,136 elements, the last raw
   `x` value is `2·19135 = 38270`, which exceeds 32768 and wraps to
   `38270 mod 32768 = 5502`. The explicit `make_grid(32768)` call remains a
...
4. **No systematic wall-clock limit test yet.** At 0.584 MiBit we remain
   ...
   backlog.
5. **Outer stride 299.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 299 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 149` for `OUTER = 299`.
   The mid-row check uses `[149][1][0][0][0][0][0]`, element index
   `149·64 + 32 = 9568`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w739.py` to
      `scripts/gen_w740.py`; set `OUTER = 299` and `MID_IDX = 149`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w740.py` to produce
      `specs/scratch/w740_bench_module_299x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w740_bench_module_299x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W740 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W740_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W740 with `Closes #1711`, record session
      log and commit count, create `wave-loop-741` with W741 cooperation
...
- [ ] `.claude/plans/wave-loop-740.md` exists and covers the above.
- [ ] `scripts/gen_w740.py` created with `OUTER = 299` and `MID_IDX = 149`.
- [ ] `specs/scratch/w740_bench_module_299x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w740_bench_module_299x2p6_aos_var_call_write` added and passing.
