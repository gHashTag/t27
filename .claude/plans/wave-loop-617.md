# Wave Loop 617 Plan — module-scope `[53][2]^6 Pt` packed AoS variable from call with indexed signed writes

## Goal
Close Issue #1588 by adding a module-scope non-power-of-two outer-dimension
witness `[53][2]^6 Pt` (3,392 elements, 108,544 bits, ≈0.103 MiBit), initialized
from a function call and exercised with indexed signed field writes, without
compiler or reference-model changes.

## Decomposition
1. **Spec / TDD** — create `specs/scratch/w617_bench_module_53x2p6_aos_var_call_write.t27`:
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [53][2]^6 Pt` returning a multi-line
   ...
   - `test module_var_53x2p6_call_write`: initial equality, corner reads,
     explicit `make_grid(32768)` modulo-wrap check.
   - `bench module_bench_53x2p6_call_write`: whole-array equality, indexed reads,
     signed writes, read-back, frame-condition, whole-array inequality.
2. **Generate** — run generator script; verify leaf count = 53 × 2⁶ = 3392 and
   bit width = 3392 × 32 = 108,544.
...
- First module-scope outer dimension 53 in the packed AoS ladder.
- Stride by 53 must be correct in generated Verilog and reference model.
...
- Icarus Verilog packed-array handling (issue #1171) — W617 stays far below the
...
## Sizing
- Elements: 53 × 64 = 3,392.
- Bits: 3,392 × 32 = 108,544 (≈0.103 MiBit).
- Expected wall-clock: seconds for parse, seconds for Icarus simulation,
  seconds for cocotb reference model.

## Success criteria
- `cargo build --release -p t27c` green.
- `cargo test -p t27c --test icarus_lowerable` includes W617 and passes 77/0.
...
## Cooperation variants for Wave Loop 618
...
3. **Variant C — `[53][2]^6 Pt` conditional whole-array reassignment inside `if`, then indexed writes.**
   108,544-bit; tests control-flow guarded packed-reg reassignment.
