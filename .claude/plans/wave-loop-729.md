...
Validate a module-scope `[277][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 598,912-bit packed vector (17,792 elements,
≈0.571 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W729 must continue using
...
4. **No systematic wall-clock limit test yet.** At 0.571 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 277.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 277 at the outer dimension; a fresh
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w728.py` to
      `scripts/gen_w729.py`; set `OUTER = 277` and `MID_IDX = 138`; fix the
      module name header.
3. [ ] **Witness generation.** Run `python3 scripts/gen_w729.py` to produce
      `specs/scratch/w729_bench_module_277x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w729_bench_module_277x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W729 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W729_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W729 with `Closes #1700`, record session
       log and commit count, create `wave-loop-730` with W730 cooperation
...
1. **Variant A (recommended): `[279][2]^6 Pt` module-scope var from call with
   ...
   - 603,264-bit packed vector, 17,936 elements.
...
- [ ] `.claude/plans/wave-loop-729.md` exists and covers the above.
- [ ] `scripts/gen_w729.py` created with `OUTER = 277` and `MID_IDX = 138`.
- [ ] `specs/scratch/w729_bench_module_277x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w729_bench_module_277x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-730` created and W729 committed with `Closes #1700`.
