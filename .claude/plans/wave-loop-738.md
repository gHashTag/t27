...
Validate a module-scope `[295][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 604,160-bit packed vector (18,880 elements,
≈0.577 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W738 must continue using
...
3. **Natural modulo wrap already present.** With 18,880 elements, the last raw
   `x` value is `2·18879 = 37758`, which exceeds 32768 and wraps to
   `37758 mod 32768 = 4990`. The explicit `make_grid(32768)` call remains a
...
4. **No systematic wall-clock limit test yet.** At 0.577 MiBit we remain
   ...
   backlog.
5. **Outer stride 295.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 295 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 147` for `OUTER = 295`.
   The mid-row check uses `[147][1][0][0][0][0][0]`, element index
   `147·64 + 32 = 9440`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w737.py` to
      `scripts/gen_w738.py`; set `OUTER = 295` and `MID_IDX = 147`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w738.py` to produce
      `specs/scratch/w738_bench_module_295x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w738_bench_module_295x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W738 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W738_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W738 with `Closes #1709`, record session
      log and commit count, create `wave-loop-739` with W739 cooperation
...
- [ ] `.claude/plans/wave-loop-738.md` exists and covers the above.
- [ ] `scripts/gen_w738.py` created with `OUTER = 295` and `MID_IDX = 147`.
- [ ] `specs/scratch/w738_bench_module_295x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w738_bench_module_295x2p6_aos_var_call_write` added and passing.
