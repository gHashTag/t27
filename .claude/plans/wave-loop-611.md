# Wave Loop 611 Plan — module-scope `[41][2]^6 Pt` packed AoS variable from call with indexed signed writes

## Goal
Close Issue #1582 by adding a module-scope non-power-of-two outer-dimension
witness `[41][2]^6 Pt` (2,624 elements, 83,968 bits, ≈0.08 MiBit), initialized
from a function call and exercised with indexed signed field writes, without
compiler or reference-model changes.

## Decomposition
1. **Spec / TDD** — create `specs/scratch/w611_bench_module_41x2p6_aos_var_call_write.t27`:
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [41][2]^6 Pt` returning a multi-line
   ...
   - `test module_var_41x2p6_call_write`: initial equality, corner reads,
     explicit `make_grid(32768)` modulo-wrap check.
   - `bench module_bench_41x2p6_call_write`: whole-array equality, indexed reads,
     signed writes, read-back, frame-condition, whole-array inequality.
2. **Generate** — run generator script; verify leaf count = 41 × 2⁶ = 2624 and
   bit width = 2624 × 32 = 83,968.
...
- First module-scope outer dimension 41 in the packed AoS ladder.
- Stride by 41 must be correct in generated Verilog and reference model.
...
- Icarus Verilog packed-array handling (issue #1171) — W611 stays far below the
...
## Sizing
- Elements: 41 × 64 = 2,624.
- Bits: 2,624 × 32 = 83,968 (≈0.08 MiBit).
- Expected wall-clock: seconds for parse, seconds for Icarus simulation,
  seconds for cocotb reference model.

## Success criteria
- `cargo build --release -p t27c` green.
- `cargo test -p t27c --test icarus_lowerable` includes W611 and passes 71/0.
...
## Cooperation variants for Wave Loop 612
...
3. **Variant C — `[41][2]^6 Pt` conditional whole-array reassignment inside `if`, then indexed writes.**
   83,968-bit; tests control-flow guarded packed-reg reassignment.
