# Wave Loop 725 — Decomposed Plan

**Issue:** #1696 (expected)  
**Branch:** `wave-loop-725`  
**Date:** 2026-07-07  
**Previous:** Wave Loop 724 (#1695, branch `wave-loop-724`)

## Goal

Validate a module-scope `[269][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 581,504-bit packed vector (17,216 elements,
≈0.554 MiBit), continuing the odd outer-dimension ladder while staying far
...
- Icarus issue #1171 — elaboration freeze on very large packed vectors; W725
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W725 must continue using
...
3. **Modulo-wrap regression signal is artificial below 16,384 elements.** With
   17,216 elements, the offset-0 schedule never wraps at 32768, so the test
   must keep an explicit `make_grid(32768)` call to retain the wrap check.
4. **No systematic wall-clock limit test yet.** At 0.554 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 269.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 269 at the outer dimension; a fresh
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w724.py` to
      `scripts/gen_w725.py`; set `OUTER = 269` and `MID_IDX = 134`; fix the
      module name header.
3. [ ] **Witness generation.** Run `python3 scripts/gen_w725.py` to produce
      `specs/scratch/w725_bench_module_269x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w725_bench_module_269x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W725 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W725_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W725 with `Closes #1696`, record session
       log and commit count, create `wave-loop-726` with W726 cooperation
...
## Next Wave Loop 726 cooperation variants

1. **Variant A (recommended): `[271][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - 585,856-bit packed vector, 17,360 elements.
   - Continues the odd outer-dimension ladder and confirms non-p2 stride 271.
   - **Recommended.**

2. **Variant B: `[269][2]^6 Pt` bench-local (function-local) packed array var
   from call with indexed signed writes.**
   - Same width as W725 but tests the mutable `reg` declared inside a bench or
     function rather than at module scope.

3. **Variant C: `[269][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   - Stays at 0.554 MiBit and tests control-flow-guarded writes on a packed reg.

## Definition of done

- [ ] `.claude/plans/wave-loop-725.md` exists and covers the above.
- [ ] `scripts/gen_w725.py` created with `OUTER = 269` and `MID_IDX = 134`.
- [ ] `specs/scratch/w725_bench_module_269x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w725_bench_module_269x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-726` created and W725 committed with `Closes #1696`.
