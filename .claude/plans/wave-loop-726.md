# Wave Loop 726 — Decomposed Plan

**Issue:** #1697 (expected)  
**Branch:** `wave-loop-726`  
**Date:** 2026-07-07  
**Previous:** Wave Loop 725 (#1696, branch `wave-loop-725`)

## Goal

Validate a module-scope `[271][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 585,856-bit packed vector (17,360 elements,
≈0.558 MiBit), continuing the odd outer-dimension ladder while staying far
...
- Icarus issue #1171 — elaboration freeze on very large packed vectors; W726
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W726 must continue using
...
3. **Modulo-wrap regression signal is artificial below 16,384 elements.** With
   17,360 elements, the offset-0 schedule never wraps at 32768, so the test
   must keep an explicit `make_grid(32768)` call to retain the wrap check.
4. **No systematic wall-clock limit test yet.** At 0.558 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 271.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 271 at the outer dimension; a fresh
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w725.py` to
      `scripts/gen_w726.py`; set `OUTER = 271` and `MID_IDX = 135`; fix the
      module name header.
3. [ ] **Witness generation.** Run `python3 scripts/gen_w726.py` to produce
      `specs/scratch/w726_bench_module_271x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w726_bench_module_271x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W726 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W726_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W726 with `Closes #1697`, record session
       log and commit count, create `wave-loop-727` with W727 cooperation
...
## Next Wave Loop 727 cooperation variants

1. **Variant A (recommended): `[273][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - 590,208-bit packed vector, 17,504 elements.
   - Continues the odd outer-dimension ladder and confirms non-p2 stride 273.
   - **Recommended.**

2. **Variant B: `[271][2]^6 Pt` bench-local (function-local) packed array var
   from call with indexed signed writes.**
   - Same width as W726 but tests the mutable `reg` declared inside a bench or
     function rather than at module scope.

3. **Variant C: `[271][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   - Stays at 0.558 MiBit and tests control-flow-guarded writes on a packed reg.

## Definition of done

- [ ] `.claude/plans/wave-loop-726.md` exists and covers the above.
- [ ] `scripts/gen_w726.py` created with `OUTER = 271` and `MID_IDX = 135`.
- [ ] `specs/scratch/w726_bench_module_271x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w726_bench_module_271x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-727` created and W726 committed with `Closes #1697`.
