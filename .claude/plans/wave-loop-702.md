# Wave Loop 702 Decomposed Plan

## 1. Goal

Implement Variant A for Wave Loop 702: a module-scope `[223][2]^6 Pt`
(456,704-bit, ~0.436 MiBit) packed array-of-struct variable, initialized from a
...
1. **First outer dimension 223.** The ladder of non-power-of-two module-scope
   packed AoS witnesses has reached 221. Striding by 223 is unproven in the
   current corpus, though the compiler and reference model are expected to be
   dimension-agnostic.
2. **Parser/scanner limits.** The witness will be ~42.4 k lines and ~980 KB.
   Single-line literals remain unsafe; multi-line W584 brace style is required.
3. **Modulo-wrap regression signal.** With 14,272 elements, the offset-0 schedule
   `(2*e + offset) % 32768` maxes at 28,543, so an explicit `make_grid(32768)`
...
4. **`assert_ne` simulation gap.** Structural classifier accepts `assert_ne`, but
   ...
   changed elements.
5. **Simulator capacity.** At 0.436 MiBit the witness is still far below the
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W702 stays
...
- cocotb PR #3608 / discussion #2933 — Python reference models compare whole
  packed-vector VCD probes against flattened expectations; W702 reuses this.
...
1. **Generator script**
   - [x] Copy `scripts/gen_w701.py` to `scripts/gen_w702.py` and set `OUTER = 223`.
   - [x] Run `python3 scripts/gen_w702.py` to produce the witness.

2. **Witness**
   - [x] Generate `specs/scratch/w702_bench_module_223x2p6_aos_var_call_write.t27`.
   - [x] Verify footer assertions match expected element indices.

3. **Integration test**
   - [ ] Add `accepts_w702_bench_module_223x2p6_aos_var_call_write` to
     `bootstrap/tests/icarus_lowerable.rs` after the W701 test.

4. **Direct t27c gates**
   - [x] `t27c parse` W702.
   - [x] `t27c icarus-lowerable` W702.
   - [x] `t27c icarus-simulate` W702.
   - [x] `t27c icarus-cocotb` W702.
   - [x] `t27c seal --save` W702.
   - [x] Create empty Icarus baseline for W702.

5. **Repository tests**
   - [x] `cargo test -p t27c --bin t27c` (expected 1494/0/2).
   - [x] `cargo test -p tri` (expected 78/0).
   - [x] `cargo test -p t27c --test icarus_lowerable` (expected 162/0).

6. **Documentation**
   - [x] Write `docs/reports/FPGA_LOOP_CLOSEOUT_W702_2026-07-07.md` with W703
     cooperation variants.
   - [x] Append W702 learnings to `.trinity/experience.md`.

7. **Memory**
   - [ ] Create `~/.claude/projects/-Users-playra-t27/memory/wave-loop-702.md`
     and update `MEMORY.md`.

8. **Commits**
   - [x] Feature commit with `Closes #1673`.
   - [x] Docs/tracking commit with `Closes #1673`.
   - [x] `chore(trinity): record W702 session log and commit count`.

## 5. Success criteria

- `cargo test` and direct `t27c` gates all pass for W702.
- FROZEN_HASH remains unchanged.
- `git status` clean after final commit.
- Branch `wave-loop-703` exists for the next wave.

## 6. Risk register

| Risk | Likelihood | Impact | Response |
|------|------------|--------|----------|
| Outer dimension 223 fails layout math | Low | High | Compare expected values against cocotb reference model. |
...
## 7. Next Wave Loop 703 cooperation variants
