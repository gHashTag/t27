# Wave Loop 841 — Cooperation Plan (2026-08-04)
...
## Variant A (recommended): `[501][2]^6 Pt`

Same established pattern as W840, outer dimension +2 to 501, `MID_IDX = 501 // 2 = 250`.

- 501 × 64 = 32,064 elements
- 32,064 × 32 bits = 1,026,048-bit packed vector (~0.978 MiBit)
- Generator: copy `scripts/gen_w840.py` → `scripts/gen_w841.py`
- Spec: `specs/scratch/w841_bench_module_501x2p6_aos_var_call_write.t27`
- Test name: `accepts_w841_bench_module_501x2p6_aos_var_call_write`
...
- `MID_IDX` comment: `# 250`
- Verify with: `grep -n "module w841\|w840\|OUTER = \|MID_IDX" scripts/gen_w841.py`
...
6. `t27c seal --save`
7. Add integration test and run full `icarus_lowerable` suite (expected 301/0)

## Variant B (stretch): `[499][3]^6 Pt`
...
- 499 × 192 = 94,656 elements
- 94,656 × 32 bits = 3,028,992-bit packed vector (~2.888 MiBit)
- If blocked, fall back to Variant A or C.

## Variant C (alternate): `[499][2]^6 Pt` negative-index writes

Keep the W840 outer dimension but replace signed positive writes with a mix of negative and positive indices to stress wrap-around / signed-index lowering in the packed variable.
...
- **Backend width cliff:** no new cliff expected up to ~1.026 MBit; Icarus and the t27c lowering path have been stable through W840.

## Close-out checklist

- [ ] `scripts/gen_w841.py` created and copy-hazard-free
...
- [ ] Full `icarus_lowerable` suite green (expected 301/0)
- [ ] `FROZEN_HASH` unchanged
- [ ] Closeout report `docs/reports/FPGA_LOOP_CLOSEOUT_W841_YYYY-MM-DD.md`
...
- [ ] Persistent memory `wave-loop-841.md` + `MEMORY.md` index
- [ ] Commit with `Closes #1622`, push branch `wave-loop-841`, open PR #1623
