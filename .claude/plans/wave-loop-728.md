...
Validate a module-scope `[275][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 594,560-bit packed vector (17,648 elements,
≈0.567 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W728 must continue using
...
4. **No systematic wall-clock limit test yet.** At 0.567 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 275.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 275 at the outer dimension; a fresh
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w727.py` to
      `scripts/gen_w728.py`; set `OUTER = 275` and `MID_IDX = 137`; fix the
      module name header.
3. [ ] **Witness generation.** Run `python3 scripts/gen_w728.py` to produce
      `specs/scratch/w728_bench_module_275x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w728_bench_module_275x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W728 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W728_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W728 with `Closes #1699`, record session
       log and commit count, create `wave-loop-729` with W729 cooperation
...
- [ ] `.claude/plans/wave-loop-728.md` exists and covers the above.
- [ ] `scripts/gen_w728.py` created with `OUTER = 275` and `MID_IDX = 137`.
- [ ] `specs/scratch/w728_bench_module_275x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w728_bench_module_275x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-729` created and W728 committed with `Closes #1699`.
