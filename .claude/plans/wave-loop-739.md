# Wave Loop 739 — Decomposed Plan

**Issue:** #1710 (expected)  
**Branch:** `wave-loop-739`  
**Date:** 2026-07-07  
**Previous:** Wave Loop 738 (#1709, branch `wave-loop-738`)

## Goal

Validate a module-scope `[297][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 608,256-bit packed vector (19,008 elements,
≈0.580 MiBit), continuing the odd outer-dimension ladder while staying far
...
- Icarus issue #1171 — elaboration freeze on very large packed vectors; W739
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W739 must continue using
...
3. **Natural modulo wrap already present.** With 19,008 elements, the last raw
   `x` value is `2·19007 = 38014`, which exceeds 32768 and wraps to
   `38014 mod 32768 = 5246`. The explicit `make_grid(32768)` call remains a
   period-identity regression check (adding one full period is a no-op modulo
   32768), not a first-time wrap test.
4. **No systematic wall-clock limit test yet.** At 0.580 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 297.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 297 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 148` for `OUTER = 297`.
   The mid-row check uses `[148][1][0][0][0][0][0]`, element index
   `148·64 + 32 = 9504`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w738.py` to
      `scripts/gen_w739.py`; set `OUTER = 297` and `MID_IDX = 148`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w739.py` to produce
      `specs/scratch/w739_bench_module_297x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w739_bench_module_297x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W739 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W739_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W739 with `Closes #1710`, record session
      log and commit count, create `wave-loop-740` with W740 cooperation
...
| Simulator capacity surprise near 0.58 MiBit | Very low | High | Direct `icarus-simulate` gate runs quickly; abort if elapsed exceeds prior waves significantly. |
...
## Next Wave Loop 740 cooperation variants

1. **Variant A (recommended): `[299][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - 612,352-bit packed vector, 19,136 elements.
   - Continues the odd outer-dimension ladder and confirms non-p2 stride 299.
   - **Recommended.**

2. **Variant B: `[297][2]^6 Pt` bench-local (function-local) packed array var
   from call with indexed signed writes.**
   - Same width as W739 but tests the mutable `reg` declared inside a bench or
     function rather than at module scope.

3. **Variant C: `[297][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   - Stays at 0.580 MiBit and tests control-flow-guarded writes on a packed reg.

## Definition of done

- [ ] `.claude/plans/wave-loop-739.md` exists and covers the above.
- [ ] `scripts/gen_w739.py` created with `OUTER = 297` and `MID_IDX = 148`.
- [ ] `specs/scratch/w739_bench_module_297x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w739_bench_module_297x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-740` created and W739 committed with `Closes #1710`.
