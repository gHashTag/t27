# Wave Loop 621 Plan — module-scope `[61][2]^6 Pt` packed AoS variable from call with indexed signed writes

## Goal
Close Issue #1592 by adding a module-scope non-power-of-two outer-dimension
witness `[61][2]^6 Pt` (3,904 elements, 124,928 bits, ≈0.119 MiBit), initialized
...
1. **Outer dimension 61** — next odd non-power-of-two stride in the module-scope
   packed AoS ladder. The compiler and reference model must correctly multiply
   by 61 at the outer dimension.
2. **Modulo-wrap signal** — 3,904 elements produce raw values up to 7,807, so the
   offset-0 schedule never wraps naturally. An explicit `make_grid(32768)` check
   preserves the regression signal.
3. **Multi-line literals** — the 6-D nested literal inside a 61-element outer
   loop remains large; W584-style multi-line brace formatting is mandatory to
   avoid parser truncation.
4. **Simulator capacity** — 0.119 MiBit is still far below the 4-MiBit literal
...
- Icarus Verilog issue #1171 — large packed-vector elaboration freezes; W621
...
## Decomposition
1. **Spec / TDD** — create `specs/scratch/w621_bench_module_61x2p6_aos_var_call_write.t27`:
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [61][2]^6 Pt` returning a multi-line
   ...
   - `test module_var_61x2p6_call_write`: initial equality, corner reads,
     explicit `make_grid(32768)` modulo-wrap check.
   - `bench module_bench_61x2p6_call_write`: whole-array equality, indexed reads,
     signed writes, read-back, frame-condition, whole-array inequality.
2. **Generate** — run generator script; verify leaf count = 61 × 2⁶ = 3904 and
   bit width = 3904 × 32 = 124,928.
...
## Sizing
- Elements: 61 × 64 = 3,904.
- Bits: 3,904 × 32 = 124,928 (≈0.119 MiBit).
- Expected wall-clock: seconds for parse, seconds for Icarus simulation,
  seconds for cocotb reference model.

## Success criteria
- `cargo build --release -p t27c` green.
- `cargo test -p t27c --test icarus_lowerable` includes W621 and passes 81/0.
...
## Cooperation variants for Wave Loop 622
...
3. **Variant C — `[61][2]^6 Pt` conditional whole-array reassignment inside `if`, then indexed writes.**
   124,928-bit; tests control-flow guarded packed-reg reassignment.
