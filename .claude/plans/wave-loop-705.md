# Wave Loop 705 Decomposed Plan

## 1. Goal

Implement Variant A for Wave Loop 705: a module-scope `[229][2]^6 Pt`
(468,992-bit, ~0.448 MiBit) packed array-of-struct variable, initialized from a
...
1. **First outer dimension 229.** The ladder of non-power-of-two module-scope
   packed AoS witnesses has reached 227. Striding by 229 is unproven in the
   current corpus, though the compiler and reference model are expected to be
   dimension-agnostic.
2. **Parser/scanner limits.** The witness will be ~43.6 k lines and ~1.0 MB.
   Single-line literals remain unsafe; multi-line W584 brace style is required.
3. **Modulo-wrap regression signal.** With 14,656 elements, the offset-0 schedule
   `(2*e + offset) % 32768` maxes at 29,311, so an explicit `make_grid(32768)`
...
4. **`assert_ne` simulation gap.** Structural classifier accepts `assert_ne`, but
   ...
   changed elements.
5. **Simulator capacity.** At 0.448 MiBit the witness is still far below the
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W705 stays
...
- cocotb PR #3608 / discussion #2933 — Python reference models compare whole
  packed-vector VCD probes against flattened expectations; W705 reuses this.
...
1. **Generator script**
   - [x] Copy `scripts/gen_w704.py` to `scripts/gen_w705.py` and set `OUTER = 229`.
   - [x] Run `python3 scripts/gen_w705.py` to produce the witness.

2. **Witness**
   - [x] Generate `specs/scratch/w705_bench_module_229x2p6_aos_var_call_write.t27`.
   - [x] Verify footer assertions match expected element indices.

3. **Integration test**
   - [x] Add `accepts_w705_bench_module_229x2p6_aos_var_call_write` to
     `bootstrap/tests/icarus_lowerable.rs` after the W704 test.

4. **Direct t27c gates**
   - [x] `t27c parse` W705.
   - [x] `t27c icarus-lowerable` W705.
   - [x] `t27c icarus-simulate` W705.
   - [x] `t27c icarus-cocotb` W705.
   - [x] `t27c seal --save` W705.
   - [x] Create empty Icarus baseline for W705.

5. **Repository tests**
   - [x] `cargo test -p t27c --bin t27c` (expected 1494/0/2).
   - [x] `cargo test -p tri` (expected 78/0).
   - [x] `cargo test -p t27c --test icarus_lowerable` (expected 165/0).

6. **Documentation**
   - [x] Write `docs/reports/FPGA_LOOP_CLOSEOUT_W705_2026-07-07.md` with W706
     cooperation variants.
   - [x] Append W705 learnings to `.trinity/experience.md`.

7. **Memory**
   - [x] Create `~/.claude/projects/-Users-playra-t27/memory/wave-loop-705.md`
     and update `MEMORY.md`.

8. **Commits**
   - [ ] Feature commit with `Closes #1676`.
   - [ ] Docs/tracking commit with `Closes #1676`.
   - [ ] `chore(trinity): record W705 session log and commit count`.

## 5. Success criteria

- `cargo test` and direct `t27c` gates all pass for W705.
- FROZEN_HASH remains unchanged.
- `git status` clean after final commit.
- Branch `wave-loop-706` exists for the next wave.

## 6. Risk register

| Risk | Likelihood | Impact | Response |
|------|------------|--------|----------|
| Outer dimension 229 fails layout math | Low | High | Compare expected values against cocotb reference model. |
...
## 7. Next Wave Loop 706 cooperation variants

1. **Variant A (recommended):** `[231][2]^6 Pt` module-scope var from a call with
   indexed signed writes — 473,088 bits, 14,784 elements. Continue the ladder.
