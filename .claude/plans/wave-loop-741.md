...
Validate a module-scope `[301][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed
signed field writes. The witness is a 616,448-bit packed vector (19,264
elements, ≈0.588 MiBit), continuing the odd outer-dimension ladder while staying
far below the ~4-MiBit simulation ceiling.
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W741 must continue using
...
3. **Natural modulo wrap already present.** With 19,264 elements, the last raw
   `x` value is `2·19263 = 38526`, which exceeds 32768 and wraps to
   `38526 mod 32768 = 5758`. The explicit `make_grid(32768)` call remains a
...
4. **No systematic wall-clock limit test yet.** At 0.588 MiBit we remain
   ...
   backlog.
5. **Outer stride 301.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 301 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 150` for `OUTER = 301`.
   The mid-row check uses `[150][1][0][0][0][0][0]`, element index
   `150·64 + 32 = 9632`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w740.py` to
      `scripts/gen_w741.py`; set `OUTER = 301` and `MID_IDX = 150`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w741.py` to produce
      `specs/scratch/w741_bench_module_301x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w741_bench_module_301x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W741 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W741_2026-07-21.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W741 with `Closes #1712`, record session
      log and commit count, create `wave-loop-742` with W742 cooperation
...
| Simulator capacity surprise near 0.59 MiBit | Very low | High | Direct `icarus-simulate` gate runs quickly; abort if elapsed exceeds prior waves significantly. |
...
- [ ] `.claude/plans/wave-loop-741.md` exists and covers the above.
- [ ] `scripts/gen_w741.py` created with `OUTER = 301` and `MID_IDX = 150`.
- [ ] `specs/scratch/w741_bench_module_301x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w741_bench_module_301x2p6_aos_var_call_write` added and passing.
