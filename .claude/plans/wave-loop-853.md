# Wave Loop 853 — Cooperation Plan (2026-08-04)
...
- **Parent branch:** `wave-loop-852` HEAD because earlier wave PRs remain open
- **Goal:** mechanical increment of the non-power-of-two packed array-of-struct ladder

## Variant A (recommended): `[525][2]^6 Pt`

Same established pattern as W852, outer dimension +2 to 525, `MID_IDX = 525 // 2 = 262`.

- 525 × 64 = 33,600 elements
- 33,600 × 32 bits = 1,075,200-bit packed vector (~1.026 MiBit)
- Generator: copy `scripts/gen_w852.py` → `scripts/gen_w853.py`
- Spec: `specs/scratch/w853_bench_module_525x2p6_aos_var_call_write.t27`
- Test name: `accepts_w853_bench_module_525x2p6_aos_var_call_write`

**Expected copy-hazard fixes:**
- destination path: `specs/scratch/w853_bench_module_525x2p6_aos_var_call_write.t27`
- module header f-string: `module w853_bench_module_525x2p6_aos_var_call_write`
- `MID_IDX` comment: `# 262`
- Verify with: `grep -n "module w853\|w852\|OUTER = \|MID_IDX" scripts/gen_w853.py`
...
6. `t27c seal --save`
7. Add integration test and run full `icarus_lowerable` suite (expected 313/0)

## Variant B (stretch): `[523][3]^6 Pt`
...
- 523 × 192 = 100,224 elements
- 100,224 × 32 bits = 3,207,168-bit packed vector (~3.058 MiBit)
- If blocked, fall back to Variant A or C.

## Variant C (alternate): `[523][2]^6 Pt` negative-index writes

Keep the W852 outer dimension but replace signed positive writes with a mix of negative and positive indices to stress wrap-around / signed-index lowering in the packed variable.
...
- [ ] `scripts/gen_w853.py` created and copy-hazard-free
...
- [ ] Full `icarus_lowerable` suite green (expected 313/0)
- [ ] `FROZEN_HASH` unchanged
- [ ] Closeout report `docs/reports/FPGA_LOOP_CLOSEOUT_W853_YYYY-MM-DD.md`
...
- [ ] Persistent memory `wave-loop-852.md` + `MEMORY.md` index
- [ ] Commit with `Closes #1644`, push branch `wave-loop-852`, open PR #1645
