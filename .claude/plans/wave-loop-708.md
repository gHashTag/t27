# Wave Loop 708 Decomposed Plan

## 1. Goal

Implement Variant A for Wave Loop 708: a module-scope `[235][2]^6 Pt`
(479,488-bit, ~0.457 MiBit) packed array-of-struct variable, initialized from a
...
1. **First outer dimension 235.** The ladder of non-power-of-two module-scope
   packed AoS witnesses has reached 233. Striding by 235 is unproven in the
   current corpus, though the compiler and reference model are expected to be
   dimension-agnostic.
2. **Parser/scanner limits.** The witness will be ~44.8 k lines and ~1.0 MB.
   Single-line literals remain unsafe; multi-line W584 brace style is required.
3. **Modulo-wrap regression signal.** With 15,040 elements, the offset-0 schedule
   `(2*e + offset) % 32768` maxes at 30,079, so an explicit `make_grid(32768)`
...
4. **`assert_ne` simulation gap.** Structural classifier accepts `assert_ne`, but
   ...
   changed elements.
5. **Simulator capacity.** At 0.457 MiBit the witness is still far below the
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W708 stays
...
- cocotb PR #3608 / discussion #2933 — Python reference models compare whole
  packed-vector VCD probes against flattened expectations; W708 reuses this.
...
1. **Generator script**
   - [x] Copy `scripts/gen_w707.py` to `scripts/gen_w708.py` and set `OUTER = 235`.
   - [x] Fix `MID_IDX = OUTER // 2` comment to the runtime value (`# 117`).
   - [x] Run `python3 scripts/gen_w708.py` to produce the witness.

2. **Witness**
   - [x] Generate `specs/scratch/w708_bench_module_235x2p6_aos_var_call_write.t27`.
   - [x] Verify footer assertions match expected element indices.

3. **Integration test**
   - [x] Add `accepts_w708_bench_module_235x2p6_aos_var_call_write` to
     `bootstrap/tests/icarus_lowerable.rs` after the W707 test.

4. **Direct t27c gates**
   - [x] `t27c parse` W708.
   - [x] `t27c icarus-lowerable` W708.
   - [x] `t27c icarus-simulate` W708.
   - [x] `t27c icarus-cocotb` W708.
   - [x] `t27c seal --save` W708.
   - [x] Create empty Icarus baseline for W708.

5. **Repository tests**
   - [x] `cargo test -p t27c --bin t27c` (expected 1494/0/2).
   - [x] `cargo test -p tri` (expected 78/0).
   - [x] `cargo test -p t27c --test icarus_lowerable` (expected 168/0).

6. **Documentation**
   - [x] Write `docs/reports/FPGA_LOOP_CLOSEOUT_W708_2026-07-07.md` with W709
     cooperation variants.
   - [x] Append W708 learnings to `.trinity/experience.md`.

7. **Memory**
   - [x] Create `~/.claude/projects/-Users-playra-t27/memory/wave-loop-708.md`
     and update `MEMORY.md`.

8. **Commits**
   - [ ] Feature commit with `Closes #1679`.
   - [ ] Docs/tracking commit with `Closes #1679`.
   - [ ] `chore(trinity): record W708 session log and commit count`.

## 5. Success criteria

- `cargo test` and direct `t27c` gates all pass for W708.
- FROZEN_HASH remains unchanged.
- `git status` clean after final commit.
- Branch `wave-loop-709` exists for the next wave.

## 6. Risk register

| Risk | Likelihood | Impact | Response |
|------|------------|--------|----------|
| Outer dimension 235 fails layout math | Low | High | Compare expected values against cocotb reference model. |
...
## 7. Next Wave Loop 709 cooperation variants
