...
Validate a module-scope `[287][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 587,776-bit packed vector (18,368 elements,
≈0.561 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W734 must continue using
...
3. **Natural modulo wrap already present.** With 18,368 elements, the last raw
   `x` value is `2·18367 = 36734`, which exceeds 32768 and wraps to
   `36734 mod 32768 = 3966`. The explicit `make_grid(32768)` call remains a
...
4. **No systematic wall-clock limit test yet.** At 0.561 MiBit we remain
   ...
   backlog.
5. **Outer stride 287.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 287 at the outer dimension; a fresh
   witness is needed to prove end-to-end correctness.
6. **Mid-row index parity.** `MID_IDX = OUTER // 2 = 143` for `OUTER = 287`.
   The mid-row check uses `[143][1][0][0][0][0][0]`, element index
   `143·64 + 32 = 9184`. Expected values must be computed with the full row-major
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w733.py` to
      `scripts/gen_w734.py`; set `OUTER = 287` and `MID_IDX = 143`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w734.py` to produce
      `specs/scratch/w734_bench_module_287x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w734_bench_module_287x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W734 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W734_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W734 with `Closes #1705`, record session
      log and commit count, create `wave-loop-735` with W735 cooperation
...
- [ ] `.claude/plans/wave-loop-734.md` exists and covers the above.
- [ ] `scripts/gen_w734.py` created with `OUTER = 287` and `MID_IDX = 143`.
- [ ] `specs/scratch/w734_bench_module_287x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w734_bench_module_287x2p6_aos_var_call_write` added and passing.
