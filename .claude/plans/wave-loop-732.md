...
Validate a module-scope `[283][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 579,584-bit packed vector (18,112 elements,
≈0.553 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W732 must continue using
...
4. **No systematic wall-clock limit test yet.** At 0.553 MiBit we remain
   ...
   backlog.
5. **Outer stride 283.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 283 at the outer dimension; a fresh
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w731.py` to
      `scripts/gen_w732.py`; set `OUTER = 283` and `MID_IDX = 141`; fix the
      module name header (note the f-string `{OUTER}` in the header line).
3. [ ] **Witness generation.** Run `python3 scripts/gen_w732.py` to produce
      `specs/scratch/w732_bench_module_283x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w732_bench_module_283x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W732 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W732_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W732 with `Closes #1703`, record session
       log and commit count, create `wave-loop-733` with W733 cooperation
...
- [ ] `.claude/plans/wave-loop-732.md` exists and covers the above.
- [ ] `scripts/gen_w732.py` created with `OUTER = 283` and `MID_IDX = 141`.
- [ ] `specs/scratch/w732_bench_module_283x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w732_bench_module_283x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-733` created and W732 committed with `Closes #1703`.
