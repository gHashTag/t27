# Wave Loop 750 — Decomposed Plan

**Issue:** #1721 (expected)
**Branch:** `wave-loop-750`
**Date:** 2026-07-23
**Previous:** Wave Loop 749 (#1720, branch `wave-loop-749`)

## Goal

Validate a module-scope `[319][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 653,312-bit packed vector (20,416 elements,
~0.624 MiBit), continuing the odd outer-dimension ladder while staying far below
...
Wave Loop 751 and the next ring.

## Weak-point audit (current snapshot)

| Law / area | Finding | Severity |
|---|---|---|
| L1 TRACEABILITY | 516 commits in the last 30 days; 406 carry `Closes #N`, leaving 110 without issue linkage. Wave-loop closeouts themselves now carry it, but historical backlog is large. | Medium |
...
These are **not** W750 blockers; they are documented for the next ring stewards.
...
- Icarus issue #1171 — elaboration freeze on very large packed vectors; W750
...
- **Generalized Multiple-Valued FPGA Architecture Based on Improved T-Gate**
  (2025, *IEEE Access*) — T-gate based MVL FPGA architecture merging LUT and
  flip-flop functionality in CLBs; applicable to any MVL level, reduces power
  and improves PVT robustness. DOI: <https://doi.org/10.1109/access.2025.3605842>.
...
1. [ ] **Select variant.** Variant A: module-scope `[319][2]^6 Pt` variable
   initialized from a call, with indexed signed field writes and read-back.
2. [ ] **Create generator.** Copy `scripts/gen_w749.py` to `scripts/gen_w750.py`,
   set `OUTER = 319` and `MID_IDX = 159`, verify module name uses `w750`.
3. [ ] **Generate witness.** Run `python3 scripts/gen_w750.py` to produce
   `specs/scratch/w750_bench_module_319x2p6_aos_var_call_write.t27`.
4. [ ] **Add integration test.** Append
   `accepts_w750_bench_module_319x2p6_aos_var_call_write` to
...
8. [ ] **Closeout report.** Write `docs/reports/FPGA_LOOP_CLOSEOUT_W750_2026-07-23.md`.
9. [ ] **Experience update.** Prepend W750 learnings to `.trinity/experience.md`.
10. [ ] **Current issue handoff.** Update `.trinity/current-issue.md` to W751.
11. [ ] **Memory update.** Write
    `~/.claude/projects/-Users-playra-t27/memory/wave-loop-750.md` and append
    one-line pointer to `MEMORY.md`.
12. [ ] **Branch / merge.** Commit W750 with `Closes #1721`; create
    `wave-loop-751`.
