# Wave Loop 616 Plan — module-scope `[51][2]^6 Pt` packed AoS variable from call with indexed signed writes

## Goal
Close Issue #1587 by adding a module-scope non-power-of-two outer-dimension
witness `[51][2]^6 Pt` (3,264 elements, 104,448 bits, ≈0.10 MiBit), initialized
from a function call and exercised with indexed signed field writes, without
compiler or reference-model changes.

## Decomposition
1. **Spec / TDD** — create `specs/scratch/w616_bench_module_51x2p6_aos_var_call_write.t27`:
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [51][2]^6 Pt` returning a multi-line
   ...
   - `test module_var_51x2p6_call_write`: initial equality, corner reads,
     explicit `make_grid(32768)` modulo-wrap check.
   - `bench module_bench_51x2p6_call_write`: whole-array equality, indexed reads,
     signed writes, read-back, frame-condition, whole-array inequality.
2. **Generate** — run generator script; verify leaf count = 51 × 2⁶ = 3264 and
   bit width = 3264 × 32 = 104,448.
...
- First module-scope outer dimension 51 in the packed AoS ladder.
- Stride by 51 must be correct in generated Verilog and reference model.
...
- Icarus Verilog packed-array handling (issue #1171) — W616 stays far below the
...
## Sizing
- Elements: 51 × 64 = 3,264.
- Bits: 3,264 × 32 = 104,448 (≈0.10 MiBit).
- Expected wall-clock: seconds for parse, seconds for Icarus simulation,
  seconds for cocotb reference model.

## Success criteria
- `cargo build --release -p t27c` green.
- `cargo test -p t27c --test icarus_lowerable` includes W616 and passes 76/0.
...
## Cooperation variants for Wave Loop 617
1. **Variant A — `[53][2]^6 Pt` module-scope var from call with indexed signed writes.**
   108,544-bit, 3,392 elements, next odd outer dimension. Recommended.
...
3. **Variant C — `[51][2]^6 Pt` conditional whole-array reassignment inside `if`, then indexed writes.**
   104,448-bit; tests control-flow guarded packed-reg reassignment.
