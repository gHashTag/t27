# Wave Loop 834 Cooperation Plan
...
| Parent branch | `wave-loop-833` HEAD because earlier waves' PRs remain open |
| Status | planned |
...
### A — Recommended: `[487][2]^6 Pt` module-scope AoS variable from call with indexed signed writes

- Outer dimension += 2 (485 → 487), keeping the inner `2^6` and `Pt { x : i16, y : i16 }` pattern.
- Expected 31,168 elements, 997,376-bit packed vector (~0.951 MiBit).
- Still well under the 4-MiBit packed-vector cliff.
- Expected zero compiler / reference-model / `FROZEN_HASH` changes.
- Mechanical generator copy-hazard fix required (`w834` / `487` / `MID_IDX = 243`).

### B — Implementation-heavy: `[485][3]^6 Pt` stride scaling

- Keep outer dimension at 485 but grow the second inner dimension from `2` to `3`.
- Changes the flattened stride and element count (485 × 3^6 = 354,375 elements, ~11.34 MiBit), which may cross simulation wall-clock or width limits.
...
### C — Process/tooling: `[485][2]^6 Pt` with negative-index writes

- Same shape as W833 but add signed negative indices to exercise wrap-around addressing.
...
- [ ] Generator `scripts/gen_w834.py` with correct `OUTER` and `MID_IDX`.
- [ ] Witness generated and passes `t27c parse|icarus-lowerable|icarus-simulate|icarus-cocotb|seal --save`.
- [ ] Integration test added; full `icarus_lowerable` suite expected to be 294/0.
