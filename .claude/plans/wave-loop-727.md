...
Validate a module-scope `[273][2]^6 Pt` non-power-of-two outer-dimension
array-of-struct variable initialized from a function call, with indexed signed
field writes. The witness is a 590,208-bit packed vector (17,504 elements,
≈0.563 MiBit), continuing the odd outer-dimension ladder while staying far
...
1. **`assert_ne` gap.** The structural classifier accepts `assert_ne`, but the
   Icarus simulation emitter only lowers `assert_eq`. W727 must continue using
...
4. **No systematic wall-clock limit test yet.** At 0.563 MiBit we remain
   comfortable, but a stress wave near the 4-MiBit boundary remains on the
   backlog.
5. **Outer stride 273.** This is the first time the compiler/reference model
   strides a module-scope packed reg by 273 at the outer dimension; a fresh
...
1. [ ] **Science survey.** Confirm the background above is current; no new
      ...
      unchanged.
2. [ ] **Generator preparation.** Copy `scripts/gen_w726.py` to
      `scripts/gen_w727.py`; set `OUTER = 273` and `MID_IDX = 136`; fix the
      module name header.
3. [ ] **Witness generation.** Run `python3 scripts/gen_w727.py` to produce
      `specs/scratch/w727_bench_module_273x2p6_aos_var_call_write.t27`.
4. [ ] **Integration test.** Append
      `accepts_w727_bench_module_273x2p6_aos_var_call_write` to
      `bootstrap/tests/icarus_lowerable.rs`.
5. [ ] **Release build.** `cargo build --release -p t27c`.
6. [ ] **Direct gates.** Run `t27c parse`, `icarus-lowerable`, `icarus-simulate`,
      `icarus-cocotb`, and `seal --save` on the W727 witness.
...
9. [ ] **Closeout and memory.** Write
      `docs/reports/FPGA_LOOP_CLOSEOUT_W727_2026-07-07.md`, update
      `.trinity/experience.md`, and update persistent memory.
10. [ ] **Land and next branch.** Commit W727 with `Closes #1698`, record session
       log and commit count, create `wave-loop-728` with W728 cooperation
...
- [ ] `.claude/plans/wave-loop-727.md` exists and covers the above.
- [ ] `scripts/gen_w727.py` created with `OUTER = 273` and `MID_IDX = 136`.
- [ ] `specs/scratch/w727_bench_module_273x2p6_aos_var_call_write.t27` generated.
- [ ] Integration test `accepts_w727_bench_module_273x2p6_aos_var_call_write` added and passing.
...
- [ ] Branch `wave-loop-728` created and W727 committed with `Closes #1698`.
