# Wave Loop 852 — Cooperation Plan (2026-08-04)
...
- **Parent branch:** `wave-loop-851` HEAD because earlier wave PRs remain open
- **Goal:** mechanical increment of the non-power-of-two packed array-of-struct ladder

## Variant A (recommended): `[523][2]^6 Pt`

Same established pattern as W851, outer dimension +2 to 523, `MID_IDX = 523 // 2 = 261`.

- 523 × 64 = 33,472 elements
- 33,472 × 32 bits = 1,071,104-bit packed vector (~1.022 MiBit)
- Generator: copy `scripts/gen_w851.py` → `scripts/gen_w852.py`
- Spec: `specs/scratch/w852_bench_module_523x2p6_aos_var_call_write.t27`
- Test name: `accepts_w852_bench_module_523x2p6_aos_var_call_write`

**Expected copy-hazard fixes:**
- destination path: `specs/scratch/w852_bench_module_523x2p6_aos_var_call_write.t27`
- module header f-string: `module w852_bench_module_523x2p6_aos_var_call_write`
- `MID_IDX` comment: `# 261`
- Verify with: `grep -n "module w852\|w851\|OUTER = \|MID_IDX" scripts/gen_w852.py`
...
6. `t27c seal --save`
7. Add integration test and run full `icarus_lowerable` suite (expected 312/0)

## Variant B (stretch): `[521][3]^6 Pt`
...
- 521 × 192 = 99,792 elements
- 99,792 × 32 bits = 3,193,344-bit packed vector (~3.043 MiBit)
- If blocked, fall back to Variant A or C.

## Variant C (alternate): `[521][2]^6 Pt` negative-index writes

Keep the W851 outer dimension but replace signed positive writes with a mix of negative and positive indices to stress wrap-around / signed-index lowering in the packed variable.
...
- [ ] `scripts/gen_w852.py` created and copy-hazard-free
...
- [ ] Full `icarus_lowerable` suite green (expected 312/0)
- [ ] `FROZEN_HASH` unchanged
- [ ] Closeout report `docs/reports/FPGA_LOOP_CLOSEOUT_W852_YYYY-MM-DD.md`
