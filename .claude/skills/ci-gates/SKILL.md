...
## 273. Build the rule you rejected, run it, and read the number
...
Neither of those is deducible from reading the rule. Both took one build
and one sweep to see. **A rule about a corpus is a claim about the
corpus: the cost of testing it is a build, and the cost of not testing
it is shipping the second row.**

## 274. What the comment was hiding

Promoting those discards from comments to code turned two silent defects
into loud ones:

- `(BOARD_NAME != )` — an operator with nothing after it, which is what a
  string literal that lost its quotes leaves behind.
...
`/* ... */`. The discard was not merely losing checks; it was **acting as
a silencer for the renderer that fed it**.
