...
**1507. REPAIR THE WHOLE CLASS THE SAME WAVE YOU NAME IT.** W983 found 14 of 28 on-die clauses were folded constants and repaired ONE wrapper, leaving the rest as a named debt for the next loop. Clearing all eight took one wave and one primitive -- an opaque zero built from two counters the optimiser will not try to prove equal. The instrument that found the class also verified each repair in six seconds, so the marginal cost per wrapper was almost nothing while the context was still loaded. A class-wide defect is cheapest to close in the wave that discovered it, because that is the only moment when the tool, the recipe and the reason are all in hand at once. (T839, W984)

**1508. A RESULT THAT ARRIVES WITH ITS OWN REPAIR DESERVES EXTRA SUSPICION.** Making gft_smul's clauses real turned its die word from 1010 to 1111. That is the outcome the repair was hoping for, which is exactly why it cannot be taken at face value: the wrapper changed, so the netlist changed, so the placement changed. It is a NEW measurement at ONE placement, not a re-run of the old one. The seed sweep that would distinguish 'the failure is gone' from 'the failure is absent here' was not run, and the record says so in the same breath as the number. State the confound in the sentence that states the result, or the number travels without it. (T840, W984)
...
**1524. SCOPE THE DELETION BY PATH, NOT BY PATTERN.** Before removing a 0.73 GiB chipdb, four independent searches all returned SAFE -- and one of them attached two dependencies it labelled out-of-scope itself: fpga/tools/bscan_spi_xc7a100t_fgg676.bit is compile-time embedded in cli/dlc10 via include_bytes!, and a second 100T bitstream is the wrong-part control that must drive Done to 0 before any die read is trusted. Both match the pattern xc7a100t and neither is a chipdb. A naive rm over the pattern would have broken the build AND destroyed the control that makes every silicon result meaningful. The fan-out earned its cost not by saying yes but by naming what the yes did not cover. Delete by enumerated path, never by glob, and read the out-of-scope notes hardest. (W992)

**1525. AN IRREVERSIBLE ACTION WAITS FOR THE HALF THAT DISAGREES.** A 0.73 GiB deletion was preceded by an adversarial audit -- four search angles, each verdict attacked by two skeptics -- precisely because it could not be undone. The rm was issued after the four SAFE verdicts and before the refutation phase returned. The refuters then found a consumer that CONSTRUCTS the path at runtime (main.rs builds format!("{}.bba", device)), which no grep for a literal could see. Nothing broke, and that is luck about which invocation is default, not evidence. Commissioning a contradiction and then acting on the agreement is the same error as quoting the passing clause of a split verdict. If the action cannot be undone, the whole verification is a precondition, not a report. (T851b, W992)

**1526. A SHELL THAT CALLS 'NO MATCH' A FAILURE WILL FAIL ON THE EMPTY CASE.** tri seeds ran two designs, one of them thirty-plus minutes of place-and-route, and printed a header with nothing under it. Under set -o pipefail a grep that matches nothing fails its pipeline, fails the substitution, and set -e ends the run before the row prints -- so the tool threw away the result in exactly the case the row exists to report. Third instance of this shape in one session: grep -l aborting tri audit when the corpus became clean, printf | grep -q inverting a successful SAT proof, and this. The fix is not another || true bolted onto each call; it is one helper that cannot fail and a row printed from whatever it returned, with an explicit ABSENT when the verdict is missing. Then validate it on the output that broke it. (T853, W993)
...
**1529. A NAME IN THE WRONG UNIT IS SELF-CONSISTENT AND THEREFORE NEVER TRIPS.** The ladder names count SYMBOLS -- 1 sign + E trits + M mantissa bits -- and spec_ladder asserts it, so TNF16 really is sixteen storage elements. Every comparison in the corpus and the literature is in BITS, where it is nineteen. The name was right in a unit nobody else uses, wrong in the unit that matters, and self-consistent enough that it never contradicted itself: the cost surfaced three separate times as something else -- a physical_bits field bolted onto a record, a standing rule after two sign-flipped results, and a table reporting 516096 values from a '16-bit' format. And 4/8/16/32/64 is the IEEE sequence, so the name read as a matched comparison it never was. When a quantity has two plausible units, put the unit in the identifier or the identifier will pick one silently. (T857, W995)

**1530. PUT THE CORRECTION WHERE THE READER ENTERS, NOT THROUGH THE HISTORY.** The rename from symbol-count names to bit-width names could have been swept through 1344 citations in 107 files. It was not. Rewriting them would break every cross-reference, touch every committed record indexed by the old name, and change no measurement -- each figure was already computed at the physical width, and the records were written when the name meant what it meant. The mapping box goes at the top of each forward-facing document instead, where a reader enters. Two counts also appear -- 1506 from a raw substring grep over more roots, 1344 from word-boundary matches in text files -- and both are recorded rather than the smaller quietly replacing the larger. Correct forward; annotate backward; never silently rewrite a record to match a later convention. (T859, W996)
...
**1533. ONE SYMPTOM, FOUR CAUSES, NONE OF THEM THE DESIGN.** gft_xorpercep read ABSENT from W977 to W998 and it was never about the operator. Four separate causes printed the identical word: a 600 s place-and-route cap, then an 1800 s cap, then a JTAG cable that had left the bus, then a bus address that moved on replug. Each time the row said ABSENT, which reads as a property of the design, and twice it was written into the record as one. The fix was not one insight but four instruments -- a slope measured from completed builds, a cap derived from the worst rate, a stage column in the sweep row, and a cable check that runs before the build. When a symptom has survived many waves, stop looking for the cause and start counting how many different causes the symptom can express. (T863, W998)

**1534. NAMING A WRONG NUMBER IS NOT FIXING IT.** The clause repair moved every wrapper area by 1.43x to 3.22x, so the MHz-per-kLUT curve stands on figures that measured designs with their checks deleted. This wave recorded the corrected areas and did NOT recompute the curve -- eight designs at several placements each, up to fifty minutes a build, is the next wave's work. What matters is that the record says so in those words rather than presenting the corrected table as though the derived curve had moved with it. A partially-corrected corpus that announces exactly which part is still wrong is safe; one that quietly leaves the reader to assume the correction propagated is not. (W998)

**1535. A CENSUS TAKEN ONCE IS A CLAIM, NOT A MEASUREMENT.** W983 published 'half the on-die clauses are folded constants -- 14 of 28' from a census run once across seven wrappers. W998 turned that check into a gate that runs every audit, and the gate immediately contradicted it: gft_signed_dot4, recorded as 0 folded, has c_ann folded. It is also the ONLY row still checkable -- the other six wrappers were repaired in W984, so their pre-repair sources are gone, and dot4 survived unrepaired precisely BECAUSE the census said it was clean. One verifiable row, and it disagrees. The class of defect is real and reproduced independently on an eighth wrapper the census never covered. But the count rests on six unverifiable numbers, and should be read as a floor. Turn a census into a gate the same wave you publish it, or you publish a number nobody can ever check again. (W998)

**1536. MOST OF THE LAST TWO PERCENT WAS BOOKKEEPING.** Asked to finish to 100 %, I enumerated what was open: five items. Three of the five were STALE FLAGS on work already done -- a defect marked 'not yet repaired' that W994 had repaired, an operator marked IN FLIGHT that W998 had read off silicon, a bound marked IN FLIGHT that had resolved. The debt was in the bookkeeping, not the work. A fourth, six census rows I had myself declared unverifiable, turned out to be sitting in git the whole time and reproduced on re-measurement. Before doing more work to close a gap, check whether the gap is still there: a record that says OPEN and a thing that is open are different objects, and the first accumulates faster. (W999)

**1537. AN UNVERIFIED CLAIM OF UNVERIFIABILITY IS STILL AN UNVERIFIED CLAIM.** W998 corrected a census row and wrote that the other six 'cannot be rechecked, the wrappers having been repaired in W984'. That sentence was never tested. Git preserved every pre-repair source at the parent commit of the repair, and all six re-measured in one command, all six agreeing. The correction was right and the reason given for stopping was wrong -- and stopping is exactly what an unverifiability claim licenses. Treat 'this cannot be checked' as a hypothesis with the same standard of evidence as the claim it excuses, because it is the only kind of claim that ends the investigation by being believed. (W999)

**1538. A VERIFIER PINNED TO THE UNFIXED STATE TURNS EVERY REPAIR INTO A FALSE ALARM.** Closing four stale open-flags made two derived checks fail -- they asserted the records still said 'not yet repaired' and 'IN FLIGHT'. The verifier was right to fire: the records had changed. But the checks were written to pin the defect open rather than to pin the FACT, so a genuine repair reads as drift. Write the check against the invariant you want to hold after the fix, not against the sentence that happens to be there before it, or the corpus will punish you for improving it. (W999)

**1539. KILLING A SCRIPT DOES NOT KILL THE PIPELINE IT LAUNCHED.** I killed curve999.sh and every nextpnr, restarted the collection, and forty minutes later found the run producing two rows instead of ten. The old tri curve loop had survived -- killing the wrapper script and the current child left the bash loop between iterations, which simply spawned the next build. Two runs were then iterating over the same wrapper list and WRITING THE SAME OUTPUT JSON, with the loser's partial table overwriting the winner's. It also halved the CPU: nextpnr went from 41.6 % to 86.9 % the moment the orphan died. Kill the process TREE, then verify by walking ps for descendants of the script you meant to stop -- and check that no survivor shares the output path, because a race for one file is worse than a race for the machine. (W1000)

**1540. A NUMBER YOU CANNOT EXPLAIN IS WORSE THAN A GAP.** The honest curve came with a CARRY4 column that reported 1604 where the yosys line read 160 -- above the LUT count, impossible on its face. It is not part of the curve and nothing depended on it, so the temptation was to publish it anyway and move on. It was dropped instead, with the reason recorded. A table with a stated gap invites the reader to fill it; a table with an unexplained number invites the reader to trust it, and every other number beside it. This is the same discipline that caught the mismatched-width comparison in W991 -- there the impossible count was the thing that saved the result. (T867a, W1000)

**1541. KEEP THE FIT YOU REPLACED, OR YOU HIDE HOW MUCH THE LAST POINT MOVED IT.** The honest MHz/kLUT curve fitted Fmax = 2050*LUT^-0.592 on twelve points. The thirteenth, gft_xorpercep at 28217 LUT, sat 1.46x beyond the previous widest and moved the exponent to -0.648 -- 9 % -- while leaving R^2 at 0.918. Reporting only the new fit would have looked like more confidence from more data; reporting both shows it was an EXTRAPOLATION test that the law passed, and tells a reader how much the next point outside the range might move it again. Publish the superseded fit beside the current one whenever the new point extends the range rather than filling it. (T869, W1001)

**1542. CLEAR-THE-CACHE-AND-RETRY IS A DESTRUCTIVE REMEDY.** A font failed to load in a LaTeX build that had succeeded minutes earlier. The reflex remedy -- clear the cache, rebuild -- turned a one-font failure into a build that cannot run at all: tectonic's 2.82 GB bundle host times out after 64 KB from this machine, so nothing can be re-fetched, while GitHub downloads 29 MB without complaint. The PDF survives only because it had been copied aside first. This is T851 three waves later, in a session that had already written the theorem: an irreversible act was taken on a cache whose restorability had not been checked, and it was taken as a FIX, which is the disguise that gets such acts past the check. Before clearing any cache, fetch one byte from wherever it would be refilled. (W1001)
