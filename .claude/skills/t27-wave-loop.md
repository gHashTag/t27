...
## Worked example — Wave Loop 778

Wave Loop 778 extended the odd outer-dimension ladder to `[375][2]^6 Pt` with no
compiler changes, branching from `wave-loop-777` HEAD because PR #1484 (W774),
PR #1486 (W775), PR #1488 (W776), PR #1489 (README/W774-W776 merge), and
PR #1491 (W777) remained open or unstable:

- Generated `scripts/gen_w778.py` from `scripts/gen_w777.py` with `OUTER = 375`
  and `MID_IDX = 187`.
- Produced `specs/scratch/w778_bench_module_375x2p6_aos_var_call_write.t27`
...
- Added integration test `accepts_w778_bench_module_375x2p6_aos_var_call_write`
  after the existing W777 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py`.
- Validation: `cargo build --release -p t27c` green,
  ...
  PASS, 53/0 cocotb PASS, 0 seal mismatches.
- Refreshed weak-point audit and 2025-2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W778_2026-07-24.md` and
...
subject-line traceability rate can regress quickly when wave-loop activity
pauses, so keep monitoring it. Always fix the module header prefix after copying
a generator, keep the `make_grid(32768)` period-identity check because
...
## Worked example — Wave Loop 781

Wave Loop 781 extended the odd outer-dimension ladder to `[381][2]^6 Pt` with no
compiler changes, branching from `wave-loop-780` HEAD because earlier wave PRs
remained open or unstable:

- Generated `scripts/gen_w781.py` from `scripts/gen_w780.py` with `OUTER = 381`
  and `MID_IDX = 190`.
- Produced `specs/scratch/w781_bench_module_381x2p6_aos_var_call_write.t27`
...
- Added integration test `accepts_w781_bench_module_381x2p6_aos_var_call_write`
  after the existing W780 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- Fixed four actionable weak points discovered in the 2026-07-24 audit:
  `cli/flash-spi/src/main.rs` (new `FlashOpts` fields), `bootstrap/src/sensitivity.rs:126`
  (clippy `approx_constant`), `bootstrap/tests/bitnet_pipeline.rs:143` (IDLE-state
  drift), and `bootstrap/tests/bitnet_top.rs` (busy/mem drift).
- Documented the remaining `verilog_array_literal_expr` regression and 627 release
  warnings as separate cleanup work.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py` for the witness itself.
- Validation: `cargo build --release -p t27c` green,
  `cargo test -p t27c --bin t27c` 1494/0/2, `cargo test -p tri` 78/0,
  `cargo test -p flash-spi` 2/0, `cargo clippy -p t27c` green,
  ...
  PASS, 53/0 cocotb PASS, 0 seal mismatches.
- Refreshed weak-point audit and 2025-2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W781_2026-07-24.md` and
  `.claude/plans/wave-loop-782.md` with three cooperation variants.

Key learning: the mechanical generator discipline continues to be the cheapest
way to extend the non-power-of-two packed-vector ladder. A wave-loop closeout
is also a good moment to knock out small blocking test drifts, but deeper
compiler regressions should be split into their own issues rather than bundled
with a witness closeout.

## Worked example — Wave Loop 780

Wave Loop 780 extended the odd outer-dimension ladder to `[379][2]^6 Pt` with no
compiler changes, branching from `wave-loop-779` HEAD because PR #1484 (W774),
PR #1486 (W775), PR #1488 (W776), PR #1489 (README/W774-W776 merge),
PR #1491 (W777), PR #1493 (W778), and PR #1495 (W779) remained open or unstable:

- Generated `scripts/gen_w780.py` from `scripts/gen_w779.py` with `OUTER = 379`
  and `MID_IDX = 189`.
- Produced `specs/scratch/w780_bench_module_379x2p6_aos_var_call_write.t27`
...
- Added integration test `accepts_w780_bench_module_379x2p6_aos_var_call_write`
  after the existing W779 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py`.
- Validation: `cargo build --release -p t27c` green,
  ...
  PASS, 53/0 cocotb PASS, 0 seal mismatches.
- Refreshed weak-point audit and 2025-2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W780_2026-07-24.md` and
  `.claude/plans/wave-loop-781.md` with three cooperation variants.

Key learning: the mechanical generator discipline continues to be the cheapest
way to extend the non-power-of-two packed-vector ladder. The 30-day
subject-line traceability rate can vary sharply with wave-loop activity density,
so keep monitoring it. Always fix the module header prefix after copying
a generator, keep the `make_grid(32768)` period-identity check because
`32768 ≡ 0 (mod 32768)`, and use `assert_eq` on changed elements because
`assert_ne` is not emitted by the Icarus simulation path.

## Worked example — Wave Loop 779

Wave Loop 779 extended the odd outer-dimension ladder to `[377][2]^6 Pt` with no
compiler changes, branching from `wave-loop-778` HEAD because PR #1484 (W774),
PR #1486 (W775), PR #1488 (W776), PR #1489 (README/W774-W776 merge),
PR #1491 (W777), and PR #1493 (W778) remained open or unstable:

- Generated `scripts/gen_w779.py` from `scripts/gen_w778.py` with `OUTER = 377`
  and `MID_IDX = 188`.
- Produced `specs/scratch/w779_bench_module_377x2p6_aos_var_call_write.t27`
  (24,128 elements, 772,096-bit packed vector, ~0.737 MiBit).
- Added integration test `accepts_w779_bench_module_377x2p6_aos_var_call_write`
  after the existing W778 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py`.
- Validation: `cargo build --release -p t27c` green,
  ...
  PASS, 53/0 cocotb PASS, 0 seal mismatches.
- Refreshed weak-point audit and 2025-2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W779_2026-07-24.md` and
  `.claude/plans/wave-loop-780.md` with three cooperation variants.
...
## Worked example — Wave Loop 783

Wave Loop 783 extended the odd outer-dimension ladder to `[385][2]^6 Pt` with no
compiler changes, branching from `wave-loop-782` HEAD because earlier wave PRs
remained open awaiting review:

- Generated `scripts/gen_w783.py` from `scripts/gen_w782.py` with `OUTER = 385`
  and `MID_IDX = 192`.
- Produced `specs/scratch/w783_bench_module_385x2p6_aos_var_call_write.t27`
  (24,640 elements, 788,480-bit packed vector, ~0.752 MiBit).
- Added integration test `accepts_w783_bench_module_385x2p6_aos_var_call_write`
  after the existing W782 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- Fixed one actionable weak point from the 2026-07-24 audit:
  `bootstrap/tests/verilog_const_array.rs:166` now accepts any
  `TODO: array literal` or `TODO: struct literal` substring instead of stale exact
  TODO strings, so `cargo test -p t27c --test verilog_const_array` is green.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py` for the witness itself.
- Validation: `cargo build --release -p t27c` green,
  `cargo clippy -p t27c` green,
  `cargo test -p t27c --bin t27c` 1494/0/2, `cargo test -p tri` 78/0,
  ...
  PASS, 53/0 cocotb PASS, 0 seal mismatches.
- Refreshed weak-point audit and 2025-2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W783_2026-07-24.md` and
  `.claude/plans/wave-loop-784.md` with three cooperation variants.

Key learning: the mechanical generator discipline continues to be the cheapest
way to extend the non-power-of-two packed-vector ladder. A wave-loop closeout is
also a good moment to knock out small test-drift weak points, but deeper
compiler regressions should be split into their own issues rather than bundled
with a witness closeout. Always fix the module header prefix after copying a
generator, keep the `make_grid(32768)` period-identity check because
`32768 ≡ 0 (mod 32768)`, and use `assert_eq` on changed elements because
`assert_ne` is not emitted by the Icarus simulation path.
...
## Worked example — Wave Loop 793

Wave Loop 793 extended the odd outer-dimension ladder to `[405][2]^6 Pt` with no
compiler changes, branching from `wave-loop-792` HEAD because earlier wave PRs
remained open awaiting review:

- Generated `scripts/gen_w793.py` from `scripts/gen_w792.py` with `OUTER = 405`
  and `MID_IDX = 202`.
- Fixed the generator destination path from the copied
  `w793_bench_module_403x2p6_aos_var_call_write` string to
  `w793_bench_module_405x2p6_aos_var_call_write` before regenerating.
- Produced `specs/scratch/w793_bench_module_405x2p6_aos_var_call_write.t27`
  (25,920 elements, 829,440-bit packed vector, ~0.791 MiBit).
- Added integration test `accepts_w793_bench_module_405x2p6_aos_var_call_write`
  after the existing W792 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py` for the witness itself.
- Validation: `cargo build --release -p t27c` green,
  ...
  `cargo test -p t27c --test verilog_const_array` 2/0.
- Refreshed weak-point audit and 2025–2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W793_2026-07-24.md` and
  `.claude/plans/wave-loop-794.md` with three cooperation variants.

Key learning: the mechanical ladder is now 21 waves deep (W774–W793) with zero
compiler changes, confirming the packed-vector AoS lowering is robust up to at
least `[405][2]^6 Pt` (25,920 elements, ~0.791 MiBit). The generator copy hazard
now affects both the module header prefix inside the f-string and the destination
path after copying; both must be grepped for the old wave number and outer
dimension before the first generation. Parameterizing both the wave prefix and
the destination path inside the generator template would make the flow fully
mechanical. Continue keeping the `make_grid(32768)` period-identity check and use
`assert_eq` on changed elements because `assert_ne` is not emitted by the Icarus
simulation path.

## Worked example — Wave Loop 794

Wave Loop 794 extended the odd outer-dimension ladder to `[407][2]^6 Pt` with no
compiler changes, branching from `wave-loop-793` HEAD because earlier wave PRs
remained open awaiting review:

- Generated `scripts/gen_w794.py` from `scripts/gen_w793.py` with `OUTER = 407`
  and `MID_IDX = 203`.
- Fixed both the generator destination path and the module header f-string from
  stale `w793` / `405` references to `w794_bench_module_407x2p6_aos_var_call_write`
  before regenerating.
- Produced `specs/scratch/w794_bench_module_407x2p6_aos_var_call_write.t27`
  (26,048 elements, 833,536-bit packed vector, ~0.795 MiBit).
- Added integration test `accepts_w794_bench_module_407x2p6_aos_var_call_write`
  after the existing W793 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py` for the witness itself.
- Validation: `cargo build --release -p t27c` green,
  ...
  `cargo test -p t27c --test verilog_const_array` 2/0.
- Refreshed weak-point audit and 2025–2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W794_2026-07-24.md` and
  `.claude/plans/wave-loop-795.md` with three cooperation variants.

Key learning: the mechanical ladder is now 22 waves deep (W774–W794) with zero
compiler changes, confirming the packed-vector AoS lowering is robust up to at
least `[407][2]^6 Pt` (26,048 elements, ~0.795 MiBit). The generator copy hazard
...
## Worked example — Wave Loop 795

Wave Loop 795 continued the module-scope packed-array-of-struct ladder with no
compiler changes, branching from `wave-loop-794` HEAD because earlier wave PRs
remained open awaiting review:

- Generated `scripts/gen_w795.py` from `scripts/gen_w794.py` with `OUTER = 409`
  and `MID_IDX = 204`.
- Fixed both the generator destination path and the module header f-string from
  stale `w794` / `407` references to `w795_bench_module_409x2p6_aos_var_call_write`
  before regenerating.
- Produced `specs/scratch/w795_bench_module_409x2p6_aos_var_call_write.t27`
  (26,176 elements, 837,632-bit packed vector, ~0.799 MiBit).
- Added integration test `accepts_w795_bench_module_409x2p6_aos_var_call_write`
  after the existing W794 tests in `bootstrap/tests/icarus_lowerable.rs`.
- Ran `t27c parse`, `icarus-lowerable`, `icarus-simulate` (17 cycles, PASSED),
  `icarus-cocotb` (reference-model OK), and `t27c seal --save`.
- No changes to `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, or
  `scripts/cocotb_ref_model.py` for the witness itself.
- Validation: `cargo build --release -p t27c` green,
  `cargo clippy -p t27c` green (627 warnings, 0 errors),
  `cargo test -p t27c --bin t27c` 1494/0/2, `cargo test -p tri` 78/0,
  `cargo test -p flash-spi` 2/0,
  `cargo test -p t27c --test bitnet_pipeline` 20/0,
  `cargo test -p t27c --test bitnet_top` 17/0,
  ...
  `cargo test -p t27c --test verilog_const_array` 2/0.
- Refreshed weak-point audit and 2025–2026 ternary/MVL literature scan.
- Wrote `docs/reports/FPGA_LOOP_CLOSEOUT_W795_2026-07-24.md` and
  `.claude/plans/wave-loop-796.md` with three cooperation variants.

Key learning: the mechanical ladder is now 23 waves deep (W774–W795) with zero
compiler changes, confirming the packed-vector AoS lowering is robust up to at
least `[409][2]^6 Pt` (26,176 elements, ~0.799 MiBit). The generator copy hazard
continues to be the only manual failure mode, and it now spans two distinct text
locations (destination path + module header f-string). A single parameterized
wave-prefix variable in the generator template would eliminate both. Continue
grepping for stale wave numbers and outer dimensions after each copy, keep the
...
Key takeaway: the generator copy hazard (stale wave number and outer dimension
in both destination path and module header f-string) remains the only manual
failure mode across W782–W821. Parameterizing `WAVE` and `OUTER` in the
generator template would eliminate it.
...
| **Parent branch** | `wave-loop-821` HEAD because earlier wave PRs remain open |
| **Recommended variant** | A — module-scope `[463][2]^6 Pt` packed array-of-struct variable from call with indexed signed writes |
| **Status** | READY TO START |
| **Next wave variants queued** | W823 Variant A `[465][2]^6 Pt`; Variant B `[463][3]^6 Pt` stride scaling; Variant C `[463][2]^6 Pt` negative-index wrap-around |
