...
- the population is now named once, in one function, and both lists draw from it. Totals moved from `32 + 18` to **`32 + 122 = 154`**, and the **104** lines that appeared (`122 - 18`; 90 subtracted the counted bucket instead) are not new defects: they are what the first version was silently declining to mention
