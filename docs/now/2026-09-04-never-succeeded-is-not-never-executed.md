...
- **the path is resolved from the repository ROOT, and running the controls said so:** the first version used a relative path and refused from `cli/tri` with a safe and useless **2**, while the checker itself, run from `cli/`, still read all **7870** tracked files
- **a fifth control was written and withdrawn rather than claimed:** an arm returning 2 outside a work tree returned **1** when run from `/tmp`, because `now_gate` errors on `git rev-parse` first. Unreachable here, so it is ordinary defence and says so -- a comment claiming to be a control is worse than no comment
