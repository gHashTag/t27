...
- New gate `tools/check_catalog_integrity.py`: every `source=` resolves, the three prefix-sharing neighbours (`gfternary`, the binary `gf*` ladder, the `tef*` ladder) are each present **on disk** and distinct, and the former name is still there. Verified red on each of those failures individually, not just green on the happy path
  - **Correction (2026-08-23):** the sentence above claimed a verification that left no trace. The gate shipped with no control in it and none anywhere else, so "verified red on each of those failures individually" was a report of hand-work nobody could re-run -- indistinguishable, a fortnight later, from a gate that had only ever been seen green. It has now been made true rather than deleted: `--self-check` plants each failure and asserts the branch by its message, plus two end-to-end runs of the whole program. That last part earned its place -- with `main()` neutered to `return 0` the gate printed OK on a catalog with a dangling `source=`, and every per-branch case still reported "RED, correct branch"
