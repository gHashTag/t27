...
### Prop. 193 — search for the question, not for the shape you already found — `MEASURED`

**Gate:** `formal-yosys.yml` → *Delimiter balance scan — a spec whose delimiters cannot close*

Two members of one corruption family were found six waves apart, each after a
long chase of downstream symptoms:

| prop | shape | sites | how found |
|---|---|---|---|
| 186 | `bits : [[]Usize",` — `"` for `]` | 107 | 33 specs captured nothing; six waves of parser work |
| 192 | `x : [[]Const [,` — `[` for `]` | 18 | instrumenting a loop whose repair had "failed" twice |

Both were one generator writing a **wrong character**. Prop. 188's gate was then
written for the first shape — odd quote count — and could not see the second.
Prop. 192's addition was written for the second and cannot see a third.

**This gate asks the question both are instances of:** *can this file's
delimiters close at all?* That is decidable, costs one pass, and does not require
knowing which character the generator will corrupt next.

It immediately widened the family. The shape-specific scan found 18 field types
in **10** specs, all `[`. The question-shaped scan finds **18 specs and 21
class-level imbalances across three delimiter classes** — including 4 specs with
unbalanced parentheses and 4 with unbalanced braces, which no `[`-shaped search
could ever have returned. `runtime/instance.t27` is off by **ten** braces.

**Theorem (shape-search coverage).** Let a defect class `D` be generated by an
unknown corruption process, and let a scan `S_σ` match a syntactic shape `σ`
observed in some `d ∈ D`. Then `S_σ` covers exactly `{d ∈ D : σ ⊑ d}`, and the
residue `D \ S_σ` is *undetectable by construction* — not merely unfound. Since
each newly discovered `d'` yields a new `σ'` and a new gate, shape-search
converges to full coverage only after every member has been found by some other
means, at which point the gates are redundant. **A shape search cannot bound its
own residue; a question search can.**

Corollary, and the reason this is worth a proposition rather than a commit
message: *the existence of a green shape-specific gate is evidence about the
shape, never about the class.* Prop. 188's gate has been green since it landed.
It was green throughout the 18 sites Prop. 192 found.

**Honest scope.** A non-zero global balance is *necessary* evidence of a defect,
not sufficient proof: no construct in this language legitimately leaves a file
unbalanced, but the scanner's string/comment handling is a model of the lexer,
not the lexer. The 21 are therefore **ratcheted, not repaired**, and the baseline
stores file+class+delta triples rather than a count, so a file that swaps one
imbalance for another still fails.

Three bars, measured: **TRUE** — exits 0 on the corpus. **ALIVE** — 497 files
scanned, 18 reported, so the scan is not vacuous. **BITING** — a planted
`const X = [1, 2;` returns exit 1 naming the file and class; removing it returns
0.
