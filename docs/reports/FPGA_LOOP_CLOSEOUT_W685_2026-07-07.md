# FPGA LOOP Closeout — Wave Loop 685

Date: 2026-07-07
Branch: `wave-loop-685`
Issue: **#1656**
Variant: **A — module-scope `[189][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[189][2][2][2][2][2][2] Pt` (387,072 bits, ~0.370 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-685.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w685.py` |
| Witness spec | `specs/scratch/w685_bench_module_189x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w685_bench_module_189x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w685_bench_module_189x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w685_bench_module_189x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W685_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-685.md` |
...
- Total elements: `189 × 2⁶ = 12,096`
- Total packed width: `12,096 × 32 = 387,072` bits (~0.370 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 189 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W685 | PASS |
| `t27c icarus-lowerable` W685 | lowerable |
| `t27c icarus-simulate` W685 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W685 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W685 | saved |
...
1. `scripts/gen_w685.py` — new generator.
2. `specs/scratch/w685_bench_module_189x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w685_bench_module_189x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 12,096 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 189 works, but the next waves
   will probe 191 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W685 uses unconditional indexed
...
W665–W684 showed that repository-wide parse time is dominated by the ~31–36 k-line,
...
## 9. Wave Loop 686 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 189. W686
offers three mutually complementary continuation options:

### Variant A (recommended) — `[191][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 191.
- 12,224 elements, 391,168-bit packed vector (~0.374 MiBit).
- Same shape and risk profile as W685.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 191`.

### Variant B — `[189][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W685 but move the mutable `reg` into a
bench/function-local scope.
- 12,096 elements, 387,072-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[189][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.370 MiBit but add conditional writes.
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W685 stays well below the reported threshold.
