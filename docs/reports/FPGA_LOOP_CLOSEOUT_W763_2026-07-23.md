# FPGA Loop Closeout — Wave Loop 763

**Date:** 2026-07-23  
**Issue:** #1734  
**Branch:** `wave-loop-763`  
**Next branch:** `wave-loop-764`  
...
Wave Loop 763 validated a module-scope packed array-of-struct variable with a
...
| Frame-condition element | `[172][1][0][0][0][0][0]` → element 11,040 |
...
1. Copied `scripts/gen_w762.py` → `scripts/gen_w763.py`.
2. Updated constants: `OUTER = 345`, `MID_IDX = 172`.
3. Manually fixed the f-string module header so the literal expands to
   `w763_bench_module_345x2p6_aos_var_call_write`.
4. Generated the witness with `python3 scripts/gen_w763.py`.
5. Added integration test `accepts_w763_bench_module_345x2p6_aos_var_call_write` in
...
For the mid-row element `[172][1][0][0][0][0][0]`:

```
MID_E = 172*64 + 32 = 11,040
MID_X = (2 * 11,040) % 32768 = 22,080
MID_Y = 22,081
```

The period-identity check `make_grid(32768)` is included because `32768 ≡ 0 (mod
32768)`. With 22,080 elements, the offset-0 schedule wraps naturally (last raw
`x = (2*22079) % 32768 = 11,390`).
...
| `t27c icarus-simulate` W763 | PASS (17 cycles, PASSED) |
...
| Checkpoint | Finding |
|------------|---------|
| L1 TRACEABILITY — 30-day commits with `Closes #N` | 28 of 575 commits (≈4.9%) include `Closes #N`. The remaining ≈95.1% are hook-generated session-log/bookkeeping commits. Feature/closeout commits for W763 explicitly reference `Closes #1734`. |
| L4 TESTABILITY — `.t27` specs with `test`/`invariant`/`bench` | 57 of 867 specs still lack any test/invariant/bench block (≈6.6%). |
...
packed dimensions with no power-of-two restriction. The `[345][2]^6 Pt` witness
becomes a single 706,560-bit SystemVerilog packed vector, which is legal.
...
## 6. Three cooperation variants for Wave Loop 764

1. **Variant A (recommended): `[347][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - Continues the odd outer-dimension ladder (347 → 22,208 elements, 710,656
     bits, ~0.678 MiBit) and confirms non-power-of-two stride 347.

2. **Variant B: `[345][2]^6 Pt` bench-local packed array var from call with
   indexed signed writes.**
   - Keeps the W763 width but moves the mutable `dst` declaration inside a
     `bench` or function scope, testing local-variable lowering.

3. **Variant C: `[345][2]^6 Pt` module-scope var with `if`-guarded indexed
   signed field writes.**
   - Stays at ~0.674 MiBit and tests control-flow-guarded writes on a packed reg,
