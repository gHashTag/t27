...
while read -r f; do
  ...
  if echo "$log" | grep -q 'error: not yet implemented'; then
    # W586: a spec whose functions have no bodies is UNWRITTEN, not broken.
    printf '%s\tUNIMPLEMENTED\t0\n' "$f" >> "$OUT"
  elif echo "$log" | grep -qE '^[^ ]+\.zig:[0-9]+:[0-9]+: error:'; then
