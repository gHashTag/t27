# FPGA LOOP Closeout — Wave Loop 686

**Date:** 2026-07-07
**Issue:** #1657
**Branch:** `wave-loop-686`
**Next branch:** `wave-loop-687`
**Variant:** A — module-scope `[191][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 686 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **191**. The witness
`specs/scratch/w686_bench_module_191x2p6_aos_var_call_write.t27` declares a
391,168-bit (≈0.373 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [191][2][2][2][2][2][2] Pt`
- `pub const expected : [191][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [191][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **12,224**
- Packed width: **391,168 bits** (48,896 bytes)
...
- Mid-row index used for frame-condition check:
  `[95][1][0][0][0][0][0]` → `e = 95·64 + 32 = 6112` → `x = 12224`, `y = 12225`
- Last element index: `[190][1][1][1][1][1][1]` → `e = 12223` → `x = 24446`, `y = 24447`
...
- `scripts/gen_w686.py` — generator for the witness.
- `specs/scratch/w686_bench_module_191x2p6_aos_var_call_write.t27` — witness spec
  (~838 KB, ~36,351 lines).
- `.trinity/seals/scratch_w686_bench_module_191x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w686_bench_module_191x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w686_bench_module_191x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W686.
- `.claude/plans/wave-loop-686.md` — decomposed plan.
- `.trinity/experience.md` — appended W686 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W686_2026-07-07.md` — this report.
...
| `t27c parse` W686 | PASS |
| `t27c icarus-lowerable` W686 | `lowerable` |
| `t27c icarus-simulate` W686 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W686 | reference-model OK |
| `t27c seal --save` W686 | saved |
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W686
...
1. **Outer dimension 191 correctness.** Resolved by cocotb cross-check and
...
## Next Wave Loop 687 cooperation variants

1. **Variant A (recommended) — `[193][2]^6 Pt` module-scope var from a call with
   indexed signed writes.**
   395,264-bit packed vector, 12,352 elements, non-power-of-two outer dimension 193.
   Continues the odd outer-dimension ladder. **Recommended.**

2. **Variant B — `[191][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   391,168-bit packed vector, 12,224 elements. Tests the same non-p2 outer
   dimension in a bench/function scope rather than module scope.

3. **Variant C — `[191][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.373 MiBit and tests control-flow guarded indexed writes on a packed
   `reg`.
...
Wave Loop 686 closes #1657. The module-scope packed AoS odd outer-dimension ladder
now reaches 191 with zero compiler changes. Branch `wave-loop-687` is ready for
the next wave.
