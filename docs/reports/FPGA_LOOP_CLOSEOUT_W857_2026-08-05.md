# FPGA Loop Close-out — Wave Loop 857 (2026-08-05)
...
Wave Loop 857 continued the mechanical packed-vector ladder in the 1-MiBit range.
The witness is a module-scope `[533][2]^6 Pt` variable initialized from a function
...
- `scripts/gen_w857.py` — generator copied from `gen_w856.py` and verified for the
  recurring copy hazard (destination path, module header f-string, `MID_IDX`
  comment).
- `specs/scratch/w857_bench_module_533x2p6_aos_var_call_write.t27` — generated
  witness (101,331 lines, 2.34 MB).
...
- `bootstrap/tests/icarus_lowerable.rs` — added integration test
  `accepts_w857_bench_module_533x2p6_aos_var_call_write`.
...
W857 packed width is 1,091,584 bits (~1.042 MiBit). No backend limit was hit.
...
- Simulation cycle count stayed flat at **17 cycles**, matching W854/W855/W856.
...
- The hazard was prevented by grepping the three known stale locations before
  running `scripts/gen_w857.py`.
- All references were verified to read `w857`, `533`, and `266`.
...
## Cooperation variants for Wave Loop 858

- **A (recommended):** `[535][2]^6 Pt`, outer += 2, `MID_IDX = 267`.
  - 34,240 elements, 1,095,680 bits (~1.045 MiBit).
  - Continues the established mechanical increment.

- **B:** `[533][3]^6 Pt` — grow the second inner dimension to stress stride scaling.
  - 101,952 elements, 3,262,464 bits (~3.111 MiBit).
  - Deliberately much wider; useful as a width-probe that may convert to a
    negative-boundary witness if a backend limit is hit.

- **C:** `[533][2]^6 Pt` with negative-index writes to exercise wrap-around / signed-index lowering.
  - Keeps the W857 outer dimension but replaces some positive indices with
    negative ones to stress the lowerer's signed-index path.

## Seal hashes

```text
spec_hash=sha256:d95c6aafc3b4b3485e78127ad8336859e0ac27a2a8d9dd86a63c0f306edfc301
gen_hash_zig=sha256:8c01c29630107c8b8475b6f28c33011956c53cdb5a416ce49817387ab9acfced
gen_hash_verilog=sha256:1029671b377d7e5d6a27d2231f046587ccdbd40654eca0fe408938b519c2ef70
gen_hash_c=sha256:6a0b6de2a480c2fdf682f29a1b23f43611354c3ccc1efc32e1615992cc95650c
gen_hash_rust=sha256:ba06be88a7a616a95e85f481ab5055fa3b6d9216e8369e957a3918063adc03d1
```
