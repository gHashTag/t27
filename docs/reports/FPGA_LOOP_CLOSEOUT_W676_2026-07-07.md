# FPGA LOOP Closeout — Wave Loop 676

Date: 2026-07-07
Branch: `wave-loop-676`
Issue: **#1647**
Variant: **A — module-scope `[171][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[171][2][2][2][2][2][2] Pt` (350,208 bits, ~0.334 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-676.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w676.py` |
| Witness spec | `specs/scratch/w676_bench_module_171x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w676_bench_module_171x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w676_bench_module_171x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w676_bench_module_171x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W676_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-676.md` |
...
- Total elements: `171 × 2⁶ = 10,944`
- Total packed width: `10,944 × 32 = 350,208` bits (~0.334 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 171 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W676 | PASS |
| `t27c icarus-lowerable` W676 | lowerable |
| `t27c icarus-simulate` W676 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W676 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W676 | saved |
| `cargo test -p t27c --bin t27c` | 1494 passed; 0 failed; 2 ignored |
| `cargo test -p tri` | 78 passed; 0 failed |
| `cargo test -p t27c --test icarus_lowerable` | 136 passed; 0 failed |
...
1. `scripts/gen_w676.py` — new generator.
2. `specs/scratch/w676_bench_module_171x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w676_bench_module_171x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 10,944 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 171 works, but the next waves
   will probe 173 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W676 uses unconditional indexed
...
W665–W675 showed that repository-wide parse time is dominated by the ~31–33 k-line,
~730+ KB literal, and a 15-minute timeout typically only reaches the end of Phase 1
...
## 9. Wave Loop 677 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 171. W677
offers three mutually complementary continuation options:

### Variant A (recommended) — `[173][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 173.
- 11,072 elements, 354,304-bit packed vector (~0.338 MiBit).
- Same shape and risk profile as W676.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 173`.

### Variant B — `[171][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W676 but move the mutable `reg` into a
bench/function-local scope.
- 10,944 elements, 350,208-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[171][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.334 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 676 — module-scope [171][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1647`.
- Tracking commit: `docs(w676): Wave Loop 676 issue #1647 + branch wave-loop-676
