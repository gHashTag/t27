# FPGA Loop Closeout — Wave Loop 786

**Date:** 2026-07-24
**Issue:** #1501
**Branch:** `wave-loop-786`
**Parent:** `wave-loop-785` HEAD (`af5c29cca`)
**Cooperation variant:** A (recommended)
**Next wave:** `wave-loop-787`
...
ladder: a `[391][2]^6 Pt` variable initialized from a function call, exercised
with indexed signed field writes and `assert_eq` read-back. The witness is
800,768 bits (~0.763 MiBit), still well below the 4-MiBit packed-vector cliff,
...
### 2.1 Witness `[391][2]^6 Pt`

- Generator: `scripts/gen_w786.py` (copied from `scripts/gen_w785.py`, updated to
  `OUTER = 391`, `MID_IDX = 195`, and module prefix `w786_bench_module_391x2p6_aos_var_call_write`).
- Generated spec: `specs/scratch/w786_bench_module_391x2p6_aos_var_call_write.t27`
  (~1,714 KB, ~74,351 lines, 25,024 elements, 800,768-bit packed vector).
- Integration test: `accepts_w786_bench_module_391x2p6_aos_var_call_write` in
  `bootstrap/tests/icarus_lowerable.rs`.
- Frame-condition element: `[195][1][0][0][0][0][0]` → element
  `195*64 + 32 = 12,512`.
...
| `cargo test -p t27c --test icarus_lowerable` | 246 passed; 0 failed |
| `t27c parse` W786 | PASS |
| `t27c icarus-lowerable` W786 | PASS (`lowerable`) |
| `t27c icarus-simulate` W786 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W786 | PASS (`reference-model OK`) |
| `t27c seal --save` W786 | PASS |
...
5. **Open PR stack** — W774-W785 PRs remain open; W786 branched from
   `wave-loop-785` HEAD to keep the sequence unblocked.

### 4.3 Hygiene checks

- No secrets found in working tree or `.env.example` files.
- `icarus_lowerable` coverage: 246 tests pass.
- 57 of 892 `.t27` specs lack `test`/`invariant`/`bench` (≈6.39%).
...
## 6. Cooperation variants for Wave Loop 787
...
3. Set `OUTER = 393`, `MID_IDX = 196`, fix module prefix to
   `w787_bench_module_393x2p6_aos_var_call_write`.
...
7. Write closeout report and W788 cooperation variants.

**Why recommended:** keeps the established mechanical generator discipline, tests
non-power-of-two stride 393, and stays well under the 4-MiBit cliff.

### Variant B — `[391][2]^6 Pt` bench/function-scope packed var from call

Keep the W786 width but move the mutable `dst` declaration inside a `bench` or
function scope:

1. Use `scripts/gen_w786.py` with `OUTER = 391` but emit `dst` as a local var.
2. Verify local-variable packed-vector lowering and lifetime handling.
3. Keep the same mid-index / frame-condition element as W786 (`MID_IDX = 195`).
...
Stay at the W786 width and add conditional indexed signed field writes:

1. Generate a W786-shaped witness.
...
1. Open PR for `wave-loop-786` against `master` (or stack after earlier waves land).
2. Link PR body to issue #1501 with `Closes #1501`.
3. After merge, create `wave-loop-787` from `wave-loop-786` HEAD and execute
