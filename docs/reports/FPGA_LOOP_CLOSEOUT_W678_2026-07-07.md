# FPGA LOOP Closeout — Wave Loop 678

Date: 2026-07-07
Branch: `wave-loop-678`
Issue: **#1649**
Variant: **A — module-scope `[175][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[175][2][2][2][2][2][2] Pt` (358,400 bits, ~0.342 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-678.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w678.py` |
| Witness spec | `specs/scratch/w678_bench_module_175x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w678_bench_module_175x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w678_bench_module_175x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w678_bench_module_175x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W678_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-678.md` |
...
- Total elements: `175 × 2⁶ = 11,200`
- Total packed width: `11,200 × 32 = 358,400` bits (~0.342 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 175 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W678 | PASS |
| `t27c icarus-lowerable` W678 | lowerable |
| `t27c icarus-simulate` W678 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W678 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W678 | saved |
| `cargo test -p t27c --bin t27c` | 1494 passed; 0 failed; 2 ignored |
| `cargo test -p tri` | 78 passed; 0 failed |
| `cargo test -p t27c --test icarus_lowerable` | 138 passed; 0 failed |
...
1. `scripts/gen_w678.py` — new generator.
2. `specs/scratch/w678_bench_module_175x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w678_bench_module_175x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 11,200 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 175 works, but the next waves
   will probe 177 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W678 uses unconditional indexed
...
W665–W677 showed that repository-wide parse time is dominated by the ~31–34 k-line,
...
## 9. Wave Loop 679 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 175. W679
offers three mutually complementary continuation options:

### Variant A (recommended) — `[177][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 177.
- 11,328 elements, 362,496-bit packed vector (~0.346 MiBit).
- Same shape and risk profile as W678.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 177`.

### Variant B — `[175][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W678 but move the mutable `reg` into a
bench/function-local scope.
- 11,200 elements, 358,400-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[175][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.342 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 678 — module-scope [175][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1649`.
- Tracking commit: `docs(w678): Wave Loop 678 issue #1649 + branch wave-loop-678
