# FPGA Loop Closeout — Wave Loop 752

**Issue:** #1723
**Branch:** `wave-loop-752`
**Date:** 2026-07-23
**Variant:** A — module-scope `[323][2]^6 Pt` array-of-struct variable
initialized from a function call, with indexed signed field writes and read-back.

## Summary

Wave Loop 752 validated a module-scope `[323][2]^6 Pt` packed array-of-struct
variable, initialized from a function call, and exercised with indexed signed
field writes. The witness is 661,504 bits (~0.631 MiBit, 20,672 elements). No
...
| Packed vector width | 661,504 bits |
| Approximate size | ~0.631 MiBit |
| Generator script | `scripts/gen_w752.py` |
| Witness spec | `specs/scratch/w752_bench_module_323x2p6_aos_var_call_write.t27` |
| Spec lines | ~61,431 |
| Spec size | ~1,381 KB |
| Mid element | `[161][1][0][0][0][0][0]` (element 10,336) |
| Last element | `[322][1][1][1][1][1][1]` (element 20,671) |
| Last raw x | 41,342 (wraps modulo 32,768 → 8,574) |
| Last raw y | 41,343 (wraps modulo 32,768 → 8,575) |
...
- `t27c parse` W752: PASS.
- `t27c icarus-lowerable` W752: PASS (`lowerable`).
- `t27c icarus-simulate` W752: PASS (17 cycles, PASSED).
- `t27c icarus-cocotb` W752: PASS (reference-model OK).
- `t27c seal --save` W752: PASS (saved to `.trinity/seals/scratch_w752_bench_module_323x2p6_aos_var_call_write.json`).
- Empty Icarus baseline saved:
  `.trinity/icarus-baselines/specs/scratch/w752_bench_module_323x2p6_aos_var_call_write.t27.baseline`.

## What changed

- Added `scripts/gen_w752.py` (copied from `scripts/gen_w751.py`, `OUTER=323`, `MID_IDX=161`).
- Added witness spec `specs/scratch/w752_bench_module_323x2p6_aos_var_call_write.t27`.
- Added integration test `accepts_w752_bench_module_323x2p6_aos_var_call_write` in
  `bootstrap/tests/icarus_lowerable.rs`.
- Saved seal and empty Icarus baseline under `.trinity/`.
- Updated `.trinity/experience.md` and `.trinity/current-issue.md`.
- Created closeout plan `.claude/plans/wave-loop-752.md`.
...
- **L1 TRACEABILITY:** 519 commits in the last 30 days; 111 still lack `Closes #N`.
...
- 2025–2026 literature reinforces the packed-vector strategy:
  ...
  - **CNFET ternary logic cells for low-power VLSI** — STI/PTI/NTI ternary
    inverters using CNFET and MOSFET at 45 nm, reporting 20–30% lower PDP for
    CNFET variants (VLSID 2025).
...
## Cooperation variants for Wave Loop 753

1. **Variant A (recommended): `[325][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - 665,600-bit packed vector, 20,800 elements.
   - Continues the odd outer-dimension ladder and confirms non-p2 stride 325.
   - **Recommended.**

2. **Variant B: `[323][2]^6 Pt` bench-local (function-local) packed array var
   from call with indexed signed writes.**
   - Same width as W752 but tests the mutable `reg` declared inside a bench or
     function rather than at module scope.

3. **Variant C: `[323][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   - Stays at ~0.631 MiBit and tests control-flow-guarded writes on a packed reg.
...
- [x] W752 committed with `Closes #1723`; branch `wave-loop-753` created.
