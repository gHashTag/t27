# FPGA Loop Close-out — Wave Loop 864 (2026-08-05)
...
Wave Loop 864 continued the mechanical packed-vector ladder in the 1-MiBit range.
The witness is a module-scope `[547][2]^6 Pt` variable initialized from a function
call and exercised with indexed signed field writes.

| Metric | Value |
|--------|-------|
| Outer dimension | 547 (odd, non-power-of-two) |
| Inner shape | `[2]^6` = 64 elements/row |
| Element | `Pt { x : i16, y : i16 }` (32 bits) |
| Total elements | 35,008 |
| Packed vector width | 1,120,256 bits |
| Approximate size | ~1.068 MiBit |
| Simulator cycles | 17 |
...
- `scripts/gen_w864.py` — generator copied from `gen_w863.py` and verified for the
  recurring copy hazard (destination path, module header f-string, `MID_IDX`
  comment).
- `specs/scratch/w864_bench_module_547x2p6_aos_var_call_write.t27` — generated
  witness (103,991 lines, 2.29 MB).
- `.trinity/seals/scratch_w864_bench_module_547x2p6_aos_var_call_write.json` —
  seal saved by `t27c seal --save`.
- `bootstrap/tests/icarus_lowerable.rs` — added integration test
  `accepts_w864_bench_module_547x2p6_aos_var_call_write`.
...
| `cargo test --release --test icarus_lowerable` | 324 passed; 0 failed |
...
W864 packed width is 1,120,256 bits (~1.068 MiBit). No backend limit was hit.
...
- Simulation cycle count stayed flat at **17 cycles**, matching W854–W863.
...
- The hazard was prevented by grepping the three known stale locations before
  running `scripts/gen_w864.py`.
- All references were verified to read `w864`, `547`, and `273`.
...
|---------|-------|-------|-----------|----------|------|-------|---------|
| **A (recommended)** | `[549][2]^6 Pt` | 549 | 274 | 35,136 | 1,124,352 | ~1.072 | Mechanical outer += 2 increment. |
| **B** | `[547][3]^6 Pt` | 547 | — | 52,704 | 1,686,528 | ~1.607 | Grow the second inner dimension to stress stride scaling. |
| **C** | `[547][2]^6 Pt` (neg-index writes) | 547 | 273 | 35,008 | 1,120,256 | ~1.068 | Negative-index writes to exercise wrap-around addressing. |
...
1. Create issue #1674 and branch `wave-loop-865` from `wave-loop-864` HEAD.
2. Implement variant A (`[549][2]^6 Pt`, `MID_IDX = 274`).
