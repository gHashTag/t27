# Wave Loop 798 Closeout Report

**Date:** 2026-07-24
**Branch:** `wave-loop-798`
**Parent branch:** `wave-loop-797` HEAD (`8ca780630`)
**Issue:** #1525
**PR:** #1526
...
Wave Loop 798 extended the module-scope packed array-of-struct ladder to `[415][2]^6 Pt`.
The witness contains 26,560 elements flattened to a single 849,920-bit packed
SystemVerilog vector (~0.810 MiBit), still well under the 4-MiBit simulation cliff,
and required zero compiler, reference-model, or `FROZEN_HASH` changes.

## What landed

- `specs/scratch/w798_bench_module_415x2p6_aos_var_call_write.t27`
  - 26,560 elements, 849,920-bit packed vector (~0.810 MiBit).
  - Module-scope `pub var dst : [415][2]^6 Pt` initialized from a function call and
    exercised with indexed signed field writes.
  - `assert_eq` read-back in a `bench` block (Icarus path does not emit `assert_ne`).
- `scripts/gen_w798.py`
  - Generator for the W798 witness; `OUTER = 415`, `MID_IDX = 207`.
  - Both the destination path and the module header f-string were manually fixed
    after copying from W797 (generator copy hazard).
- `bootstrap/tests/icarus_lowerable.rs`
  - Added `accepts_w798_bench_module_415x2p6_aos_var_call_write`.
- `.trinity/seals/scratch_w798_bench_module_415x2p6_aos_var_call_write.json`
...
| `cargo test -p t27c --test icarus_lowerable` | 258 passed; 0 failed |
| `cargo test -p t27c --test verilog_const_array` | 2 passed; 0 failed |
| `t27c parse` W798 | PASS |
| `t27c icarus-lowerable` W798 | PASS (`lowerable`) |
| `t27c icarus-simulate` W798 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W798 | PASS (reference-model OK) |
| `t27c seal --save` W798 | PASS |

## Weak-point audit (2026-07-24)

- **Open PR ladder:** PRs #1362, #1364, #1369, #1372, #1373, #1375, #1378, #1382,
  ...
  #1508, #1510, #1512, #1514, #1516, #1518, #1520, #1522, #1524 remain open
...
- IEEE 1800-2017 §7.4.1/7.4.3 define packed-array width as the product of packed
  ...
  849,920-bit packed vector, which is legal SystemVerilog.
...
- **Variant A (recommended):** continue odd outer-dimension ladder with `[417][2]^6 Pt`
  (~0.814 MiBit, 26,688 elements, 854,016-bit packed vector). Zero compiler changes
  expected.
- **Variant B:** keep `[415][2]^6 Pt` width but move the packed var to bench/function
  scope to exercise function-local non-power-of-two packed arrays.
- **Variant C:** keep `[415][2]^6 Pt` width and add `if`-guarded indexed signed field
  writes to exercise control-flow + packed-vector writes.

## Artifacts

- Witness: `specs/scratch/w798_bench_module_415x2p6_aos_var_call_write.t27`
- Generator: `scripts/gen_w798.py`
- Seal: `.trinity/seals/scratch_w798_bench_module_415x2p6_aos_var_call_write.json`
- Report: `docs/reports/FPGA_LOOP_CLOSEOUT_W798_2026-07-24.md`
