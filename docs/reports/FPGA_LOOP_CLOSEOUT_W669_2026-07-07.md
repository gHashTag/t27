# FPGA LOOP Closeout — Wave Loop 669

Date: 2026-07-07
Branch: `wave-loop-669`
Issue: **#1640**
Variant: **A — module-scope `[157][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[157][2][2][2][2][2][2] Pt` (321,536 bits, ~0.307 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-669.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w669.py` |
| Witness spec | `specs/scratch/w669_bench_module_157x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w669_bench_module_157x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w669_bench_module_157x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w669_bench_module_157x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W669_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-669.md` |
...
- Total elements: `157 × 2⁶ = 10,048`
- Total packed width: `10,048 × 32 = 321,536` bits (~0.307 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 157 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W669 | PASS |
| `t27c icarus-lowerable` W669 | lowerable |
| `t27c icarus-simulate` W669 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W669 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W669 | saved |
...
1. `scripts/gen_w669.py` — new generator.
2. `specs/scratch/w669_bench_module_157x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w669_bench_module_157x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 10,048 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 157 works, but the next waves
   will probe 159 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W669 uses unconditional indexed
...
complete Phase 1 (Parse). The 29,891-line, 687 KB W669 literal dominates the
...
## 9. Wave Loop 670 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 157. W670
offers three mutually complementary continuation options:

### Variant A (recommended) — `[159][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 159.
- 10,176 elements, 325,632-bit packed vector (~0.311 MiBit).
- Same shape and risk profile as W669.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 159`.

### Variant B — `[157][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W669 but move the mutable `reg` into a
bench/function-local scope.
- 10,048 elements, 321,536-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[157][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.307 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 669 — module-scope [157][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1640`.
- Tracking commit: `docs(w669): Wave Loop 669 issue #1640 + branch wave-loop-669
...
Next: create branch `wave-loop-670`, choose variant, update
