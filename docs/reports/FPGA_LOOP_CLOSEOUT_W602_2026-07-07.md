# FPGA Wave Loop 602 Closeout Report
...
Wave Loop 602 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **23**:

- Type: `[23][2]^6 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **47,104 bits** (≈0.045 MiBit).
- Total elements: **1,472**.
...
path and the generic indexed field-write paths handled outer stride 23
correctly.
...
1. `specs/scratch/w602_bench_module_23x2p6_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [23][2]^6 Pt`
   - `pub const expected : [23][2]^6 Pt = make_grid(0);`
   - `pub var dst : [23][2]^6 Pt = make_grid(0);`
   - Test `module_var_23x2p6_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_23x2p6_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w602_bench_module_23x2p6_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w602_bench_module_23x2p6_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w602_bench_module_23x2p6_aos_var_call_write`.
5. `.claude/plans/wave-loop-602.md`.
6. `.trinity/current-issue.md` updated for W603 cooperation variants.
7. `.trinity/experience.md` updated with W602 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-602.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 62/0 (new W602 test) |
...
dimension-agnostic for odd outer dimensions through at least 23. Because the
...
`% 32768`, even though the offset-0 path for 1,472 elements does not trigger a
wrap.
