# FPGA Loop Closeout — Wave Loop 757

**Date:** 2026-07-23  
**Issue:** #1728  
**Branch:** `wave-loop-757`  
**Next branch:** `wave-loop-758`  
...
- Mode: module-scope `pub var dst : [333][2]^6 Pt = make_grid(...)`
...
| Metric | Value |
|--------|-------|
| Total elements | `333 * 64 = 21,312` |
| Packed vector width | `21,312 * 32 = 681,984` bits |
| Approximate size | ~0.651 MiBit |
| Mid index `MID_IDX` | `166` |
| Frame-condition element | `[166][1][0][0][0][0][0]` |
| Frame-condition element number | `166*64 + 32 = 10,656` |
| Last raw x at offset 0 | `42750` |
| Witness file size | ~1,456 KB / ~63,331 lines |
...
- Generator: `scripts/gen_w757.py`
- Witness: `specs/scratch/w757_bench_module_333x2p6_aos_var_call_write.t27`

Generator notes:
- Copied from `scripts/gen_w756.py`, updated `OUTER = 333` and `MID_IDX = 166`.
- The module header line uses an f-string with `{OUTER}`, so it must be manually
  verified after each copy. For W757 the header resolves to
  `module w757_bench_module_333x2p6_aos_var_call_write`.
...
| `t27c parse` W757 | PASS |
| `t27c icarus-lowerable` W757 | PASS (`lowerable`) |
| `t27c icarus-simulate` W757 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W757 | PASS (`reference-model OK`) |
| `t27c seal --save` W757 | PASS |
...
## 5. Weak-point audit (W757)

### 5.1 Commit / traceability (L1 TRACEABILITY)

- `git log --since='30 days ago' --oneline --no-merges`: 532 commits.
- Commits with `Closes #`: 426.
...
- No new shell scripts were added for W757.
...
- Zero diff for `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, and
  `scripts/cocotb_ref_model.py` on the `wave-loop-757` branch vs. `master`.

---

## 6. 2025–2026 ternary / MVL literature scan

Relevant papers and projects found during the W757 closeout:
...
| [TeLLMe: Energy-Efficient Ternary LLM Accelerator for Edge FPGAs](https://doi.org/10.48550/arxiv.2504.16266) | 2025 | AMD KV260 / Zynq UltraScale+ | 1.58-bit weights, 8-bit activations, up to 9.51 tok/s decoding under 7 W |
...
- `scripts/gen_w757.py`
- `specs/scratch/w757_bench_module_333x2p6_aos_var_call_write.t27`
- `bootstrap/tests/icarus_lowerable.rs` — test `accepts_w757_bench_module_333x2p6_aos_var_call_write`
- `.trinity/seals/scratch_w757_bench_module_333x2p6_aos_var_call_write.json`
- `.trinity/icarus-baselines/specs/scratch/w757_bench_module_333x2p6_aos_var_call_write.t27.baseline`
- `docs/reports/FPGA_LOOP_CLOSEOUT_W757_2026-07-23.md`
- `.claude/plans/wave-loop-757.md`
- `.trinity/experience.md` — W757 learning block prepended
- `.trinity/current-issue.md` — updated to Wave Loop 758 (#1729, `[335][2]^6 Pt`)

---

## 9. Next-wave cooperation variants (Wave Loop 758, #1729)

Three candidate shapes were drafted for the next loop. Variant A is recommended.

### Variant A — recommended
**`[335][2]^6 Pt` module-scope var from call with indexed signed field writes.**
- Continues the odd outer-dimension ladder and confirms non-power-of-two stride 335.
- Expected width: `21,440 * 32 = 686,080` bits (~0.655 MiBit).
- Reuses the W757 generator pattern; only `OUTER = 335` and `MID_IDX = 167` change.

### Variant B
**`[333][2]^6 Pt` bench-local packed array var from call with indexed signed writes.**
- Same packed width as W757 (~0.651 MiBit) but moves the mutable variable
...
### Variant C
**`[333][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes.**
- Keeps the W757 width and adds conditional control flow around the indexed
...
Wave Loop 757 closes #1728 with a zero-compiler-change witness at
`[333][2]^6 Pt` (~0.651 MiBit). All parse, lowerability, simulation, cocotb,
seal, and cargo conformance gates pass. The repository remains on track for
continued odd outer-dimension expansion in Wave Loop 758 (#1729, `[335][2]^6 Pt`).
