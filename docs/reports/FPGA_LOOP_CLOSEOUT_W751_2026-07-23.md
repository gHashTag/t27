# FPGA Loop Closeout — Wave Loop 751

**Issue:** #1722
**Branch:** `wave-loop-751`
**Date:** 2026-07-23
**Variant:** A — module-scope `[321][2]^6 Pt` array-of-struct variable
initialized from a function call, with indexed signed field writes and read-back.

## Summary

Wave Loop 751 validated a module-scope `[321][2]^6 Pt` packed array-of-struct
variable, initialized from a function call, and exercised with indexed signed
field writes. The witness is 657,408 bits (~0.627 MiBit, 20,544 elements). No
...
|--------|-------|
| Outer dimension | 321 |
| Total elements | 20,544 |
| Packed vector width | 657,408 bits |
| Approximate size | ~0.627 MiBit |
| Generator script | `scripts/gen_w751.py` |
| Witness spec | `specs/scratch/w751_bench_module_321x2p6_aos_var_call_write.t27` |
| Spec lines | ~61,051 |
| Spec size | ~1,405 KB |
| Mid element | `[160][1][0][0][0][0][0]` (element 10,272) |
| Last element | `[320][1][1][1][1][1][1]` (element 20,543) |
| Last raw x | 41,086 (wraps modulo 32,768 → 8,318) |
| Last raw y | 41,087 (wraps modulo 32,768 → 8,319) |
...
- `t27c parse` W751: PASS.
- `t27c icarus-lowerable` W751: PASS (`lowerable`).
- `t27c icarus-simulate` W751: PASS (17 cycles, PASSED).
- `t27c icarus-cocotb` W751: PASS (reference-model OK).
- `t27c seal --save` W751: PASS (saved to `.trinity/seals/scratch_w751_bench_module_321x2p6_aos_var_call_write.json`).
- Empty Icarus baseline saved:
  `.trinity/icarus-baselines/specs/scratch/w751_bench_module_321x2p6_aos_var_call_write.t27.baseline`.

## What changed

- Added `scripts/gen_w751.py` (copied from `scripts/gen_w750.py`, `OUTER=321`, `MID_IDX=160`).
- Added witness spec `specs/scratch/w751_bench_module_321x2p6_aos_var_call_write.t27`.
- Added integration test `accepts_w751_bench_module_321x2p6_aos_var_call_write` in
  `bootstrap/tests/icarus_lowerable.rs`.
- Saved seal and empty Icarus baseline under `.trinity/`.
- Updated `.trinity/experience.md` and `.trinity/current-issue.md`.
- Created closeout plan `.claude/plans/wave-loop-751.md`.
...
- **L1 TRACEABILITY:** 518 commits in the last 30 days; 111 still lack `Closes #N`.
...
## Cooperation variants for Wave Loop 752

1. **Variant A (recommended): `[323][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - 661,504-bit packed vector, 20,672 elements.
   - Continues the odd outer-dimension ladder and confirms non-p2 stride 323.
   - **Recommended.**

2. **Variant B: `[321][2]^6 Pt` bench-local (function-local) packed array var
   from call with indexed signed writes.**
   - Same width as W751 but tests the mutable `reg` declared inside a bench or
     function rather than at module scope.

3. **Variant C: `[321][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   - Stays at ~0.627 MiBit and tests control-flow-guarded writes on a packed reg.
...
- [x] W751 committed with `Closes #1722`; branch `wave-loop-752` created.
