# FPGA Wave Loop 603 Closeout Report

**Issue:** #1574  
**Branch:** `wave-loop-603`  
**Previous:** Wave Loop 602 (#1573, `wave-loop-602`)  
**Date:** 2026-07-07  
...
Wave Loop 603 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **25**:

- Type: `[25][2]^6 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **51,200 bits** (≈0.049 MiBit).
...
path and the generic indexed field-write paths handled outer stride 25
correctly.
...
|------------|----------------|------------|
| Outer dimension 25 untested at module scope | Stride-25 multiplication and row-major flattening must be correct end-to-end. | Added direct Icarus simulation and cocotb reference-model cross-check. |
| Element count below the modulo-wrap point | With 1,600 elements the offset-0 schedule `(2*e + offset) % 32768` never wraps (`max raw = 3,199`). Earlier large waves used `e_wrap = 16384` to assert wrap behavior. | Added an explicit shifted call `make_grid(32768)` and asserted its first/last elements equal `(offset + raw) % 32768`, proving the compiler and reference model both evaluate modulo correctly even when the offset-0 path does not wrap. |
...
1. `specs/scratch/w603_bench_module_25x2p6_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [25][2]^6 Pt`
   - `pub const expected : [25][2]^6 Pt = make_grid(0);`
   - `pub var dst : [25][2]^6 Pt = make_grid(0);`
   - Test `module_var_25x2p6_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_25x2p6_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w603_bench_module_25x2p6_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w603_bench_module_25x2p6_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w603_bench_module_25x2p6_aos_var_call_write`.
5. `.claude/plans/wave-loop-603.md`.
6. `.trinity/current-issue.md` updated for W604 cooperation variants.
7. `.trinity/experience.md` updated with W603 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-603.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 63/0 (new W603 test) |
...
dimension-agnostic for odd outer dimensions through at least 25. Because the
...
`% 32768`, even though the offset-0 path for 1,600 elements does not trigger a
wrap.
