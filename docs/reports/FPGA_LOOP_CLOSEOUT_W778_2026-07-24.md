# FPGA Loop Closeout — Wave Loop 778

**Date:** 2026-07-24
**Issue:** #1492
**Branch:** `wave-loop-778`
**Parent branch:** `wave-loop-777` HEAD (`0867846cf`)
**Next branch:** `wave-loop-779`
**Witness:** `specs/scratch/w778_bench_module_375x2p6_aos_var_call_write.t27`
**Generator:** `scripts/gen_w778.py`
**PR:** #1493
...
Wave Loop 778 validated a module-scope packed array-of-struct variable with a
non-power-of-two outer dimension:

- Shape: `[375][2]^6 Pt`
- Type: `pub struct Pt { x : i16, y : i16 }`
- Mode: module-scope `pub var dst : [375][2]^6 Pt = make_grid(...)`
...
|--------|-------|
| Outer dimension | 375 |
| Total elements | 375 × 64 = 24,000 |
| Packed vector width | 24,000 × 32 = 768,000 bits |
| Approximate size | ~0.733 MiBit |
| Mid index | `MID_IDX = 187` |
...
1. Created `wave-loop-778` from `wave-loop-777` HEAD because PR #1484 (W774),
   PR #1486 (W775), PR #1488 (W776), PR #1489 (README/W774-W776 merge), and
   PR #1491 (W777) remain open and/or unstable.
2. Copied `scripts/gen_w777.py` → `scripts/gen_w778.py`.
3. Updated constants: `OUTER = 375`, `MID_IDX = 187`.
4. Fixed the module header prefix to
   `w778_bench_module_375x2p6_aos_var_call_write`.
5. Generated the witness with `python3 scripts/gen_w778.py`.
6. Added integration test `accepts_w778_bench_module_375x2p6_aos_var_call_write`
...
For the mid-row element `[187][1][0][0][0][0][0]`:

```
MID_E = 187*64 + 32 = 12,000
MID_X = (2 * 12,000) % 32768 = 24,000
MID_Y = 24,001
```

The period-identity check `make_grid(32768)` is included because `32768 ≡ 0 (mod
32768)`. With 24,000 elements, the offset-0 schedule wraps naturally (last raw
`x = (2*23999) % 32768 = 15,230`).
...
| `t27c parse` W778 | PASS |
| `t27c icarus-lowerable` W778 | PASS (`lowerable`) |
| `t27c icarus-simulate` W778 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W778 | PASS (`reference-model OK`) |
...
| Checkpoint | Finding |
|------------|---------|
| L1 TRACEABILITY — 30-day commits with `Closes #N` / `Fixes #N` | **~17.8%** (13 of 73 commits). Regressed from the local wave-loop spike; the remote 30-day rate needs continuous monitoring. |
| L4 TESTABILITY — `.t27` specs with `test`/`invariant`/`bench` | 57 of 884 `.t27` files still lack any test/invariant/bench block (≈6.45%). One new scratch witness (W778) was added and contains both `test` and `bench` blocks. |
| L7 UNITY — `scripts/*.sh` on critical path | 19 shell scripts remain under `scripts/`; none were added this wave. |
| Open PRs in flight | **PR #1484 (W774), #1486 (W775), #1488 (W776), #1489 (README update), and #1491 (W777) remain OPEN.** W778 branched from `wave-loop-777` HEAD to avoid blocking on the merge gate. |
| PR #1489 merge blocker | `fpga-synthesis` fails with pre-existing Yosys error: `Static cast is only supported in SystemVerilog mode` at `build/fpga/generated/uart.v:31`. `--admin` merge rejected while required checks are red. Out of scope for W778. |
| Test drift: `bitnet_pipeline::sequencer_idle_arms_on_start` | **STILL PRESENT**: `bootstrap/tests/bitnet_pipeline.rs:143` expects exact string `IDLE: if(start) begin state<=RUN; ...` but actual `gen-layer-sequencer` output wraps it as `IDLE: begin done<=0; if(start) begin state<=RUN; ... end end`. This pre-existing test-vs-emitter drift is unrelated to the wave-loop ladder and causes `cargo test -p t27c --test bitnet_pipeline` to fail. |
| Worktree hygiene | Untracked stale W485 artefacts remain: `specs/scratch/w485_*.t27` (×3) and `.claude/plans/wave-loop-485.md`. These are unrelated to W778 and should be resolved on a dedicated W485 branch or removed. |
...
| `NOW.md` currency | `NOW.md` refreshed to reflect W778 landed / W779 next state. |
...
IEEE Std 1800-2017 remains the authoritative basis for the W778 witness: packed
arrays of structs and arbitrary-width packed vectors are defined in Clause 7
(Aggregate Data Types). The `[375][2]^6 Pt` shape flattens to a single
768,000-bit SystemVerilog packed vector, which is legal and simulator-portable
...
## 6. Three cooperation variants for Wave Loop 779

1. **Variant A (recommended): `[377][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - Continues the odd outer-dimension ladder (377 → 24,128 elements, 772,096
     bits, ~0.737 MiBit) and confirms non-power-of-two stride 377.

2. **Variant B: `[375][2]^6 Pt` bench-local packed array var from call with
   indexed signed writes.**
   - Keeps the W778 width but moves the mutable `dst` declaration inside a
     `bench` or function scope, testing local-variable lowering.

3. **Variant C: `[375][2]^6 Pt` module-scope var with `if`-guarded indexed
   signed field writes.**
   - Stays at ~0.733 MiBit and adds conditional indexed signed field writes,
