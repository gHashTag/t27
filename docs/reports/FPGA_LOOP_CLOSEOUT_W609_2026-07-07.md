# FPGA Wave Loop 609 Closeout Report
...
Wave Loop 609 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **37**:

- Type: `[37][2]^6 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **75,776 bits** (≈0.072 MiBit).
- Total elements: **2,368**.
...
path and the generic indexed field-write paths handled outer stride 37
correctly.
...
| Syntax compatibility with the lowerable subset | W606 showed that bench-local `mut dst` and compact struct literals can parse but produce invalid Verilog. | Reused the exact W605/W606/W607/W608 module-scope lowerable style: `pub var dst`, `pub const expected`, explicit array-type annotations, `.x = ...` field initializers, separate `test`/`bench` blocks. |

## What was added

1. `specs/scratch/w609_bench_module_37x2p6_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [37][2]^6 Pt`
   - `pub const expected : [37][2]^6 Pt = make_grid(0);`
   - `pub var dst : [37][2]^6 Pt = make_grid(0);`
   - Test `module_var_37x2p6_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_37x2p6_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w609_bench_module_37x2p6_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w609_bench_module_37x2p6_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w609_bench_module_37x2p6_aos_var_call_write`.
5. `.claude/plans/wave-loop-609.md`.
6. `.trinity/current-issue.md` updated for W610 cooperation variants.
7. `.trinity/experience.md` updated with W609 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-609.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 69/0 (new W609 test) |
...
dimension-agnostic for odd outer dimensions through at least 37. Because the
...
`% 32768`, even though the offset-0 path for 2,368 elements does not trigger a
wrap.

W609 further confirms the W606 lesson: when extending a working witness pattern,
