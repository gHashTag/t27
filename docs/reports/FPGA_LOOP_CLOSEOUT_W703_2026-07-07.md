# FPGA LOOP Closeout — Wave Loop 703

**Date:** 2026-07-07
**Issue:** #1674
**Branch:** `wave-loop-703`
**Next branch:** `wave-loop-704`
**Variant:** A — module-scope `[225][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 703 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **225**. The witness
`specs/scratch/w703_bench_module_225x2p6_aos_var_call_write.t27` declares a
460,800-bit (≈0.440 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [225][2][2][2][2][2][2] Pt`
- `pub const expected : [225][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [225][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **14,400**
- Packed width: **460,800 bits** (57,600 bytes)
...
- Mid-row index used for frame-condition check:
  `[112][1][0][0][0][0][0]` → `e = 112·64 + 32 = 7200` → `x = 14400`, `y = 14401`
- Last element index: `[224][1][1][1][1][1][1]` → `e = 14399` → `x = 28798`, `y = 28799`
...
- `scripts/gen_w703.py` — generator for the witness.
- `specs/scratch/w703_bench_module_225x2p6_aos_var_call_write.t27` — witness spec
  (~989 KB, 42,811 lines).
- `.trinity/seals/scratch_w703_bench_module_225x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w703_bench_module_225x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w703_bench_module_225x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W703.
- `.claude/plans/wave-loop-703.md` — decomposed plan.
- `.trinity/experience.md` — appended W703 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W703_2026-07-07.md` — this report.
...
| `t27c parse` W703 | PASS |
| `t27c icarus-lowerable` W703 | `lowerable` |
| `t27c icarus-simulate` W703 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W703 | reference-model OK |
| `t27c seal --save` W703 | saved |
...
- IEEE Std 1800-2017 §7.4.1/7.4.3: packed-array width is the product of packed
  dimensions, with no power-of-two restriction. The 460,800-bit vector emitted
  for W703 is legal SystemVerilog.
...
- cocotb PR #3608 / discussion #2933: Python reference models integrate cleanly
  with SystemVerilog benches via VCD probe comparison; the W703 cocotb check
  reuses this pattern.
...
|------|------------|--------|---------|
| Outer dimension 225 breaks layout math | Low | High | Did not occur; cocotb reference model agreed with simulation. |
...
## Next Wave Loop 704 cooperation variants
...
- The odd outer-dimension ladder (211, 213, 215, 217, 219, 221, 223, 225) is now
  established and dimension-agnostic; W704 can stride directly to 227 without
