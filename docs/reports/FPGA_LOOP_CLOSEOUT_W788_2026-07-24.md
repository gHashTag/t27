# FPGA / IGLA Wave Loop 788 Closeout Report
...
Wave Loop 788 extended the module-scope packed-array-of-struct ladder to
`[395][2]^6 Pt`. A module-level `pub var dst : [395][2]^6 Pt` is initialized
...
| File | Purpose |
|------|---------|
| `specs/scratch/w788_bench_module_395x2p6_aos_var_call_write.t27` | Witness spec (25,280 elements, 808,960-bit packed vector, ~0.771 MiBit) |
| `scripts/gen_w788.py` | Generator (`OUTER = 395`, `MID_IDX = 197`) |
| `.trinity/seals/scratch_w788_bench_module_395x2p6_aos_var_call_write.json` | Saved seal |
| `bootstrap/tests/icarus_lowerable.rs` | Integration test `accepts_w788_bench_module_395x2p6_aos_var_call_write` |
...
- Outer dimension `395` is non-power-of-two.
- Total elements: `395 × 64 = 25,280`.
- Packed vector width: `25,280 × 32 = 808,960` bits (~0.771 MiBit).
...
- The witness includes an explicit `make_grid(32768)` period-identity check
  ...
  25,280 elements (last raw `x = 15902`).
...
| `cargo test -p t27c --test icarus_lowerable` | 248 passed; 0 failed |
| `cargo test -p t27c --test verilog_const_array` | 2 passed; 0 failed |
| `t27c parse` W788 | PASS |
| `t27c icarus-lowerable` W788 | PASS (`lowerable`) |
| `t27c icarus-simulate` W788 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W788 | PASS (`reference-model OK`) |
| `t27c seal --save` W788 | PASS |

The `icarus_lowerable` integration-test count advanced from 247 (W787) to 248 (W788).
...
- Open PR stack W774-W787 still awaits review, so W788 was branched from `wave-loop-787` HEAD.
...
dimensions, with no power-of-two restriction. The W788 witness emits a single
808,960-bit packed vector, which is legal SystemVerilog. The continued use of a
non-power-of-two outer dimension (395) exercises t27's row-major flattening and
...
## 6. Three cooperation variants for Wave Loop 789

### Variant A — `[397][2]^6 Pt` module-scope var from call (recommended)

Continue the odd outer-dimension ladder. Expected 25,408 elements, 813,056-bit
packed vector (~0.775 MiBit), still well under the 4-MiBit cliff. This is the
...
Stay at W788 width and add conditional indexed signed field writes (e.g.
`if (index % 2 == 0) { dst[index] = ... }`). Tests control-flow emission for
packed reg writes.

**Recommendation:** Variant A. The width ladder has been stable for 14
consecutive waves (W774–W788) with zero compiler changes; continuing it is the
...
Wave Loop 788 closed successfully. The `[395][2]^6 Pt` module-scope packed
