...
# --- W846: this was a REPORT, not a gate ---------------------------------
# It ended in an unconditional sys.exit(0), so it reported 174 unsourced
# literals and passed. Thirteen iterations counted it among the green ones.
# A baseline makes it a gate WITHOUT declaring the current 210 a defect: what
# is there now is accepted, and anything NEW fails. That is the same contract
# check_withdrawn_live uses, and the same negative-test discipline applies --
# a checker that cannot fail is indistinguishable from one that has gone blind.
BASE = pathlib.Path(__file__).with_name("paper_numbers_baseline.txt")
uniq = sorted(set(missing))
if "--update-baseline" in sys.argv:
    BASE.write_text("\n".join(uniq) + ("\n" if uniq else ""))
    print(f"\nbaseline written: {len(uniq)} known unsourced literal(s)")
    sys.exit(0)
known = {l.strip() for l in BASE.read_text().splitlines() if l.strip()} if BASE.exists() else set()
new = [v for v in uniq if v not in known]
if new:
    print(f"\nFAIL: {len(new)} NEW unsourced literal(s) carrying a unit\n")
    for v in new:
        ctx = re.search(r'.{0,70}(?<![\w.])' + re.escape(v) + r'(?![\w]).{0,50}', paper_txt, re.S)
        c = " ".join(ctx.group(0).split()) if ctx else ""
        print(f"  {v:>12s}  {c[:100]}")
    sys.exit(1)
print(f"\nOK: no new unsourced literal ({len(known)} known)")
sys.exit(0)
