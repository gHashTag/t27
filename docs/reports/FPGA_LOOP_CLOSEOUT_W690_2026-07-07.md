# FPGA LOOP Closeout — Wave Loop 690

**Date:** 2026-07-07
**Issue:** #1661
**Branch:** `wave-loop-690`
**Next branch:** `wave-loop-691`
**Variant:** A — module-scope `[199][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 690 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **199**. The witness
`specs/scratch/w690_bench_module_199x2p6_aos_var_call_write.t27` declares a
407,552-bit (≈0.387 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [199][2][2][2][2][2][2] Pt`
- `pub const expected : [199][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [199][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **12,736**
- Packed width: **407,552 bits** (50,944 bytes)
...
- Mid-row index used for frame-condition check:
  `[99][1][0][0][0][0][0]` → `e = 99·64 + 32 = 6368` → `x = 12736`, `y = 12737`
- Last element index: `[198][1][1][1][1][1][1]` → `e = 12735` → `x = 25470`, `y = 25471`
...
- `scripts/gen_w690.py` — generator for the witness.
- `specs/scratch/w690_bench_module_199x2p6_aos_var_call_write.t27` — witness spec
  (~874 KB, ~37,871 lines).
- `.trinity/seals/scratch_w690_bench_module_199x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w690_bench_module_199x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w690_bench_module_199x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W690.
- `.claude/plans/wave-loop-690.md` — decomposed plan.
- `.trinity/experience.md` — appended W690 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W690_2026-07-07.md` — this report.
...
| `t27c parse` W690 | PASS |
| `t27c icarus-lowerable` W690 | `lowerable` |
| `t27c icarus-simulate` W690 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W690 | reference-model OK |
| `t27c seal --save` W690 | saved |
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W690
...
1. **Outer dimension 199 correctness.** Resolved by cocotb cross-check and
...
## Next Wave Loop 691 cooperation variants

1. **Variant A (recommended) — `[201][2]^6 Pt` module-scope var from a call with
   indexed signed writes.**
   411,648-bit packed vector, 12,864 elements, non-power-of-two outer dimension 201.
   Continues the odd outer-dimension ladder. **Recommended.**

2. **Variant B — `[199][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   407,552-bit packed vector, 12,736 elements. Tests the same non-p2 outer
   dimension in a bench/function scope rather than module scope.

3. **Variant C — `[199][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.387 MiBit and tests control-flow guarded indexed writes on a packed
   `reg`.
...
Wave Loop 690 closes #1661. The module-scope packed AoS odd outer-dimension ladder
now reaches 199 with zero compiler changes. Branch `wave-loop-691` is ready for
the next wave.
