# FPGA Wave Loop 600 Closeout Report
...
Wave Loop 600 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **19**:

- Type: `[19][2]^8 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **155,648 bits** (≈0.15 MiBit).
- Total elements: **4,864**.
...
path and the generic indexed field-write paths handled outer stride 19
correctly.
...
1. `specs/scratch/w600_bench_module_19x2p8_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [19][2]^8 Pt`
   - `pub const expected : [19][2]^8 Pt = make_grid(0);`
   - `pub var dst : [19][2]^8 Pt = make_grid(0);`
   - Test `module_var_19x2p8_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_19x2p8_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w600_bench_module_19x2p8_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w600_bench_module_19x2p8_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w600_bench_module_19x2p8_aos_var_call_write`.
5. `.claude/plans/wave-loop-600.md`.
6. `.trinity/current-issue.md` updated for W601 cooperation variants.
7. `.trinity/experience.md` updated with W600 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-600.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 60/0 (new W600 test) |
...
dimension-agnostic for odd outer dimensions through at least 19. Because the
...
`% 32768`, even though the offset-0 path for 4,864 elements does not trigger a
wrap.
