# FPGA Loop Closeout — Wave Loop 759

**Date:** 2026-07-23  
**Issue:** #1730  
**Branch:** `wave-loop-759`  
**Next branch:** `wave-loop-760`  
...
- Mode: module-scope `pub var dst : [337][2]^6 Pt = make_grid(...)`
...
| Metric | Value |
|--------|-------|
| Total elements | `337 * 64 = 21,568` |
| Packed vector width | `21,568 * 32 = 690,176` bits |
| Approximate size | ~0.659 MiBit |
| Mid index `MID_IDX` | `168` |
| Frame-condition element | `[168][1][0][0][0][0][0]` |
| Frame-condition element number | `168*64 + 32 = 10,784` |
| Last raw x at offset 0 | `43454` |
| Witness file size | ~1,474 KB / ~64,091 lines |
...
- Generator: `scripts/gen_w759.py`
- Witness: `specs/scratch/w759_bench_module_337x2p6_aos_var_call_write.t27`

Generator notes:
- Copied from `scripts/gen_w758.py`, updated `OUTER = 337` and `MID_IDX = 168`.
- The module header line uses an f-string with `{OUTER}`, so it must be manually
  verified after each copy. For W759 the header resolves to
  `module w759_bench_module_337x2p6_aos_var_call_write`.
...
| `t27c parse` W759 | PASS |
| `t27c icarus-lowerable` W759 | PASS (`lowerable`) |
| `t27c icarus-simulate` W759 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W759 | PASS (`reference-model OK`) |
| `t27c seal --save` W759 | PASS |
...
## 5. Weak-point audit (W759)

### 5.1 Commit / traceability (L1 TRACEABILITY)

- `git log --since='30 days ago' --oneline --no-merges`: 540 commits.
- Commits with `Closes #`: 434.
...
- No new shell scripts were added for W759.
...
- Zero diff for `bootstrap/src/compiler.rs`, `bootstrap/stage0/FROZEN_HASH`, and
  `scripts/cocotb_ref_model.py` on the `wave-loop-759` branch vs. `master`.

---

## 6. 2025–2026 ternary / MVL literature scan

Relevant papers and projects found during the W759 closeout:
...
| [TeLLMe v2: Efficient End-to-End Ternary LLM Prefill and Decode Accelerator](https://doi.org/10.48550/arxiv.2510.15926) | 2025 | AMD Kria KV260 | Table-lookup matmul, 25 tok/s decode / 143 tok/s prefill under 5 W |
| [TeLLMe: Energy-Efficient Ternary LLM Accelerator for Edge FPGAs](https://arxiv.org/pdf/2504.16266) | 2025 | AMD Kria KV260 | 1.58-bit weights, 8-bit activations, up to 9.51 tok/s decoding under 7 W |
...
| [Design of unbalanced 9:2 ternary encoder and 2:9 ternary decoder circuits in RRAM and CNTFET technology](https://doi.org/10.1002/cta.4022) | 2024 | CNTFET-RRAM | 9:2 encoder / 2:9 decoder with 62-94% delay/power/PDP reductions |
...
gHashTag/trinity, TeLLMe v2, TernaryCore) converging on zero-DSP, LUT-only
...
- `scripts/gen_w759.py`
- `specs/scratch/w759_bench_module_337x2p6_aos_var_call_write.t27`
- `bootstrap/tests/icarus_lowerable.rs` — test `accepts_w759_bench_module_337x2p6_aos_var_call_write`
- `.trinity/seals/scratch_w759_bench_module_337x2p6_aos_var_call_write.json`
- `.trinity/icarus-baselines/specs/scratch/w759_bench_module_337x2p6_aos_var_call_write.t27.baseline`
- `docs/reports/FPGA_LOOP_CLOSEOUT_W759_2026-07-23.md`
- `.claude/plans/wave-loop-759.md`
- `.trinity/experience.md` — W759 learning block prepended
- `.trinity/current-issue.md` — updated to Wave Loop 760 (#1731, `[339][2]^6 Pt`)

---

## 9. Next-wave cooperation variants (Wave Loop 760, #1731)

Three candidate shapes were drafted for the next loop. Variant A is recommended.

### Variant A — recommended
**`[339][2]^6 Pt` module-scope var from call with indexed signed field writes.**
- Continues the odd outer-dimension ladder and confirms non-power-of-two stride 339.
- Expected width: `21,696 * 32 = 694,272` bits (~0.663 MiBit).
- Reuses the W759 generator pattern; only `OUTER = 339` and `MID_IDX = 169` change.

### Variant B
**`[337][2]^6 Pt` bench-local packed array var from call with indexed signed writes.**
- Same packed width as W759 (~0.659 MiBit) but moves the mutable variable
...
### Variant C
**`[337][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes.**
- Keeps the W759 width and adds conditional control flow around the indexed
...
Wave Loop 759 closes #1730 with a zero-compiler-change witness at
`[337][2]^6 Pt` (~0.659 MiBit). All parse, lowerability, simulation, cocotb,
seal, and cargo conformance gates pass. The repository remains on track for
continued odd outer-dimension expansion in Wave Loop 760 (#1731, `[339][2]^6 Pt`).
