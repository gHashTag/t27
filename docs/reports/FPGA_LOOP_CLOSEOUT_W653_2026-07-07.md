# FPGA / Icarus Wave Loop 653 Closeout Report
...
Wave Loop 653 extended the module-scope packed array-of-struct odd outer-dimension
ladder from 123 (W652) to 125. The witness is a module-scope mutable `reg` of type
`[125][2]^6 Pt` initialized from a function call, then exercised with signed
...
- **Spec:** `specs/scratch/w653_bench_module_125x2p6_aos_var_call_write.t27`
- **Generator:** `scripts/gen_w653.py`
- **Size:** 256,000-bit packed vector, 8,000 scalar elements, ~0.244 MiBit.
- **Lines:** ~23,811 (multi-line W584 brace style).
- **Seal:** `.trinity/seals/scratch_w653_bench_module_125x2p6_aos_var_call_write.json`
  - `spec_hash`: `sha256:22fdca6f6bb7df6ebee9492ab71709f9be34f68fca24484f90df7b24d55ffbe7`
...
- `pub fn make_grid(offset : u16) -> [125][2][2][2][2][2][2] Pt`
  - 8,000 elements, each element numbered `e` from 0 to 7,999.
  - `x = (2*e + offset) % 32768`
  - `y = (2*e + offset + 1) % 32768`
- `pub const expected : [125][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [125][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_125x2p6_call_write`:
  - initial whole-array equality to `expected`;
  - first-element read (`0/1`);
  - last-element read `[124][1][1][1][1][1][1]` (`15998/15999`);
  - mid-row read `[62][1][0][0][0][0][0]` (`8000/8001`);
  - explicit modulo-wrap check via `make_grid(32768)`.
- `bench module_bench_125x2p6_call_write`:
...
The mid-row index is `125 // 2 = 62`. Element index for
`dst[62][1][0][0][0][0][0]` in row-major LSB-first layout is
`62*64 + 1*32 = 4000`, giving expected values `x = 8000`, `y = 8001`. The
...
| Parse | `t27c parse specs/scratch/w653_bench_module_125x2p6_aos_var_call_write.t27` | PASS |
...
| Icarus lowerable integration | `cargo test -p t27c --test icarus_lowerable` | **113/0** |
...
|---|------------|---------|--------|
| 1 | **First outer dimension 125** | The compiler and reference model correctly stride by 125 at the outer dimension. | Resolved, no compiler changes. |
| 2 | **Modulo-wrap regression signal** | With 8,000 elements the offset-0 schedule never wraps; explicit `make_grid(32768)` preserves the wrap path. | Resolved. |
| 3 | **Multi-line mega-literals** | W584-style multi-line brace splitting remains necessary and valid for the 6-D literal inside the 125× outer shape. | Resolved. |
| 4 | **Simulator capacity** | 0.244 MiBit is far below the 4-MiBit Icarus/Yosys cliff; simulation completes in 17 cycles and is interactive. | Resolved. |
...
| 6 | **Spec size** | ~545 KB, ~23,811 lines; generator extended W652 by two row blocks rather than rewriting the literal. | Resolved. |
| 7 | **`assert_ne` simulation gap** | `assert_ne` is accepted by `icarus-lowerable` but not emitted by the Icarus simulation path. W653 continues using `assert_eq` on changed elements. | Documented, unchanged. |
...
## 6. Next Wave Loop 654 cooperation variants
...
- Reuses the exact W653 module-scope lowerable style with two extra rows.
...
- Keeps the 256,000-bit vector and moves the mutable `reg` from module scope
...
- Feature: `feat(igla): Wave Loop 653 — module-scope [125][2]^6 Pt non-p2 AoS var from call with indexed signed writes` (`Closes #1624`).
- Tracking: `chore(trinity): record W653 session log and commit count`.

---

## 8. Sign-off

Wave Loop 653 is closed. All success criteria are met, FROZEN_HASH is stable,
and three cooperation variants are proposed for Wave Loop 654.
