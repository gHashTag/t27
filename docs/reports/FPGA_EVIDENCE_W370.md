...
No changes to `fpga/verilog/ternary_mac_demo_top.v` or the bitstream were required during Wave Loop 370 because:
- Wave Loop 370 fixed a `gen-verilog` parser defect (`const` ordering) in `bootstrap/src/compiler.rs`, not the emitted Verilog of already-generated designs.
- The bitstream target (`ternary_mac_demo_top`) does not depend on multiple `const` declarations, so the W370 fix has no effect on its output.
...
```bash
sudo target/release/dlc10 idcode
```
...
This is the same result as Waves 362–369. The host cannot enumerate the Xilinx DLC10 Platform Cable. Possible causes:
...
5. If re-synthesis is needed with W370-level Verilog fixes, regenerate the bitstream through the OpenXC7 or Vivado-in-Docker path and update this evidence file.
