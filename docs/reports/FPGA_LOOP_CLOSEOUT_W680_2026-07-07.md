# FPGA LOOP Closeout — Wave Loop 680

Date: 2026-07-07
Branch: `wave-loop-680`
Issue: **#1651**
Variant: **A — module-scope `[179][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[179][2][2][2][2][2][2] Pt` (366,592 bits, ~0.350 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-680.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w680.py` |
| Witness spec | `specs/scratch/w680_bench_module_179x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w680_bench_module_179x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w680_bench_module_179x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w680_bench_module_179x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W680_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-680.md` |
...
- Total elements: `179 × 2⁶ = 11,456`
- Total packed width: `11,456 × 32 = 366,592` bits (~0.350 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 179 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W680 | PASS |
| `t27c icarus-lowerable` W680 | lowerable |
| `t27c icarus-simulate` W680 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W680 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W680 | saved |
| `cargo test -p t27c --bin t27c` | 1494 passed; 0 failed; 2 ignored |
| `cargo test -p tri` | 78 passed; 0 failed |
| `cargo test -p t27c --test icarus_lowerable` | 140 passed; 0 failed |
...
1. `scripts/gen_w680.py` — new generator.
2. `specs/scratch/w680_bench_module_179x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w680_bench_module_179x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 11,456 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 179 works, but the next waves
   will probe 181 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W680 uses unconditional indexed
...
W665–W679 showed that repository-wide parse time is dominated by the ~31–35 k-line,
~760+ KB literal, and a 15-minute timeout typically only reaches the end of Phase 1
...
## 9. Wave Loop 681 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 179. W681
offers three mutually complementary continuation options:

### Variant A (recommended) — `[181][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 181.
- 11,584 elements, 370,688-bit packed vector (~0.354 MiBit).
- Same shape and risk profile as W680.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 181`.

### Variant B — `[179][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W680 but move the mutable `reg` into a
bench/function-local scope.
- 11,456 elements, 366,592-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[179][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.350 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 680 — module-scope [179][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1651`.
- Tracking commit: `docs(w680): Wave Loop 680 issue #1651 + branch wave-loop-680
