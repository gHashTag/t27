...
Wave Loop 795 extended the module-scope packed array-of-struct ladder to `[409][2]^6 Pt`.
The witness contains 26,176 elements flattened to a single 837,632-bit packed
SystemVerilog vector (~0.799 MiBit), still well under the 4-MiBit simulation cliff,
...
- `.trinity/seals/scratch_w795_bench_module_409x2p6_aos_var_call_write.json`
...
| `cargo test -p t27c --test icarus_lowerable` | 255 passed; 0 failed |
| `cargo test -p t27c --test verilog_const_array` | 2 passed; 0 failed |
| `t27c parse` W795 | PASS |
| `t27c icarus-lowerable` W795 | PASS (`lowerable`) |
| `t27c icarus-simulate` W795 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W795 | PASS (reference-model OK) |
| `t27c seal --save` W795 | PASS |

## Weak-point audit (2026-07-24)

- **Open PR ladder:** PRs #1484, #1486, #1488, #1489, #1491, #1493, #1494, #1496,
  #1498, #1500, #1502, #1504, #1506, #1508, #1510, #1512, #1514, #1516, #1518 remain
  open awaiting review. Continue branching from the previous wave HEAD.
...
- **Warning debt:** 627 release warnings and 780 clippy warnings need a dedicated
  cleanup sprint.
- **Vivado-in-Docker CI gap:** private image not yet published.
- **30-day traceability by subject:** ~9.4% (99/1058), down from earlier waves;
  continue putting `Closes #N` / `Fixes #N` in commit subjects.
...
- IEEE 1800-2017 §7.4.1/7.4.3 define packed-array width as the product of packed
  ...
  837,632-bit packed vector, which is legal SystemVerilog.
...
- Recent 2025–2026 ternary/MVL literature reinforces that flattening ternary
  ...
  - **Optimized Ternary Wallace Multiplier Using Ternary Excess-One Converter for
    FPGA Applications** (IEEE Discover 2025): approximate ternary Wallace multiplier
    for Xilinx FPGA, relevant to future ternary arithmetic lowering.
  - **Tlsys: A Synthesis Framework for Ternary Logic from RTL to CNFET-Based
    Gate-Level Netlist** (Chinese Journal of Electronics 2026): RTL-to-netlist
    ternary synthesis flow for 500k+ gate designs.
  - **Ternary VHDL: Simplifying the Design and Verification of Mixed-radix VLSI
    Circuits** (IEEE ISMVL 2026): VHDL library extension for mixed-radix design and
    verification.
  - **An RTL-Based General Synthesis Methodology for Device-Independent Ternary
    Logic Circuits** (IEEE Access 2025): Verilog-based ternary syntax and generic
    cell library, device-independent flow.

## Cooperation variants for Wave Loop 796

- **Variant A (recommended):** continue odd outer-dimension ladder with `[411][2]^6 Pt`
  (~0.803 MiBit, 26,304 elements, 841,728-bit packed vector). Zero compiler changes
  expected.
- **Variant B:** keep `[409][2]^6 Pt` width but move the packed var to bench/function
  scope to exercise function-local non-power-of-two packed arrays.
- **Variant C:** keep `[409][2]^6 Pt` width and add `if`-guarded indexed signed field
  writes to exercise control-flow + packed-vector writes.

## Artifacts

- Witness: `specs/scratch/w795_bench_module_409x2p6_aos_var_call_write.t27`
- Generator: `scripts/gen_w795.py`
- Seal: `.trinity/seals/scratch_w795_bench_module_409x2p6_aos_var_call_write.json`
