# FPGA Wave Loop 715 Closeout

**Date:** 2026-07-07
**Issue:** #1686
**Branch:** `wave-loop-715`
**Previous:** Wave Loop 714 (#1685, `wave-loop-714`)
**Witness:** `specs/scratch/w715_bench_module_249x2p6_aos_var_call_write.t27`

## What was validated

Wave Loop 715 extended the module-scope packed array-of-structs (AoS) odd
outer-dimension ladder to **249**. The witness declares:

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [249][2][2][2][2][2][2] Pt`
- `pub const expected : [249][2]^6 Pt = make_grid(0);`
- `pub var dst : [249][2]^6 Pt = make_grid(0);`
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W715 stays far below the reported threshold.
...
| # | Task | Result |
|---|------|--------|
| 1 | Update `.trinity/current-issue.md` for #1686 | Done |
| 2 | Create `.claude/plans/wave-loop-715.md` | Done |
| 3 | Create `scripts/gen_w715.py` and generate witness | Done |
| 4 | Add `accepts_w715_...` integration test | Done |
...
| 10 | Create `wave-loop-716` branch | Done |
...
- `t27c parse` W715: PASS.
- `t27c icarus-lowerable` W715: PASS (`lowerable`).
- `t27c icarus-simulate` W715: PASS (17 cycles, PASSED).
- `t27c icarus-cocotb` W715: PASS (`reference-model OK`).
- `t27c seal --save` W715: saved.
- `FROZEN_HASH`: unchanged `68a0b933c00ba5efd7facb5997f00880c3eecae55e6ac5e8cea2aee399b92adc`.

## Key learnings

- The odd outer-dimension ladder (3, 5, 7, ..., 247, 249) continues to work at
...
- At 0.496 MiBit, Icarus simulation still completes in 17 cycles and remains
  comfortably interactive.
- The explicit `make_grid(32768)` modulo-wrap check is still required because the
  offset-0 schedule `(2*e + offset) % 32768` does not wrap for 16,064 elements
  (max raw value 32,127).
...
1. **Variant A (recommended) — `[251][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
...
2. **Variant B — `[249][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   520,704-bit packed vector, 16,064 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[249][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.496 MiBit and tests that control-flow guarded indexed writes on a
...
Wave Loop 715 is complete. Issue #1686 closed. Branch `wave-loop-716` exists for
continued work. All invariant laws satisfied:
- L1 TRACEABILITY: commits include `Closes #1686`.
