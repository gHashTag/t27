# FPGA LOOP Closeout — Wave Loop 677

Date: 2026-07-07
Branch: `wave-loop-677`
Issue: **#1648**
Variant: **A — module-scope `[173][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[173][2][2][2][2][2][2] Pt` (354,304 bits, ~0.338 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-677.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w677.py` |
| Witness spec | `specs/scratch/w677_bench_module_173x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w677_bench_module_173x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w677_bench_module_173x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w677_bench_module_173x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W677_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-677.md` |
...
- Total elements: `173 × 2⁶ = 11,072`
- Total packed width: `11,072 × 32 = 354,304` bits (~0.338 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 173 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W677 | PASS |
| `t27c icarus-lowerable` W677 | lowerable |
| `t27c icarus-simulate` W677 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W677 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W677 | saved |
| `cargo test -p t27c --bin t27c` | 1494 passed; 0 failed; 2 ignored |
| `cargo test -p tri` | 78 passed; 0 failed |
| `cargo test -p t27c --test icarus_lowerable` | 137 passed; 0 failed |
...
1. `scripts/gen_w677.py` — new generator.
2. `specs/scratch/w677_bench_module_173x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w677_bench_module_173x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 11,072 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 173 works, but the next waves
   will probe 175 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W677 uses unconditional indexed
...
W665–W676 showed that repository-wide parse time is dominated by the ~31–33 k-line,
...
## 9. Wave Loop 678 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 173. W678
offers three mutually complementary continuation options:

### Variant A (recommended) — `[175][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 175.
- 11,200 elements, 358,400-bit packed vector (~0.342 MiBit).
- Same shape and risk profile as W677.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 175`.

### Variant B — `[173][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W677 but move the mutable `reg` into a
bench/function-local scope.
- 11,072 elements, 354,304-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[173][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.338 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 677 — module-scope [173][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1648`.
- Tracking commit: `docs(w677): Wave Loop 677 issue #1648 + branch wave-loop-677
