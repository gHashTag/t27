# FPGA LOOP Closeout — Wave Loop 701

**Date:** 2026-07-07
**Issue:** #1672
**Branch:** `wave-loop-701`
**Next branch:** `wave-loop-702`
**Variant:** A — module-scope `[221][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 701 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **221**. The witness
`specs/scratch/w701_bench_module_221x2p6_aos_var_call_write.t27` declares a
452,608-bit (≈0.432 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [221][2][2][2][2][2][2] Pt`
- `pub const expected : [221][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [221][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **14,144**
- Packed width: **452,608 bits** (56,576 bytes)
...
- Mid-row index used for frame-condition check:
  `[110][1][0][0][0][0][0]` → `e = 110·64 + 32 = 7072` → `x = 14144`, `y = 14145`
- Last element index: `[220][1][1][1][1][1][1]` → `e = 14143` → `x = 28286`, `y = 28287`
...
- `scripts/gen_w701.py` — generator for the witness.
- `specs/scratch/w701_bench_module_221x2p6_aos_var_call_write.t27` — witness spec
  (~971 KB, 42,051 lines).
- `.trinity/seals/scratch_w701_bench_module_221x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w701_bench_module_221x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w701_bench_module_221x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W701.
- `.claude/plans/wave-loop-701.md` — decomposed plan.
- `.trinity/experience.md` — appended W701 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W701_2026-07-07.md` — this report.
...
| `t27c parse` W701 | PASS |
| `t27c icarus-lowerable` W701 | `lowerable` |
| `t27c icarus-simulate` W701 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W701 | reference-model OK |
| `t27c seal --save` W701 | saved |
...
- IEEE Std 1800-2017 §7.4.1/7.4.3: packed-array width is the product of packed
  dimensions, with no power-of-two restriction. The 452,608-bit vector emitted
  for W701 is legal SystemVerilog.
...
- cocotb PR #3608 / discussion #2933: Python reference models integrate cleanly
  with SystemVerilog benches via VCD probe comparison; the W701 cocotb check
  reuses this pattern.
...
|------|------------|--------|---------|
| Outer dimension 221 breaks layout math | Low | High | Did not occur; cocotb reference model agreed with simulation. |
...
## Next Wave Loop 702 cooperation variants
...
- The odd outer-dimension ladder (211, 213, 215, 217, 219, 221) is now established
  and dimension-agnostic; W702 can stride directly to 223 without compiler
  risk assessment.
- At ~0.43 MiBit the witness is still far below the ~4-MiBit Icarus/Yosys comfort
