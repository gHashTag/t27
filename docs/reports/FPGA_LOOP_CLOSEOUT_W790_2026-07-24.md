# FPGA / IGLA Wave Loop 790 Closeout Report
...
Wave Loop 790 extended the module-scope packed-array-of-struct ladder to
`[399][2]^6 Pt`. A module-level `pub var dst : [399][2]^6 Pt` is initialized
...
| File | Purpose |
|------|---------|
| `specs/scratch/w790_bench_module_399x2p6_aos_var_call_write.t27` | Witness spec (25,536 elements, 817,152-bit packed vector, ~0.779 MiBit) |
| `scripts/gen_w790.py` | Generator (`OUTER = 399`, `MID_IDX = 199`) |
| `.trinity/seals/scratch_w790_bench_module_399x2p6_aos_var_call_write.json` | Saved seal |
| `bootstrap/tests/icarus_lowerable.rs` | Integration test `accepts_w790_bench_module_399x2p6_aos_var_call_write` |
...
- Outer dimension `399` is non-power-of-two.
- Total elements: `399 × 64 = 25,536`.
- Packed vector width: `25,536 × 32 = 817,152` bits (~0.779 MiBit).
...
- The witness includes an explicit `make_grid(32768)` period-identity check
  ...
  25,536 elements (last raw `x = 16126`).
...
| `t27c parse` W790 | PASS |
| `t27c icarus-lowerable` W790 | PASS (`lowerable`) |
| `t27c icarus-simulate` W790 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W790 | PASS (`reference-model OK`) |
| `t27c seal --save` W790 | PASS |

The `icarus_lowerable` integration-test count advanced from 249 (W789) to 250 (W790).
...
- Open PR stack W774-W789 still awaits review, so W790 was branched from `wave-loop-789` HEAD.

### Generator copy hazard — observed and fixed

The generator copy hazard struck again in W790:

- `scripts/gen_w790.py` line 77 initially contained `module w789_bench_module_{OUTER}x2p6_aos_var_call_write` because the hardcoded wave prefix inside the f-string was copied from W789.
- First generation produced a spec with module name `w789_bench_module_399x2p6_aos_var_call_write`.
- The prefix was corrected to `w790`, the witness was regenerated, and the correct seal path was produced.

This is the same hazard documented in W782–W789 learnings and remains the only
...
- 30-day commit traceability by subject: 0 of 87 commits carry `Closes #N`/`Fixes #N` (0.0%). This metric has dropped because recent wave commits place the closing reference in the commit body rather than the subject; it is a formatting signal, not a L1 violation, but it continues the downward trend noted since W774.
...
dimensions, with no power-of-two restriction. The W790 witness emits a single
817,152-bit packed vector, which is legal SystemVerilog. The continued use of a
non-power-of-two outer dimension (399) exercises t27's row-major flattening and
...
- **An Innovative Approach to Multi-Valued Logic**
  (IEEJ Trans. Electrical & Electronic Engineering, 2026,
  [PDF](https://iris.univr.it/retrieve/f58cef5d-364d-4e07-beb6-db6482d12b89/IEEJ%20Transactions%20Elec%20Engng%20-%202026%20-%20Mokhtari%20-%20An%20Innovative%20Approach%20to%20Multi%e2%80%90Valued%20Logic.pdf)) —
  decenary (10-valued) analog MVL family with Min/Max/Optimism/Pessimism operators;
  180 nm TSMC PSPICE proof-of-concept.
...
## 6. Three cooperation variants for Wave Loop 791

### Variant A — `[401][2]^6 Pt` module-scope var from call (recommended)

Continue the odd outer-dimension ladder. Expected 25,664 elements, 821,248-bit
packed vector (~0.783 MiBit), still well under the 4-MiBit cliff. This is the
...
Stay at W790 width and add conditional indexed signed field writes (e.g.
`if (index % 2 == 0) { dst[index] = ... }`). Tests control-flow emission for
packed reg writes.

**Recommendation:** Variant A. The width ladder has been stable for 17
consecutive waves (W774–W790) with zero compiler changes; continuing it is the
...
Wave Loop 790 closed successfully. The `[399][2]^6 Pt` module-scope packed
