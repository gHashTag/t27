...
Board flash is **blocked by missing DLC10 cable**, same as Waves 361–377. The QMTech Wukong V1 target and the in-tree Rust `dlc10` driver remain correct per `fpga/HARDWARE_SSOT.md`.

The W378 work did not change the RTL or bitstream path; it closed the last tracked `gen-verilog` syntax defect, expanded the in-runner yosys smoke gate to all 27 IGLA specs, and left the hardware path ready for the next cable availability window.
