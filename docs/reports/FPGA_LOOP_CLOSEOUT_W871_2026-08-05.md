# Wave Loop 871 — Closeout Report

**Date:** 2026-08-05  
**Issue:** [#1690](https://github.com/gHashTag/t27/issues/1690)  
**Branch:** `wave-loop-871`  
**Parent:** `wave-loop-870` HEAD (earlier waves' PRs remain open)  
**PR:** [#1692](https://github.com/gHashTag/t27/pull/1692)  
...
Wave Loop 871 continues the module-scope packed array-of-struct ladder, selecting
Variant A from the W871 plan:

```text
module-scope [561][2]^6 Pt variable from call with indexed signed writes
```

- `Pt { x : i16, y : i16 }` → 32 bits/element.
- Outer dimension `561` is odd and non-power-of-two, preserving the boundary-stress
  cadence established by W837+.
- Inner shape `[2]^6` = 64 elements/row.
- Total elements: `561 × 64 = 35,904`.
- Packed vector width: `35,904 × 32 = 1,148,928 bits` ≈ **1.096 MiBit**.

Artifacts produced:

| Artifact | Path | Notes |
|----------|------|-------|
| Generator | `scripts/gen_w871.py` | Copied from `gen_w870.py`; copy hazard fixed before first run (`w871`, `OUTER = 561`, `MID_IDX = 280`). |
| Spec | `specs/scratch/w871_bench_module_561x2p6_aos_var_call_write.t27` | 106,651 lines, ~2.3 MB. |
| Seal | `.trinity/seals/scratch_w871_bench_module_561x2p6_aos_var_call_write.json` | Saved by `t27c seal --save`. |
| Test | `bootstrap/tests/icarus_lowerable.rs` | `accepts_w871_bench_module_561x2p6_aos_var_call_write`. |
...
The practical landscape remains as observed in W837–W870:
...
**Interpretation:** W871's 1.096-MiBit vector is still comfortably below both the
...
edge case. No new limit appeared between W870 and W871.
...
**Interpretation for t27:** our generated internal vector (`1,148,928 bits`) is far wider
...
**Interpretation:** W871 is still on the "soft" side of the memory wall. The kernel
...
|------|----------------|--------|
| Parse | `t27c parse specs/scratch/w871_bench_module_561x2p6_aos_var_call_write.t27` | PASS |
...
| Targeted test | `cargo test --release --test icarus_lowerable -- accepts_w871_bench_module_561x2p6_aos_var_call_write` | 1/0 |
| Full suite | `cargo test --release --test icarus_lowerable` | **331/0** |
...
## 5. Cooperation variants for Wave Loop 872
...
| **B** | `[561][3]^6 Pt` | 561 | `[3]^6` | 53,760 | 1,720,320 | ~1.641 | Grow second inner dimension, stress stride scaling (~1.6 MiBit). |
...
1. Land W871 commit (`Closes #1690`) and open PR #1692 to `master`.
2. Create W872 issue #1691 and branch `wave-loop-872` from `wave-loop-871` HEAD.
3. Implement selected W872 variant per the standing charter.
