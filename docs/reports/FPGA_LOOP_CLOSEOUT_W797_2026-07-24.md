...
Wave Loop 797 extended the module-scope packed array-of-struct ladder to `[413][2]^6 Pt`.
The witness contains 26,432 elements flattened to a single 845,824-bit packed
SystemVerilog vector (~0.807 MiBit), still well under the 4-MiBit simulation cliff,
...
| `cargo test -p t27c --test icarus_lowerable` | 257 passed; 0 failed |
| `cargo test -p t27c --test verilog_const_array` | 2 passed; 0 failed |
| `t27c parse` W797 | PASS |
| `t27c icarus-lowerable` W797 | PASS (`lowerable`) |
| `t27c icarus-simulate` W797 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W797 | PASS (reference-model OK) |
| `t27c seal --save` W797 | PASS |
...
- **Open PR ladder:** PRs #1484, #1486, #1488, #1489, #1491, #1493, #1494, #1496,
  ...
  #1522 remain open awaiting review. Continue branching from the previous wave HEAD.
...
- IEEE 1800-2017 §7.4.1/7.4.3 define packed-array width as the product of packed
  ...
  845,824-bit packed vector, which is legal SystemVerilog.
...
- **Variant A (recommended):** continue odd outer-dimension ladder with `[415][2]^6 Pt`
  (~0.810 MiBit, 26,560 elements, 849,920-bit packed vector). Zero compiler changes
  expected.
- **Variant B:** keep `[413][2]^6 Pt` width but move the packed var to bench/function
  scope to exercise function-local non-power-of-two packed arrays.
- **Variant C:** keep `[413][2]^6 Pt` width and add `if`-guarded indexed signed field
  writes to exercise control-flow + packed-vector writes.

## Artifacts

- Witness: `specs/scratch/w797_bench_module_413x2p6_aos_var_call_write.t27`
- Generator: `scripts/gen_w797.py`
- Seal: `.trinity/seals/scratch_w797_bench_module_413x2p6_aos_var_call_write.json`
