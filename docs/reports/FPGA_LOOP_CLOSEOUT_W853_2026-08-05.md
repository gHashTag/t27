# FPGA Loop Close-out — Wave Loop 853 (2026-08-05)

**Issue:** #1646  
**Branch:** `wave-loop-853` (from `wave-loop-852` HEAD)  
**PR:** #1647  
**Variant:** A — module-scope `[525][2]^6 Pt` non-power-of-two outer-dimension array-of-struct variable from call with indexed signed writes.

## Summary

Wave Loop 853 continued the mechanical packed-vector ladder in the 1-MiBit range.
The witness is a module-scope `[525][2]^6 Pt` variable initialized from a function
call and exercised with indexed signed field writes.

| Metric | Value |
|--------|-------|
| Outer dimension | 525 (odd, non-power-of-two) |
| Inner shape | `[2]^6` = 64 elements/row |
| Element | `Pt { x : i16, y : i16 }` (32 bits) |
| Total elements | 33,600 |
| Packed vector width | 1,075,200 bits |
| Approximate size | ~1.026 MiBit |
| Simulator cycles | 17 |
...
- `scripts/gen_w853.py` — generator copied from `gen_w852.py` and verified for the
  recurring copy hazard (destination path, module header f-string, `MID_IDX`
  comment).
- `specs/scratch/w853_bench_module_525x2p6_aos_var_call_write.t27` — generated
  witness (99,811 lines, 2.31 MB).
...
- `bootstrap/tests/icarus_lowerable.rs` — added integration test
  `accepts_w853_bench_module_525x2p6_aos_var_call_write`.
...
| `cargo test --release --test icarus_lowerable` | 313 passed; 0 failed |
...
W853 packed width is 1,075,200 bits (~1.026 MiBit). No backend limit was hit.
...
- Simulation cycle count stayed flat at **17 cycles**, matching W850/W851/W852.
...
- The hazard was prevented by grepping the three known stale locations before
  running `scripts/gen_w853.py`.
- All references were verified to read `w853`, `525`, and `262`.
...
## Cooperation variants for Wave Loop 854

- **A (recommended):** `[527][2]^6 Pt`, outer += 2, `MID_IDX = 263`.
  - 33,728 elements, 1,079,296 bits (~1.030 MiBit).
  - Continues the established mechanical increment.

- **B:** `[525][3]^6 Pt` — grow the second inner dimension to stress stride scaling.
  - 100,656 elements, 3,220,992 bits (~3.073 MiBit).
  - Deliberately much wider; useful as a width-probe that may convert to a
    negative-boundary witness if a backend limit is hit.

- **C:** `[525][2]^6 Pt` with negative-index writes to exercise wrap-around / signed-index lowering.
  - Keeps the W853 outer dimension but replaces some positive indices with
    negative ones to stress the lowerer's signed-index path.

## Seal hashes

```text
spec_hash=sha256:448260199b4c8440e1ca4f4cbb876ca9fbe538ccca0f03f79176083d8e2cf5a8
gen_hash_zig=sha256:099d7ed439dbf456c9764c7c897cebe080a4e9e2130c374068d807e8ac66acaa
gen_hash_verilog=sha256:89a396013a72126e44b3ff9acdbf472a52c8227d4c6604734809dc7ffd0ef1d1
gen_hash_c=sha256:420a43532a982da4d2a92fd22b99bdf4d962eeabede7f3c670684b9f41a61770
gen_hash_rust=sha256:4779e63c876e095cb4b54ad6737eb0d1c9d20d12ba041628edcb99be77117966
```

## Next steps

1. Open PR #1647 to `master` with `Closes #1646`.
2. Create GitHub issue #1648 for Wave Loop 854.
3. Branch `wave-loop-854` from `wave-loop-853` HEAD and execute the selected
   W854 variant.
