# FPGA LOOP Closeout — Wave Loop 706

**Date:** 2026-07-07
**Issue:** #1677
**Branch:** `wave-loop-706`
**Next branch:** `wave-loop-707`
**Variant:** A — module-scope `[231][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 706 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **231**. The witness
`specs/scratch/w706_bench_module_231x2p6_aos_var_call_write.t27` declares a
473,088-bit (≈0.452 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [231][2][2][2][2][2][2] Pt`
- `pub const expected : [231][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [231][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **14,784**
- Packed width: **473,088 bits** (59,136 bytes)
...
- Mid-row index used for frame-condition check:
  `[115][1][0][0][0][0][0]` → `e = 115·64 + 32 = 7392` → `x = 14784`, `y = 14785`
- Last element index: `[230][1][1][1][1][1][1]` → `e = 14783` → `x = 29566`, `y = 29567`
...
- `scripts/gen_w706.py` — generator for the witness.
- `specs/scratch/w706_bench_module_231x2p6_aos_var_call_write.t27` — witness spec
  (~1,015 KB, 43,951 lines).
- `.trinity/seals/scratch_w706_bench_module_231x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w706_bench_module_231x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w706_bench_module_231x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W706.
- `.claude/plans/wave-loop-706.md` — decomposed plan.
- `.trinity/experience.md` — appended W706 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W706_2026-07-07.md` — this report.
...
| `t27c parse` W706 | PASS |
| `t27c icarus-lowerable` W706 | `lowerable` |
| `t27c icarus-simulate` W706 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W706 | reference-model OK |
| `t27c seal --save` W706 | saved |
...
- IEEE Std 1800-2017 §7.4.1/7.4.3: packed-array width is the product of packed
  dimensions, with no power-of-two restriction. The 473,088-bit vector emitted
  for W706 is legal SystemVerilog.
...
- cocotb PR #3608 / discussion #2933: Python reference models integrate cleanly
  with SystemVerilog benches via VCD probe comparison; the W706 cocotb check
  reuses this pattern.
...
|------|------------|--------|---------|
| Outer dimension 231 breaks layout math | Low | High | Did not occur; cocotb reference model agreed with simulation. |
...
## Next Wave Loop 707 cooperation variants
...
- The odd outer-dimension ladder (211, 213, 215, 217, 219, 221, 223, 225, 227,
  229, 231) is now established and dimension-agnostic; W707 can stride directly to
  233 without compiler risk assessment.
