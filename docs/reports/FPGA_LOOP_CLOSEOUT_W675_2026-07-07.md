# FPGA LOOP Closeout — Wave Loop 675

Date: 2026-07-07
Branch: `wave-loop-675`
Issue: **#1646**
Variant: **A — module-scope `[169][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[169][2][2][2][2][2][2] Pt` (346,112 bits, ~0.330 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-675.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w675.py` |
| Witness spec | `specs/scratch/w675_bench_module_169x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w675_bench_module_169x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w675_bench_module_169x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w675_bench_module_169x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W675_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-675.md` |
...
- Total elements: `169 × 2⁶ = 10,816`
- Total packed width: `10,816 × 32 = 346,112` bits (~0.330 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 169 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W675 | PASS |
| `t27c icarus-lowerable` W675 | lowerable |
| `t27c icarus-simulate` W675 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W675 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W675 | saved |
| `cargo test -p t27c --bin t27c` | 1494 passed; 0 failed; 2 ignored |
| `cargo test -p tri` | 78 passed; 0 failed |
| `cargo test -p t27c --test icarus_lowerable` | 135 passed; 0 failed |
...
1. `scripts/gen_w675.py` — new generator.
2. `specs/scratch/w675_bench_module_169x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w675_bench_module_169x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 10,816 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 169 works, but the next waves
   will probe 171 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W675 uses unconditional indexed
...
W665–W674 showed that repository-wide parse time is dominated by the ~31–32 k-line,
...
## 9. Wave Loop 676 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 169. W676
offers three mutually complementary continuation options:

### Variant A (recommended) — `[171][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 171.
- 10,944 elements, 350,208-bit packed vector (~0.334 MiBit).
- Same shape and risk profile as W675.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 171`.

### Variant B — `[169][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W675 but move the mutable `reg` into a
bench/function-local scope.
- 10,816 elements, 346,112-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[169][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.330 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 675 — module-scope [169][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1646`.
- Tracking commit: `docs(w675): Wave Loop 675 issue #1646 + branch wave-loop-675
