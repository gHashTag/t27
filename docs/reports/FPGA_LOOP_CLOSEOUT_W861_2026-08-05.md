# FPGA Loop Close-out — Wave Loop 861 (2026-08-05)

**Issue:** #1666  
**Branch:** `wave-loop-861` (from `wave-loop-860` HEAD)  
**PR:** TBD  
**Variant:** A — module-scope `[541][2]^6 Pt` non-power-of-two outer-dimension array-of-struct variable from call with indexed signed writes.

## Summary

Wave Loop 861 continued the mechanical packed-vector ladder in the 1-MiBit range.
The witness is a module-scope `[541][2]^6 Pt` variable initialized from a function
...
- `scripts/gen_w861.py` — generator copied from `gen_w860.py` and verified for the
  recurring copy hazard (destination path, module header f-string, `MID_IDX`
  comment).
- `specs/scratch/w861_bench_module_541x2p6_aos_var_call_write.t27` — generated
  witness (102,851 lines, 2.37 MB).
...
- `bootstrap/tests/icarus_lowerable.rs` — added integration test
  `accepts_w861_bench_module_541x2p6_aos_var_call_write`.
...
| `cargo test --release --test icarus_lowerable` | 321 passed; 0 failed |
...
W861 packed width is 1,107,968 bits (~1.056 MiBit). No backend limit was hit.
...
- Simulation cycle count stayed flat at **17 cycles**, matching W854–W860.
- The `.t27` parse dump is ~339 MB of AST text; this is an output-artifact cost,
...
- The hazard was prevented by grepping the three known stale locations before
  running `scripts/gen_w861.py`.
- All references were verified to read `w861`, `541`, and `270`.
...
## Cooperation variants for Wave Loop 862

| Variant | Shape | Outer | `MID_IDX` | Elements | Bits | MiBit | Purpose |
|---------|-------|-------|-----------|----------|------|-------|---------|
| **A (recommended)** | `[543][2]^6 Pt` | 543 | 271 | 34,752 | 1,112,064 | ~1.060 | Mechanical outer += 2 increment. |
| **B** | `[541][3]^6 Pt` | 541 | — | 51,936 | 1,661,952 | ~1.585 | Grow the second inner dimension to stress stride scaling. |
| **C** | `[541][2]^6 Pt` (neg-index writes) | 541 | 270 | 34,624 | 1,107,968 | ~1.056 | Negative-index writes to exercise wrap-around addressing. |
...
1. Create issue #1668 and branch `wave-loop-862` from `wave-loop-861` HEAD.
2. Implement variant A (`[543][2]^6 Pt`, `MID_IDX = 271`).
