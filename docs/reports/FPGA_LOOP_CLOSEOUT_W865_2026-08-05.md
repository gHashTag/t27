# Wave Loop 865 — Closeout Report

**Date:** 2026-08-05  
**Issue:** [#1678](https://github.com/gHashTag/t27/issues/1678)  
**Branch:** `wave-loop-865`  
**Parent:** `wave-loop-864` HEAD (earlier waves' PRs remain open)  
**PR:** #1679  
...
Wave Loop 865 is the next mechanical rung of the module-scope packed array-of-struct
ladder. We selected Variant A from the W865 plan:

```text
module-scope [549][2]^6 Pt variable from call with indexed signed writes
```

- `Pt { x : i16, y : i16 }` → 32 bits/element.
- Outer dimension `549` is odd and non-power-of-two, keeping the boundary-stress
  pattern established by W837+.
- Inner shape `[2]^6` = 64 elements/row.
- Total elements: `549 × 64 = 35,136`.
- Packed vector width: `35,136 × 32 = 1,124,352 bits` ≈ **1.072 MiBit**.

Artifacts produced:

| Artifact | Path | Notes |
|----------|------|-------|
| Generator | `scripts/gen_w865.py` | Copied from `gen_w864.py`; copy hazard fixed before first run. |
| Spec | `specs/scratch/w865_bench_module_549x2p6_aos_var_call_write.t27` | 104,371 lines, 2.41 MB. |
| Seal | `.trinity/seals/scratch_w865_bench_module_549x2p6_aos_var_call_write.json` | Saved by `t27c seal --save`. |
| Test | `bootstrap/tests/icarus_lowerable.rs` | `accepts_w865_bench_module_549x2p6_aos_var_call_write`. |
...
At **1.072 MiBit** we are still deep in the 1-MiBit neighborhood and far below the
...
|------|----------------|--------|
| Parse | `t27c parse specs/scratch/w865_bench_module_549x2p6_aos_var_call_write.t27` | PASS |
...
| Targeted test | `cargo test --release --test icarus_lowerable -- accepts_w865_bench_module_549x2p6_aos_var_call_write` | 1/0 |
| Full suite | `cargo test --release --test icarus_lowerable` | **325/0** |
...
## 5. Cooperation variants for Wave Loop 866

| Variant | Shape | Outer | Inner | Elements | Bits | MiBit | Purpose |
|---------|-------|-------|-------|----------|------|-------|---------|
| **A (recommended)** | `[551][2]^6 Pt` | 551 | `[2]^6` | 35,264 | 1,128,448 | ~1.076 | Continue mechanical `outer += 2` ladder. |
| **B** | `[549][3]^6 Pt` | 549 | `[3]^6` | 52,992 | 1,695,744 | ~1.615 | Grow second inner dimension, stress stride scaling. |
| **C** | `[549][2]^6 Pt` (neg-index writes) | 549 | `[2]^6` | 35,136 | 1,124,352 | ~1.072 | Negative-index / wrap-around writes. |
...
1. Land W865 commit (`Closes #1678`) and open PR to `master`.
2. Create W866 issue and branch `wave-loop-866` from `wave-loop-865` HEAD.
3. Implement selected W866 variant per the standing charter.
