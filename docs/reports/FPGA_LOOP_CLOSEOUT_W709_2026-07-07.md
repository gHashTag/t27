# FPGA Wave Loop 709 Closeout

**Date:** 2026-07-07
**Issue:** #1680
**Branch:** `wave-loop-709`
**Previous:** Wave Loop 708 (#1679, `wave-loop-708`)
**Witness:** `specs/scratch/w709_bench_module_237x2p6_aos_var_call_write.t27`

## What was validated

Wave Loop 709 extended the module-scope packed array-of-structs (AoS) odd
outer-dimension ladder to **237**. The witness declares:

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [237][2][2][2][2][2][2] Pt`
- `pub const expected : [237][2]^6 Pt = make_grid(0);`
- `pub var dst : [237][2]^6 Pt = make_grid(0);`
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W709 stays far below the reported threshold.
...
| # | Task | Result |
|---|------|--------|
| 1 | Update `.trinity/current-issue.md` for #1680 | Done |
| 2 | Create `.claude/plans/wave-loop-709.md` | Done |
| 3 | Create `scripts/gen_w709.py` and generate witness | Done |
| 4 | Add `accepts_w709_...` integration test | Done |
...
| 10 | Create `wave-loop-710` branch | Done |
...
- `t27c parse` W709: PASS.
- `t27c icarus-lowerable` W709: PASS (`lowerable`).
- `t27c icarus-simulate` W709: PASS (17 cycles, PASSED).
- `t27c icarus-cocotb` W709: PASS (`reference-model OK`).
- `t27c seal --save` W709: saved.
- `FROZEN_HASH`: unchanged `68a0b933c00ba5efd7facb5997f00880c3eecae55e6ac5e8cea2aee399b92adc`.

## Key learnings

- The odd outer-dimension ladder (3, 5, 7, ..., 235, 237) continues to work at
...
- At 0.461 MiBit, Icarus simulation still completes in 17 cycles and remains
  comfortably interactive.
- The explicit `make_grid(32768)` modulo-wrap check is still required because the
  offset-0 schedule `(2*e + offset) % 32768` does not wrap for 15,168 elements
  (max raw value 30,335).
...
1. **Variant A (recommended) — `[239][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
...
2. **Variant B — `[237][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   483,200-bit packed vector, 15,168 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[237][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.461 MiBit and tests that control-flow guarded indexed writes on a
...
Wave Loop 709 is complete. Issue #1680 closed. Branch `wave-loop-710` exists for
continued work. All invariant laws satisfied:
- L1 TRACEABILITY: commits include `Closes #1680`.
