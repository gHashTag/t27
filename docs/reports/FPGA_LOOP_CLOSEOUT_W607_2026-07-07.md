# FPGA Wave Loop 607 Closeout Report

**Issue:** #1578  
**Branch:** `wave-loop-607`  
**Previous:** Wave Loop 606 (#1577, `wave-loop-606`)  
**Date:** 2026-07-07  
...
Wave Loop 607 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **33**:

- Type: `[33][2]^6 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **67,584 bits** (≈0.064 MiBit).
- Total elements: **2,112**.
...
path and the generic indexed field-write paths handled outer stride 33
correctly.
...
|------------|----------------|------------|
| Outer dimension 33 untested at module scope | Stride-33 multiplication and row-major flattening must be correct end-to-end. | Added direct Icarus simulation and cocotb reference-model cross-check. |
| Element count below the modulo-wrap point | With 2,112 elements the offset-0 schedule `(2*e + offset) % 32768` never wraps (`max raw = 4,223`). Earlier large waves used `e_wrap = 16384` to assert wrap behavior. | Added an explicit shifted call `make_grid(32768)` and asserted its first/last elements equal `(offset + raw) % 32768`, proving the compiler and reference model both evaluate modulo correctly even when the offset-0 path does not wrap. |
...
| Syntax compatibility with the lowerable subset | W606 showed that bench-local `mut dst` and compact struct literals can parse but produce invalid Verilog. | Reused the exact W605/W606 module-scope lowerable style: `pub var dst`, `pub const expected`, explicit array-type annotations, `.x = ...` field initializers, separate `test`/`bench` blocks. |

## What was added

1. `specs/scratch/w607_bench_module_33x2p6_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [33][2]^6 Pt`
   - `pub const expected : [33][2]^6 Pt = make_grid(0);`
   - `pub var dst : [33][2]^6 Pt = make_grid(0);`
   - Test `module_var_33x2p6_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_33x2p6_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w607_bench_module_33x2p6_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w607_bench_module_33x2p6_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w607_bench_module_33x2p6_aos_var_call_write`.
5. `.claude/plans/wave-loop-607.md`.
6. `.trinity/current-issue.md` updated for W608 cooperation variants.
7. `.trinity/experience.md` updated with W607 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-607.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 67/0 (new W607 test) |
...
dimension-agnostic for odd outer dimensions through at least 33. Because the
...
`% 32768`, even though the offset-0 path for 2,112 elements does not trigger a
wrap.

W607 confirms the W606 lesson: when extending a working witness pattern, clone
the syntax of the last passing witness exactly. The lowerable subset has a
