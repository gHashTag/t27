# FPGA LOOP Closeout — Wave Loop 671

Date: 2026-07-07
Branch: `wave-loop-671`
Issue: **#1642**
Variant: **A — module-scope `[161][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[161][2][2][2][2][2][2] Pt` (329,728 bits, ~0.315 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-671.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w671.py` |
| Witness spec | `specs/scratch/w671_bench_module_161x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w671_bench_module_161x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w671_bench_module_161x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w671_bench_module_161x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W671_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-671.md` |
...
- Total elements: `161 × 2⁶ = 10,304`
- Total packed width: `10,304 × 32 = 329,728` bits (~0.315 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 161 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W671 | PASS |
| `t27c icarus-lowerable` W671 | lowerable |
| `t27c icarus-simulate` W671 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W671 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W671 | saved |
...
1. `scripts/gen_w671.py` — new generator.
2. `specs/scratch/w671_bench_module_161x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w671_bench_module_161x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 10,304 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 161 works, but the next waves
   will probe 163 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W671 uses unconditional indexed
...
W665–W670 showed that repository-wide parse time is dominated by the ~30 k-line,
...
## 9. Wave Loop 672 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 161. W672
offers three mutually complementary continuation options:

### Variant A (recommended) — `[163][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 163.
- 10,432 elements, 333,824-bit packed vector (~0.319 MiBit).
- Same shape and risk profile as W671.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 163`.

### Variant B — `[161][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W671 but move the mutable `reg` into a
bench/function-local scope.
- 10,304 elements, 329,728-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[161][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.315 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 671 — module-scope [161][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1642`.
- Tracking commit: `docs(w671): Wave Loop 671 issue #1642 + branch wave-loop-671
...
Next: create branch `wave-loop-672`, choose variant, update
