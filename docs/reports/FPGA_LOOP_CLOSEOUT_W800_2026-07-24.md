...
Wave Loop 800 extended the module-scope packed array-of-struct ladder to `[419][2]^6 Pt`.
The witness contains 26,816 elements flattened to a single 858,112-bit packed
SystemVerilog vector (~0.818 MiBit), still well under the 4-MiBit simulation cliff,
...
- `scripts/gen_w800.py`
  ...
  - Both the destination path and the module header f-string were manually fixed
    after copying from W799 (generator copy hazard).
...
- `.claude/skills/t27-wave-loop.md`
  - Added a live tracker section that is updated at the end of every Wave Loop.
...
| `cargo test -p t27c --test icarus_lowerable` | 260 passed; 0 failed |
| `cargo test -p t27c --test verilog_const_array` | 2 passed; 0 failed |
| `t27c parse` W800 | PASS |
| `t27c icarus-lowerable` W800 | PASS (`lowerable`) |
| `t27c icarus-simulate` W800 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W800 | PASS (reference-model OK) |
| `t27c seal --save` W800 | PASS |
...
- IEEE 1800-2017 §7.4.1/7.4.3 define packed-array width as the product of packed
  ...
  858,112-bit packed vector, which is legal SystemVerilog.
...
- **Variant A (recommended):** continue odd outer-dimension ladder with `[421][2]^6 Pt`
  (~0.822 MiBit, 26,944 elements, 862,208-bit packed vector). Zero compiler changes
  expected.
- **Variant B:** keep `[419][2]^6 Pt` width but move the packed var to bench/function
  scope to exercise function-local non-power-of-two packed arrays.
- **Variant C:** keep `[419][2]^6 Pt` width and add `if`-guarded indexed signed field
  writes to exercise control-flow + packed-vector writes.

## Artifacts

- Witness: `specs/scratch/w800_bench_module_419x2p6_aos_var_call_write.t27`
- Generator: `scripts/gen_w800.py`
- Seal: `.trinity/seals/scratch_w800_bench_module_419x2p6_aos_var_call_write.json`
