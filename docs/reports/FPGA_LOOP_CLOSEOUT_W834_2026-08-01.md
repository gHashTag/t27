# FPGA Loop Closeout — Wave Loop 834 (2026-08-01)
...
**PR:** #1609  
**Variant:** A — module-scope `[487][2]^6 Pt` packed array-of-struct variable from call with indexed signed writes
...
- Added generator `scripts/gen_w834.py` (copied from `scripts/gen_w833.py` with the
  recurring copy hazard fixed: destination path, module header f-string, and
  `MID_IDX` comment updated to `w834` / `487` / `243`).
...
- Added integration test `accepts_w834_bench_module_487x2p6_aos_var_call_write` to
...
| `cargo test -p t27c --test icarus_lowerable --release` | 294 passed; 0 failed |
| `t27c parse` W834 | PASS |
| `t27c icarus-lowerable` W834 | `lowerable` |
| `t27c icarus-simulate` W834 | `PASSED` (17 cycles) |
| `t27c icarus-cocotb` W834 | `reference-model OK` |
| `t27c seal --save` W834 | seal saved |
...
- IEEE 1800-2017 §7.4.1/7.4.3 define packed-array width as the product of packed
  ...
  997,376-bit packed vector, which is legal SystemVerilog.
...
## Cooperation variants for Wave Loop 835

- **A (recommended):** `[489][2]^6 Pt`, outer += 2, `MID_IDX = 244`.
  Smallest mechanical increment; expected 31,296 elements, 1,001,472 bits (~0.955 MiBit).
- **B:** `[487][3]^6 Pt` — grow the second inner dimension to stress stride scaling.
  Expected 355,023 elements, ~11.36 MiBit; may cross the 4-MiBit cliff or expose a
