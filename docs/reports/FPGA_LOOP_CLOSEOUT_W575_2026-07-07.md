# FPGA Loop Closeout — Wave Loop 575

**Issue:** #1546  
**Branch:** `wave-loop-575`  
**Variant:** A — 9-D array-of-struct return call deduplication  
**φ² + 1/φ² = 3 | TRINITY**
...
Wave Loop 575 extended the rank-agnostic packed-array-of-struct path from eight
dimensions to nine. A function returning `[2][2][2][2][2][2][2][2][2]Pt` (512
elements of 32-bit scalar struct `Pt`, total packed width 16,384 bits) was used
as a local initializer, as the base of indexed field accesses, and in whole-array
`assert_eq` sites.

The compiler and reference model required **no code changes**. Reusing the W574
witness-level workaround — binding the 9-D array literal to a local `expected`
variable before any whole-array `assert_eq` — kept the generated `$display` calls
free of 16,384-bit nested concatenations, and Icarus 12.0 accepted the resulting
16,384-bit flattened packed vector.
...
- Added `specs/scratch/w575_bench_9d_aos_call_dedup.t27` with the W574-style
  local-`expected` workaround for the Icarus `$display` buffer overflow.
- Saved t27 seal `.trinity/seals/scratch_w575_bench_9d_aos_call_dedup.json`.
- Saved Icarus baseline `.trinity/icarus-baselines/specs/scratch/w575_bench_9d_aos_call_dedup.json`.
- Added integration test `accepts_w575_bench_9d_aos_call_dedup` to
...
message. For the assertion comparing `make_nona()` against the 9-D literal, the
second operand would be a 16,384-bit, nine-level nested concatenation. Passing
that literal directly to `$display` would exercise the Icarus 12.0 VPI argument
buffer limit observed in W573/W574.
...
Both in `test nona_test` and `bench "nona_bench"`. The 9-D literal emission,
...
- `./scripts/tri test --icarus-lowerable --icarus-simulate --cocotb --fast`:
  ...
  - Seal verify: 679 passed, 0 mismatches
  - 24 pre-existing yosys smoke baseline failures unchanged
- Direct `./target/release/t27c icarus-simulate specs/scratch/w575_bench_9d_aos_call_dedup.t27`: PASS
- Direct `./target/release/t27c icarus-cocotb specs/scratch/w575_bench_9d_aos_call_dedup.t27`: PASS
...
- IEEE Std 1800-2017 clause 7.4.1 requires compliant tools to support packed
  ...
  receiver's implementation limits. A 16,384-bit flattened vector is one quarter
...
- Icarus 12.0 has an implementation-side VPI argument-formatting buffer limit
  ...
  W573/W574. t27's flattening strategy avoids Icarus's multi-D packed-array
...
- C++23 `std::mdspan` default `layout_right` row-major mapping generalizes to any
  rank. For nine dimensions t27 emits the canonical nested linear-index
  expression `((((((((((((((((i0*2+i1)*2+i2)*2+i3)*2+i4)*2+i5)*2+i6)*2+i7)*2+i8)*32 +: 16)`
...
- The rank-agnostic paths are real: nine dimensions, 512 elements, and 16,384
...
- Pre-compute expected indexed values with the documented row-major layout. For
  ...
  - `nona[0][1][0][1][1][1][1][1][1]` → linear element 191 → `x=382`
  - `nona[1][0][1][0][1][0][1][0][1]` → linear element 341 → `y=683`
...
## Three cooperation variants for Wave Loop 576
...
Create branch `wave-loop-576` from the W575 result and select one of the three
