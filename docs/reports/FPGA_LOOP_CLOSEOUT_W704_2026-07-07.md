# FPGA LOOP Closeout — Wave Loop 704

**Date:** 2026-07-07
**Issue:** #1675
**Branch:** `wave-loop-704`
**Next branch:** `wave-loop-705`
**Variant:** A — module-scope `[227][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 704 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **227**. The witness
`specs/scratch/w704_bench_module_227x2p6_aos_var_call_write.t27` declares a
464,896-bit (≈0.444 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [227][2][2][2][2][2][2] Pt`
- `pub const expected : [227][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [227][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **14,528**
- Packed width: **464,896 bits** (58,112 bytes)
...
- Mid-row index used for frame-condition check:
  `[113][1][0][0][0][0][0]` → `e = 113·64 + 32 = 7264` → `x = 14528`, `y = 14529`
- Last element index: `[226][1][1][1][1][1][1]` → `e = 14527` → `x = 29054`, `y = 29055`
...
- `scripts/gen_w704.py` — generator for the witness.
- `specs/scratch/w704_bench_module_227x2p6_aos_var_call_write.t27` — witness spec
  (~998 KB, 43,191 lines).
- `.trinity/seals/scratch_w704_bench_module_227x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w704_bench_module_227x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w704_bench_module_227x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W704.
- `.claude/plans/wave-loop-704.md` — decomposed plan.
- `.trinity/experience.md` — appended W704 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W704_2026-07-07.md` — this report.
...
| `t27c parse` W704 | PASS |
| `t27c icarus-lowerable` W704 | `lowerable` |
| `t27c icarus-simulate` W704 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W704 | reference-model OK |
| `t27c seal --save` W704 | saved |
...
- IEEE Std 1800-2017 §7.4.1/7.4.3: packed-array width is the product of packed
  dimensions, with no power-of-two restriction. The 464,896-bit vector emitted
  for W704 is legal SystemVerilog.
...
- cocotb PR #3608 / discussion #2933: Python reference models integrate cleanly
  with SystemVerilog benches via VCD probe comparison; the W704 cocotb check
  reuses this pattern.
...
|------|------------|--------|---------|
| Outer dimension 227 breaks layout math | Low | High | Did not occur; cocotb reference model agreed with simulation. |
...
## Next Wave Loop 705 cooperation variants

1. **Variant A (recommended):** `[229][2]^6 Pt` module-scope var from a call with
   indexed signed writes — 468,992 bits, 14,656 elements. Continue the ladder.
2. **Variant B:** `[227][2]^6 Pt` bench-local packed array var from a call with
   indexed signed writes — same size, different scope.
3. **Variant C:** `[227][2]^6 Pt` module-scope var with `if`-guarded indexed
...
- The odd outer-dimension ladder (211, 213, 215, 217, 219, 221, 223, 225, 227)
  is now established and dimension-agnostic; W705 can stride directly to 229
