# FPGA Wave Loop 716 Closeout

**Date:** 2026-07-07
**Issue:** #1687
**Branch:** `wave-loop-716`
**Previous:** Wave Loop 715 (#1686, `wave-loop-715`)
**Witness:** `specs/scratch/w716_bench_module_251x2p6_aos_var_call_write.t27`

## What was validated

Wave Loop 716 extended the module-scope packed array-of-structs (AoS) odd
outer-dimension ladder to **251**. The witness declares:

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [251][2][2][2][2][2][2] Pt`
- `pub const expected : [251][2]^6 Pt = make_grid(0);`
- `pub var dst : [251][2]^6 Pt = make_grid(0);`
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W716 stays far below the reported threshold.
...
| # | Task | Result |
|---|------|--------|
| 1 | Update `.trinity/current-issue.md` for #1687 | Done |
| 2 | Create `.claude/plans/wave-loop-716.md` | Done |
| 3 | Create `scripts/gen_w716.py` and generate witness | Done |
| 4 | Add `accepts_w716_...` integration test | Done |
...
| 10 | Create `wave-loop-717` branch | Done |
...
- `t27c parse` W716: PASS.
- `t27c icarus-lowerable` W716: PASS (`lowerable`).
- `t27c icarus-simulate` W716: PASS (17 cycles, PASSED).
- `t27c icarus-cocotb` W716: PASS (`reference-model OK`).
- `t27c seal --save` W716: saved.
- `FROZEN_HASH`: unchanged `68a0b933c00ba5efd7facb5997f00880c3eecae55e6ac5e8cea2aee399b92adc`.

## Key learnings

- The odd outer-dimension ladder (3, 5, 7, ..., 249, 251) continues to work at
...
- At 0.502 MiBit, Icarus simulation still completes in 17 cycles and remains
  comfortably interactive.
- The explicit `make_grid(32768)` modulo-wrap check is still required because the
  offset-0 schedule `(2*e + offset) % 32768` does not wrap for 16,176 elements
  (max raw value 32,351).
...
1. **Variant A (recommended) — `[253][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
...
2. **Variant B — `[251][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   526,976-bit packed vector, 16,176 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[251][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.502 MiBit and tests that control-flow guarded indexed writes on a
...
Wave Loop 716 is complete. Issue #1687 closed. Branch `wave-loop-717` exists for
continued work. All invariant laws satisfied:
- L1 TRACEABILITY: commits include `Closes #1687`.
