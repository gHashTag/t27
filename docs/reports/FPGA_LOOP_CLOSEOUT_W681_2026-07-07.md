# FPGA LOOP Closeout — Wave Loop 681

Date: 2026-07-07
Branch: `wave-loop-681`
Issue: **#1652**
Variant: **A — module-scope `[181][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[181][2][2][2][2][2][2] Pt` (370,688 bits, ~0.354 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-681.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w681.py` |
| Witness spec | `specs/scratch/w681_bench_module_181x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w681_bench_module_181x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w681_bench_module_181x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w681_bench_module_181x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W681_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-681.md` |
...
- Total elements: `181 × 2⁶ = 11,584`
- Total packed width: `11,584 × 32 = 370,688` bits (~0.354 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 181 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W681 | PASS |
| `t27c icarus-lowerable` W681 | lowerable |
| `t27c icarus-simulate` W681 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W681 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W681 | saved |
| `cargo test -p t27c --bin t27c` | 1494 passed; 0 failed; 2 ignored |
| `cargo test -p tri` | 78 passed; 0 failed |
| `cargo test -p t27c --test icarus_lowerable` | 141 passed; 0 failed |
...
1. `scripts/gen_w681.py` — new generator.
2. `specs/scratch/w681_bench_module_181x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w681_bench_module_181x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 11,584 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 181 works, but the next waves
   will probe 183 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W681 uses unconditional indexed
...
## 9. Wave Loop 682 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 181. W682
offers three mutually complementary continuation options:

### Variant A (recommended) — `[183][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 183.
- 11,712 elements, 374,784-bit packed vector (~0.358 MiBit).
- Same shape and risk profile as W681.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 183`.

### Variant B — `[181][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W681 but move the mutable `reg` into a
bench/function-local scope.
- 11,584 elements, 370,688-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[181][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.354 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 681 — module-scope [181][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1652`.
- Tracking commit: `docs(w681): Wave Loop 681 issue #1652 + branch wave-loop-681
