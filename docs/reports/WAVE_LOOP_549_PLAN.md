...
| **W5** | **IGLA spec vacuity.** `2160 / 3788` (**57.0 %**) of `test`/`bench` blocks under `specs/igla/**` contain nothing but `assert true`; `1917 / 3314` (**57.8 %**) of invariants are the literal tautology `true`. IGLA is 2160 of the 2165 vacuous tests and 1917 of the 1918 vacuous invariants in the entire `specs/` tree. Every wave loop appends the same 2 tests + 1 invariant to every IGLA spec. | vacuity scan | high — L4 TESTABILITY is satisfied in letter, void in spirit |
