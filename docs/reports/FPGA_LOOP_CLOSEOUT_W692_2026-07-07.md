# FPGA LOOP Closeout — Wave Loop 692

**Date:** 2026-07-07
**Issue:** #1663
**Branch:** `wave-loop-692`
**Next branch:** `wave-loop-693`
**Variant:** A — module-scope `[203][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 692 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **203**. The witness
`specs/scratch/w692_bench_module_203x2p6_aos_var_call_write.t27` declares a
415,744-bit (≈0.396 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [203][2][2][2][2][2][2] Pt`
- `pub const expected : [203][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [203][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **13,056**
- Packed width: **415,744 bits** (51,968 bytes)
...
- Mid-row index used for frame-condition check:
  `[101][1][0][0][0][0][0]` → `e = 101·64 + 32 = 6496` → `x = 12992`, `y = 12993`
- Last element index: `[202][1][1][1][1][1][1]` → `e = 13055` → `x = 26110`, `y = 26111`
...
- `scripts/gen_w692.py` — generator for the witness.
- `specs/scratch/w692_bench_module_203x2p6_aos_var_call_write.t27` — witness spec
  (~891 KB, ~38,631 lines).
- `.trinity/seals/scratch_w692_bench_module_203x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w692_bench_module_203x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w692_bench_module_203x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W692.
- `.claude/plans/wave-loop-692.md` — decomposed plan.
- `.trinity/experience.md` — appended W692 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W692_2026-07-07.md` — this report.
...
| `t27c parse` W692 | PASS |
| `t27c icarus-lowerable` W692 | `lowerable` |
| `t27c icarus-simulate` W692 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W692 | reference-model OK |
| `t27c seal --save` W692 | saved |
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W692
...
1. **Outer dimension 203 correctness.** Resolved by cocotb cross-check and
...
## Next Wave Loop 693 cooperation variants

1. **Variant A (recommended) — `[205][2]^6 Pt` module-scope var from a call with
   indexed signed writes.**
   419,840-bit packed vector, 13,248 elements, non-power-of-two outer dimension 205.
   Continues the odd outer-dimension ladder. **Recommended.**

2. **Variant B — `[203][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   415,744-bit packed vector, 13,056 elements. Tests the same non-p2 outer
   dimension in a bench/function scope rather than module scope.

3. **Variant C — `[203][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.396 MiBit and tests control-flow guarded indexed writes on a packed
   `reg`.
...
Wave Loop 692 closes #1663. The module-scope packed AoS odd outer-dimension ladder
now reaches 203 with zero compiler changes. Branch `wave-loop-693` is ready for
the next wave.
