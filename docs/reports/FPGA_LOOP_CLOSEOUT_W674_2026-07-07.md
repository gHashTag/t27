# FPGA LOOP Closeout — Wave Loop 674

Date: 2026-07-07
Branch: `wave-loop-674`
Issue: **#1645**
Variant: **A — module-scope `[167][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[167][2][2][2][2][2][2] Pt` (342,016 bits, ~0.327 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-674.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w674.py` |
| Witness spec | `specs/scratch/w674_bench_module_167x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w674_bench_module_167x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w674_bench_module_167x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w674_bench_module_167x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W674_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-674.md` |
...
- Total elements: `167 × 2⁶ = 10,688`
- Total packed width: `10,688 × 32 = 342,016` bits (~0.327 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 167 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W674 | PASS |
| `t27c icarus-lowerable` W674 | lowerable |
| `t27c icarus-simulate` W674 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W674 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W674 | saved |
...
1. `scripts/gen_w674.py` — new generator.
2. `specs/scratch/w674_bench_module_167x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w674_bench_module_167x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 10,688 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 167 works, but the next waves
   will probe 169 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W674 uses unconditional indexed
...
W665–W673 showed that repository-wide parse time is dominated by the ~31–32 k-line,
...
## 9. Wave Loop 675 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 167. W675
offers three mutually complementary continuation options:

### Variant A (recommended) — `[169][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 169.
- 10,816 elements, 346,112-bit packed vector (~0.330 MiBit).
- Same shape and risk profile as W674.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 169`.

### Variant B — `[167][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W674 but move the mutable `reg` into a
bench/function-local scope.
- 10,688 elements, 342,016-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[167][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.327 MiBit but add conditional writes.
...
- Feature commit: `feat(igla): Wave Loop 674 — module-scope [167][2]^6 Pt
  non-power-of-two outer-dimension array-of-struct variable from a call with
  indexed signed writes`
  - Body must include `Closes #1645`.
- Tracking commit: `docs(w674): Wave Loop 674 issue #1645 + branch wave-loop-674
...
Next: create branch `wave-loop-675`, choose variant, update
