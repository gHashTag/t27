# FPGA LOOP Closeout — Wave Loop 688

**Date:** 2026-07-07
**Issue:** #1659
**Branch:** `wave-loop-688`
**Next branch:** `wave-loop-689`
**Variant:** A — module-scope `[195][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 688 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **195**. The witness
`specs/scratch/w688_bench_module_195x2p6_aos_var_call_write.t27` declares a
399,360-bit (≈0.380 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [195][2][2][2][2][2][2] Pt`
- `pub const expected : [195][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [195][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **12,480**
- Packed width: **399,360 bits** (49,920 bytes)
...
- Mid-row index used for frame-condition check:
  `[97][1][0][0][0][0][0]` → `e = 97·64 + 32 = 6240` → `x = 12480`, `y = 12481`
- Last element index: `[194][1][1][1][1][1][1]` → `e = 12479` → `x = 24958`, `y = 24959`
...
- `scripts/gen_w688.py` — generator for the witness.
- `specs/scratch/w688_bench_module_195x2p6_aos_var_call_write.t27` — witness spec
  (~856 KB, ~37,111 lines).
- `.trinity/seals/scratch_w688_bench_module_195x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w688_bench_module_195x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w688_bench_module_195x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W688.
- `.claude/plans/wave-loop-688.md` — decomposed plan.
- `.trinity/experience.md` — appended W688 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W688_2026-07-07.md` — this report.
...
| `t27c parse` W688 | PASS |
| `t27c icarus-lowerable` W688 | `lowerable` |
| `t27c icarus-simulate` W688 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W688 | reference-model OK |
| `t27c seal --save` W688 | saved |
...
- Icarus issue #1171 — very large packed vectors can freeze elaboration; W688
...
1. **Outer dimension 195 correctness.** Resolved by cocotb cross-check and
...
## Next Wave Loop 689 cooperation variants

1. **Variant A (recommended) — `[197][2]^6 Pt` module-scope var from a call with
   indexed signed writes.**
   403,456-bit packed vector, 12,608 elements, non-power-of-two outer dimension 197.
   Continues the odd outer-dimension ladder. **Recommended.**

2. **Variant B — `[195][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   399,360-bit packed vector, 12,480 elements. Tests the same non-p2 outer
   dimension in a bench/function scope rather than module scope.

3. **Variant C — `[195][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.380 MiBit and tests control-flow guarded indexed writes on a packed
   `reg`.
...
Wave Loop 688 closes #1659. The module-scope packed AoS odd outer-dimension ladder
now reaches 195 with zero compiler changes. Branch `wave-loop-689` is ready for
the next wave.
