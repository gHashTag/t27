# FPGA LOOP Closeout — Wave Loop 700

**Date:** 2026-07-07
**Issue:** #1671
**Branch:** `wave-loop-700`
**Next branch:** `wave-loop-701`
**Variant:** A — module-scope `[219][2]^6 Pt` array-of-struct variable, initialized
...
Wave Loop 700 extended the module-scope packed array-of-struct ladder to a
non-power-of-two outer dimension of **219**. The witness
`specs/scratch/w700_bench_module_219x2p6_aos_var_call_write.t27` declares a
448,512-bit (≈0.428 MiBit) mutable packed `reg`, initializes it from
...
- `pub fn make_grid(offset : u16) -> [219][2][2][2][2][2][2] Pt`
- `pub const expected : [219][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [219][2][2][2][2][2][2] Pt = make_grid(0);`
- Element count: **14,016**
- Packed width: **448,512 bits** (56,064 bytes)
...
- Mid-row index used for frame-condition check:
  `[109][1][0][0][0][0][0]` → `e = 109·64 + 32 = 7008` → `x = 14016`, `y = 14017`
- Last element index: `[218][1][1][1][1][1][1]` → `e = 14015` → `x = 28030`, `y = 28031`
...
- `scripts/gen_w700.py` — generator for the witness.
- `specs/scratch/w700_bench_module_219x2p6_aos_var_call_write.t27` — witness spec
  (~962 KB, 41,671 lines).
- `.trinity/seals/scratch_w700_bench_module_219x2p6_aos_var_call_write.json` — seal.
- `.trinity/icarus-baselines/specs/scratch/w700_bench_module_219x2p6_aos_var_call_write.json` —
  empty Icarus baseline.
- `bootstrap/tests/icarus_lowerable.rs` — added
  `accepts_w700_bench_module_219x2p6_aos_var_call_write`.
- `.trinity/current-issue.md` — updated for W700.
- `.claude/plans/wave-loop-700.md` — decomposed plan.
- `.trinity/experience.md` — appended W700 learnings.
- `docs/reports/FPGA_LOOP_CLOSEOUT_W700_2026-07-07.md` — this report.
...
| `t27c parse` W700 | PASS |
| `t27c icarus-lowerable` W700 | `lowerable` |
| `t27c icarus-simulate` W700 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W700 | reference-model OK |
| `t27c seal --save` W700 | saved |
...
- IEEE Std 1800-2017 §7.4.1/7.4.3: packed-array width is the product of packed
  dimensions, with no power-of-two restriction. The 448,512-bit vector emitted
  for W700 is legal SystemVerilog.
...
- cocotb PR #3608 / discussion #2933: Python reference models integrate cleanly
  with SystemVerilog benches via VCD probe comparison; the W700 cocotb check
  reuses this pattern.
...
|------|------------|--------|---------|
| Outer dimension 219 breaks layout math | Low | High | Did not occur; cocotb reference model agreed with simulation. |
...
## Next Wave Loop 701 cooperation variants
...
- The odd outer-dimension ladder (211, 213, 215, 217, 219) is now established
  and dimension-agnostic; W701 can stride directly to 221 without compiler
  risk assessment.
