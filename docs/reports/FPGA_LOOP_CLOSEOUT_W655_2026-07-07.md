# FPGA / Icarus Wave Loop 655 Closeout Report
...
Wave Loop 655 extended the module-scope packed array-of-struct odd outer-dimension
ladder from 127 (W654) to 129. The witness is a module-scope mutable `reg` of type
`[129][2]^6 Pt` initialized from a function call, then exercised with signed
...
- **Spec:** `specs/scratch/w655_bench_module_129x2p6_aos_var_call_write.t27`
- **Generator:** `scripts/gen_w655.py`
- **Size:** 264,192-bit packed vector, 8,256 scalar elements, ~0.252 MiBit.
- **Lines:** ~24,571 (multi-line W584 brace style).
- **Seal:** `.trinity/seals/scratch_w655_bench_module_129x2p6_aos_var_call_write.json`
  - `spec_hash`: `sha256:0dc290e4c9fca37afcc6e11438a9f418bac8d4dcc02861c2ec4913b3753f81e8`
...
- `pub fn make_grid(offset : u16) -> [129][2][2][2][2][2][2] Pt`
  - 8,256 elements, each element numbered `e` from 0 to 8,255.
  - `x = (2*e + offset) % 32768`
  - `y = (2*e + offset + 1) % 32768`
- `pub const expected : [129][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [129][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_129x2p6_call_write`:
  - initial whole-array equality to `expected`;
  - first-element read (`0/1`);
  - last-element read `[128][1][1][1][1][1][1]` (`16510/16511`);
  - mid-row read `[64][1][0][0][0][0][0]` (`8256/8257`);
  - explicit modulo-wrap check via `make_grid(32768)`.
- `bench module_bench_129x2p6_call_write`:
...
The mid-row index is `129 // 2 = 64`. Element index for
`dst[64][1][0][0][0][0][0]` in row-major LSB-first layout is
`64*64 + 1*32 = 4128`, giving expected values `x = 8256`, `y = 8257`. The
...
| Parse | `t27c parse specs/scratch/w655_bench_module_129x2p6_aos_var_call_write.t27` | PASS |
...
| Icarus lowerable integration | `cargo test -p t27c --test icarus_lowerable` | **115/0** |
...
|---|------------|---------|--------|
| 1 | **First outer dimension 129** | The compiler and reference model correctly stride by 129 at the outer dimension. | Resolved, no compiler changes. |
| 2 | **Modulo-wrap regression signal** | With 8,256 elements the offset-0 schedule never wraps; explicit `make_grid(32768)` preserves the wrap path. | Resolved. |
| 3 | **Multi-line mega-literals** | W584-style multi-line brace splitting remains necessary and valid for the 6-D literal inside the 129× outer shape. | Resolved. |
| 4 | **Simulator capacity** | 0.252 MiBit is far below the 4-MiBit Icarus/Yosys cliff; simulation completes in 17 cycles and is interactive. | Resolved. |
...
| 6 | **Spec size** | ~563 KB, ~24,571 lines; generator extended W654 by two row blocks rather than rewriting the literal. | Resolved. |
| 7 | **`assert_ne` simulation gap** | `assert_ne` is accepted by `icarus-lowerable` but not emitted by the Icarus simulation path. W655 continues using `assert_eq` on changed elements. | Documented, unchanged. |
...
## 6. Next Wave Loop 656 cooperation variants
...
- Reuses the exact W655 module-scope lowerable style with two extra rows.
...
- Keeps the 264,192-bit vector and moves the mutable `reg` from module scope
...
- Feature: `feat(igla): Wave Loop 655 — module-scope [129][2]^6 Pt non-p2 AoS var from call with indexed signed writes` (`Closes #1626`).
- Tracking: `chore(trinity): record W655 session log and commit count`.

---

## 8. Sign-off

Wave Loop 655 is closed. All success criteria are met, FROZEN_HASH is stable,
and three cooperation variants are proposed for Wave Loop 656.
