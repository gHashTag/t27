# FPGA LOOP Closeout — Wave Loop 683

Date: 2026-07-07
Branch: `wave-loop-683`
Issue: **#1654**
Variant: **A — module-scope `[185][2]^6 Pt` array-of-struct variable from a call with indexed signed writes**
...
`[185][2][2][2][2][2][2] Pt` (378,880 bits, ~0.362 MiBit) can be:
...
| Artifact | Path |
|----------|------|
| Decomposed plan | `.claude/plans/wave-loop-683.md` |
| Issue + variant definition | `.trinity/current-issue.md` |
| Witness generator | `scripts/gen_w683.py` |
| Witness spec | `specs/scratch/w683_bench_module_185x2p6_aos_var_call_write.t27` |
| Structural test | `bootstrap/tests/icarus_lowerable.rs` — `accepts_w683_bench_module_185x2p6_aos_var_call_write` |
| Seal | `.trinity/seals/scratch_w683_bench_module_185x2p6_aos_var_call_write.json` |
| Icarus baseline | `.trinity/icarus-baselines/specs/scratch/w683_bench_module_185x2p6_aos_var_call_write.json` |
| Closeout report | `docs/reports/FPGA_LOOP_CLOSEOUT_W683_2026-07-07.md` |
| Experience update | `.trinity/experience.md` |
| Persistent memory | `~/.claude/projects/-Users-playra-t27/memory/wave-loop-683.md` |
...
- Total elements: `185 × 2⁶ = 11,840`
- Total packed width: `11,840 × 32 = 378,880` bits (~0.362 MiBit)
- Element values: `x = (2*e + offset) % 32768`, `y = (2*e + offset + 1) % 32768`
- Outer dimension 185 is non-power-of-two; no padding is added in the packed
...
| `t27c parse` W683 | PASS |
| `t27c icarus-lowerable` W683 | lowerable |
| `t27c icarus-simulate` W683 | PASSED (17 cycles) |
| `t27c icarus-cocotb` W683 | OK (1 test / 1 bench passed + VCD probe check) |
| `t27c seal --save` W683 | saved |
...
1. `scripts/gen_w683.py` — new generator.
2. `specs/scratch/w683_bench_module_185x2p6_aos_var_call_write.t27` — new
   generated witness.
3. `bootstrap/tests/icarus_lowerable.rs` — one new integration test.
4. `.trinity/seals/scratch_w683_bench_module_185x2p6_aos_var_call_write.json`
...
- Multi-line W584-style brace literals are still required; a single-line mega
  ...
  it in principle.
- Because the offset-0 schedule for 11,840 elements never reaches 32768, an
...
1. **Upper bound on outer non-p2 dimension.** 185 works, but the next waves
   will probe 187 and beyond. The 4-MiBit threshold is still far away, but
...
3. **No control-flow-guarded writes yet.** W683 uses unconditional indexed
...
W665–W682 showed that repository-wide parse time is dominated by the ~31–35 k-line,
...
## 9. Wave Loop 684 cooperation variants

The odd outer-dimension module-scope ladder has been extended to 185. W684
offers three mutually complementary continuation options:

### Variant A (recommended) — `[187][2]^6 Pt` module-scope var from a call with indexed signed writes

Continue the ladder to outer dimension 187.
- 11,968 elements, 382,976-bit packed vector (~0.366 MiBit).
- Same shape and risk profile as W683.
- Zero expected compiler/reference-model changes.
- Fits the existing generator template with `OUTER = 187`.

### Variant B — `[185][2]^6 Pt` bench-local packed array var from a call with indexed signed writes

Keep the same dimensions and size as W683 but move the mutable `reg` into a
bench/function-local scope.
- 11,840 elements, 378,880-bit packed vector.
- Tests scope handling for large packed arrays.
- Useful complement to the module-scope ladder.

### Variant C — `[185][2]^6 Pt` module-scope var with `if`-guarded indexed signed field writes

Stay at 0.362 MiBit but add conditional writes.
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W683 stays well below the reported threshold.
