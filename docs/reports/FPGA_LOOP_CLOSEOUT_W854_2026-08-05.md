# FPGA Loop Close-out — Wave Loop 854 (2026-08-05)

**Issue:** #1648  
**Branch:** `wave-loop-854` (from `wave-loop-853` HEAD)  
**PR:** #1649  
**Variant:** A — module-scope `[527][2]^6 Pt` non-power-of-two outer-dimension array-of-struct variable from call with indexed signed writes.

## Summary

Wave Loop 854 continued the mechanical packed-vector ladder in the 1-MiBit range.
The witness is a module-scope `[527][2]^6 Pt` variable initialized from a function
call and exercised with indexed signed field writes.

| Metric | Value |
|--------|-------|
| Outer dimension | 527 (odd, non-power-of-two) |
| Inner shape | `[2]^6` = 64 elements/row |
| Element | `Pt { x : i16, y : i16 }` (32 bits) |
| Total elements | 33,728 |
| Packed vector width | 1,079,296 bits |
| Approximate size | ~1.030 MiBit |
| Simulator cycles | 17 |
...
- `scripts/gen_w854.py` — generator copied from `gen_w853.py` and verified for the
  recurring copy hazard (destination path, module header f-string, `MID_IDX`
  comment).
- `specs/scratch/w854_bench_module_527x2p6_aos_var_call_write.t27` — generated
  witness (100,191 lines, 2.31 MB).
...
- `bootstrap/tests/icarus_lowerable.rs` — added integration test
  `accepts_w854_bench_module_527x2p6_aos_var_call_write`.
...
| `cargo test --release --test icarus_lowerable` | 314 passed; 0 failed |
...
W854 packed width is 1,079,296 bits (~1.030 MiBit). No backend limit was hit.
...
- Simulation cycle count stayed flat at **17 cycles**, matching W851/W852/W853.
...
- The hazard was prevented by grepping the three known stale locations before
  running `scripts/gen_w854.py`.
- All references were verified to read `w854`, `527`, and `263`.
...
## Cooperation variants for Wave Loop 855

- **A (recommended):** `[529][2]^6 Pt`, outer += 2, `MID_IDX = 264`.
  - 33,856 elements, 1,083,392 bits (~1.034 MiBit).
  - Continues the established mechanical increment.

- **B:** `[527][3]^6 Pt` — grow the second inner dimension to stress stride scaling.
  - 101,088 elements, 3,234,816 bits (~3.087 MiBit).
  - Deliberately much wider; useful as a width-probe that may convert to a
    negative-boundary witness if a backend limit is hit.

- **C:** `[527][2]^6 Pt` with negative-index writes to exercise wrap-around / signed-index lowering.
  - Keeps the W854 outer dimension but replaces some positive indices with
    negative ones to stress the lowerer's signed-index path.

## Seal hashes

```text
spec_hash=sha256:105b1d696390aeb01001efbbb946f078159265f8136c96f488475a1959ae6955
gen_hash_zig=sha256:e2f27cfd558cf63e767c9eb11ad241d872a9bc13703eadc8cc4e34fdb12a6920
gen_hash_verilog=sha256:efca578e00bb897fbbac21e56e243e1f05ea6e54eb5c57233c4b65214a20d954
gen_hash_c=sha256:42c2848ecf532ab431df730159060d59a04791532a2965d71869c387e6e0b9a1
gen_hash_rust=sha256:eb11f51832335d6512ae36ddbab0de9bf3f5a811eab0a92280a477c5ab309e27
```

## Next steps

1. Open PR #1649 to `master` with `Closes #1648`.
2. Create GitHub issue #1650 for Wave Loop 855.
3. Branch `wave-loop-855` from `wave-loop-854` HEAD and execute the selected
   W855 variant.
