# Wave Loop 879 — Closeout Report

**Date:** 2026-08-05  
**Issue:** [#1708](https://github.com/gHashTag/t27/issues/1708)  
**Branch:** `wave-loop-879`  
**Parent:** `wave-loop-878` HEAD (earlier waves' PRs remain open)  
**PR:** [#1711](https://github.com/gHashTag/t27/pull/1711)  
...
Wave Loop 879 continues the module-scope packed array-of-struct ladder, selecting
Variant A from the W879 plan:

```text
module-scope [577][2]^6 Pt variable from call with indexed signed writes
```

- `Pt { x : i16, y : i16 }` → 32 bits/element.
- Outer dimension `577` is odd and non-power-of-two, preserving the boundary-stress
  cadence established by W837+.
- Inner shape `[2]^6` = 64 elements/row.
- Total elements: `577 × 64 = 36,928`.
- Packed vector width: `36,928 × 32 = 1,181,696 bits` ≈ **1.128 MiBit**.

Artifacts produced:

| Artifact | Path | Notes |
|----------|------|-------|
| Generator | `scripts/gen_w879.py` | Copied from `gen_w878.py`; copy hazard fixed before first run (`w879`, `OUTER = 577`, `MID_IDX = 288`). |
| Spec | `specs/scratch/w879_bench_module_577x2p6_aos_var_call_write.t27` | 109,691 lines, ~2.53 MB. |
| Seal | `.trinity/seals/scratch_w879_bench_module_577x2p6_aos_var_call_write.json` | Saved by `t27c seal --save`. |
| Test | `bootstrap/tests/icarus_lowerable.rs` | `accepts_w879_bench_module_577x2p6_aos_var_call_write`. |
...
This is the twenty-ninth consecutive zero-compiler-change wave in the mechanical
packed-vector AoS ladder (W851–W879), confirming that the lowering, simulation,
...
At 1.128 MiBit, W879 remains far below any practical Icarus memory boundary.
...
because the vector is internal. The 1.128-MiBit W879 vector is trivial compared to
...
| Parse | `./target/release/t27c parse specs/scratch/w879_bench_module_577x2p6_aos_var_call_write.t27` | PASS |
...
| Targeted test | `cargo test --release --test icarus_lowerable accepts_w879_bench_module_577x2p6_aos_var_call_write` | 1/0 |
| Full suite | `cargo test --release --test icarus_lowerable` | **339/0** |

---

## 5. Integration

- [x] Generator `scripts/gen_w879.py` with copy hazard fixed.
- [x] Witness generated and all direct gates green.
- [x] Integration test added immediately after W878's test.
- [x] `FROZEN_HASH` verified unchanged.
- [x] Closeout report written.
- [ ] Commit with `Closes #1708`, push branch `wave-loop-879`, open PR to `master`.
- [ ] Create W880 issue and branch `wave-loop-880` from `wave-loop-879` HEAD.

---

## 6. Next steps

1. Land W879 commit (`Closes #1708`) and open PR to `master`.
2. Create W880 issue and branch `wave-loop-880` from `wave-loop-879` HEAD.
3. Implement selected W880 variant per the standing charter.
