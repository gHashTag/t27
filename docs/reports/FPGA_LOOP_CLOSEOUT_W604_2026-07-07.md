# FPGA Wave Loop 604 Closeout Report

**Issue:** #1575  
**Branch:** `wave-loop-604`  
**Previous:** Wave Loop 603 (#1574, `wave-loop-603`)  
**Date:** 2026-07-07  
...
Wave Loop 604 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **27**:

- Type: `[27][2]^6 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **55,296 bits** (≈0.053 MiBit).
- Total elements: **1,728**.
...
path and the generic indexed field-write paths handled outer stride 27
correctly.
...
|------------|----------------|------------|
| Outer dimension 27 untested at module scope | Stride-27 multiplication and row-major flattening must be correct end-to-end. | Added direct Icarus simulation and cocotb reference-model cross-check. |
| Element count below the modulo-wrap point | With 1,728 elements the offset-0 schedule `(2*e + offset) % 32768` never wraps (`max raw = 3,455`). Earlier large waves used `e_wrap = 16384` to assert wrap behavior. | Added an explicit shifted call `make_grid(32768)` and asserted its first/last elements equal `(offset + raw) % 32768`, proving the compiler and reference model both evaluate modulo correctly even when the offset-0 path does not wrap. |
...
1. `specs/scratch/w604_bench_module_27x2p6_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [27][2]^6 Pt`
   - `pub const expected : [27][2]^6 Pt = make_grid(0);`
   - `pub var dst : [27][2]^6 Pt = make_grid(0);`
   - Test `module_var_27x2p6_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_27x2p6_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w604_bench_module_27x2p6_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w604_bench_module_27x2p6_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w604_bench_module_27x2p6_aos_var_call_write`.
5. `.claude/plans/wave-loop-604.md`.
6. `.trinity/current-issue.md` updated for W605 cooperation variants.
7. `.trinity/experience.md` updated with W604 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-604.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 64/0 (new W604 test) |
...
dimension-agnostic for odd outer dimensions through at least 27. Because the
...
`% 32768`, even though the offset-0 path for 1,728 elements does not trigger a
wrap.
