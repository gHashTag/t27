# FPGA / Icarus Wave Loop 634 Closeout Report

**Date:** 2026-07-07  
**Issue:** #1605  
**Branch:** `wave-loop-634`  
**Variant:** A — module-scope `[87][2]^6 Pt` non-power-of-two array-of-struct
...
Wave Loop 634 pushed the module-scope packed array-of-struct odd outer-dimension
ladder from 85 (W633) to 87. The witness is a module-scope mutable `reg` of type
`[87][2]^6 Pt` initialized from a function call, then exercised with signed
...
- **Spec:** `specs/scratch/w634_bench_module_87x2p6_aos_var_call_write.t27`
- **Generator:** `scripts/gen_w634.py`
- **Size:** 178,176-bit packed vector, 5,568 scalar elements, ~0.17 MiBit.
- **Lines:** ~16,591 (multi-line W584 brace style).
- **Seal:** `.trinity/seals/scratch_w634_bench_module_87x2p6_aos_var_call_write.json`
  - `spec_hash`: `sha256:48330ac011da3a87645eeb75733b1787200d82acf452121fa7fd7399fce50d85`
...
- `pub fn make_grid(offset : u16) -> [87][2][2][2][2][2][2] Pt`
  - 5,568 elements, each element numbered `e` from 0 to 5,567.
  - `x = (2*e + offset) % 32768`
  - `y = (2*e + offset + 1) % 32768`
- `pub const expected : [87][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [87][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_87x2p6_call_write`:
  - initial whole-array equality to `expected`;
  - first-element read (`0/1`);
  - last-element read `[86][1][1][1][1][1][1]` (`11134/11135`);
  - mid-row read `[43][1][0][0][0][0][0]` (`5568/5569`);
  - explicit modulo-wrap check via `make_grid(32768)`.
- `bench module_bench_87x2p6_call_write`:
...
The mid-row index is `87 // 2 = 43`. Element index for
`dst[43][1][0][0][0][0][0]` in row-major LSB-first layout is
`43*64 + 1*32 = 2784`, giving expected values `x = 5568`, `y = 5569`. The
...
| Parse | `t27c parse specs/scratch/w634_bench_module_87x2p6_aos_var_call_write.t27` | PASS |
...
| Icarus lowerable integration | `cargo test -p t27c --test icarus_lowerable` | **94/0** |
...
|---|------------|---------|--------|
| 1 | **First outer dimension 87** | The compiler and reference model correctly stride by 87 at the outer dimension. | Resolved, no compiler changes. |
| 2 | **Modulo-wrap regression signal** | With 5,568 elements the offset-0 schedule never wraps; explicit `make_grid(32768)` preserves the wrap path. | Resolved. |
| 3 | **Multi-line mega-literals** | W584-style multi-line brace splitting remains necessary and valid for the 6-D literal inside the 87× outer shape. | Resolved. |
| 4 | **Simulator capacity** | 0.17 MiBit is far below the 4-MiBit Icarus/Yosys cliff; simulation completes in 17 cycles and is interactive. | Resolved. |
...
| 6 | **Spec size** | ~377 KB, ~16,591 lines; generator extended W633 by two row blocks rather than rewriting the literal. | Resolved. |
| 7 | **`assert_ne` simulation gap** | `assert_ne` is accepted by `icarus-lowerable` but not emitted by the Icarus simulation path. W634 continues using `assert_eq` on changed elements. | Documented, unchanged. |
...
## 6. Next Wave Loop 635 cooperation variants

### Variant A — `[89][2]^6 Pt` module-scope var from a call with indexed signed writes *(Recommended)*

- Outer dimension 89 (next odd non-power-of-two).
- 5,696 elements, 182,272-bit packed vector (~0.174 MiBit).
- Continues the established ladder with zero expected compiler changes.
- Reuses the exact W634 module-scope lowerable style with two extra rows.
...
### Variant C — `[87][2]^6 Pt` with `if`-guarded whole-array reassignment

- Keeps the 0.17 MiBit vector and adds a conditional `if` branch that
...
- Feature: `feat(igla): Wave Loop 634 — module-scope [87][2]^6 Pt non-p2 AoS var from call with indexed signed writes` (`Closes #1605`).
- Tracking: `chore(trinity): record W634 session log and commit count`.

---

## 8. Sign-off

Wave Loop 634 is closed. All success criteria are met, FROZEN_HASH is stable,
and three cooperation variants are proposed for Wave Loop 635.
