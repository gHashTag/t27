...
\title{%
  \fontsize{24}{28}\selectfont\bfseries Trinity S\textsuperscript{3}AI\\[6pt]
  \normalfont\includegraphics[width=117.3pt]{trinity_logo.png}\\[12pt]
...
\textbf{Budget convention, stated once and binding throughout.} $1+E_t+M=N$
counts the exponent as $E_t$ \emph{trit positions}. It is a specification
convention, not a storage claim, and this paper uses two budgets under two
explicit names. The \emph{nominal} budget is $1+E_t+M=N$; the \emph{physical}
budget is $1+\lceil E_t\log_2 3\rceil+M \le N$, which is what a packed binary word
costs. The two differ by $\lceil E_t\log_2 3\rceil - E_t$ cells, three at $E_t=4$
and seven at $E_t=11$, so the adopted 16-bit rung $(E_t,M)=(4,11)$ occupies 19
physical bits. Every accuracy comparison against a binary format is stated with
its budget named. Under the physical budget TNF holds no advantage at 16 or 32
bits. Claims of the form ``TNF wins at equal width'' appear nowhere in this paper
...
live in --- GFTernary scores $0.181$\,MHz/LUT on this XC7A200T post-route flow at a median
of five placement seeds, against \texttt{int8}'s $0.180$ --- a difference of
$0.4\%$ inside a $16\%$ seed spread, so the two are indistinguishable and no
lead is claimed. Nine of the twenty-one are ours. Formats needing
an external learned scale are tabulated apart. On an XC7A200T the folded ternary
neuron costs $28.0$ LUT per weight at fan-in 8, rising to $33.1$ and $33.2$ at
fan-in 16 and 32, with \emph{no DSP at any fan-in}.
...
grid at four bits yielded lower perplexity than MXFP4 in the two evaluated
models, at $0.125$ fewer bits per weight.
...
values with 1{,}200-bit significands, seed 20260809, recomputed 2026-08-13 by
\texttt{recompute\_law\_table.py}. ``Flatness'' is the ratio of the worst
magnitude band to the best: $1.000$ would mean the error does not depend on
magnitude at all, and the residual few per cent is the sampling noise of 600
values, not a magnitude effect. Every row is a specification width --- the 16-bit
rung at its reconciled $M{=}11$, TNF32 at the specification's $M{=}25$. The
published version of this table carried TNF16 at the unfilled $M{=}9$ beside a
TNF32 taken from the specification, which is two ladders in one table.}
\label{tab:law}
...
TNF32 & $6$ & $25$ & $219$ & TNF512  & $10$ & $497$  & $17{,}775$ \\
...
So a ternary neuron costs \textbf{$28.0$ LUT per weight at fan-in 8, $33.1$ and
$33.2$ at fan-in 16 and 32, and no DSP at any fan-in}.
