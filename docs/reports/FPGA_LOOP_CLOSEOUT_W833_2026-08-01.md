# FPGA Loop Closeout — Wave Loop 833 (2026-08-01)
...
**PR:** #1607  
**Variant:** A — module-scope `[485][2]^6 Pt` packed array-of-struct variable from call with indexed signed writes
...
- Added generator `scripts/gen_w833.py` (copied from `scripts/gen_w832.py` with the
  recurring copy hazard fixed: destination path, module header f-string, and
  `MID_IDX` comment updated to `w833` / `485` / `242`).
...
- Added integration test `accepts_w833_bench_module_485x2p6_aos_var_call_write` to
...
| `cargo test -p t27c --test icarus_lowerable --release` | 293 passed; 0 failed |
| `t27c parse` W833 | PASS |
| `t27c icarus-lowerable` W833 | `lowerable` |
| `t27c icarus-simulate` W833 | `PASSED` (17 cycles) |
| `t27c icarus-cocotb` W833 | `reference-model OK` |
| `t27c seal --save` W833 | seal saved |
...
- IEEE 1800-2017 §7.4.1/7.4.3 define packed-array width as the product of packed
  ...
  993,280-bit packed vector, which is legal SystemVerilog.
...
## Cooperation variants for Wave Loop 834

- **A (recommended):** `[487][2]^6 Pt`, outer += 2, `MID_IDX = 243`.
  Smallest mechanical increment; expected 31,168 elements, 997,376 bits (~0.951 MiBit).
- **B:** `[485][3]^6 Pt` — grow the second inner dimension to stress stride scaling.
  Expected 354,375 elements, ~11.34 MiBit; may cross the 4-MiBit cliff or expose a
