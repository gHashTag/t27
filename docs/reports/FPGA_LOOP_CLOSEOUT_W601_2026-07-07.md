# FPGA Wave Loop 601 Closeout Report
...
Wave Loop 601 validated a module-scope, mutable, packed array-of-scalar-struct
variable with a non-power-of-two outer dimension of **21**:

- Type: `[21][2]^7 Pt` where `Pt { x : i16, y : i16 }`.
- Total packed width: **86,016 bits** (≈0.08 MiBit).
- Total elements: **2,688**.
...
path and the generic indexed field-write paths handled outer stride 21
correctly.
...
1. `specs/scratch/w601_bench_module_21x2p7_aos_var_call_write.t27`
   - `pub struct Pt { x : i16, y : i16 }`
   - `pub fn make_grid(offset : u16) -> [21][2]^7 Pt`
   - `pub const expected : [21][2]^7 Pt = make_grid(0);`
   - `pub var dst : [21][2]^7 Pt = make_grid(0);`
   - Test `module_var_21x2p7_call_write`: initial state, corner indexed
     reads, and explicit modulo-wrap check via `make_grid(32768)`.
   - Bench `module_bench_21x2p7_call_write`: indexed reads, signed indexed
     writes, read-back, frame-condition checks, whole-array inequality.
2. `.trinity/seals/scratch_w601_bench_module_21x2p7_aos_var_call_write.json`
3. `.trinity/icarus-baselines/specs/scratch/w601_bench_module_21x2p7_aos_var_call_write.json`
4. `bootstrap/tests/icarus_lowerable.rs`: new integration test
   `accepts_w601_bench_module_21x2p7_aos_var_call_write`.
5. `.claude/plans/wave-loop-601.md`.
6. `.trinity/current-issue.md` updated for W602 cooperation variants.
7. `.trinity/experience.md` updated with W601 learnings.
8. Persistent memory: `~/.claude/projects/-Users-playra-t27/memory/wave-loop-601.md`
...
| `cargo test -p t27c --test icarus_lowerable` | 61/0 (new W601 test) |
...
dimension-agnostic for odd outer dimensions through at least 21. Because the
...
`% 32768`, even though the offset-0 path for 2,688 elements does not trigger a
wrap.
