# FPGA Loop Closeout — Wave Loop 780

**Date:** 2026-07-24
**Issue:** #1496
**Branch:** `wave-loop-780`
**Parent branch:** `wave-loop-779` HEAD (`eadd9cfbcb`)
**Next branch:** `wave-loop-781`
**Witness:** `specs/scratch/w780_bench_module_379x2p6_aos_var_call_write.t27`
**Generator:** `scripts/gen_w780.py`
**PR:** #1497
...
Wave Loop 780 validated a module-scope packed array-of-struct variable with a
non-power-of-two outer dimension:

- Shape: `[379][2]^6 Pt`
- Type: `pub struct Pt { x : i16, y : i16 }`
- Mode: module-scope `pub var dst : [379][2]^6 Pt = make_grid(...)`
...
1. Created `wave-loop-780` from `wave-loop-779` HEAD because PR #1484 (W774),
   PR #1486 (W775), PR #1488 (W776), PR #1489 (README/W774-W776 merge),
   PR #1491 (W777), PR #1493 (W778), and PR #1495 (W779) remain open/unmerged.
2. Copied `scripts/gen_w779.py` → `scripts/gen_w780.py`.
3. Updated constants: `OUTER = 379`, `MID_IDX = 189`, and fixed the module
   header prefix to `w780_bench_module_379x2p6_aos_var_call_write`.
4. Generated the witness with `python3 scripts/gen_w780.py`.
5. Added integration test `accepts_w780_bench_module_379x2p6_aos_var_call_write`
   in `bootstrap/tests/icarus_lowerable.rs`.
6. Sealed the witness with `t27c seal --save`.
...
For the mid-row element `[189][1][0][0][0][0][0]`:

```
MID_E = 189*64 + 32 = 12,128
MID_X = (2 * 12,128) % 32768 = 24,256
MID_Y = 24,257
```

The period-identity check `make_grid(32768)` is included because `32768 ≡ 0 (mod
32768)`. With 24,256 elements, the offset-0 schedule wraps naturally (last raw
`x = (2*24255) % 32768 = 15,742`).
...
| `t27c parse` W780 | PASS |
| `t27c icarus-lowerable` W780 | PASS (`lowerable`) |
| `t27c icarus-simulate` W780 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W780 | PASS (`reference-model OK`) |
...
| Checkpoint | Finding |
|------------|---------|
| L1 TRACEABILITY — 30-day commits with `Closes #N` / `Fixes #N` | **~56.7%** (17 of 30 commits in the current activity window). Improved versus W779's ~17.6%, but still below the prior ~80% local spike. Continue monitoring the remote 30-day rate separately. |
| L4 TESTABILITY — `.t27` specs with `test`/`invariant`/`bench` | 57 of 886 `.t27` files still lack any test/invariant/bench block (≈6.43%). One new scratch witness (W780) was added and contains both `test` and `bench` blocks. |
| L7 UNITY — `scripts/*.sh` on critical path | 19 shell scripts remain under `scripts/`; none were added this wave. |
| Open PRs in flight | **PR #1484 (W774), #1486 (W775), #1488 (W776), #1489 (README update), #1491 (W777), #1493 (W778), and #1495 (W779) remain OPEN.** W780 branched from `wave-loop-779` HEAD to avoid blocking on the merge gate. |
| PR #1489 merge blocker | `fpga-synthesis` fails with pre-existing Yosys error: `Static cast is only supported in SystemVerilog mode` at `build/fpga/generated/uart.v:31`. `--admin` merge rejected while required checks are red. Out of scope for W780. |
| Test drift: `bitnet_pipeline::sequencer_idle_arms_on_start` | **STILL PRESENT**: `bootstrap/tests/bitnet_pipeline.rs:143` expects exact string `IDLE: if(start) begin ...` but actual `gen-layer-sequencer` output wraps it as `IDLE: begin done<=0; if(start) begin ... end end`. Pre-existing; unrelated to wave-loop work. |
| Worktree hygiene | Untracked stale W485 artefacts remain: `specs/scratch/w485_*.t27` (×3) and `.claude/plans/wave-loop-485.md`. These are unrelated to W780 and should be resolved on a dedicated W485 branch or removed. |
| `main` vs `master` divergence | Separate `main` branch still contains useful work (dlc10, Verilog struct access, TRI-NET) with unresolved add/add conflicts against `master`; needs a dedicated merge/reconciliation session. |
| `NOW.md` currency | `NOW.md` refreshed to reflect W780 landed / W781 next state. |
| FPGA synthesis / formal pre-existing failures | `fpga-formal`, `fpga-synthesis`, and `fpga-synthesis-arty` remain failing for infrastructure reasons unrelated to the wave. |
...
IEEE Std 1800-2017 remains the authoritative basis for the W780 witness: packed
arrays of structs and arbitrary-width packed vectors are defined in Clause 7.
The `[379][2]^6 Pt` shape flattens to a single 776,192-bit SystemVerilog packed
vector, which is legal and simulator-portable when scalar-flattened for Icarus.
AMD UG901 (2026.1) and AR 51836 confirm that Vivado simulation and synthesis
accept packed structs/arrays as wide vectors, with DPI mapping them to
`svLogicVecVal` arrays.
...
## 6. Three cooperation variants for Wave Loop 781

1. **Variant A (recommended): `[381][2]^6 Pt` module-scope var from call with
   indexed signed field writes.**
   - Continues the odd outer-dimension ladder (381 → 24,384 elements, 780,288
     bits, ~0.745 MiBit) and confirms non-power-of-two stride 381.

2. **Variant B: `[379][2]^6 Pt` bench-local packed array var from call with
   indexed signed writes.**
   - Keeps the W780 width but moves the mutable `dst` declaration inside a
     `bench` or function scope, testing local-variable lowering.

3. **Variant C: `[379][2]^6 Pt` module-scope var with `if`-guarded indexed
   signed field writes.**
   - Stays at ~0.741 MiBit and adds conditional indexed signed field writes,
