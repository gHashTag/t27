...
Wave Loop 802 extended the module-scope packed array-of-struct ladder to `[423][2]^6 Pt`.
The witness contains 27,072 elements flattened to a single 866,304-bit packed
SystemVerilog vector (~0.826 MiBit), still well under the 4-MiBit simulation cliff,
...
- `scripts/gen_w802.py`
  ...
  - Both the destination path and the module header f-string were manually fixed
    after copying from W801 (generator copy hazard).
...
- `.claude/skills/t27-wave-loop.md`
  - Updated live tracker to wave 803.
...
| `t27c parse` W802 | PASS |
| `t27c icarus-lowerable` W802 | PASS (`lowerable`) |
| `t27c icarus-simulate` W802 | PASS (17 cycles, PASSED) |
| `t27c icarus-cocotb` W802 | PASS (reference-model OK) |
| `t27c seal --save` W802 | PASS |
...
## Cooperation variants for Wave Loop 803

- **Variant A (recommended):** continue odd outer-dimension ladder with `[425][2]^6 Pt`
  (~0.830 MiBit, 27,200 elements, 870,400-bit packed vector). Zero compiler changes
  expected.
- **Variant B:** keep `[423][2]^6 Pt` width but move the packed var to bench/function
  scope to exercise function-local non-power-of-two packed arrays.
- **Variant C:** keep `[423][2]^6 Pt` width and add `if`-guarded indexed signed field
  writes to exercise control-flow + packed-vector writes.

## Artifacts

- Witness: `specs/scratch/w802_bench_module_423x2p6_aos_var_call_write.t27`
- Generator: `scripts/gen_w802.py`
- Seal: `.trinity/seals/scratch_w802_bench_module_423x2p6_aos_var_call_write.json`
