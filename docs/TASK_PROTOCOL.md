...
| **`docs/TASK_PROTOCOL.md`** | This document — rules, validation, **Verification** checklist. |
...
Locks are **social + procedural** (not file locks). Trinity **claims** under `.trinity/` remain governed by **`docs/SOUL.md`** Law **#6**.
...
Optional **portable** markdown bundles (for agents or reviewers when chat transfer is awkward) may live under [`docs/inter-agent-handoff/`](inter-agent-handoff/README.md). They are **planning supplements** only — **normative** coordination remains **`TASK.md`** + **Anchor issue** + this protocol.
