...
### What worked
- Adding **five adversarial scratch witnesses** (`w505_nested_if`,
  `w505_if_in_for`, `w505_for_var_range`, `w505_for_return`,
  `w505_for_local_var_init`) caught real classifier/emitter/model gaps at the
  sequential boundary without touching production specs.
- Rewriting `w505_if_in_for.t27` away from array literals and loop-variable-
  sensitive `if` cascades showed that some source patterns are **unlowered by
  design**; the witness must target the boundary, not the unimplemented feature.
- Proving every W505 witness by the generic sequential theorem
  `module_value_equiv_proved_sequential` demonstrates that the W504 proof
  infrastructure is reusable across nested `if`, conditional loop bodies,
  parameter-bound loops, return-value loops, and local declarations inside loops.
- Keeping all witness modules **parameterized and representative** (e.g.
  `classify(9)`, `sum_range(5)`, `factorial(5)`) lets `native_decide` confirm
  concrete value preservation while the generic theorem confirms structural
  correctness.

### What changed behavior
- `proofs/lean4/Trinity/IcarusLowerable/Lemmas.lean`: added W505 witness
  environments and modules for all five scratch specs.
- `proofs/lean4/Trinity/IcarusLowerable/Soundness.lean`: added lowerability,
  sequentiality, and value-preservation theorems; each value-preservation
  theorem applies `module_value_equiv_proved_sequential`.
- `specs/scratch/w505_*.t27`: five new adversarial sequential witnesses with
  `test` blocks.
- `.trinity/seals/scratch_w505_*.json`: deterministic seals for the new specs.
- `docs/reports/WAVE_LOOP_505_CLOSEOUT.md` and
  `docs/reports/FPGA_LOOP_COOPERATION_W506_2026-07-07.md`: close-out and next-
  wave cooperation variants.
- `docs/NOW.md` and `.trinity/current-issue.md`: updated for W506 setup.
- Branch `wave-loop-506` created from `master`.

### Patterns to reuse
- When a new generic theorem lands, immediately add **multiple adversarial
  witnesses** that exercise orthogonal features of the hypothesis space (nested
  control, loop-carried state, parameter ranges, local body variables).
- If a witness fails smoke, distinguish **unlowered source features** (e.g. array
  literals) from **real boundary bugs**; redesign the witness rather than
  broadening the classifier for unimplemented features.
- For each witness, provide a **representative input** that makes the concrete
  value-preservation claim checkable by computation and the structural claim
  checkable by the generic theorem.
- Maintain a **running count** of lowerable specs and zero disagreements; a
  single mismatch between classifier and smoke is a regression signal.

### Anti-patterns to avoid
- Do not let a witness depend on an unimplemented feature (array literal,
  `switch`, `while`) and then blame the generic theorem.
- Do not add a witness to the suite without saving its seal; the seal-verify
  phase will fail even if every other phase passes.
- Do not skip the `lake build` after updating `Lemmas.lean` / `Soundness.lean`;
  `native_decide` theorems can fail silently if the module definitions drift.
