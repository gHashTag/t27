# Wave Loop 670 — Current Issue

**Issue #1641** — Module-scope `[159][2]^6 Pt` array-of-struct variable with a
non-power-of-two outer dimension, initialized from a function call, with indexed
signed field writes.
**Branch:** `wave-loop-670`.
**Previous:** Wave Loop 669 (#1640, branch `wave-loop-669`).

## Chosen cooperation variant

**Variant A — `[159][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w670_bench_module_159x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [159][2][2][2][2][2][2] Pt` returning a 325,632-bit packed
  literal with 10,176 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [159][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [159][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_159x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_159x2p6_call_write`: whole-array equality before writes,
...
(159), reaching 325,632 bits (≈0.311 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 669

W669 validated a module-scope `[157][2]^6 Pt` (321,536-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 10,048 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W669 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W670

1. **First outer dimension 159.** The compiler and reference model must
   multiply/stride by 159 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 155, 157) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With 10,176 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 20,351).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 159×
   outer shape.
4. **Simulator capacity.** At 0.311 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W670 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W670 stays far below the reported threshold.
...
## Next Wave Loop 671 cooperation variants

1. **Variant A (recommended) — `[161][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   329,728-bit packed vector, 10,304 elements, non-power-of-two outer dimension 161
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[159][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   325,632-bit packed vector, 10,176 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[159][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.311 MiBit and tests that control-flow guarded indexed writes on a
