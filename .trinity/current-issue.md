# Wave Loop 645 — Current Issue

**Issue #1616** — Module-scope `[109][2]^6 Pt` array-of-struct variable with a
non-power-of-two outer dimension, initialized from a function call, with indexed
signed field writes.
**Branch:** `wave-loop-645`.
**Previous:** Wave Loop 644 (#1615, branch `wave-loop-644`).

## Chosen cooperation variant

**Variant A — `[109][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w645_bench_module_109x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [109][2][2][2][2][2][2] Pt` returning a 222,528-bit packed
  literal with 6,976 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [109][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [109][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_109x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_109x2p6_call_write`: whole-array equality before writes,
...
(109), reaching 222,528 bits (≈0.212 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 644

W644 validated a module-scope `[107][2]^6 Pt` (218,752-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 6,848 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W644 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W645

1. **First outer dimension 109.** The compiler and reference model must
   multiply/stride by 109 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 105, 107) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With only 6,976 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 13,951).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 109×
   outer shape.
4. **Simulator capacity.** At 0.212 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W645 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
## Next Wave Loop 646 cooperation variants

1. **Variant A — `[111][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   226,304-bit packed vector, 7,104 elements, non-power-of-two outer dimension 111
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[109][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   222,528-bit packed vector, 6,976 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[109][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.212 MiBit and tests that control-flow guarded indexed writes on a
