# Wave Loop 662 — Current Issue

**Issue #1633** — Module-scope `[143][2]^6 Pt` array-of-struct variable with a
...
**Variant A — `[143][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w662_bench_module_143x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [143][2][2][2][2][2][2] Pt` returning a 292,864-bit packed
  literal with 9,152 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [143][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [143][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_143x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_143x2p6_call_write`: whole-array equality before writes,
...
(143), reaching 292,864 bits (≈0.279 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 661

W661 validated a module-scope `[141][2]^6 Pt` (288,768-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 9,024 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W661 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W662

1. **First outer dimension 143.** The compiler and reference model must
   multiply/stride by 143 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 139, 141) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With 9,152 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 18,303).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 143×
   outer shape.
4. **Simulator capacity.** At 0.279 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W662 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W662 stays far below the reported threshold.
...
## Next Wave Loop 663 cooperation variants

1. **Variant A — `[145][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   296,960-bit packed vector, 9,280 elements, non-power-of-two outer dimension 145
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[143][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   292,864-bit packed vector, 9,152 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[143][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.279 MiBit and tests that control-flow guarded indexed writes on a
