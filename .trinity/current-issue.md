# Wave Loop 642 — Current Issue

**Issue #1613** — Module-scope `[103][2]^6 Pt` array-of-struct variable with a
non-power-of-two outer dimension, initialized from a function call, with indexed
signed field writes.
**Branch:** `wave-loop-642`.
**Previous:** Wave Loop 641 (#1612, branch `wave-loop-641`).

## Chosen cooperation variant

**Variant A — `[103][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w642_bench_module_103x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [103][2][2][2][2][2][2] Pt` returning a 211,264-bit packed
  literal with 6,592 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [103][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [103][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_103x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_103x2p6_call_write`: whole-array equality before writes,
...
(103), reaching 211,264 bits (≈0.201 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 641

W641 validated a module-scope `[101][2]^6 Pt` (206,848-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 6,464 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W641 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W642

1. **First outer dimension 103.** The compiler and reference model must
   multiply/stride by 103 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 99, 101) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With only 6,592 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 13,183).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 103×
   outer shape.
4. **Simulator capacity.** At 0.201 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W642 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W642 stays far below the reported threshold.
...
## Next Wave Loop 643 cooperation variants

1. **Variant A — `[105][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   215,040-bit packed vector, 6,720 elements, non-power-of-two outer dimension 105
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[103][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   211,264-bit packed vector, 6,592 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[103][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.201 MiBit and tests that control-flow guarded indexed writes on a
