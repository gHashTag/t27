# Wave Loop 640 — Current Issue

**Issue #1611** — Module-scope `[99][2]^6 Pt` array-of-struct variable with a
non-power-of-two outer dimension, initialized from a function call, with indexed
signed field writes.
**Branch:** `wave-loop-640`.
**Previous:** Wave Loop 639 (#1610, branch `wave-loop-639`).

## Chosen cooperation variant

**Variant A — `[99][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w640_bench_module_99x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [99][2][2][2][2][2][2] Pt` returning a 202,752-bit packed
  literal with 6,336 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [99][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [99][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_99x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_99x2p6_call_write`: whole-array equality before writes,
...
(99), reaching 202,752 bits (≈0.193 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 639

W639 validated a module-scope `[97][2]^6 Pt` (198,656-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 6,208 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W639 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W640

1. **First outer dimension 99.** The compiler and reference model must
   multiply/stride by 99 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 95, 97) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With only 6,336 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 12,671).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 99×
   outer shape.
4. **Simulator capacity.** At 0.193 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W640 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W640 stays far below the reported threshold.
...
## Next Wave Loop 641 cooperation variants

1. **Variant A — `[101][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   206,848-bit packed vector, 6,464 elements, non-power-of-two outer dimension 101
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[2]^23 Pt` module-scope var from a call with indexed signed
   writes.**
   268,435,456-bit packed vector, 8,388,608 elements. Crosses the 4-MiBit cliff by
   64× and will likely hit Icarus/Yosys compile-time or memory limits
   interactively. Not recommended without chunked-literal design.

3. **Variant C — `[99][2]^6 Pt` module-scope var initialized from a call, then
   conditionally reassigned inside an `if` statement, followed by indexed signed
   field writes.**
   Stays at 0.193 MiBit and tests that control-flow guarded whole-array
