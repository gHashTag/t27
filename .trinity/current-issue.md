# Wave Loop 599 — Current Issue

**Issue #1570** — Module-scope `[17][2]^9 Pt` array-of-struct variable with a
non-power-of-two outer dimension, initialized from a function call, with indexed
signed field writes.
**Branch:** `wave-loop-599`.
**Previous:** Wave Loop 598 (#1569, branch `wave-loop-598`).

## Chosen cooperation variant

**Variant A — `[17][2]^9 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w599_bench_module_17x2p9_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [17][2]^9 Pt` returning a 278,528-bit packed
  literal with 8,704 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [17][2]^9 Pt = make_grid(0);`
- `pub var dst : [17][2]^9 Pt = make_grid(0);`
- `test module_var_17x2p9_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_17x2p9_call_write`: whole-array equality before writes,
...
dimension (17), reaching 278,528 bits (≈0.27 MiBit), well under the 4-MiBit
cliff, without requiring new compiler support.

## Background from Wave Loop 598

W598 validated a module-scope `[15][2]^10 Pt` (491,520-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 15,360 elements are below the natural modulo-wrap
point, the test added an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal.

## Open risks for W599

1. **First outer dimension 17.** The compiler and reference model must
   multiply/stride by 17 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, 9, 11, 13, 15) suggest this is safe, but a module-scope witness is
   needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With only 8,704 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 17,407).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 9-D nested literal inside the 17×
   outer shape.
4. **Simulator capacity.** At 0.27 MiBit the witness is expected to be fast and
