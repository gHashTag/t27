# Wave Loop 653 — Current Issue

**Issue #1624** — Module-scope `[125][2]^6 Pt` array-of-struct variable with a
non-power-of-two outer dimension, initialized from a function call, with indexed
signed field writes.
**Branch:** `wave-loop-653`.
**Previous:** Wave Loop 652 (#1623, branch `wave-loop-652`).

## Chosen cooperation variant

**Variant A — `[125][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w653_bench_module_125x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [125][2][2][2][2][2][2] Pt` returning a 256,000-bit packed
  literal with 8,000 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [125][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [125][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_125x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_125x2p6_call_write`: whole-array equality before writes,
...
(125), reaching 256,000 bits (≈0.244 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 652

W652 validated a module-scope `[123][2]^6 Pt` (252,096-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 7,872 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W652 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W653

1. **First outer dimension 125.** The compiler and reference model must
   multiply/stride by 125 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 121, 123) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With only 8,000 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 15,999).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 125×
   outer shape.
4. **Simulator capacity.** At 0.244 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W653 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
## Next Wave Loop 654 cooperation variants

1. **Variant A — `[127][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   260,096-bit packed vector, 8,128 elements, non-power-of-two outer dimension 127
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[125][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   256,000-bit packed vector, 8,000 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[125][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.244 MiBit and tests that control-flow guarded indexed writes on a
