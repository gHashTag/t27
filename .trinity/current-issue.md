# Wave Loop 665 — Current Issue

**Issue #1636** — Module-scope `[149][2]^6 Pt` array-of-struct variable with a
...
**Variant A — `[149][2]^6 Pt` initialized from a call, with indexed signed field
writes and read-back.**

Witness: `specs/scratch/w665_bench_module_149x2p6_aos_var_call_write.t27`.

- `pub struct Pt { x : i16, y : i16 }`
- `pub fn make_grid(offset : u16) -> [149][2][2][2][2][2][2] Pt` returning a 305,152-bit packed
  literal with 9,536 elements, leaf values `x=(2*e + offset)%32768`,
  `y=(2*e + offset + 1)%32768`.
- `pub const expected : [149][2][2][2][2][2][2] Pt = make_grid(0);`
- `pub var dst : [149][2][2][2][2][2][2] Pt = make_grid(0);`
- `test module_var_149x2p6_call_write`: initial state equals `expected`, plus
  corner indexed reads (first element, last element, mid element, and an explicit
  modulo-wrap check using `make_grid(32768)`).
- `bench module_bench_149x2p6_call_write`: whole-array equality before writes,
...
(149), reaching 305,152 bits (≈0.291 MiBit), well under the 4-MiBit cliff, without
requiring new compiler support.

## Background from Wave Loop 664

W664 validated a module-scope `[147][2]^6 Pt` (301,056-bit) mutable packed reg
initialized from a function call, with indexed signed field writes, with zero
compiler changes. Because 9,408 elements are below the natural modulo-wrap
point, the test retained an explicit `make_grid(32768)` call to preserve the
modulo-wrap regression signal. W605–W664 all use the same module-scope
lowerable style after W606 showed that alternative syntax can parse but produce
invalid Verilog.

## Open risks for W665

1. **First outer dimension 149.** The compiler and reference model must
   multiply/stride by 149 at the outer dimension. Prior non-p2 witnesses
   (3, 5, 7, ..., 145, 147) suggest this is safe, but a module-scope
   witness is needed for end-to-end proof.
2. **Element count below the modulo-wrap point.** With 9,536 elements,
   the offset-0 schedule `(2*e + offset) % 32768` never wraps (max raw 19,071).
...
3. **Parser tolerance for single-line mega-literals.** Multi-line W584-style
   brace style remains mandatory for the 6-D nested literal inside the 149×
   outer shape.
4. **Simulator capacity.** At 0.291 MiBit the witness is expected to be very fast
...
5. **`assert_ne` is not emitted by the Icarus simulation path.** The structural
   ...
   W665 replaces the whole-array `assert_ne(dst, expected)` with checks on the
...
- Icarus issue #1171 — freezes during elaboration of very large packed vectors;
  W665 stays far below the reported threshold.
...
## Next Wave Loop 666 cooperation variants

1. **Variant A — `[151][2]^6 Pt` module-scope var from a call with indexed signed
   writes.**
   309,248-bit packed vector, 9,664 elements, non-power-of-two outer dimension 151
   well under the 4-MiBit cliff. Continues the odd outer-dimension ladder.
   **Recommended.**

2. **Variant B — `[149][2]^6 Pt` bench-local (function-local) packed array var
   from a call with indexed signed writes.**
   305,152-bit packed vector, 9,536 elements. Tests that the same non-p2 outer
...
3. **Variant C — `[149][2]^6 Pt` module-scope var with `if`-guarded indexed signed
   field writes.**
   Stays at 0.291 MiBit and tests that control-flow guarded indexed writes on a
