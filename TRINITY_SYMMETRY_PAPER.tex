...
\]
This is Conjecture GI1: the true Immirzi parameter for Loop Quantum Gravity satisfies Domagala-Lewandowski bounds $\varphi[\ln(2)/\pi, \ln(3)/\pi] \approx [0.2206, 0.3497]$. This value differs from the Meissner 2004 value $\gamma_1 = 0.2375$ by $0.603\%$.\footnote{\textbf{Note, 2026-08-12.} The phrase ``the true Immirzi parameter'' overstates the status of this conjecture. The identification of $\gamma = \varphi^{-3}$ with the loop-quantum-gravity Barbero--Immirzi parameter was tested and rejected in this corpus (DELTA-001, 2026-03-28, status FALSIFIED): $\varphi^{-3} = 0.236068$ versus the canonical $0.237533$, a $+0.617\%$ error (Rovelli \& Vidotto, \emph{Covariant Loop Quantum Gravity}, 2014). $\gamma$ is used here as notation for $\varphi^{-3}$ only; the physical identification is withdrawn. The algebra $\varphi^{-3} = \sqrt5 - 2$ and the quoted gap are unaffected.}
\end{description}
...
\noindent\textbf{Note on row P01, 2026-08-12.} The row above labels $\varphi^{-3}$ as $\gamma_{BI}$, the LQG Immirzi parameter. That identification was tested and rejected in this corpus (DELTA-001, 2026-03-28, status FALSIFIED): $\varphi^{-3} = 0.236068$ versus the canonical Barbero--Immirzi value $0.237533$, a $+0.617\%$ error (Rovelli \& Vidotto, \emph{Covariant Loop Quantum Gravity}, 2014). P01 is retained as a numerical comparison only; the physical identification is withdrawn, and at $0.62\%$ it does not meet the $\Delta < 0.1\%$ VERIFIED threshold used elsewhere in this table.
...
\begin{enumerate}
\item \textbf{P01: $\gamma_\varphi = \varphi^{-3} = \sqrt{5} - 2 \approx 0.23607$} (LQG) -- The only pure power of $\varphi$ within Domagala-Lewandowski bounds for the Barbero-Immirzi parameter in Loop Quantum Gravity. This is Conjecture GI1.\footnote{\textbf{Note, 2026-08-12.} Listing P01 among the ``most significant discoveries'' is no longer defensible. The identification of $\gamma = \varphi^{-3}$ with the loop-quantum-gravity Barbero--Immirzi parameter was tested and rejected in this corpus (DELTA-001, 2026-03-28, status FALSIFIED): $\varphi^{-3} = 0.236068$ versus the canonical $0.237533$, a $+0.617\%$ error (Rovelli \& Vidotto, \emph{Covariant Loop Quantum Gravity}, 2014). $\gamma_\varphi$ is retained as notation for $\varphi^{-3}$ only; the physical identification is withdrawn. Note also that P01 is quoted at $-0.62\%$ in Table~\ref{tab:catalog}, outside the $\Delta < 0.1\%$ threshold the abstract claims for the catalogue.}
