...
const createDb = () => ({
  ...
  select: (fields?: unknown) => ({
    from: (_table: unknown) => ({
      ...
      where: fields
        ? // When select() receives fields (e.g. { value: dbCount() })
          // it's the MAX_SESSIONS count query — return [{value: N}]
          async () => {
            const active = store.filter(
              (s) => s.status !== "deleted" && s.status !== "failed",
            );
            return [{ value: active.length }];
          }
        : async (pred: (s: Session) => boolean) => store.filter(pred),
    }),
  }),
