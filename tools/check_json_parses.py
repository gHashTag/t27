...
def _reject_constant(name):
    ...
    CPython's acceptance is not evidence the file parses.
    """
    raise ValueError(f"RFC 8259 forbids bare {name}: use JSON string format")
...
def scan(root=ROOT):
    ...
    for rel in tracked_json(root):
        ...
        if not raw.strip():
            ...
            continue
        try:
            # STRICT on both axes, because a lenient reader here is a gate that
            # passes what its consumers refuse.
            #
            # `errors="replace"` turned any invalid byte into U+FFFD and then
            # parsed the repaired text, so a file no strict reader accepts came
            ...
            #
            # `parse_constant` refuses bare `Infinity`, which CPython accepts and 
            # documents as "an extension to the JSON specification" while RFC 8259 
            # has no such literal. `node -e JSON.parse(...)` rejects it, so this 
            # gate passes what another consumer refuses -- a real defect, fixed.
