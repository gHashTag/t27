...
pub const EXP_BIAS: u16 = 255;
pub const PHI_DISTANCE: f64 = 0.02482317991669112;
pub const MEMORY_RATIO_VS_FP32: f32 = 24.0 / 32.0; // 0.75

const EXP_MASK: u32 = (1 << EXP_BITS) - 1; // 0x1FF = 511
const MANT_MASK: u32 = (1 << MANT_BITS) - 1; // 0x3FFF = 16383
const MANT_SCALE: f32 = 16384.0; // 2^14

// =====================================================================
// GF24 Type
// =====================================================================

pub const GF24 = struct {
    raw: u32, // 24-bit value stored in u32

    pub fn init(raw: u32) GF24 {
        return GF24{ .raw = raw & 0xFFFFFF }; // mask to 24 bits
    }
};
...
pub fn decode(gf: GF24) f32 {
    const sign: u8 = @intCast((gf.raw >> 23) & 1);
...
pub fn maxValue() f32 {
    const mant_max: f32 = 1.0 + 16383.0 / MANT_SCALE;
...
pub fn epsilon() f32 {
    return 1.0 / MANT_SCALE; // 0.00006103515625
}
...
test "gf24_phi_distance_within_tolerance" {
    try std.testing.expect(PHI_DISTANCE < 0.03);
}

test "gf24_memory_ratio_vs_fp32" {
    try std.testing.expect(@abs(MEMORY_RATIO_VS_FP32 - 0.75) < 0.01);
}
...
// Invariant tests
test "gf24_bits_constant" {
    try std.testing.expectEqual(@as(u8, 24), BITS);
}

test "gf24_sign_bits_is_one" {
    try std.testing.expectEqual(@as(u8, 1), SIGN_BITS);
}

test "gf24_exp_bits_is_nine" {
    try std.testing.expectEqual(@as(u8, 9), EXP_BITS);
}

test "gf24_mant_bits_is_fourteen" {
    try std.testing.expectEqual(@as(u8, 14), MANT_BITS);
}

test "gf24_max_ge_min_positive" {
    try std.testing.expect(maxValue() >= minPositive());
}

test "gf24_phi_distance_below_threshold" {
    try std.testing.expect(PHI_DISTANCE < 0.03);
}

test "gf24_exp_bias_positive" {
    try std.testing.expect(EXP_BIAS > 0);
}

test "gf24_ln_exp_inversion" {
    const x: f32 = 2.0;
    const y = lnApprox(x);
    try std.testing.expect(@abs(expApprox(y) - x) < 0.01);
}

test "gf24_floor_monotonic" {
