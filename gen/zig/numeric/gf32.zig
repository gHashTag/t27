...
// 19-bit mantissa (vs IEEE 23-bit) -- still good precision
// =====================================================================

pub const BITS: u8 = 32;
pub const SIGN_BITS: u8 = 1;
pub const EXP_BITS: u8 = 12;
pub const MANT_BITS: u8 = 19;
pub const EXP_BIAS: u16 = 2047;
pub const PHI_DISTANCE: f64 = 0.01354495894042812;
pub const MEMORY_RATIO_VS_FP32: f32 = 1.0;

const EXP_MASK: u32 = (1 << EXP_BITS) - 1; // 0xFFF = 4095
const MANT_MASK: u32 = (1 << MANT_BITS) - 1; // 0x7FFFF = 524287
const MANT_SCALE: f32 = 524288.0; // 2^19

// =====================================================================
// GF32 Type
// =====================================================================

pub const GF32 = struct {
    raw: u32, // 32-bit raw value

    pub fn init(raw: u32) GF32 {
        return GF32{ .raw = raw };
    }
};
...
pub fn decode(gf: GF32) f32 {
    const sign: u8 = @intCast((gf.raw >> 31) & 1);
    const exp_biased: u16 = @intCast((gf.raw >> MANT_BITS) & EXP_MASK);
    ...
    const exp_unbiased: i16 = if (exp_biased == 0)
        ...
        @as(i16, @intCast(exp_biased)) - @as(i16, EXP_BIAS);
...
pub fn maxValue() f32 {
    const mant_max: f32 = 1.0 + 524287.0 / MANT_SCALE;
    const exp_max: i16 = @as(i16, @intCast(EXP_MASK)) - @as(i16, EXP_BIAS);
...
pub fn epsilon() f32 {
    return 1.0 / MANT_SCALE; // 0.000001907348633
}
...
test "gf32_phi_distance_near_optimal" {
    try std.testing.expect(PHI_DISTANCE < 0.015);
}

test "gf32_memory_ratio_equals_one" {
    try std.testing.expectEqual(@as(f32, 1.0), MEMORY_RATIO_VS_FP32);
}
...
// Invariant tests
test "gf32_bits_constant" {
    try std.testing.expectEqual(@as(u8, 32), BITS);
}

test "gf32_sign_bits_is_one" {
    try std.testing.expectEqual(@as(u8, 1), SIGN_BITS);
}

test "gf32_exp_bits_is_twelve" {
    try std.testing.expectEqual(@as(u8, 12), EXP_BITS);
}

test "gf32_mant_bits_is_nineteen" {
    try std.testing.expectEqual(@as(u8, 19), MANT_BITS);
}

test "gf32_max_ge_min_positive" {
    try std.testing.expect(maxValue() >= minPositive());
}

test "gf32_phi_distance_near_optimal_invariant" {
    try std.testing.expect(PHI_DISTANCE < 0.015);
}

test "gf32_exp_bias_positive" {
    try std.testing.expect(EXP_BIAS > 0);
}

test "gf32_exp_wider_than_ieee" {
    try std.testing.expect(EXP_BITS > 8);
}

test "gf32_mant_narrower_than_ieee" {
    try std.testing.expect(MANT_BITS < 23);
}

test "gf32_ln_exp_inversion" {
    const x: f32 = 2.0;
    const y = lnApprox(x);
    try std.testing.expect(@abs(expApprox(y) - x) < 0.01);
}

test "gf32_floor_monotonic" {
