...
static int32_t gf20_clamp_i32(int32_t x, int32_t min_val, int32_t max_val) {
...
static uint32_t gf20_extract_mantissa(float value, int16_t exp_val, uint8_t mant_bits) {
...
float gf20_pow(float base, float exponent) {
    ...
    float ln_val = gf20_ln_approx(base);
    return gf20_exp_approx(exponent * ln_val);
}
...
GF20 gf20_encode(float value) {
    ...
    int16_t exp_unbiased = gf20_floor_log2(abs_val);
    int32_t exp_biased = (int32_t)exp_unbiased + (int32_t)GF20_EXP_BIAS;

    /* Clamp exponent */
    int32_t exp_clamped = gf20_clamp_i32(exp_biased, 0, (int32_t)GF20_EXP_MASK);

    /* Extract mantissa (12 bits) */
    uint32_t mant = gf20_extract_mantissa(abs_val, exp_unbiased, GF20_MANT_BITS);

    gf.raw = (sign << 19) |
             ((uint32_t)exp_clamped << GF20_MANT_BITS) |
             (mant & GF20_MANT_MASK);
...
float gf20_decode(GF20 gf) {
    uint8_t sign = (uint8_t)((gf.raw >> 19) & 1);
    uint8_t exp_biased = (uint8_t)((gf.raw >> GF20_MANT_BITS) & GF20_EXP_MASK);
    uint32_t mant = gf.raw & GF20_MANT_MASK;
    ...
    if (exp_biased == 0) {
        exp_unbiased = -(int16_t)GF20_EXP_BIAS + 1;
    } else {
        exp_unbiased = (int16_t)exp_biased - (int16_t)GF20_EXP_BIAS;
    }

    /* Mantissa */
    float mant_normalized;
    if (exp_biased == 0) {
        mant_normalized = (float)mant / GF20_MANT_SCALE;
    } else {
        mant_normalized = 1.0f + (float)mant / GF20_MANT_SCALE;
    }
...
float gf20_max_value(void) {
    float mant_max = 1.0f + 4095.0f / GF20_MANT_SCALE;
    int16_t exp_max = (int16_t)GF20_EXP_MASK - (int16_t)GF20_EXP_BIAS;
    return mant_max * powf(2.0f, (float)exp_max);
}

float gf20_min_positive(void) {
    float mant_min = 1.0f / GF20_MANT_SCALE;
    int16_t exp_min = -(int16_t)GF20_EXP_BIAS + 1;
    return mant_min * powf(2.0f, (float)exp_min);
}

float gf20_epsilon(void) {
    return 1.0f / GF20_MANT_SCALE; /* 0.000244140625 */
}

bool gf20_validate_format(void) {
    return (GF20_BITS == GF20_SIGN_BITS + GF20_EXP_BITS + GF20_MANT_BITS) &&
           (GF20_SIGN_BITS == 1) &&
           (GF20_EXP_BITS == 7) &&
           (GF20_MANT_BITS == 12);
}
...
void test_gf20(void) {
    ...
    {
        float result = gf20_ln_approx(1.0f);
    ...
    {
        float result = gf20_exp_approx(0.0f);
        assert(fabsf(result - 1.0f) < 1e-6f);
    }
    ...
    {
        float e = 2.718281828459045f;
        float result = gf20_exp_approx(1.0f);
    ...
    {
        float result = gf20_floor(3.7f);
        assert(fabsf(result - 3.0f) < 1e-6f);
    }

    /* test gf20_floor_negative */
    {
        float result = gf20_floor(-3.2f);
        assert(fabsf(result - (-4.0f)) < 1e-6f);
    }

    /* test gf20_floor_integer */
    {
        float result = gf20_floor(5.0f);
        assert(fabsf(result - 5.0f) < 1e-6f);
    }

    /* Invariant: max >= min_positive */
    {
        assert(gf20_max_value() >= gf20_min_positive());
    }

    /* Invariant: ln/exp inversion */
    {
        float x = 2.0f;
        float y = gf20_ln_approx(x);
        assert(fabsf(gf20_exp_approx(y) - x) < 0.01f);
    }

    /* Invariant: floor monotonic */
    {
        assert(gf20_floor(2.5f) <= gf20_floor(3.5f));
    }

    printf("GF20: all tests passed\n");
}
