...
\texttt{tnf-vectors-3}, with its own manifest of SHA-256 digests. Neither name is
resolvable at this revision --- \texttt{git ls-remote --tags} returns no
\texttt{tnf-vectors} tag and no manifest is committed --- so a reader cannot today
check a digest against either set. The vectors themselves are under
\texttt{conformance/vectors/}; the versioned manifests are the part that is
missing, and until they are published this paragraph records an intent rather
than a released artefact. Any transcript
