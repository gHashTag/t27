...
\texttt{proofs/trinity/CorePhi.v}, theorem \texttt{fib\_step\_is\_phi\_mul}
...
\paragraph{Where the artefacts are.} Everything this paper measures is in one
repository, \url{https://github.com/gHashTag/t27}, at commit
\texttt{0bf0a309d}. Every occurrence of ``this revision'' below and above means
that commit. An earlier draft pinned the tag \texttt{v4.0-trinity}, which does
not exist on that remote and never did, so each of the twelve provenance claims
in this paper resolved to nothing; a commit hash is used here because it cannot
drift.

\paragraph{What is machine-checked, and where.} The $\varphi$ identities on
which the closure argument rests are in
\texttt{proofs/trinity/CorePhi.v}: \texttt{phi\_square} ($\varphi^2=\varphi+1$),
