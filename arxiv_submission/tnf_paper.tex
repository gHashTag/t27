...
Table~\ref{tab:rejected}. \textbf{Three of the four TNF rungs are at their
specified parameters here; the TNF64 row is not}. Its $65$ bits are the
pre-reconciliation module at $M{=}52$, where the reconciled rung is $69$ bits
and has not been placed --- so that row is named by its stored width and should
not be read against a specified TNF64. The other three moved: the modules
previously measured
