...
\paragraph{Budget convention, stated once and binding throughout.}
...
TNF32 and TNF64 are 6, 10, 19, 36 and 69 bits. The nominal names are
\emph{deprecated}, not merely secondary, and the rule this paper binds itself to
is that no comparison may be stated in them: a row reading ``TNF16 against
binary16'' looks matched and is not, because $4,8,16,32,64$ is exactly the IEEE
width sequence and the coincidence does the arguing. Where a nominal name appears
below it names a module that was built, and its stored width is printed beside
it. A reader who sees a nominal name in a comparison anywhere in this paper has
found a defect.
...
multiplier-free at \emph{one addition and no shifter} whenever it is an
algebraic integer whose companion matrix has entries in $\{0,\pm1\}$ --- a
sufficient condition, not a characterisation, since $\{0,\pm2^k\}$ coefficients
are shifts and stay multiplier-free too. Under that budget degree two admits
$\varphi$ alone, and nothing lies between the shift and $\varphi$ at two
registers; at three the tribonacci constant does. The family $r^d=r+1$ reaches
...
binary field: four trits use $81$ of $128$ codes, leaving $200{,}704$ of the
$2^{19}$ words unreachable. That figure read $8{,}190$ in v1, because the oracle
behind the table decoded all $128$ exponent codes rather than the $81$ the trits
name; the correction enlarges the deficit against us and leaves the step at
unity, and so the ratios above, unchanged. The no-survivors corollary is
accordingly withdrawn to a
...
\textbf{The third contribution is methodological and is not about ternary.}
Section~\ref{sec:checklist} states six ways an arithmetic-hardware result dies
while reporting success, each found in our own numbers and each costing us a
published claim: a synthesiser that folds away the on-die checks whose area is
being measured; a frequency column that is a size proxy rather than a figure of
merit; a proof base with no admitted lemmas that never type-checked; a decoder
that accepts codes its format cannot name and so flatters it; substring matching
that sources a figure to unrelated digits; and a provenance chain that ends at a
document restating the number rather than at the tool that produced it. Each is
given as a procedure a reader can run against their own corpus. None was caught
by review; all six were caught by re-running something that had already passed.

The artefacts are at \url{https://github.com/gHashTag/ternary-network-floats}:
\texttt{python3 verify.py} recomputes the tables above from the reference
oracle and fails if any number moves, and \texttt{coqc proofs/CorePhi.v}
checks the algebra.
\end{abstract}
...
\paragraph{The third thread.} Two of the three results here are about a number
format. The third is not: Section~\ref{sec:checklist} collects six ways an
arithmetic-hardware measurement reports success while having failed, each of
which cost this paper a published claim before it was caught. They are stated as
procedures because they are properties of synthesis tools, proof checkers and
provenance matchers rather than of this format, and a reader building any
arithmetic datapath is exposed to all six.

\paragraph{A word on the plates.} Seventy of the figures here carry captions
opening ``Canon plate''. They are a companion series rather than evidence: each
sits in the section whose argument it draws, states that argument in its own
caption, and carries no number in the running text, because the text it
illustrates is on the facing page. Every other figure and every table in this
paper is referred to by number at the point its evidence is used, so a claim
that rests on a measurement can always be traced to the float that holds it.

A ternary node has three sites where a number format could live, and only one of
them needs one. Figure~\ref{fig:canon-sites} names them: the weight is a code and
multiplying by it is a sign-select; the
...
says what it does single out. A scale is applicable without a multiplier at one
addition and no shifter whenever it is an algebraic integer whose companion
matrix has entries in $\{0,\pm1\}$, which is a finite condition and therefore
enumerable rather than arguable; it is sufficient and not necessary, since
$\{0,\pm2^k\}$ coefficients are shifts. Degree one gives the shift; degree two
gives $\varphi$ and nothing else; degree three gives three more --- plastic
$1.3247$ and supergolden $1.4656$ finer than $\varphi$, tribonacci $1.8393$
coarser. Between the shift and $\varphi$ there is nothing at two registers, and
$\varphi$ is the only multiply-free refinement of the power-of-two ladder
available there. The hierarchy continues --- and
...
Three properties follow from the layout rather than from tuning, and
Figure~\ref{fig:canon-field} reads the sixteen bits off one field at a time.
...
$1.03$ and $1.11$ on every rung whose range covers the workload --- at most an
$11\%$ difference between the closest and furthest parts of a workload spanning
76 binary decades (Figure~\ref{fig:ladderacc} plots those bands rung by rung).
TNF8 is the exception at $1.883$, and it is not a counterexample to the
derivation but a rung whose six binades cannot hold the workload: the far band
leaves range, and what the ratio measures there is overflow rather than rounding.
An earlier version of this sentence gave the interval as $1.05$ to $1.07$, which
seven of the eight rows of Table~\ref{tab:law} fall outside. This is the property fixed fields
...
bit, from nothing but its round-trip error, is doing more than curve-fitting: it
inverts. Figure~\ref{fig:canon-law} draws that inversion beside the two relations
it rests on --- a rung buying fine steps with range, and an error that moves with
the mantissa alone.
...
\caption{The data of Table~\ref{tab:ladderacc}, redrawn. The three bars on each rung are the magnitude bands
...
\caption{The data of Table~\ref{tab:ladderacc} again, by count. A starred count marks values that overflowed; TNF16,
...
10 & posit10 $es{=}1$ & $\mathbf{1{,}022}$ & $\mathbf{32.0}$ & $\mathbf{0.781\%}$ \\
10 & posit10 $es{=}2$ & $\mathbf{1{,}022}$ & $\mathbf{64.0}$ & $\mathbf{1.562\%}$ \\
\midrule
6  & TNF4 $(2t,1m)$   & $28$          & $6.6$ & $25.0\%$ \\
6  & posit6 $es{=}1$  & $\mathbf{62}$ & $\mathbf{16.0}$ & $\mathbf{12.5\%}$ \\
...
$200{,}704$ of the $2^{19}$ words --- $38.3\%$ of the container --- are
unreachable. That is what a ternary exponent costs \emph{in a word of its own},
and it is the cost this paper measured throughout.

\paragraph{Most of that deficit is recoverable, and we did not recover it.} An
earlier version of this paragraph said no packing repairs the loss. That is true
per word and false per block, and the difference is worth stating because it
bounds how much of the negative result above is structural.

One value of the adopted rung carries
$\log_2(2\cdot 3^4\cdot 2^{11}) = 18.340$ bits of information in a $19$-bit word,
so the word is $63.3\%$ used. The waste is a rounding-up, $\lceil 4\log_2
3\rceil = 7$ bits for a field that needs $6.340$, and rounding up once per value
is what makes it expensive. Pack the exponents of a block of $K$ values as a
single base-3 integer and the rounding happens once per block: at $K=32$ the
block costs $587$ bits, $18.344$ per value, which is $0.004$ bits above the
information-theoretic floor and $99.7\%$ utilisation.

Base-3 packing decodes by division and is not worth its area. The implementable
point is coarser and nearly as good: \textbf{five trits in eight bits}, since
$3^5=243$ fits $2^8=256$ at $94.9\%$, decoded by one $256\times8$ table lookup
per five values and a field slice thereafter. Applied to the adopted rung it
gives $18.400$ bits per value against $19$ --- $95.9\%$ utilisation, $0.6$ bits
per value recovered, a $3.2\%$ reduction in stored width. Among group sizes, five is the first whose table is small enough to be free and
whose utilisation exceeds ninety per cent: three trits reach $84.4\%$ in five
bits, four reach only $63.3\%$ in seven, and ten reach $90.1\%$ but need a
$65{,}536$-row table for a further $0.0\%$ over five.

What this does \emph{not} do is close the comparison. Spending the recovered
$0.6$ bits on the mantissa moves the step at unity from $0.024\%$ to about
$0.015\%$, which narrows the gap to posit19 from $12\times$ to roughly
$7.6\times$. Posit still wins on precision per bit at every width; the deficit
was overstated by a factor of about two by our own choice to round the exponent
field up once per value, and stating the negative result honestly means saying
how much of it was ours to fix. It also costs the property that makes the format
cheap to decode: within a group of five, values are no longer independently
addressable without the table. Section~\ref{sec:cleantable} is where that trade belongs, and it is not a trade
this paper measured.

\paragraph{This table was wrong in v1, and the error inflated our own numbers.}
The row above read $516{,}096$ values and $127.0$ binades until the oracle
behind it was re-read. Its decoder masked the exponent to the seven bits the
field occupies and accepted all $128$ codes, including the $47$ that no
four-trit word names; those extra codes decoded as ordinary normals. The
published row therefore described a format with a seven-bit \emph{binary}
exponent, not the balanced-ternary one Section~\ref{sec:format} defines, and
$127.0$ binades is $2^7-1$ --- a number that should have been caught against the
$24$ decades this paper reports for the same rung everywhere else. Recomputed
against the trits, the rung holds $323{,}584$ values across $79.0$ binades.
The correction moves the deficit against us: the gap to posit19 was reported as
$8{,}190$ values and is $200{,}702$. The step at unity is untouched, so the
$12\times$, $4\times$ and $2\times$ ratios above are the same numbers they were.

\paragraph{The axis this format is for.} Precision per bit is the axis posit
wins, and we have now stated by how much. It is worth saying plainly which axis
runs the other way, because the two are bought with the same bits.

A tapered format buys its precision with a variable-length regime, and a
variable-length regime is decoded by counting leading zeros and then shifting by
the count. A fixed field is decoded by slicing it. The difference is measured in
Section~\ref{sec:cleantable} and it is not small: all fourteen fixed fields sit
above all three tapers on estimated maximum frequency, the worst fixed field
leading the best taper by $1.43\times$, and against posit32 the gap is
$7.8\times$ on area and $19.9\times$ on frequency. A format that is $12\times$
coarser at unity and decodes without a shifter is not strictly worse than one
that is finer and does not; it is a different point on a trade, and which point
is right depends on whether the design is starved of precision or of area.

\paragraph{Exactness is the property that has no competitor.} The linear path of
a layer in $\mathbb{Z}[\varphi]$ is exact --- not accurate to some tolerance,
exact --- and this is machine-checked rather than asserted. Every competing
low-precision format rounds at the accumulator. The consequence worth naming is
not accuracy, where the difference is small and we do not claim otherwise: it is
that a layer's output is a deterministic function of its inputs with no rounding
step to audit, identical bit for bit on any device that implements the same
integer arithmetic. Where a result has to be reproduced, or certified, or
compared across two implementations, that is a property no tapered format and no
rounded accumulator can offer at any width.

\paragraph{Zero DSP is a contention argument, not only an area one.} DSP slices
are a fixed count on a part, and a design that uses none of them does not compete
for them. That is the claim behind putting a radio and a transformer in the same
sentence: the filter bank takes the DSP columns, the network takes LUTs, and the
two fit on one device because they are starved of different things. Every
fan-in in Table~\ref{tab:node} reads zero DSP, so the property holds across the
range measured rather than at one operating point.
...
Figure~\ref{fig:canon-fabric} collects what that fabric costs, down to the
assembled ternary neuron of Table~\ref{tab:node}, which stays DSP-free at every
fan-in measured.
...
\caption{What the datapath costs. The folded ternary neuron measures $28.0$ LUT per weight at fan-in $8$ and $33.1$--$33.2$ at fan-in $16$--$32$, with no DSP at any of the three, which is what makes area and precision commensurable in the fitted cost model of this section. The throughput-per-LUT ranking this caption previously drew from the same table is withdrawn: the frequencies it rested on were measured on builds whose on-die checks had been folded to constants at synthesis.}
...
not. Each was checked rather than argued (Table~\ref{tab:equiv}).
...
$\lceil E_t \log_2 3\rceil$ bits (Table~\ref{tab:pack}).
...
is the nominal budget of the convention stated in the abstract: the adopted rung occupies
...
The last row of Table~\ref{tab:tnf16-fill} is the law's own consistency check:
adding two trits of exponent
...
number without any measurement, and the measurement then confirms it
(Table~\ref{tab:alloc}). Against a
...
16-bit class and $111.2$ at the 32-bit one, Table~\ref{tab:convert} prices the two
regime round trips in mantissa bits:
...
Table~\ref{tab:oneoverm} values the same two regime codecs at five mantissa
widths; Corollary~\ref{cor:breakeven} is the exact form of what
...
$\kappa$ alone. Figure~\ref{fig:radix} draws $\kappa$ against the radix and marks
every radix whose scaling is a shift. Sweeping $r$ continuously over $[1.1, 16]$
with $E$ integral and
...
returns E2M1 (Table~\ref{tab:blockpct}), which is the element the OCP microscaling
specification fixes. The
...
Figure~\ref{fig:canon-families} draws it: the two rules crossed with the two
radices exhaust the corners, and there is no fifth cell.
...
Table~\ref{tab:gftvstnf} sets the two ladders side by side, rung by rung. This is
why the FPGA post-route tables carry TNF rows and no separate GF-T ones: at
every rung but 32 they would be the same numbers twice. It is also why the
eight-bit measurement in Table~\ref{tab:fourfam} reports GF-T8 and ``TNF8 at
$E_t=3$'' as one line
...
it now appears in Table~\ref{tab:fullthroughput} as row 10, measured on the
...
a property of the number set, and Figure~\ref{fig:canon-closure} draws that
property against its absence: the step $(a,b)\mapsto(b,a+b)$ stays inside
$\mathbb{Z}[\varphi]$, while $F_4F_4=9$ is already outside the Fibonacci
integers.
...
A scale $r$ is applied without a multiplier \emph{whenever} $r$ is an algebraic
...
vector $(x_0,\dots,x_{d-1})$ to $(a_0x_{d-1},\;x_0+a_1x_{d-1},\;\dots)$, in which
every product is a select and every combination an addition. The condition is
finite, so the scales meeting it can be enumerated rather than argued about, and
the rest of this section enumerates them.

\textbf{The converse is false, and we do not claim it.} An earlier version of
this section wrote ``exactly when'' and justified the necessity in one clause:
that any $|a_i|>1$ reintroduces a constant multiply. It does not. Coefficients
in $\{0,\pm2^{k}\}$ are shifts, so they stay multiply-free at the cost of a
shifter, and any ratio at all is reachable multiply-free once enough adders are
spent. What $\{0,\pm1\}$ characterises is not
multiplier-freedom in general but multiplier-freedom \emph{at one addition and
no shifter}, and every enumeration below is to be read under that budget. The
biconditional is withdrawn; the sufficiency, which is what the datapath uses, is
unaffected.
...
the degree-2 discriminants $a^2+4b$ are negative for three cases
($b=-1$ at $a\in\{0,\pm1\}$), and the remaining six give roots in $\{0,\pm1\}$
except for $b=1,a=\pm1$.
\end{proof}

\begin{corollary}\label{cor:nothing-between}
Nothing lies between the shift and $\varphi$ \emph{at two registers}: $\varphi$
is the unique multiply-free refinement of the power-of-two ladder available
there. At three registers something does --- the tribonacci constant $1.8393$ of
Theorem~\ref{thm:hierarchy} sits between $\varphi$ and the shift --- so the
corollary is a statement about the two-register budget and not about the
interval.
\end{corollary}
\begin{proof}
...
Figure~\ref{fig:canon-hierarchy} draws the hierarchy by degree, with $\varphi$
the only scale at two and the plastic step one register further on.
...
ladder in Table~\ref{tab:blockbound} beats MXFP4 on squared error by factors of
three to ten, and every
...
why a fourth is not warranted. Figure~\ref{fig:canon-block} puts that ceiling
beside the axis it does not close: the element codebook and the one scale a
block carries are separate decisions.
...
choice at equal width is between exactly those two, and
Table~\ref{tab:scalegrid} measures both on both models.
...
L'H\^opital's rule gives $\lim_{x\to0^+}R(x)=1/\ln 2$.
\end{proof}
...
below the shift costs exactly one addition and only the register count grows, with the degree-4 minimum included as the exception at three.}
\label{tab:allscales}
...
$2.6$--$3.6\times$ and lands on the hand count --- a three-way select of a
...
than part of it. Frequency falls about $20\%$ at fan-in $8$ and $16$ and $12\%$
at $32$, the carry tree's root addition sitting after the main one; on
throughput per area the trade is worth $2.1$--$3.1\times$, the fan-in-$16$ row
being the weakest at $2.11\times$ and fan-in $32$ the strongest at $3.14\times$.
...
Table~\ref{tab:nodepipe} measures the trade: at fan-in 8 and 16 the cut buys $45\%$
more frequency for $16\%$ and $5\%$ more area; at 32 it buys $30\%$ for $42\%$. The crossover is where $N/2$ registers
...
of DSP, and Table~\ref{tab:oneadder} prices that step on its own.
...
What the one-adder family costs is registers, not granularity. At degree $9$ its
best member is $1.0850702$ under $\{0,\pm1\}$, $5.7\%$ from the eight-bit target
of $1.026159$; but the roots of $r^d=r+1$ descend monotonically towards $1$, as
Theorem~\ref{thm:oneadder} states, and degree $27$ reaches $1.0265049$ ---
within $0.034\%$ --- at the same single addition. An earlier version of this
paragraph said these errors ``do not improve with degree'', which contradicts
that theorem and is withdrawn. The target is reached; it is reached at $27$
registers rather than the $9$ the unrestricted enumeration needs. Any ratio is
reachable multiply-free if adders are spent; this family reaches any ratio with
one adder, and pays in state.
...
This places the alphabet exactly. $\varphi$ is the right rung at four bits on
the measured column, and the continuous optimum does not by itself say so:
$r^{*}=1.4115$ falls between plastic $1.3247$ and supergolden $1.4656$, below
$\varphi$ rather than beside it, so the nearest-ratio rule alone would select
supergolden. An earlier version of this sentence placed $r^{*}$ between
$\varphi$ and supergolden, which is arithmetically false. What selects $\varphi$
here is the measurement, not the proximity. It is the
...
The closure half of this argument is machine-checked, and the artefact is named
so it can be run rather than believed. In
\texttt{proofs/CorePhi.v} of the artefact repository named in the
Reproducibility section, theorem \texttt{fib\_step\_is\_phi\_mul}
proves that $(a,b)\mapsto(b,a+b)$ computes multiplication by $\varphi$ on the
reals; \texttt{zphi\_add\_closed}, \texttt{zphi\_opp\_closed} and
\texttt{zphi\_zero} close the lattice under the three arms of the
$\{-\varphi,0,+\varphi\}$ alphabet; and \texttt{dot\_exact} carries the result
along a whole dot product by induction on its length. The file compiles under
\texttt{coqc} (Rocq 9.2) with twenty-two theorems, twenty-two \texttt{Qed} and
no \texttt{Admitted} or \texttt{Axiom}. Until the revision named in the
Reproducibility section that file did not compile at all and stated three false
power identities; the correction is described there.
...
Table~\ref{tab:zerodsp} is post-route estimated for an XC7A200T through a fully
open flow, with no DSP blocks used anywhere.
...
Table~\ref{tab:rejected}. \textbf{Two of the four TNF rungs are at their
specified parameters here; two are not}. TNF8 at $10$ bits and TNF32 at $36$
bits are the reconciled rungs. TNF16 reads $17$ bits and TNF64 reads $65$: both
are pre-reconciliation modules, at $M{=}9$ and $M{=}52$ against the specified
$M{=}11$ and $M{=}56$, whose reconciled packed words are $19$ and $69$ bits and
have not been placed. Each is named here by the width of the module that was
actually placed, and neither row should be read against a specified rung of the
same name. All four moved from where they were: the modules
previously measured
...
\caption{Canon plate. With every decoder bit folded into one observed output, a tapered format is paid in delay rather than in area: the two families overlap on area, posit8 at 214 LUT against IBM hexadecimal's 243, while every fixed field leads every taper on frequency -- by 1.43 times at the boundary and 19.9 times at the extremes.}
...
without any individual measurement being wrong. Table~\ref{tab:cleandecode} is
measured on the only harness that passes the observation gate we wrote
afterwards: decoder
...
one cheaply and pay for the other (Table~\ref{tab:bothops}).
...
the wrong thing. It is reported apart because it declines the task the table
sets, not because of where it would place: on the surviving numbers this flow
does not separate the two, \texttt{int8} at $0.1736$\,MHz/LUT against
GFTernary's $0.1797$. The claim that it led the table rested on the $7\%$ lead
withdrawn seven lines above.
...
modern formats live: Table~\ref{tab:packloss} counts what is lost at four widths.

\begin{table}[t]\centering
\caption{Codes lost to packing when a ternary exponent is stored in a binary
word \emph{of its own}. The rounding-up is a property of the integers; paying it
once per value rather than once per block is an implementation choice, and
Section~\ref{sec:matchedwidth} prices the alternative.}
\label{tab:packloss}
\begin{tabular}{rlrr}
\toprule
...
\caption{Canon plate. The substring gate reports 882 of 885 distinctive literals as tracing to the tree; a stricter registry restricted to frequencies finds 12 of 44 with no record file (16 of 40 before that registry was committed and could be re-run), every one of them marked in place, and five document-to-code pairs still have no gate.}
...
(\texttt{docs/}\allowbreak\texttt{reports/}\allowbreak\texttt{gates/}\allowbreak\texttt{check\_paper\_numbers.py};
 an earlier draft cited it as \texttt{tools/check\_paper\_numbers.py}, where it has never
 been, and the script's own input path pointed inside \texttt{docs/reports/}, so the
 gate could not run at all until this revision) extracts every distinctive numeric
...
of \textbf{44} frequency literals in this paper, \textbf{12} are stated in no
record file.

That reading was \textbf{16} of \textbf{40} until this revision, and the
discrepancy is worth stating rather than reconciling away, because the earlier
figure came from a script that was not in the repository and could not be re-run.
The registry is now committed at the path named above and reports the numbers
here. Six literals the table below marks untraced --- $318.47$, $78.83$, $77.00$,
$73.51$, $67.06$ and $43.04$ --- the committed registry does find, in
\texttt{docs/theory/}\allowbreak\texttt{IGLA-FORMAL-RESULTS.md} and two audit
notes. That is not a repair: those documents \emph{restate this paper's own
numbers}, and a restatement is not a measurement record, which is precisely the
conclusion the next paragraph reaches for the whole column. The registry counts
any delimited match in a prose or script file, so its ``traced'' is the weakest
sense of the word and is reported as such. Two literals it adds, $66.28$ and
$66.91$, appear in no file at all. Table~\ref{tab:untraced} is left as first
published, marked against the committed run rather than rewritten to agree with
it. Every one of the sixteen is
marked $^{\ast}$ where it appears, and
...
\texttt{BROKEN\_IS\_SMALL\_2026-08-11.md}. An ordering by MHz per LUT --- the ordering
Figure~\ref{fig:fullthroughput} draws --- in which our
...
withdrawal note said to be in \texttt{research/frontier/} states that the ladder
frequencies rest on $298$ \texttt{nextpnr-xilinx} logs under
\texttt{fpga/phiscale/}. Neither survives: there is no \texttt{research/frontier/}
in the repository at this revision, so the note and its count of $298$ cannot be
checked either, and \texttt{fpga/phiscale/} holds $145$ files of which none is a
log. So even the twenty-four
...
are corrected in Table~\ref{tab:crosscorrected}: takum32 moves from $5.9$--$10.2$ to an exact $2.6$, the
...
in Section~\ref{sec:limits} and in the erratum discussion.
...
Table~\ref{tab:zeckcost} measures both adders through the same technology
mapping. This is a negative result and we report it as one. It does not touch the closure
...
\subsection{The condition is where the bulk sits, and the threshold is \texorpdfstring{$2^{12}$}{2\textasciicircum 12}}
...
iterations against that format's $27$ (Table~\ref{tab:downstream}). This is a task-level number in this paper
...
\begin{sloppypar}
The run was a closed artefact when it was made, and is not one now: neither the
generator nor the record named below is in the repository at this revision, so the
paragraph that follows describes an artefact a reader cannot fetch. It is left
stated rather than deleted, on the same principle as
Section~\ref{sec:provenance}: an unreachable artefact is a marked gap, not a
quietly dropped claim. Command as run:
...
The intention was to bump the vector specification version rather than overwrite
files whose digests are cited, leaving the affected set as \texttt{tnf-vectors-2}
and publishing the repaired one as \texttt{tnf-vectors-3}. That is what earlier
drafts of this paragraph described, and it did not happen. No such tag exists on
the remote, no second set of files is in the tree, and every vector file in
\texttt{conformance/vectors/} carries the suffix \texttt{\_v0}: the lineage
cannot be reconstructed from what was committed, so naming it here would be
inventing a history to match a sentence.

What does exist, from this revision, is a manifest of the set that is there.
\texttt{conformance/vectors/SHA256SUMS} carries a SHA-256 digest for each of the
$100$ files, written and checked by
\texttt{conformance/}\allowbreak\texttt{make\_vector\_manifest.py}:

\begin{quote}
\texttt{python3 conformance/make\_vector\_manifest.py -{}-check}
\end{quote}

\noindent returns $0$ when every file matches and non-zero on the first drifted
digest, a missing file, or a file present in the directory and absent from the
manifest. A transcript claiming conformance should therefore quote the commit
and this manifest, never a version name, and any transcript that cites a
\texttt{tnf-vectors} tag should be re-read: the tag it names has never
resolved.

\section{Six ways an arithmetic-hardware result dies quietly}\label{sec:checklist}

Every finding in this section was found the expensive way, in our own numbers,
and none of it is about ternary. Each cost us a published claim; each is cheap to
check once you know to look; and each is stated here as a procedure rather than a
confession, because the failure modes are properties of the tools and not of this
format. A reader who never intends to build a ternary datapath can still run all
six against their own corpus in an afternoon.

\paragraph{1. Synthesis deletes the thing you are measuring.} We embedded checks
in the datapath so that a wrong result could not be silently produced, then
measured the area of the result. The synthesiser folded fifteen of twenty-eight
on-die clauses to constants and removed them, so every area figure we published
measured a design with its checks already deleted --- and the smaller the design,
the more of it had been removed, which biases exactly the comparison an area
table is for.
\emph{Procedure:} synthesise twice, once with the checks and once with them cut
by hand, and compare cell counts. If the two agree, the checks were folded in the
first build. Read the synthesis log for constant propagation into assertion
clauses and report the count, as a fraction, beside the area.

\paragraph{2. Frequency across designs of different size is a size proxy.} We
ranked formats by MHz per LUT and took the ranking for a figure of merit. Over
our own corpus, across a $41\times$ span of area, achieved frequency follows
$F_{\max}\approx 3174\cdot\mathrm{LUT}^{-0.648}$ at $R^2=0.92$: size explains
$92\%$ of frequency, so MHz per LUT is close to a restatement of area, and the
ranking it produced was a ranking by smallness.
\emph{Procedure:} before reporting frequency per area, fit $F_{\max}$ against
area over your own rows. An exponent near $-0.65$ with high $R^2$ means the
quantity is measuring size. Residuals are where the structure is: ours span a
factor of two, and that factor is the only part of the frequency column that
carries information about the design.

\paragraph{3. A proof base with no admitted lemmas can be checking nothing.} Our
Coq development closed every lemma with \texttt{Qed} and contained zero
\texttt{Admitted} and zero \texttt{Axiom}. It also did not compile, and three of
its statements were false --- $\varphi^3=2\sqrt5+3$ is $7.472$ where $\varphi^3$
is $4.236$. ``No admitted lemmas'' is a grep over a file, and a grep cannot
distinguish a proof from a file that was never type-checked.
\emph{Procedure:} report the checker, its version, the command and its exit
status, not a count of \texttt{Admitted}. Put the command in the paper so a
referee can run it. An artefact claim that cannot be executed in one line is not
an artefact claim.

\paragraph{4. An oracle that decodes what the format cannot name inflates the
format.} A ternary exponent of $E_t$ trits occupies
$\lceil E_t\log_2 3\rceil$ bits, so the field holds more code points than the
trits can name: at $E_t=4$ it holds $128$ and the trits reach $81$. Our decoder
masked to the field width and returned ordinary normals for the other $47$, which
put our own format at $516{,}096$ values and $127.0$ binades where the trits give
$323{,}584$ and $79.0$. The error flattered us by $1.6\times$.
\emph{Procedure:} the tell is a metric that comes out as a power of two minus one.
A four-trit exponent reporting $127.0$ binades is reporting $2^7-1$, which is a
binary field wearing a ternary label. Check that the value count is divisible by
the number of code points the format actually names, and check every derived
range against the format's own definition rather than against the decoder.

\paragraph{5. Substring provenance is worthless for short literals.} Our
traceability gate matched a figure as a substring of a data file with separators
stripped. A frequency of $307.69$ passed because the digits $30769$ occur inside
an unrelated conformance-vector column, and two figures reading $3.15$ passed
with no generating file at all.
\emph{Procedure:} require a delimited match, bounded by non-digit characters, and
treat any literal of three or four significant characters as unmatched until it
is. Then \emph{audit the passes.} Failures are already visible and will be
investigated; a false pass is silent, and it was auditing our passes rather than
our failures that exposed this.

\paragraph{6. A restatement is not a record.} Even after the strict registry, a
figure counted as sourced when it appeared in a prose document --- and prose
documents in a project restate the project's own numbers. Twenty-four of our
frequencies were ``sourced'' in this sense, to a document reporting a
measurement, never to the tool output that produced it. Not one
place-and-route log is in the repository: \texttt{git ls-files} finds zero.
\emph{Procedure:} a claim is closed by a command, its inputs, its log and a hash,
and by nothing weaker. Where the log is absent, say so at the point of use rather
than at the end. Sections~\ref{sec:selfcheck} and~\ref{sec:provenance} carry ours.

\paragraph{What the six have in common.} None was caught by review, and all six
were caught by re-running something that had already passed. A published number
is a claim that a procedure was executed, and the procedures above all failed
while reporting success --- a folded check, a compiling-looking proof, a decoder
answering out of range, a matcher matching the wrong thing. That is the class of
error that survives peer review, because peer review reads the paper and not the
build. The only defence we found is to distrust the passes.
...
the device, TNF64 at $7{,}479$ LUTs and $48.20$\,MHz, and Figure~\ref{fig:ladder}
plots those five rungs in area and achieved frequency. TNF128 at its reconciled
$M = 119$ does not
...
\paragraph{Where the artefacts are.} The artefacts of this paper have their own
repository: \url{https://github.com/gHashTag/ternary-network-floats}, at commit
\texttt{f29b29b}. It holds the reference oracle, the Coq development, the
matched-width data and the formal-equivalence modules, and nothing else, so a
reader can check this paper without cloning the monorepo it was developed in
(\url{https://github.com/gHashTag/t27}, commit \texttt{0bf0a309d}, where
``this revision'' below and above points).

Two commands settle the quantitative claims:

\begin{quote}
\texttt{python3 verify.py} \\
\texttt{coqc -Q proofs Trinity proofs/CorePhi.v}
\end{quote}

\noindent The first recomputes the matched-width table, the one-adder family
roots and the node area \emph{from the oracle} rather than reading them from a
stored record, and exits non-zero if any number moves; its twenty checks are
listed against the claims they settle in the repository's \textsc{readme}. The
second returns $0$. An earlier draft pinned the tag \texttt{v4.0-trinity},
which does not exist on either remote and never did, so each of the twelve
provenance claims in this paper resolved to nothing; commit hashes are used here
because they cannot drift.

\paragraph{What is machine-checked, and where.} The $\varphi$ identities on
which the closure argument rests are in \texttt{proofs/CorePhi.v} of that
repository: \texttt{phi\_square} ($\varphi^2=\varphi+1$),
\texttt{trinity\_identity} ($\varphi^2+\varphi^{-2}=3$), and the Fibonacci
powers \texttt{phi\_cubed\_fib}, \texttt{phi\_fourth\_fib},
\texttt{phi\_fifth\_fib} ($\varphi^n=F(n)\varphi+F(n-1)$, which is the step the
datapath performs). The closure results are there too:
\texttt{fib\_step\_is\_phi\_mul}, \texttt{zphi\_add\_closed},
\texttt{zphi\_opp\_closed}, \texttt{zphi\_zero} and \texttt{dot\_exact}. The
file compiles under \texttt{coqc} (Rocq 9.2) with twenty-two theorems,
twenty-two \texttt{Qed} and no \texttt{Admitted} or \texttt{Axiom}. It is checkable in one command and the reader is invited to run it rather
than take the claim.

That claim needed the correction. Until this revision the same file stated
$\varphi^3=2\sqrt5+3$, which is $7.472$ where $\varphi^3$ is $4.236$, and two
further powers with the same error --- the Fibonacci form written as
$F(n)\sqrt5+F(n+1)$. It also did not compile: \texttt{phi\_square} read
\texttt{apply phi\_quadratic; ring}, which derives nothing, and
\texttt{phi\_quadratic} called \texttt{field} on a goal requiring
$\sqrt5\cdot\sqrt5=5$, which \texttt{field} cannot know. Every lemma ended in
\texttt{Qed} and none was ever checked, so ``no admitted lemmas'' was a true
statistic over a file that type-checked nothing. Nothing outside those identities
depended on the false statements, and the closure argument is unchanged; what
changed is that it is now verified rather than asserted.

\paragraph{What is not in the repository.} Section~\ref{sec:provenance} lists,
by name, the figures whose generating data this repository does not contain.
Those are the claims a reader cannot check, and they are marked rather than
repaired.
\begin{figure}[htbp]\centering
...
current version. Source: \url{https://github.com/gHashTag/t27}, commit
\texttt{0bf0a309d}.
