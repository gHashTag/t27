...
impl Mmio for MockMmio {
    fn read32(&mut self, addr: u32) -> u32 {
        ...
        if addr == csr_map::IRQ_STAT {
            ...
            // read (see bootstrap/src/bitnet_irq.rs around line 103).
            // The mock must model the same destructive read so host code
            // exercised against MockMmio behaves identically on silicon.
