...
impl Parser {
    ...
    fn parse_expr_unary(&mut self) -> Result<Node, String> {
        ...
        // Cast: `expr as T` (e.g. `bit as u16`). `as` lexes as an Ident.
        // Wrap in ExprCast; the backends lower it to the inner expression
        // (Verilog is width-loose), so no type width is emitted.
        while self.current.kind == TokenKind::Ident && self.current.lexeme == "as" {
            self.advance(); // consume 'as'
            let cast_ty = self.parse_type_annotation();
            let mut cast = Node::new(NodeKind::ExprCast);
            cast.extra_type = cast_ty;
...
impl RustCodegen {
    ...
    pub fn gen_rust(&mut self, ast: &Node) {
        ...
        // Cross-module calls (`OtherModule::func`) need a Rust import. `ast` is
        // usually the module node itself; collect the OTHER t27 modules referenced
        // anywhere in it and alias each back to its snake_case file/module name
        // (t27 `Gf256` -> `crate::gf256`). Self-modules are excluded.
        let mut defined = std::collections::BTreeSet::new();
        if ast.kind == NodeKind::Module && !ast.name.is_empty() {
            defined.insert(ast.name.clone());
        }
        for child in &ast.children {
            if child.kind == NodeKind::Module && !child.name.is_empty() {
                defined.insert(child.name.clone());
            }
        }
        let mut refs = std::collections::BTreeSet::new();
        Self::collect_module_refs(ast, &mut refs);
        let mut emitted = false;
        for m in refs.difference(&defined) {
            self.write_line(&format!("use crate::{} as {};", Self::pascal_to_snake(m), m));
            emitted = true;
        }
        if emitted {
            self.blank_line();
        ...
        for child in &ast.children {
            match child.kind {
                NodeKind::Module => {
                    let module_name = child.name.clone();
    ...
    // Collect the names of OTHER t27 modules this module calls into (from any
    // `Module::func` reference in a fn body or test), so gen_rust can emit the
    // matching `use` imports. Self-references and non-PascalCase namespaces (e.g.
    // `base::types`) are skipped.
    fn collect_module_refs(node: &Node, refs: &mut std::collections::BTreeSet<String>) {
        if let Some(idx) = node.name.find("::") {
            let m = &node.name[..idx];
            if m.chars().next().is_some_and(|c| c.is_ascii_uppercase()) {
                refs.insert(m.to_string());
            }
        }
        for c in &node.children {
            Self::collect_module_refs(c, refs);
        }
    }

    // PascalCase t27 module name -> snake_case file/module name (Gf256 -> gf256,
    // AdaptiveMcs -> adaptive_mcs), matching the specs/ file naming.
