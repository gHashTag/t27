...
impl Compiler {
    ...
    pub fn compile_wasm(source: &str) -> Result<String, String> {
        let lexer = Lexer::new(source);
        let mut parser = Parser::new(lexer);
        let mut ast = parser.parse()?;
        optimize(&mut ast, &OptConfig::default());
        let mut codegen = WasmCodegen::new();
        codegen.gen_wasm(&ast);
        Ok(codegen.into_string())
    }
...
impl WasmCodegen {
    ...
    fn wasm_type(t27_type: &str) -> &'static str {
        match t27_type {
            "i8" | "u8" => "i32",
            "i16" | "u16" | "gf16" => "i32",
            "i32" | "u32" => "i32",
    ...
    fn gen_wasm(&mut self, ast: &Node) {
        // Header
        self.write_line(";; Generated from t27 spec");
        self.write_line(";; DO NOT EDIT — generated by t27c");
        self.write_line(";; φ² + 1/φ² = 3 | TRINITY");
        self.write_line("(module");
        self.indent();
        self.write_line("(import \"js\" \"log\" (func $log (param i32)))");
        self.write_line("");

        // Collect all constants, functions, types
            ...
            match child.kind {
                NodeKind::Module => {
                    for module_child in &child.children {
                        match module_child.kind {
                            NodeKind::ConstDecl => constants.push(module_child),
                            NodeKind::FnDecl => functions.push(module_child),
                            _ => {}
                        }
                    }
                }
                NodeKind::ConstDecl => constants.push(child),
                NodeKind::FnDecl => functions.push(child),
                _ => {}
            }
        }

        // Generate constants as data or globals
        for const_node in &constants {
            self.gen_const(const_node);
        }
        ...
        for func_node in &functions {
            exports.push(&func_node.name);
            self.gen_fn(func_node);
        }
        ...
        for export in &exports {
            self.write_line(&format!("(export \"{}\" (func ${}))", export, export));
        }
    ...
    fn gen_const(&mut self, node: &Node) {
        let value = if node.children.is_empty() {
            ...
            Self::expr_to_wasm(&node.children[0])
        };

        let wasm_type = Self::wasm_type(&node.extra_type);
        self.write_line(&format!("(global ${} {} (constant {}))", node.name, wasm_type, value));
    }
    ...
    fn gen_fn(&mut self, node: &Node) {
        ...
        let ret_type = if node.extra_return_type.is_empty() {
            ...
            Self::wasm_type(node.extra_return_type.as_str())
        };

        self.write(&format!("(func ${}", fn_name));

        // Parameters
        let mut params_str = String::new();
        for (param_name, param_type) in &node.params {
            params_str.push_str(&format!(" (param ${} {})", param_name, Self::wasm_type(param_type)));
        }
        self.write(&params_str);

        // Return type
        if !ret_type.is_empty() {
            self.write(&format!(" (result {})", ret_type));
        }
        self.write_line(")");

        self.indent();
        self.local_index = node.params.len() as u32;

        // Body
        for child in &node.children {
            match child.kind {
                NodeKind::StmtLocal => self.gen_local(child),
                NodeKind::StmtAssign => self.gen_assign(child),
                NodeKind::StmtExpr | NodeKind::ExprReturn => {
                    if child.kind == NodeKind::ExprReturn {
                        self.write_indent();
                        self.write("return ");
                        if child.children.is_empty() {
                            self.write_line("");
                        } else {
                            self.write_line(&Self::expr_to_wasm(&child.children[0]));
                        }
        ...
        if !ret_type.is_empty() && !node.children.iter().any(|c| c.kind == NodeKind::ExprReturn) {
            self.write_indent();
            match ret_type {
                "i32" | "i64" => self.write_line("i32.const 0"),
                "f32" | "f64" => self.write_line("f64.const 0"),
                _ => self.write_line("unreachable"),
            }
        }
    ...
    fn gen_local(&mut self, node: &Node) {
        let local_name = &node.name;
        let local_type = Self::wasm_type(&node.extra_type);
        self.local_index += 1;
        self.write_line(&format!("(local ${} {})", local_name, local_type));

        if !node.children.is_empty() {
            self.write_indent();
            self.write_line(&format!("local.set ${} {}", local_name, Self::expr_to_wasm(&node.children[0])));
        }
    }
    ...
    fn gen_assign(&mut self, node: &Node) {
        ...
        if node.children.len() == 1 {
            self.write_indent();
            self.write_line(&format!("local.set ${} {}", target, Self::expr_to_wasm(&node.children[0])));
        }
    }

    fn expr_to_wasm(node: &Node) -> String {
        match node.kind {
            NodeKind::ExprLiteral => {
                let value = &node.value;
                match node.extra_type.as_str() {
                    "i32" | "u32" => format!("i32.const {}", value),
                    "i64" | "u64" => format!("i64.const {}", value),
                    "f32" => format!("f32.const {}", value),
                    "f64" => format!("f64.const {}", value),
                    "bool" => format!("i32.const {}", if value == "true" { "1" } else { "0" }),
                    "str" => format!("(i32.const {})", value), // placeholder
                    _ => format!("i32.const {}", value),
                }
            }
            NodeKind::ExprIdentifier => {
                format!("local.get ${}", node.name)
            }
            NodeKind::ExprBinary => {
                let op = match node.extra_op.as_str() {
                    "+" => "add",
                ...
                let prefix = match node.extra_type.as_str() {
                    "f32" | "f64" => "f64.",
                    _ => "i32.",
                };

                if node.children.len() >= 2 {
                    let left = Self::expr_to_wasm(&node.children[0]);
                    let right = Self::expr_to_wasm(&node.children[1]);
                    format!("({}.{} {} {})", prefix, op, left, right)
                } else {
            ...
            NodeKind::ExprCall => {
                if node.children.is_empty() {
                    ...
                    let args: Vec<String> = node.children.iter()
                        .map(|c| Self::expr_to_wasm(c))
                        .collect();
                    format!("(call ${})", node.name) // args passed implicitly
                }
            }
            NodeKind::ExprReturn => {
                if node.children.is_empty() {
                    ...
                    format!("return {}", Self::expr_to_wasm(&node.children[0]))
                }
            }
