...
impl Compiler {
    ...
    pub fn compile_ts(source: &str) -> Result<String, String> {
        let lexer = Lexer::new(source);
        let mut parser = Parser::new(lexer);
        let mut ast = parser.parse()?;
        optimize(&mut ast, &OptConfig::default());
...
impl TypeScriptCodegen {
    ...
    /// Map t27 type to TypeScript type string
    fn t27_type_to_ts(ty: &str) -> String {
        match ty {
            "bool" => "boolean".to_string(),
            "u8" | "i8" | "u16" | "i16" | "u32" | "i32" | "u64" | "i64" => "number".to_string(),
            "f32" | "f64" => "number".to_string(),
            "GF16" | "gf16" | "phi" => "number".to_string(),
            "void" => "void".to_string(),
            "str" => "string".to_string(),
            "usize" => "number".to_string(),
            t if t.starts_with("[]") => {
                let inner = &t[2..];
                format!("{}[]", Self::t27_type_to_ts(inner))
            }
            t => t.to_string(), // Custom type name
        }
    }

    pub fn gen_typescript(&mut self, ast: &Node) {
        // Header
        let module_name = if !ast.name.is_empty() {
            &ast.name
        } else {
        ...
        for decl in &ast.children {
            match decl.kind {
                NodeKind::ConstDecl => self.gen_ts_const(decl),
                NodeKind::EnumDecl => self.gen_ts_enum(decl),
                NodeKind::StructDecl => self.gen_ts_struct(decl),
                NodeKind::FnDecl => self.gen_ts_fn(decl),
                _ => {}
            }
        }
    }

    fn gen_ts_const(&mut self, node: &Node) {
        let ts_type = Self::t27_type_to_ts(&node.extra_type);
        self.write_line(&format!(
            "export const {}: {} = {};",
            node.name, ts_type, if node.children.is_empty() {
                "null".to_string()
            } else {
                Self::expr_to_ts(&node.children[0])
            }
        ));
    }

    fn gen_ts_enum(&mut self, node: &Node) {
        self.write_line("export enum {");
        self.indent += 1;
        for child in &node.children {
            if child.kind == NodeKind::EnumVariant {
                self.write_indent();
    ...
    fn gen_ts_struct(&mut self, node: &Node) {
        ...
        for child in &node.children {
            if child.kind == NodeKind::ExprIdentifier {
                let field_type = Self::t27_type_to_ts(&child.extra_type);
                self.write_line(&format!("    {}: {};", child.name, field_type));
            }
        }
        self.indent -= 1;
        self.write_line("}");
    ...
    fn gen_ts_fn(&mut self, node: &Node) {
        let ret_type = if node.extra_return_type.is_empty() {
            "void".to_string()
        } else {
            Self::t27_type_to_ts(&node.extra_return_type)
        };
    ...
    fn expr_to_ts(node: &Node) -> String {
        match node.kind {
            ...
            NodeKind::ExprBinary => {
                if node.children.len() >= 2 {
                    let left = Self::expr_to_ts(&node.children[0]);
                    let right = Self::expr_to_ts(&node.children[1]);
                    let op = &node.extra_op;
            ...
            NodeKind::ExprCall => {
                let args: Vec<String> = node
                    .children
                    .iter()
                    .map(|c| Self::expr_to_ts(c))
                    .collect();
                format!("{}({})", node.name, args.join(", "))
            }
            NodeKind::ExprFieldAccess => {
                if !node.children.is_empty() {
                    format!("{}.{}", Self::expr_to_ts(&node.children[0]), node.name)
                } else {
                    node.name.clone()
                }
            }
