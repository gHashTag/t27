...
impl Parser {
    ...
    fn parse_const_decl(&mut self, is_pub: bool) -> Result<Node, String> {
        ...
        if self.current.kind == TokenKind::Colon {
            self.advance(); // consume :
                            // W470: use the shared type parser so multi-dimensional
                            // array annotations like `[2][3]Pt` are preserved intact.
        ...
        if self.current.kind == TokenKind::Equals {
            ...
            if self.current.kind == TokenKind::KwEnum {
                ...
                // W469: identifier RHS may be a plain name (type alias, enum variant,
                // or simple value) OR a struct literal `Pt{ .x = 1, .y = 2 }`.
                // If a '{' follows the identifier, parse the whole expression so the
                // struct literal is preserved in the AST; otherwise keep the legacy
                // single-token behavior for type aliases and simple constants.
                let checkpoint = self.save_state();
                let ident_name = self.current.lexeme.clone();
                self.advance();
                if self.current.kind == TokenKind::LBrace {
    ...
    fn parse_fn_body(&mut self, decl: &mut Node) -> Result<(), String> {
        while self.current.kind != TokenKind::RBrace && self.current.kind != TokenKind::Eof {
            // W468: local pragmas apply to the next statement inside a function
            // body. Parse them inline and let `parse_local_decl` pick up the
            // pending pragma.
            if self.current.kind == TokenKind::KwPragma {
                if let Err(e) = self.parse_pragma() {
    ...
    fn parse_local_decl(&mut self) -> Result<Node, String> {
        ...
        // W468: apply any pending pragma (from a preceding `pragma ...;` statement)
    ...
    fn parse_array_literal(&mut self) -> Result<Node, String> {
        ...
        // W468: consume the (outermost) bracket pair and any further dimension
        // brackets, so `[2][3]u16{...}` is parsed as a single 2D array literal
        // with extra_size="2" and extra_type="[3]u16".
        let mut outer_size = String::new();
        if self.current.kind == TokenKind::RBracket {
            self.advance();
        } else {
            ...
            outer_size = bracket_content.trim().to_string();
            self.expect(TokenKind::RBracket)?;
        }

        let mut full_elem_type = String::new();
        while self.current.kind == TokenKind::LBracket {
            self.advance();
            let mut bracket_content = String::new();
            while self.current.kind != TokenKind::RBracket
            ...
            full_elem_type.push('[');
            full_elem_type.push_str(&bracket_content);
            full_elem_type.push(']');
            self.expect(TokenKind::RBracket)?;
        }

        if self.current.kind == TokenKind::Ident {
            full_elem_type.push_str(&self.current.lexeme);
            self.advance();
        }
        node.extra_size = outer_size;
        node.extra_type = full_elem_type;
...
pub struct VerilogCodegen {
    ...
    // W468: names of functions declared with zero parameters (and a non-void
    // return). Their generated Verilog function has a dummy `input _unused;`, so
    // calls must supply a placeholder argument.
    fn_zero_arg_functions: std::collections::HashSet<String>,
    // W468: loop variables already declared in the current procedural block
    // (function body or test/bench initial block). Prevents duplicate Verilog
    // `integer` declarations when a single variable is used by multiple loops.
    loop_vars_declared: std::collections::HashSet<String>,
    ...
    // W466: element type and size for each function-local/bench-local array,
    // keyed by the original declared name. Used to resolve indexed field access
    // on local arrays whose element type is a struct, including nested fields.
    local_array_elem_info: std::collections::HashMap<String, (usize, String)>,
    // W468: dimensions for function-local/bench-local arrays with more than one
    ...
    module_struct_array_fields: std::collections::HashMap<String, Vec<(String, String)>>,
    ...
    // W467: map from local/bench-local struct variable name to its declared struct
    // type. Used to decompose whole-struct assignment into per-field scalar/memory
    // assignments.
    local_struct_var_types: std::collections::HashMap<String, String>,
    ...
    module_scalar_struct_fields: std::collections::HashMap<String, Vec<(String, String)>>,
    // W470: struct type name for each module-level scalar struct variable/constant.
    // Enables whole-struct assignment lowering between module-level scalar structs.
    module_scalar_struct_types: std::collections::HashMap<String, String>,
    // W469: scalar struct parameters of the current function, keyed by parameter
    // name. Each entry lists the flattened leaf fields (name, type) in MSB-first order.
    scalar_struct_params: std::collections::HashMap<String, Vec<(String, String)>>,
...
impl VerilogCodegen {
    ...
    fn gen_verilog_struct_return_slicing(
        ...
        let flat_fields =
            Self::struct_field_widths(struct_name, &self.struct_fields);
        if flat_fields.is_empty() {
            return;
        }
        let total_width: u32 = flat_fields.iter().map(|(_, w)| w).sum();
        ...
        for (fname, width) in &flat_fields {
    ...
    fn gen_verilog_pack_scalar_struct(
        ...
        let field_widths = Self::struct_field_widths(struct_type, &self.struct_fields);
        self.write("{");
        for (i, (fname, _width)) in field_widths.iter().enumerate() {
            if i > 0 {
                self.write(", ");
            }
            // Build the raw concatenated name first, then escape it as a whole.
            // If we escaped the base name first (e.g., \task ), appending the
            // field suffix would create an invalid escaped identifier.
            let flat = format!("{}_{}", base_name, fname);
            self.write(&Self::verilog_safe_identifier(&flat));
        }
        self.write("}");
    }
    ...
    /// W470: emit a scalar struct expression as a packed vector. Identifiers are
    /// packed from their per-field registers; other expressions (struct literals
    /// or struct-return calls) are emitted normally and are already packed.
    fn gen_verilog_pack_scalar_struct_expr(
        ...
        if node.kind == NodeKind::ExprIdentifier {
            self.gen_verilog_pack_scalar_struct(&node.name, struct_type,
            );
        } else {
    ...
    /// non-struct types so callers can fall back to scalar behavior.
    ...
    fn struct_field_widths(ty: &str, struct_fields: &std::collections::HashMap<String, Vec<(String, String)>>) -> Vec<(String, u32)> {
        if !ty.is_empty() && struct_fields.contains_key(ty) {
            let mut flat = Vec::new();
            Self::flatten_struct_fields(ty, struct_fields, "", &mut flat);
            flat.into_iter().map(|(fname, ftype)| (fname, Self::type_to_width(&ftype))).collect()
        } else {
            Vec::new()
        }
    }
    ...
    fn gen_verilog_const(&mut self, node: &Node) {
        ...
        {
            // W469: scalar struct constant. Emit one register per flattened scalar
            // leaf field and initialize it in an initial block. Register the field
            // list so field accesses and whole-struct comparison can resolve.
    ...
    fn gen_verilog_var(&mut self, node: &Node) {
            ...
            && self.struct_fields.contains_key(&node.extra_type)
        {
            ...
            for (fname, ftype) in &flat_fields {
                ...
                self.write_line(&format!(
                    "reg {}{} {}_{};",
                    fsigned_str, frange_str, safe_name, fname
                ));
            }
            if !node.children.is_empty() {
                self.write_indent();
    ...
    fn gen_verilog_fn_internal(
        ...
        // W470: unpack each scalar struct parameter into per-field local registers
        // so the function body can reference p_x / p_inner_a / etc. directly.
        for (pname, ptype) in &scalar_struct_params_to_unpack {
            let safe_pname = Self::verilog_safe_identifier(pname);
            let field_widths = Self::struct_field_widths(ptype, &self.struct_fields);
            let total_width: u32 = field_widths.iter().map(|(_, w)| w).sum();
            let mut cursor = total_width;
            for (fname, fwidth) in &field_widths {
                let high = cursor.saturating_sub(1);
                ...
                if let Some(expr) = node.children.first() {
                    if expr.kind == NodeKind::ExprCall {
                        ...
                        if expr.name == "assert" && !expr.children.is_empty() {
                            // W459: `assert(cond)` is emitted as a SystemVerilog
                            // assertion statement. The optional message argument is
                            // ignored for smoke compatibility; the condition still
                            // drives pass/fail semantics in simulation.
                            self.write_indent();
                            self.write("assert(");
                            ...
                            self.write_indent();
                            self.write_line("$display(\"[TEST] assertion failed\");");
                            self.dedent();
                            self.write_indent();
                            self.write_line("end");
                            self.dedent();
    ...
    fn gen_verilog_local_multi_dim_read(
        ...
        for (comb_idx, comb) in combos.iter().enumerate() {
            ...
            self.write(") ? (");
            let name = comb.iter().fold(safe_base.clone(), |acc, &i| {
                format!("{}_{}", acc, i)
            });
            self.write(&Self::verilog_safe_identifier(&name));
            self.write(")");
        ...
        match node.kind {
            ...
            NodeKind::StmtLocal => {
                    ...
                    && self.struct_fields.contains_key(&node.extra_type)
                {
                    let base_name = &node.name;
                    self.local_struct_var_types
                        .insert(base_name.clone(), node.extra_type.clone());
                    self.gen_verilog_local_struct_var_decl(
                        base_name,
                        &node.extra_type,
                    );
        ...
        match node.kind {
            ...
            NodeKind::ExprCall => {
                ...
                for (i, arg) in node.children.iter().enumerate() {
                    ...
                    first = false;
                    // W470: scalar struct variables passed as function arguments must
                    // be packed into a concatenation matching the callee's input width.
                    if arg.kind == NodeKind::ExprIdentifier {
            ...
            NodeKind::ExprBinary => {
                if node.children.len() >= 2 {
                    ...
                    if node.extra_op.as_str() == "*" {
                        ...
                        let lhs_ty = self.scalar_struct_expr_type(&node.children[0]);
                        let rhs_ty = self.scalar_struct_expr_type(&node.children[1]);
            ...
            NodeKind::ExprFieldAccess => {
                if !node.children.is_empty() {
                        ...
                        && child.children.len() >= 2
                    {
                        ...
                        // W466/W470: nested field access on an array of structs:
                        // arr[i].inner.a or pts[i][j].coords[0][0]. Flatten the
                        // dotted path and the index chain, then emit the appropriate
                        // field-indexed memory or per-element per-field register.
                        let mut emitted = false;
                        if let Some((idx_node, field_suffix)) =
                            Self::flatten_nested_array_field_access(node)
                        {
                            let multi_dim = Self::flatten_index_chain(idx_node);
                            let (base_name, idx, is_multi) = match &multi_dim {
                                Some((base_id, indices)) if indices.len() > 1 => {
                                    (base_id.name.clone(), indices[0], true)
                                }
                                _ => {
                                    let base_name = match idx_node.children[0].kind {
                                        NodeKind::ExprIdentifier => {
                                            idx_node.children[0].name.clone()
                                        }
                                        _ => String::new(),
                                    };
                                    (base_name, &idx_node.children[1], false)
                                }
                            };

                            // W470: multi-dimensional array parameter bound to a module-level
                            // struct array with nested field access.
                            if is_multi {
                                if let Some(bound) =
            ...
            NodeKind::ExprStructLit => {
                ...
                if self.struct_fields.contains_key(&node.name) {
                    let flat_fields = self
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let mut leaf_values: Vec<Option<&Node>> = Vec::new();
                    let mut all_found = true;
                    for (fname, ftype) in &flat_fields {
                        let val = node.children.iter().find(|f| {
                            f.kind == NodeKind::ExprFieldAccess
                                && f.name == *fname
                                && !f.children.is_empty()
                        }).map(|f| &f.children[0]);
                        if val.is_none() && self.struct_fields.contains_key(ftype) {
                            // The source literal specified the nested struct as a
                            // whole rather than field-by-field; we cannot currently
                            // pack that inline, so fall back.
                            all_found = false;
                            break;
                        }
                        leaf_values.push(val);
                    }
                    if all_found && !leaf_values.is_empty() {
                        self.write("{");
                        for (i, (val, (fname, _ftype))) in
                            leaf_values.iter().zip(flat_fields.iter()).enumerate()
                        {
                            if i > 0 {
                                self.write(", ");
                            }
                            if let Some(v) = val {
                                ...
                                // Missing field: emit zero of the appropriate width.
                                let fwidth = Self::type_to_width(_ftype);
...
mod tests_hir_pipeline_parity {
    ...
    pub fn get_a(idx: u32) -> u8 {
        ...
        // W466: variable-index reads of local struct arrays now emit a priority
        // mux over per-element per-field registers such as pairs_0_a / pairs_1_a.
        assert!(v.contains("pairs_0_a"), "expected flattened name pairs_0_a");
        assert!(v.contains("pairs_1_a"), "expected flattened name pairs_1_a");
    }
