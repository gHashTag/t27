...
pub fn build_bitnet_engine_top(module_name: &str) -> String {
    ...
    s.push_str("    // BOUND: chunk_addr counts one step per layer_valid and resets only on\n");
    s.push_str("    // layer_start, so between two layer starts it advances num_neurons *\n");
    s.push_str("    // num_chunks times. It drives weight_bram.rd_addr, whose depth is 4096 --\n");
    s.push_str("    // exactly what 12 bits hold -- so the caller must keep that product within\n");
    s.push_str("    // 4096. num_neurons is a 16-bit port and num_chunks an 8-bit one, so their\n");
    s.push_str("    // product can reach 16.7M: nothing in this module, and nothing in the\n");
    s.push_str("    // sequencer, prevents the wrap. The contract lives entirely in whoever\n");
    s.push_str("    // programs the CSRs.\n");
    s.push_str("    // \n");
    s.push_str("    // Recorded in Wave 636b, after bound_scan stopped crediting formal\n");
    s.push_str("    // assertions as design bounds. This register read as LOCAL for four waves\n");
    ...
    s.push_str("    // BOUND: act_wr_word counts one packed activation word per 27 neurons and\n");
    s.push_str("    // resets on layer_start, so it advances FLOOR(num_neurons / 27) times\n");
    s.push_str("    // per layer -- not ceil. Wave 653: this annotation said ceil, which is\n");
    s.push_str("    // what the design was INTENDED to do and is not what it does.\n");
    s.push_str("    // activation_requant raises word_valid only at trit_count == 26, so a\n");
    s.push_str("    // layer whose neuron count is not a multiple of 27 leaves its last\n");
    s.push_str("    // (num_neurons mod 27) results in a partial word that is never emitted,\n");
    s.push_str("    // and nothing constrains num_neurons to a multiple of 27. The module\'s\n");
    s.push_str("    // own inline property a_word_only_on_full ASSERTS this behaviour, so the\n");
    s.push_str("    // gap is encoded as intended -- the same shape as the Wave 628 defect,\n");
    s.push_str("    // which a unit test had pinned. Confirmed reachable in the assembled\n");
    s.push_str("    // engine by an independent harness. The BOUND argument below is\n");
    s.push_str("    // unaffected, since floor <= ceil. See Prop. 120.\n");
    s.push_str("    // (bound continues)\n");
    s.push_str("    // layer. It addresses the activation buffer through act_wr_addr, 12 bits, so\n");
    s.push_str("    // the caller must keep that count within 4096 -- i.e. num_neurons <= 110592.\n");
    s.push_str("    // A 16-bit num_neurons cannot reach that, so this one is safe BY PORT WIDTH\n");
    s.push_str("    // rather than by contract: 65535/27 = 2427 < 4096. That is a narrower margin\n");
    s.push_str("    // than it looks and it is not checked anywhere; widening num_neurons past 17\n");
    s.push_str("    // bits would wrap this counter. Props. 84, 95b.\n");
