...
pub fn build_bitnet_engine_top(module_name: &str) -> String {
    ...
    s.push_str("        // The interlock must cover the PIPELINE, not just the top-level\n");
    s.push_str("        // state. inference_active tracks the multilayer sequencer's done,\n");
    s.push_str("        // but layer_sequencer and the MAC keep work in flight after it\n");
    s.push_str("        // clears: a host writing ctrl = 2 mid-inference dropped busy while\n");
    s.push_str("        // layer_valid and mac_valid_q were still high, and the DMA started\n");
    s.push_str("        // into a draining pipeline. A busy flag at the top of a pipelined\n");
    s.push_str("        // design does not mean the pipeline is empty. See Prop. 22.\n");
    s.push_str("        .start(reg_ctrl[1] && !reg_ctrl[0] && !busy\n");
    ...
    s.push_str("    // OPEN, and now diagnosed -- no top-level gate can close this.\n");
    s.push_str("    //\n");
    s.push_str("    //   assert (!(dma_local_we && mac_valid_q))   REFUTED\n");
    s.push_str("    //\n");
    s.push_str("    // Three successive narrowings landed, each real and each insufficient:\n");
    s.push_str("    //   1. busy made a state register instead of a decode of current_layer\n");
    s.push_str("    //   2. the interlock made symmetric in both directions\n");
    s.push_str("    //   3. the gate extended across layer_valid / mac_valid_q / trit_valid\n");
    s.push_str("    //\n");
    s.push_str("    // The trace shows why none of them is enough. multilayer_sequencer runs\n");
    s.push_str("    // its own state machine and does NOT stop when the host clears\n");
    s.push_str("    // reg_ctrl[0]: inference_active falls, the DMA gate opens, and the\n");
    s.push_str("    // sequencer then re-raises layer_start on its own. Quiescence is a\n");
    s.push_str("    // property of the sequencer, and this module cannot observe it -- so\n");
    s.push_str("    // gating harder at the top can only ever narrow the window.\n");
    s.push_str("    //\n");
    s.push_str("    // The fix belongs one level down: multilayer_sequencer needs an idle\n");
    s.push_str("    // output (state == IDLE), and the interlock should key off that. That\n");
    s.push_str("    // is a module interface change, deliberately not made here as a fourth\n");
    s.push_str("    // narrowing. Recorded rather than weakened, per Prop. 17. See Prop. 22.\n");
    s.push_str("    always @(posedge clk) if (rst_n)\n");
