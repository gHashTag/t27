...
fn spec_path() -> PathBuf { PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("specs").join("ternary").join("gft_dot4.t27") }
fn scratch_dir(l:&str)->PathBuf{let d=env::temp_dir().join(format!("t27_gft4_{}_{}",std::process::id(),l)); if d.exists(){let _=fs::remove_dir_all(&d);} d}
...
module tb;
  reg [15:0] a1,b1,a2,b2,a3,b3,a4,b4; wire [15:0] y_spec; integer i,fails,o,m;
  wire [15:0] p12,p34,y_ref; wire [31:0] yo,ym;
  GftDot4 dut(.clk(1'b0),.rst_n(1'b1),.en(1'b1),.a1(a1),.b1(b1),.a2(a2),.b2(b2),.a3(a3),.b3(b3),.a4(a4),.b4(b4),.ready(),.result(y_spec));
  gft_dot2 r12(.a1(a1),.b1(b1),.a2(a2),.b2(b2),.y(p12));
  gft_dot2 r34(.a1(a3),.b1(b3),.a2(a4),.b2(b4),.y(p34));
  gft_add #(.OFFSET_MAX(80),.MANT_ONE(512),.SIG_BITS(10)) racc(.a_off({25'd0,p12[15:9]}),.a_mant({23'd0,p12[8:0]}),.b_off({25'd0,p34[15:9]}),.b_mant({23'd0,p34[8:0]}),.out_off(yo),.out_mant(ym));
  assign y_ref = {yo[6:0],ym[8:0]};
  function [15:0] rnd; input integer dummy; begin o=1+($random%79); if(o<1)o=1; if(o>79)o=79; m=$random%512; if(m<0)m=-m; rnd=(o<<9)|m; end endfunction
  initial begin
    fails=0;
    for(i=0;i<2000;i=i+1) begin
      a1=rnd(0);b1=rnd(0);a2=rnd(0);b2=rnd(0);a3=rnd(0);b3=rnd(0);a4=rnd(0);b4=rnd(0);#1;
...
"#;
#[test]
fn spec_first_gft_dot4_matches_silicon_tree() {
    ...
    assert!(v.contains("input  wire [15:0] a4")&&v.contains("output wire [15:0] result"),"missing dot4 interface:\n{}",v);
    ...
    assert!(o.contains("ALL_PASS 2000"),"dot4 differs from silicon tree:\n{}",o);
    assert!(!o.contains("FAIL"),"dot4 mismatch:\n{}",o);
}
