// ============================================================================
// Check for the spec-first GF-T relu primitive (specs/ternary/gft_relu.t27):
// relu(x)=max(0,x) for a signed GF-T16. The missing building
// block for a GF-T softmax. Bit-exact to the committed integer oracle over 606
// vectors (tests/gft_relu_vectors.txt); the oracle is itself <=1 ULP vs the true
// max(0,x) exactly (measured in the prototype).
// ============================================================================
...
fn spec_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("specs").join("ternary").join("gft_relu.t27")
}
fn vectors_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("gft_relu_vectors.txt")
}
...
#[test]
fn spec_first_gft_relu_matches_oracle() {
...
module tb;
  ...
  GftRelu dut(.clk(1'b0),.rst_n(1'b1),.en(1'b1),.x(x),.ready(),.result(y));
  initial begin
...
"#,
    ...
    let _ = fs::remove_dir_all(&dir);
    assert!(stdout.contains(&format!("ALL_PASS {}", n)), "relu differs from the oracle:\n{}", stdout);
    assert!(!stdout.contains("FAIL"), "relu mismatch:\n{}", stdout);
}
