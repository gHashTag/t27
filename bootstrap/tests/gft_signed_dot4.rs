// ============================================================================
// Check for the spec-first SIGNED GF-T16 4-term MAC (specs/ternary/
// gft_signed_dot4.t27, y = sum a_i*b_i): real-valued matmul tile with negatives
// and sign cancellation, round-to-nearest-even. Bit-exact to the ideal oracle
// balanced tree over 300 vectors (tests/gft_signed_dot4_vectors.txt). Skips w/o
// iverilog/vvp.
// ============================================================================
...
fn spec_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("specs").join("ternary").join("gft_signed_dot4.t27")
}
fn vectors_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("gft_signed_dot4_vectors.txt")
}
...
#[test]
fn spec_first_gft_signed_dot4_matches_oracle() {
    ...
    assert!(
        v.contains("input  wire [31:0] a4") && v.contains("output wire [31:0] result"),
        "signed dot4 missing interface:\n{}", v
    );

    if !tool_available("iverilog") || !tool_available("vvp") {
        eprintln!("SKIP: iverilog/vvp not on PATH; skipping signed dot4 check");
        return;
    }

    let vectors = fs::read_to_string(vectors_path()).expect("read vectors");
    let n = vectors.lines().filter(|l| !l.trim().is_empty()).count();
    let dir = env::temp_dir().join(format!("t27_sd4_{}", std::process::id()));
    ...
    let tb = format!(
        r#"`timescale 1ns/1ps
module tb;
  reg [31:0] a1,b1,a2,b2,a3,b3,a4,b4; wire [31:0] y; integer fails,nn,fd,code; reg [31:0] exp;
  GftSignedDot4 dut(.clk(1'b0),.rst_n(1'b1),.en(1'b1),.a1(a1),.b1(b1),.a2(a2),.b2(b2),.a3(a3),.b3(b3),.a4(a4),.b4(b4),.ready(),.result(y));
  initial begin
...
"#,
    ...
    let _ = fs::remove_dir_all(&dir);
    assert!(stdout.contains(&format!("ALL_PASS {}", n)), "signed dot4 differs from the oracle:\n{}", stdout);
    assert!(!stdout.contains("FAIL"), "signed dot4 mismatch:\n{}", stdout);
}
