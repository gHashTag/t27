// ============================================================================
// Check for the spec-first GF-T nll primitive (specs/ternary/gft_nll.t27):
// cross-entropy loss -log2(p) for a probability p; result a positive GF-T16. The missing building
// block for a GF-T softmax. Bit-exact to the committed integer oracle over 606
// vectors (tests/gft_nll_vectors.txt); the oracle is itself <=1 ULP vs the true
// the log2-composed model (measured in the prototype).
// ============================================================================
...
fn spec_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("specs").join("ternary").join("gft_nll.t27")
}
fn vectors_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("gft_nll_vectors.txt")
}
...
#[test]
fn spec_first_gft_nll_matches_oracle() {
    ...
    assert!(
        v.contains("input  wire [31:0] p") && v.contains("output wire [31:0] result"),
        "nll missing p -> result interface:\n{}", v
    );

    if !tool_available("iverilog") || !tool_available("vvp") {
        eprintln!("SKIP: iverilog/vvp not on PATH; skipping nll check");
        return;
    }

    let vectors = fs::read_to_string(vectors_path()).expect("read vectors");
    let n = vectors.lines().filter(|l| !l.trim().is_empty()).count();
    let dir = env::temp_dir().join(format!("t27_nll_{}", std::process::id()));
    ...
    let tb = format!(
        r#"`timescale 1ns/1ps
module tb;
  reg [31:0] p; wire [31:0] y; integer fails,nn,fd,code; reg [31:0] exp;
  GftNll dut(.clk(1'b0),.rst_n(1'b1),.en(1'b1),.p(p),.ready(),.result(y));
  initial begin
    fails=0; nn=0; fd=$fopen("{}","r");
    while(!$feof(fd)) begin code=$fscanf(fd,"%h %h\n",p,exp);
      if(code==2) begin #1; nn=nn+1;
        if(y!==exp) begin fails=fails+1; if(fails<=6)$display("FAIL p=%h y=%h exp=%h",p,y,exp); end
      end end
    $fclose(fd);
...
"#,
    ...
    let _ = fs::remove_dir_all(&dir);
    assert!(stdout.contains(&format!("ALL_PASS {}", n)), "nll differs from the oracle:\n{}", stdout);
    assert!(!stdout.contains("FAIL"), "nll mismatch:\n{}", stdout);
}
