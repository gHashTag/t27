...
// gft_layer3.t27): four shared GF-T16 activations feed three neurons, each with
...
// vectors (tests/gft_layer3_vectors.txt).
// ============================================================================
...
fn spec_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("specs").join("ternary").join("gft_layer3.t27")
}
fn vectors_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("gft_layer3_vectors.txt")
}
...
#[test]
fn spec_first_gft_layer3_matches_oracle() {
    ...
    assert!(
        ...
        "layer3 missing weight/activation -> packed-trit interface:\n{}", v
    );

    if !tool_available("iverilog") || !tool_available("vvp") {
        eprintln!("SKIP: iverilog/vvp not on PATH; skipping layer3 check");
        return;
    }

    let vectors = fs::read_to_string(vectors_path()).expect("read vectors");
    let n = vectors.lines().filter(|l| !l.trim().is_empty()).count();
    let dir = env::temp_dir().join(format!("t27_l3_{}", std::process::id()));
    ...
    let tb = format!(
        r#"`timescale 1ns/1ps
module tb;
  reg [7:0] w00,w01,w02,w03,w10,w11,w12,w13,w20,w21,w22,w23;
  reg [31:0] a0,a1,a2,a3; wire [31:0] y; integer fails,nn,fd,code; reg [31:0] exp;
  GftLayer3 dut(.clk(1'b0),.rst_n(1'b1),.en(1'b1),
  ...
  initial begin
    ...
    while(!$feof(fd)) begin
      code=$fscanf(fd,"%d %d %d %d %d %d %d %d %d %d %d %d %h %h %h %h %d\n",
        w00,w01,w02,w03,w10,w11,w12,w13,w20,w21,w22,w23,a0,a1,a2,a3,exp);
      if(code==17) begin #1; nn=nn+1;
...
"#,
    ...
    let _ = fs::remove_dir_all(&dir);
    assert!(stdout.contains(&format!("ALL_PASS {}", n)), "layer3 differs from the oracle:\n{}", stdout);
    assert!(!stdout.contains("FAIL"), "layer3 mismatch:\n{}", stdout);
}
