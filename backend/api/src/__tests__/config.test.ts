...
describe("localMode", () => {
  ...
  it("is true when SANDBOX_LOCAL_BASE_URL is set in non-production (actual behavior)", async () => {
    // The actual config evaluates localMode as true when SANDBOX_LOCAL_BASE_URL
  ...
  it("is false when SANDBOX_LOCAL_BASE_URL is set but NODE_ENV=production", async () => {
    const { config } = await loadConfig({
      ...BASE_ENV,
      SANDBOX_LOCAL_BASE_URL: "http://localhost:8080",
      NODE_ENV: "production",
  ...
  it("is true when SANDBOX_LOCAL_MAP is set in non-production", async () => {
    const { config } = await loadConfig({
      ...BASE_ENV,
