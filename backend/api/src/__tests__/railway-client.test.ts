...
describe("railwayRequest", () => {
  ...
  it("round-robin advances the counter on each call without explicit accountIndex", async () => {
    ...
    // Without explicit accountIndex, client defaults to index 0 (round-robin
    // is now handled by the sessions service, not the client).
    expect(capturedTokens[0]).toBe(`Bearer ${TOKEN_POOL[0]}`);
    expect(capturedTokens[1]).toBe(`Bearer ${TOKEN_POOL[0]}`);
  });
});
