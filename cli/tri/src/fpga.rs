...
pub fn run(cmd: &FpgaCmd) -> Result<()> {
    match cmd {
        ...
        FpgaCmd::BitConfig { bit } => bit_config(bit, &[]).map(|out| print!("{out}")),
...
fn resolve_pvt_context_for_boot(
    ...
    cable: &str,
    read_xadc: bool,
) -> Result<(Option<PvtContext>, serde_json::Value, bool)> {
    if let Some(path) = pvt_context {
...
fn bit_config(bit: &PathBuf, extra_args: &[&str]) -> Result<String> {
    ...
    if !output.status.success() {
        bail!("dump_bit_config.py exited with {:?}", output.status);
    }
...
mod tests {
    ...
    fn test_smoke_gate_missing_bitstream_matches_snapshot() {
        ...
        check_smoke_gate_snapshot(&report, &fixture_dir, "missing_bitstream_snapshot");
    }
