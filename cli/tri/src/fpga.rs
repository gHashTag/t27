...
pub enum FpgaCmd {
    ...
    ColdPor {
        ...
        #[arg(long)]
        xadc: bool,
        /// Use a deterministic synthetic operating point instead of reading the
        /// board. Useful for CI when no FPGA is connected. The source label in
        /// the boot log will be "synthetic".
    ...
    /// Validate a `.lean` theorem generated by `tri fpga measured-to-lean`.
    /// Checks the generated source labels against a JSON summary and counts
    /// the theorem declarations so downstream CI can assert the artifact is
    /// internally consistent.
    VerifyLean {
        /// Path to the generated `.lean` file.
        lean: PathBuf,
        /// Optional JSON summary emitted by `tri fpga measured-to-lean --json`.
        /// When supplied, the source label is read from the summary rather
        /// than parsed from theorem comments.
        #[arg(long)]
        summary: Option<PathBuf>,
        /// Expected closed-vocabulary `operating_point` source label. Fails
        /// if the summary or the theorem comments disagree.
        #[arg(long)]
...
pub fn run(cmd: &FpgaCmd) -> Result<()> {
    match cmd {
        ...
        FpgaCmd::VerifyLean {
            ...
            expected_source,
            json,
        } => verify_lean(lean, summary.as_ref(), expected_source.as_deref(), *json),
...
fn cclk_sweep(
    ...
    // PVT context files are still honored.
    let resolved = resolve_pvt_context_for_boot(
        pvt_context,
        corner,
        to_pvt_context,
        cable,
        if dry_run { false } else { read_xadc },
        synthetic_operating_point,
    )?;
    let pvt_ctx = resolved.pvt_ctx;
    let xadc_json = resolved.xadc_json;
    let operating_point = operating_point_json(&pvt_ctx, resolved.source);
...
impl XadcContext {
    ...
    fn to_pvt_context(&self, corner: ProcessCorner) -> Result<PvtContext> {
        let temp_c = self.temp_c.round().clamp(i64::MIN as f64, i64::MAX as f64) as i64;
        let vccint_mv = (self.vccint_v * 1000.0).round().clamp(0.0, u64::MAX as f64) as u64;
        let vccaux_mv = (self.vccaux_v * 1000.0).round().clamp(0.0, u64::MAX as f64) as u64;
        Ok(PvtContext {
            temp_c,
            vccint_mv,
            vccaux_mv,
...
/// Resolved PVT context and provenance for a cold-POR or CCLK-sweep boot log.
#[derive(Debug, Clone)]
pub struct ResolvedPvtContext {
    /// Resolved operating point, if any.
    pub pvt_ctx: Option<PvtContext>,
    /// XADC-shaped JSON object to store in the boot log (`source` tells
    /// downstream tooling how the values were obtained).
    pub xadc_json: serde_json::Value,
    /// Closed-vocabulary source label: `pvt_context_file`, `xadc`, `synthetic`,
    /// or `not_read`.
    pub source: &'static str,
    /// True iff the context came from a live XADC readout.
    pub from_xadc: bool,
}

/// Build a deterministic synthetic `PvtContext` for CI / dry-run use.
///
/// Values are chosen well inside the documented N25Q128_3V operating envelope
/// and approximate a cool, nominal-voltage lab bench.
pub fn synthetic_pvt_context(process_corner: ProcessCorner) -> PvtContext {
    PvtContext {
        temp_c: 42,
        vccint_mv: 1000,
        vccaux_mv: 1800,
...
///
/// Priority:
/// 1. An explicit `--pvt-context` file (`source: "pvt_context_file"`).
/// 2. A live XADC readout when `--xadc` is set (`source: "xadc"`), optionally
///    persisted via `--to-pvt-context`.
/// 3. A deterministic synthetic operating point when
///    `--synthetic-operating-point` is set (`source: "synthetic"`), optionally
///    persisted via `--to-pvt-context`.
/// 4. None (`source: "not_read"`) when none of the above are available.
///
/// # Examples
///
/// Explicit PVT file wins over synthetic:
///
/// ```rust,no_run
/// use std::path::PathBuf;
/// use tri::fpga::{resolve_pvt_context_for_boot, ProcessCorner};
///
/// let result = resolve_pvt_context_for_boot(
///     Some(&PathBuf::from("pvt.json")),
///     ProcessCorner::Ss,
///     None,
///     "digilent_hs2",
///     false,
///     true,
/// ).unwrap();
/// assert_eq!(result.source, "pvt_context_file");
/// ```
///
/// Synthetic operating point is used when no file or live readout is requested:
///
/// ```rust
/// use tri::fpga::{resolve_pvt_context_for_boot, ProcessCorner};
///
...
/// assert_eq!(result.pvt_ctx.as_ref().unwrap().process_corner, ProcessCorner::Tt);
/// ```
///
/// No source yields the `not_read` placeholder:
///
/// ```rust
/// use tri::fpga::{resolve_pvt_context_for_boot, ProcessCorner};
///
...
///     false,
/// ).unwrap();
/// assert_eq!(result.source, "not_read");
/// assert!(result.pvt_ctx.is_none());
/// ```
pub fn resolve_pvt_context_for_boot(
    pvt_context: Option<&PathBuf>,
    process_corner: ProcessCorner,
    to_pvt_context: Option<&PathBuf>,
    cable: &str,
    ...
    if read_xadc {
        match read_xadc_via_openfpgaloader(cable) {
            ...
            Err(e) => {
                ...
                Ok(ResolvedPvtContext {
                    pvt_ctx: None,
                    xadc_json: xadc_context_json("not_read", None),
                    source: "not_read",
                    from_xadc: false,
                })
            }
        ...
        let pvt = synthetic_pvt_context(process_corner);
        if let Some(path) = to_pvt_context {
            std::fs::write(path, serde_json::to_string_pretty(&pvt)?)
                .with_context(|| format!("write PVT context {}", path.display()))?;
            println!("[boot] wrote synthetic PVT context to {}", path.display());
        }
...
fn build_measured_to_lean_summary(
    ...
    let operating_point = pvt_ctx.as_ref().map(|ctx| {
        ...
        if let Some(obj) = value.as_object_mut() {
            obj.insert(
                "source".to_string(),
                serde_json::Value::String(pvt_source.to_string()),
            );
        }
        value
    });
...
fn measured_to_lean(
    ...
    let summary =
        build_measured_to_lean_summary(name, raw_ns, margin, &pvt_ctx, pvt_source, &text)?;
...
/// Extract the closed-vocabulary `operating_point` source label from the
/// comments of a generated `.lean` file. The label is emitted by
/// `measured-to-lean` as a `--` line when a PVT context is present.
fn extract_source_from_lean(text: &str) -> Option<String> {
    for line in text.lines() {
        let trimmed = line.trim();
        if let Some(rest) = trimmed.strip_prefix("--") {
            let rest = rest.trim();
            if let Some(src) = rest.strip_prefix("operating_point source:") {
...
/// Validate a `.lean` theorem generated by `tri fpga measured-to-lean`.
///
/// When a JSON summary is supplied (the `--json` output of `measured-to-lean`),
/// the `operating_point.source` field is read from it; otherwise the source
/// label is parsed from theorem comments. If `--expected-source` is provided,
/// the command fails when the actual source disagrees. The command also counts
/// `theorem` declarations and fails when none are found.
fn verify_lean(
    lean: &PathBuf,
    summary: Option<&PathBuf>,
    ...
    let lean_text = std::fs::read_to_string(lean)
        .with_context(|| format!("verify-lean: read {}", lean.display()))?;

    let summary_json: Option<serde_json::Value> = summary
        .map(|p| {
            let text = std::fs::read_to_string(p)
                .with_context(|| format!("verify-lean: read summary {}", p.display()))?;
            serde_json::from_str(&text)
                .with_context(|| format!("verify-lean: parse summary JSON {}", p.display()))
        })
        .transpose()?;

    let source = if let Some(ref s) = summary_json {
        s.get("operating_point")
            .and_then(|op| op.get("source"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| extract_source_from_lean(&lean_text))
    } else {
        extract_source_from_lean(&lean_text)
    };

    let theorem_re = regex::Regex::new(r"theorem\s+([A-Za-z0-9_]+)\s*:").unwrap();
    let theorems: Vec<String> = theorem_re
        .captures_iter(&lean_text)
        .map(|c| c[1].to_string())
        .collect();

    if theorems.is_empty() {
        bail!(
            "verify-lean: no theorem declarations found in {}",
            lean.display()
        );
    }

    if let Some(expected) = expected_source {
        let actual = source.as_deref().unwrap_or("not_read");
        if actual != expected {
            bail!(
                "verify-lean: expected operating_point source '{}' but found '{}'",
                expected,
                actual
            );
        }
    ...
    let passed = expected_source.map_or(true, |e| source.as_deref() == Some(e));

    if json {
        let result = serde_json::json!({
            "lean_file": lean.to_string_lossy().to_string(),
            "summary_file": summary.map(|p| p.to_string_lossy().to_string()),
        ...
        println!("== verify-lean ==");
        println!("Lean file: {}", lean.display());
        if let Some(p) = summary {
            println!("Summary: {}", p.display());
        }
        println!(
            "Operating point source: {}",
            source.as_deref().unwrap_or("not_read")
        );
        println!("Theorem count: {}", theorems.len());
        for t in &theorems {
            println!("  - {}", t);
        }
        if let Some(e) = expected_source {
            println!("Expected source: {}", e);
        }
...
fn bit_config(bit: &PathBuf, extra_args: &[&str]) -> Result<String> {
    ...
    let output = cmd.output().with_context(|| "spawn dump_bit_config.py")?;
    if !output.status.success() {
        let err = String::from_utf8_lossy(&output.stderr);
        bail!(
            "dump_bit_config.py exited with {:?}: {}",
            output.status,
            err.trim()
        );
    }
...
fn boot_log(
    ...
    if let Some(obj) = log_entry.as_object_mut() {
        ...
        obj.insert(
            "pvt_envelope_margin_ns".to_string(),
            serde_json::json!(Option::<i64>::None),
        );
        obj.insert(
            "recommendation".to_string(),
            recommendation_from_conclusion(conclusion, None, None),
        );
    }
    ...
    if bit_path.is_file() {
        ...
        if run_theorem_matrix {
            ...
            if replay_fixtures.is_some() {
                ...
                let (entries, elapsed_ms, replay_source) =
                    replay_theorem_matrix(&matrix_fixture_dir)
                ...
                if validate_lean_standalone {
                    ...
                    std::fs::create_dir_all(&pkg_dir).with_context(|| {
                        format!("create temp lake package {}", pkg_dir.display())
                    })?;
                    let lakefile = format!(
                        "import Lake\n\
                         open Lake DSL\n\n\
...
mod tests {
    ...
    fn test_build_measured_to_lean_summary_freq() {
        ...
        let json = serde_json::to_string(&m).unwrap();
        let summary =
            build_measured_to_lean_summary("measured_cclk", false, false, &None, "", &json)
                .unwrap();
        assert_eq!(
    ...
    fn test_pvt_envelope_json_report_inside_envelope_true() {
        ...
        assert_eq!(
            report["pvt_context"]["source"].as_str(),
            Some("pvt_context_file")
        );
        std::fs::remove_file(&pvt).unwrap();
    }
    ...
    fn test_theorem_matrix_synthetic_fixture_and_summary() {
        let tmp =
            std::env::temp_dir().join(format!("tri_theorem_matrix_test_{}", std::process::id()));
    ...
    fn test_theorem_matrix_fixture_roundtrip() {
        ...
        let generated = generate_theorem_matrix(&tmp, &serde_json::json!({}), "synthetic")
            .expect("generate theorem matrix should succeed");
        assert_eq!(generated.len(), 24, "expected 24 variants");

        let (replayed, _elapsed_ms, _source) =
            replay_theorem_matrix(&tmp).expect("replay theorem matrix should succeed");
    ...
    fn test_theorem_matrix_replay_does_not_regenerate() {
        let tmp =
            std::env::temp_dir().join(format!("tri_theorem_matrix_replay_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();

        generate_theorem_matrix(&tmp, &serde_json::json!({}), "synthetic")
            .expect("generate theorem matrix should succeed");
        ...
        let mut raw_ns: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&corrupt_path).unwrap()).unwrap();
    ...
    fn test_theorem_matrix_golden_replay_passes() {
        ...
        let (entries, elapsed_ms, _source) =
            replay_theorem_matrix(&golden).expect("golden fixture replay should succeed");
        assert_eq!(
            entries.len(),
            24,
    ...
    fn test_theorem_matrix_golden_replay_matches_snapshot() {
        ...
        if std::env::var("UPDATE_EXPECTED").is_ok() || !expected_path.is_file() {
            std::fs::write(
                &expected_path,
                serde_json::to_string_pretty(&snapshot_report).expect("serialize report snapshot"),
            )
        ...
        let expected_text =
            std::fs::read_to_string(&expected_path).expect("read expected_report.json");
        let expected: serde_json::Value =
            serde_json::from_str(&expected_text).expect("parse expected_report.json");
    ...
    fn test_theorem_matrix_dry_run_live_replay_matches_golden_shape() {
        ...
        let dry_entries =
            generate_theorem_matrix(&dry_run_live, &serde_json::json!({}), "dry_run_live")
        ...
        let (dry_replayed, dry_elapsed_ms, dry_source) =
            replay_theorem_matrix(&dry_run_live).expect("dry-run-live replay should succeed");
        assert_eq!(
            dry_replayed.len(),
            24,
            "expected 24 replayed dry-run-live variants"
        );
        assert_eq!(
        ...
        let dry_report =
            build_theorem_matrix_report(&dry_replayed, dry_elapsed_ms, true, &dry_source);
        assert_eq!(
        ...
        let golden_report =
            build_theorem_matrix_report(&golden_replayed, golden_elapsed_ms, true, &golden_source);
