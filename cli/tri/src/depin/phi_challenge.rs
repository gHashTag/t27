...
mod tests_v2 {
    ...
    fn test_v2_champion_weights_full_rank() {
        ...
        for col in 0..16 {
            let pivot = (col..16).find(|&r| m[r][col] != 0)
                .expect("zero pivot: matrix is singular!");
            m.swap(col, pivot);
            let inv = gf16_inv(m[col][col]);
            for j in col..16 { m[col][j] = gf16_mul(m[col][j], inv) & 0xF; }
            for r in 0..16 {
                if r != col && m[r][col] != 0 {
                    let factor = m[r][col];
                    for j in col..16 {
    ...
    fn test_v2_diffusion() {
        ...
        assert!(diff_bytes >= 8, "avalanche too weak: only {}/32 bytes changed", diff_bytes);
    }
}
