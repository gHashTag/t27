...
| CI | Formal (Yosys) | GREEN | **42 properties proved** across 6 modules + **20 integration properties** on `bitnet_engine_top`, all proving — including the **first property that spans two layers**. Plus an **8-property zero-sized-request sweep**, a **baseline gate** so a probe verdict is only read after the unprobed design proves, 6 liveness witnesses, **0 vacuous guards** — and a **documentation gate**: all **27 propositions** name the CI step that re-checks them, and every runnable block in the document is verified to run ([Props. 14–27](docs/FORMAL_FOUNDATIONS.md)) |
