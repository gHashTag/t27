...
| CI | Formal (Yosys) | GREEN | **42 properties proved** across 6 modules + **20 integration properties** on `bitnet_engine_top`, all proving — including the **first property that spans two layers**. Plus an **8-property zero-sized-request sweep** over every module that takes a count, a **baseline gate** so a probe verdict is only read after the unprobed design proves, 6 liveness witnesses, and **0 vacuous guards** ([Props. 14–26](docs/FORMAL_FOUNDATIONS.md)) |
