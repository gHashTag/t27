...
| CI | Formal (Yosys) | GREEN | **42 properties proved** across 6 modules + **21 integration properties** on `bitnet_engine_top`, all proving **at `-seq 40`, `DEPTH 4`** — and the claim carries its ceiling: the set still proves at `-seq 80` (2× the CI bound), at `DEPTH 16`, and at raised seq and depth together, and is undecided at `-seq 120` ([Prop. 34](docs/FORMAL_FOUNDATIONS.md)). **No property is gated as an expected refutation**, so nothing in the engine is knowingly broken. Includes cross-layer and write-contiguity properties, zero- and maximum-size sweeps, a baseline gate, liveness witnesses, **0 vacuous guards**, a documentation gate covering all **42 propositions**, a validated counterexample reader, and weekly mutation and scale-ceiling harnesses ([Props. 14–42](docs/FORMAL_FOUNDATIONS.md)). A **free-property gate** fails the build if any assertion body is discharged by syntax alone, and is mutation-tested in the same step. One suite is proved **unboundedly** by k-induction rather than to a depth; the bounded ones were mapped property-by-property and their bounds raised where that is meaningful — `dma_controller` 12→80, `layer_sequencer` 12→48 ([Props. 35–36](docs/FORMAL_FOUNDATIONS.md)) |
...
| BitNet HLS | RTL blocks **integrated** | YELLOW | **10 of 10** modules, 12 instances, and **one open defect**, CI-gated as an expected refutation. Both ends of every count are swept; the sweeps produced six distinct RTL defects across three modules — address wrap, word *N* written at *N+1*, a dual-role pointer, a misplaced reset, a write strobe held as a level, and a layer reading a buffer nothing wrote ([Props. 26–33](docs/FORMAL_FOUNDATIONS.md)). All fixed, with `overflow` and `buffer_unwritten` driving the error IRQ that had been tied off |
