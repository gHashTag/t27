...
|--------|-----------|--------|---------|
| Compiler | `t27c parse` | GREEN | **497 / 497** specs parse, **154 recovery events, 0 declarations swallowed, 0 specs declaring but capturing nothing** — down from 1741 / 788 / 40. Ratcheted by `formal/spec_parse_gate.py`. Separately, **18 field types across 10 specs carry unbalanced brackets** (`[[]Const [,`) — the same generator defect as the 107 sites repaired in [Prop. 186](docs/FORMAL_FOUNDATIONS.md); these are **not repaired**, because balance determines the bracket but not the element type the generator also lost, and guessing 18 of those is a decision about what the corpus meant rather than a scanner's call. They are ratcheted instead — a 19th fails the build ([Prop. 192](docs/FORMAL_FOUNDATIONS.md)). Searching for the *question* rather than the shape widens this to **18 specs and 21 imbalances across three delimiter classes** — including parens and braces, which no `[`-shaped search could return ([Prop. 193](docs/FORMAL_FOUNDATIONS.md)). Turning that theorem on the instruments: **22 of 29 checking scripts publish no denominator**, and two that do have been printing their own blindness in green since they landed — `units_scan` compares 41 of 163 connections (**25%**), `width_scan` 3 of 16 declarations (**19%**) ([Prop. 194](docs/FORMAL_FOUNDATIONS.md)). And every gate above passes **8 specs, 2,865 code lines, that the parser reads as anonymous empty shells** — 2 AST nodes each regardless of size; the declarative spec form captures **0.4 nodes per 100 lines against the module form's 69.3** ([Prop. 195](docs/FORMAL_FOUNDATIONS.md)). One contextual keyword — `spec Ident {` as an alias for `module`, which cannot be a plain keyword because `spec` is an ordinary identifier 31 times over — took that from **8 shells to 1** and 161 recovery events to **154** ([Prop. 196](docs/FORMAL_FOUNDATIONS.md)). Emitting the recovery *messages* then showed the headline number is not one population: **16 of 497 specs are Markdown documents and account for 55 of the 154 events — 36%** ([Prop. 197](docs/FORMAL_FOUNDATIONS.md)). Auditing all **13 hardcoded exemptions** across the gates then found one too narrow and **three simply false** — `bench.py` and `mutate.py` can both fail CI and were excused from stating a denominator as \"not checking scripts\" ([Prop. 198](docs/FORMAL_FOUNDATIONS.md)). Continuing the audit, `units_scan`'s single `KNOWN_OPEN` entry — recorded as *\"Real, unfixed\"* — **had been fixed in the RTL and stopped firing in silence**; an expected refutation that stops firing produces no output, so the failure looks exactly like success ([Prop. 199](docs/FORMAL_FOUNDATIONS.md)). Then the sweep itself: **red since wave 694**, hidden because six waves ended \"all 20 gates green\" against a hand-typed list while the suite has 58 steps — and the absence-swept count rose 44→47 *because* the new steps were unexempted, so the defect read as growth ([Prop. 200](docs/FORMAL_FOUNDATIONS.md)). Replacing the hand-typed list with an enumerator over the workflow files — **43 invocations of 30 scripts** — then showed that of 5 `continue-on-error` steps, 2 are checks, and one has been **red since it landed** behind the note *\"non-blocking until observed green\"*: three Coq files use `Forall` without importing `List` ([Prop. 201](docs/FORMAL_FOUNDATIONS.md)). Repairing behind that one flag took the Coq tree from **10 of 28 proof files building to 25**, across **five unrelated mechanical defect classes** that had accumulated because a suspended check reports success whether it is green or has been red for a year ([Prop. 202](docs/FORMAL_FOUNDATIONS.md)). The last obstacle was `lia` itself: a `nat` literal of 142,857,143 is kept abstract, so the solver reports a missing witness on a trivially true goal. **28 of 28 build, `make` exits 0, and the `continue-on-error` flag is off** ([Prop. 203](docs/FORMAL_FOUNDATIONS.md)). Asking Coq what those proofs rest on then found a lower-bound theorem **resting on an axiom that asserts the bound** — `Qed` certifies a derivation, never the truth of its premises, and the two are indistinguishable in every `Qed` count published here ([Prop. 204](docs/FORMAL_FOUNDATIONS.md)). The 26% that scan could not resolve turned out to be **entirely the instrument's** — two load-path roots and module-qualified names — taking resolution to **419 of 460** and revealing 19 conditional theorems where 12 were visible ([Prop. 205](docs/FORMAL_FOUNDATIONS.md)). The outage that exposed it is now fixed structurally: a `finally` cannot survive `SIGKILL`, so the sweep's restore runs **at startup of the next run** instead ([Prop. 206](docs/FORMAL_FOUNDATIONS.md)). The outage also measured something no design review would: on one identically starved tree **23 of 44 gates failed and 21 passed**, and which group a gate landed in was a property of how it was written, not of the tree ([Prop. 207](docs/FORMAL_FOUNDATIONS.md)). Executing all 16 exempt gates' absence cases instead of asserting them, 15 held and one did not: a scanner **matched its own source**, satisfying the very liveness floor written to stop a silent clean sweep ([Prop. 208](docs/FORMAL_FOUNDATIONS.md)). Starving a gate *partly* rather than completely then showed the deeper problem: with **249 of 497 specs deleted, every ratchet still reported \"holds\"** and three read as substantial improvement — a finding count is unbounded below in its own denominator ([Prop. 209](docs/FORMAL_FOUNDATIONS.md)). Hollowing 249 specs to a single comment line — **every file still present** — then defeated that gate too, and the fix is one line of arithmetic: ratchet total content alongside file count ([Prop. 210](docs/FORMAL_FOUNDATIONS.md)). |
...
# ...but 69 of those 546, in 7 files, are in NO _CoqProject and are
# type-checked by nothing. Counting `Qed.` measures proof TERMINATORS IN
# TEXT; only membership in a build measures proofs. Across coq/,
# trios-coq/ and proofs/: 641 Qed built, 42 unbuilt. Gate 20
# (formal/coq_build_scan.py) ratchets the unbuilt set. Prop. 154.
```
