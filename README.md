...
| CI | Formal (Yosys) | GREEN | **42 properties proved** across 6 modules + **23 integration properties** on `bitnet_engine_top`, all proving **at `-seq 40`, `DEPTH 4`** — and the claim carries its ceiling: the set still proves at `-seq 60`, at `DEPTH 8`, and at both together, and is undecided beyond ([Prop. 34](docs/FORMAL_FOUNDATIONS.md)). **No property is gated as an expected refutation**, so nothing in the engine is knowingly broken. Includes cross-layer and write-contiguity properties, zero- and maximum-size sweeps, a baseline gate, liveness witnesses, **0 vacuous guards**, a documentation gate covering all **35 propositions**, a validated counterexample reader, and weekly mutation and scale-ceiling harnesses ([Props. 14–35](docs/FORMAL_FOUNDATIONS.md)). Module suites are proved **one property per invocation**, which raised the shallowest module's verified bound from 14 to 40 — a batch verdict is the minimum over its members ([Prop. 35](docs/FORMAL_FOUNDATIONS.md)) |
