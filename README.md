...
| CI | Formal (Yosys) | GREEN | **42 properties proved** across 6 modules + **15 integration properties** on `bitnet_engine_top`, spanning CSR aperture → sequencing → memory → MAC → requantizer ([Props. 14–19](docs/FORMAL_FOUNDATIONS.md)) |
...
| BitNet HLS | RTL blocks **integrated** | YELLOW | host-driven over AXI-Lite: CSRs → sequencers → weight prefetch → MAC → requantizer → activation double buffer → next layer, with IRQ. **9 of 10** modules, 11 instances; `dma_controller` alone remains standalone. [`docs/BITNET_V2_POSITION.md`](docs/BITNET_V2_POSITION.md) §2.3 |
