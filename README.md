...
| CI | Formal (Yosys) | GREEN | **42 properties proved** across 6 modules + **23 integration properties** on `bitnet_engine_top`, all proving — **no property is gated as an expected refutation**, so nothing in the engine is knowingly broken. Includes cross-layer and write-contiguity properties, zero- and maximum-size sweeps, a baseline gate, liveness witnesses, **0 vacuous guards**, a documentation gate covering all **33 propositions**, a validated counterexample reader, and a weekly mutation harness proving the gates bite ([Props. 14–33](docs/FORMAL_FOUNDATIONS.md)) |
