...
def main() -> None:
    ...
    if "ternaryMacAccumulateFiftyFivePlusGeneric" in text:
        print("W379 Lean theorems already present.")
        return
...
theorem ternaryMacAccumulateFiftyFivePlusGeneric ({plus_vars} : Int) :
...
theorem ternaryMacAccumulateFiftyFourMinusGeneric ({minus_vars} : Int) :
...
theorem ternaryMacZeroWeightTrevigintupleClosureGeneric (x {zero_vars} : Int) :
    {lhs_zero} =
    {rhs_zero} := by
  simp [ternaryMac_eq_acc_plus_mul, ternaryMul, ternaryDecode]
  <;> try omega
'''
    ...
    print("Appended W379 theorems to TernaryInference.lean")
