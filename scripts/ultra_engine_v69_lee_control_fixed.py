...
def lee_experiment():
    ...
    # Generate exponent grids
    phi_pows = np.arange(-10, 11)
    ...
    for target_name, target_val in PDG_TARGETS.items():
        ...
        # FIX: Recalculate hit_rate and p_value in this loop
        hit_rate = k_observed / n_total

        # Use scipy.stats.binom_test for older scipy or binomtest for newer
        try:
            from scipy.stats import binom_test
            p_value = binom_test(k_observed, n_total, 0.5, alternative='greater')
        except ImportError:
            from scipy.stats import binomtest
            p_value = binomtest(k_observed, n_total, 0.5, alternative='greater').pvalue
        ...
        lee_analysis[target_name] = {
            ...
            "significance": "highly_significant" if p_value < 0.001 else
