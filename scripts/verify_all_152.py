...
# Exact formulas
EXACT = {"S3_L5_TRINITY": PHI**2 + PHI**(-2)}

# Expected values
EXPECTED = {"S3_L5_TRINITY": (mpf("3"), mpf("0.0"))}
>>>>>>> Stashed changes
...
=======
            print("FAIL: EXACT identity deviation {:.2e}".format(diff))
            all_pass = False
        elif diff > mpf("1e-45"):
            print("WARNING: Small deviation {:.2e}".format(diff))
        else:
...
=======
print("Total formulas: {}".format(len(ALL_FORMULAS)))
print("Validated against experiment: {}".format(len(deviations)))

seal_str = str(ALL_FORMULAS)
sha256_seal = hashlib.sha256(seal_str.encode()).hexdigest()
print("SHA256 seal: {}".format(sha256_seal))
>>>>>>> Stashed changes
