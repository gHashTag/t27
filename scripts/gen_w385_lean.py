#!/usr/bin/env python3
"""Append Wave Loop 385 generic theorems to Trinity.TernaryInference.lean."""
...
def main() -> None:
    ...
    if "ternaryMacZeroWeightTwentyPairClosureGeneric" in text:
        print("W385 Lean theorems already present.")
        return

    blocks: list[str] = []

    # 63-variable plus accumulation
    lhs, rhs = plus_chain(63)
    vars = " ".join(var_names(63))
    blocks.append(theorem_block(
        "ternaryMacAccumulateSixtyThreePlusGeneric",
        "Generic theorem: 63-variable plus accumulation. **281 generic ∀ milestone.**",
        vars, lhs, rhs,
    ))

    # 62-variable minus accumulation
    lhs, rhs = minus_chain(62)
    vars = " ".join(var_names(62))
    blocks.append(theorem_block(
        "ternaryMacAccumulateSixtyTwoMinusGeneric",
        "Generic theorem: 62-variable minus accumulation lattice.",
        vars, lhs, rhs,
    ))

    # depth-45 cancellation (odd depth leaves residual plus-weight MAC)
    lhs, rhs = cancellation_chain(45)
    blocks.append(theorem_block(
        "ternaryMacQuadragintupleQuinqueCancellationGeneric",
        "Generic theorem: depth-45 activation cancellation. mac^45(x,a,[.plus,.minus,...]) = mac(x,a,.plus).",
        "x a", lhs, rhs,
    ))

    # zero-weight closure: 20 zero before + plus + 20 zero after
    lhs, rhs = zero_weight_closure(20, 20)
    vars = " ".join(var_names(41))
    blocks.append(theorem_block(
        "ternaryMacZeroWeightTwentyPairClosureGeneric",
    ...
    print("Appended W385 theorems to TernaryInference.lean")
