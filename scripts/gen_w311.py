#!/usr/bin/env python3
"""Batch append W311 tests and invariants to all IGLA specs."""
...
def transform_block(block_lines):
    """Replace wave suffix to w311."""
    text = '\n'.join(block_lines)
    text = text.replace('_w310', '_w311')
    text = text.replace('_w309', '_w311')
    text = text.replace('_w308', '_w311')
    return text + '\n'
