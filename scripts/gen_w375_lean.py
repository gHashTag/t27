#!/usr/bin/env python3
"""Append Wave Loop 375 generic theorems to Trinity.TernaryInference.lean."""
...
def main() -> None:
    ...
    if "ternaryMacAccumulateFiftyOnePlusGeneric" in text:
        print("W375 Lean theorems already present.")
        return

    # 51-variable plus accumulation
    lhs_plus, rhs_plus, title_plus = plus_chain(51)
    plus_vars = " ".join(var_names(51))
    plus_thm = f'''/-- Generic theorem: accumulating fifty-one independent activations with plus-weights is quinquaginta-unary addition.
...
theorem ternaryMacAccumulateFiftyOnePlusGeneric ({plus_vars} : Int) :
    {lhs_plus} = {rhs_plus} := by
  simp [ternaryMac_eq_acc_plus_mul, ternaryMul, ternaryDecode]
  <;> try omega
'''

    # 50-variable minus accumulation
    lhs_minus, rhs_minus, title_minus = minus_chain(50)
    minus_vars = " ".join(var_names(50))
    minus_thm = f'''/-- Generic theorem: accumulating fifty independent activations with minus-weights is negated quinquagintal addition.
...
theorem ternaryMacAccumulateFiftyMinusGeneric ({minus_vars} : Int) :
    {lhs_minus} = -({rhs_minus}) := by
  simp [ternaryMac_eq_acc_plus_mul, ternaryMul, ternaryDecode]
  <;> try omega
'''

    # depth-28 cancellation (even depth -> identity x)
    cancel_lhs = cancellation_chain(28)
    cancel_thm = f'''/-- Generic theorem: octovigintuple activation cancellation for ternary MAC.
...
'''
    ...
    # zero-weight octodecuple closure: 9 before + 1 plus + 9 after = 19 vars, 18 zero-weight MACs.
    lhs_zero, rhs_zero = zero_weight_closure(9, 9)
    zero_vars = " ".join(var_names(19))
    zero_thm = f'''/-- Generic theorem: zero-weight octodecuple closure for ternary MAC.
...
theorem ternaryMacZeroWeightOctodecupleClosureGeneric (x {zero_vars} : Int) :
    {lhs_zero} =
    {rhs_zero} := by
  simp [ternaryMac_eq_acc_plus_mul, ternaryMul, ternaryDecode]
  <;> try omega
'''
    ...
    print("Appended W375 theorems to TernaryInference.lean")
