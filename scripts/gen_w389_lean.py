#!/usr/bin/env python3
"""Append Wave Loop 389 generic theorems to Trinity.TernaryInference.lean."""
...
def main() -> None:
    ...
    if "ternaryMacZeroWeightTwentyFourPairClosureGeneric" in text:
        print("W389 Lean theorems already present.")
        return

    blocks: list[str] = []

    # 67-variable plus accumulation
    lhs, rhs = plus_chain(67)
    vars = " ".join(var_names(67))
    blocks.append(theorem_block(
        "ternaryMacAccumulateSixtySevenPlusGeneric",
        "Generic theorem: 67-variable plus accumulation. **297 generic ∀ milestone.**",
        vars, lhs, rhs,
    ))

    # 66-variable minus accumulation
    lhs, rhs = minus_chain(66)
    vars = " ".join(var_names(66))
    blocks.append(theorem_block(
        "ternaryMacAccumulateSixtySixMinusGeneric",
        "Generic theorem: 66-variable minus accumulation lattice.",
        vars, lhs, rhs,
    ))

    # depth-49 residual cancellation (odd depth leaves a residual plus-weight MAC)
    lhs, rhs = cancellation_chain(49)
    blocks.append(theorem_block(
        "ternaryMacQuadragintupleNovemCancellationGeneric",
        "Generic theorem: depth-49 activation cancellation. mac^49(x,a,[.plus,.minus,...]) = mac(x,a,.plus).",
        "x a", lhs, rhs,
    ))

    # zero-weight closure: 24 zero before + plus + 24 zero after
    lhs, rhs = zero_weight_closure(24, 24)
    vars = " ".join(var_names(49))
    blocks.append(theorem_block(
        "ternaryMacZeroWeightTwentyFourPairClosureGeneric",
    ...
    print("Appended W389 theorems to TernaryInference.lean")
